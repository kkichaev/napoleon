...
void Config::SetValue(const std::string& key, const std::string& value) {
    ...
    else if (_stricmp(k, "readtimeout") == 0) readtimeout = atoi(value.c_str()) * 1000;
    else if (_stricmp(k, "devicePingTimeout") == 0) devicePingTimeout = atoi(value.c_str()) * 1000;
    ...
    else if (_stricmp(k, "maxDataSize") == 0) maxDataSize = atoi(value.c_str());
}

void Config::SetDefaults() {
    port = 7654;
    readtimeout = 40 * 1000;
    devicePingTimeout = 30 * 1000;
