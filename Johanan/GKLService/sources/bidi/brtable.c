...
BDPTR dp;
    ...
    dp = getBidiDataPtr ( c );
    return ( dp != NULL ) ? dp->bpt : BIDI_None ;
}
...
static int br_parsePropertyData ( int status, int version, const char* workFolder )
{
FILE *fdi;
int rc;
int i;
...
int br_InitTable ( int version, const char* workFolder )
{
int rc;
	...
	rc = br_parsePropertyData ( UNICODE_DATA, version, workFolder );
    if ( rc < 0 )
    {
        if ( Trace ( Trace15 ) )
        {
    ...
    if ( version == UBA63 )
    {
        rc = br_parsePropertyData ( BRACKET_DATA, version, workFolder );
        if ( rc < 0 )
        {
            if ( Trace ( Trace15 ) )
            {
...
int br_Init ( int version, const char* workFolder )
{
int rc;
    ...
    rc = br_InitTable ( version, workFolder );
