 ...
 * It is set to half the MAXINPUTLEN, because in principle an
...
#define MAXPAIRINGDEPTH (BR_MAXINPUTLEN / 2)
...
typedef struct {
  U_Int_32 c;
  BIDIPROP bc;
  BIDIPROP orig_bc; /* store original value for bc */
...
extern int br_InitTable ( int version, const char* workFolder );
