...
UBACTXTPTR br_ConstructContext ( int len, U_Int_32 *text, 
...
int rc;
    ...
    if ( rc != 1 )
    {
        if ( Trace ( Trace0 ) )
        {
            printf ( "Error: bad return in UBA processing\n" );
        }
        return ( rc );
    }
 ...
 *
 * Return  1 if o.k.
 * Return -1 and print a truncation error if the output parameter is too short.
 */
