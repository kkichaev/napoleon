 ...
 * 2013-Jun-05 kenw   Added use of dirtyBit in rules and display.
 * 2013-Jun-07 kenw   Add canonical equivalence for bracket matching.
 * 2013-Jun-19 kenw   Add use of br_ErrPrint.
 * 2013-Jun-27 kenw   Correct error in N0 handling of EN/AN.
 * 2013-Jun-29 kenw   Correct another error in br_ResolveOnePair.
 ...
 * statusStack
 *
 * For simplicity the statuc stack is allocated here using the larger 
	...
	for ( i = 1; i <= brp->len; i++ )
	{
		if ( i == 1 )
		{
			if ( brp->len == 1 )
			{
				if ( brp->sor == BIDI_L )
				{
					if ( seqSyntax && ( br_IsIsolateInitiator ( brp->first->bc ) ) )
					{
		...
		{
			if ( seqSyntax && br_IsIsolateInitiator ( brp->last->bc ) )
			{
...
BIDIUNITPTR bdu = current + 1;
	...
	/* If we hit another isolate initiator, increment the level counter */
		else if ( br_IsIsolateInitiator ( bc ) )
		{
			dumblevelctr++;
		}
...
BIDIUNITPTR bdu;
	...
	while ( bdu <= last )
	{
		...
		if ( level != NOLEVEL )
		{
			if ( ( GetUBAVersion() == UBA63 ) && ( br_IsIsolateInitiator ( bdu->bc ) ) )
			{
...
IRSEQPTR tirp;
	...
	while ( tirp != NULL )
	{
		 ...
		 * initiator.
		 */
		if ( br_IsIsolateInitiator ( tirp->lastRun->run->last->bc ) )
		{
			nextRunLevel = ctxt->paragraphEmbeddingLevel;
...
int savelevel;
	...
	while ( brp != NULL )
	{
		...
		if ( brp->seqID == 0 )
		{
			 ...
			 */
			if ( br_IsIsolateInitiator ( brp->last->bc ) )
			{
				if ( Trace ( Trace5 ))
				{
				...
				while ( brp2->next != NULL )
				{
					 ...
					 */
						 ...
						 */
						if ( !br_IsIsolateInitiator ( brp2->last->bc ) )
						{
							break;
						}
					}
...
static int br_IsNeutralType ( BIDIPROP bc )
{
	...
	else if ( ( GetUBAVersion() == UBA63 ) && ( br_IsIsolateControl ( bc ) ) )
	{
		return ( 1 );
	}
	else
...
BIDIUNITPTR *tbupp;
	...
	{
		if ( ( (*tbupp)->bc == BIDI_R ) || 
			( (*tbupp)->bc == BIDI_AN ) || ( (*tbupp)->bc == BIDI_EN ) )
		{
			return ( 1 );
		}
		else if ( (*tbupp)->level == NOLEVEL )
 ...
 * Push an element on the stack. 
 */
static void br_PushBracketStack ( BRACKETSTACKPTR sptr )
{
	/* Check for stack full */
...
int stackDepth;
	...
	stackDepth = bracketStackTop - bracketStack;
	if ( stackDepth > n )
	{
		bracketStackTop -= n;
	}
...
int strongtypefound;
	...
	if ( firstpos < lastpos - 1 )
	{
		for ( bupp = buppopening + 1; bupp <= buppclosing - 1; bupp++ )
		{
			...
			tempbc = ( (((*bupp)->bc) == BIDI_R )  || 
			           (((*bupp)->bc) == BIDI_EN ) || 
                       (((*bupp)->bc) == BIDI_AN ) ) ? BIDI_R :
                       (((*bupp)->bc) == BIDI_L ) ? BIDI_L : BIDI_None ;
			if ( tempbc == embeddingdirection )
			{
				/* N0 Step b, */
				(*buppopening)->bc = embeddingdirection;
				(*buppclosing)->bc = embeddingdirection;
				if ( Trace ( Trace7 ) )
				{
	...
	if ( strongtypefound )
	{
		...
		{
			(*buppopening)->bc = oppositedirection;
			(*buppclosing)->bc = oppositedirection;
		}
		/*
	...
	for ( bupp = buppfirst, pos = 0; bupp <= bupplast; bupp++, pos++ )
	{
		if ( (*bupp)->bpt != BPT_None )
		{
			if ( (*bupp)->bpt == BPT_O )
			{
				...
				bracketData.pos = pos;
				br_PushBracketStack ( &bracketData );
			}
			else
			{
				/* 
	...
	while ( bdu < endOfText )
	{
		if ( bdu->level != NOLEVEL )
		{
			...
			else
			{
				...
				else if ( ( bdu->bc == BIDI_AN ) || ( bdu->bc == BIDI_EN ) )
				{
					bdu->level += 2;
					ctxt->dirtyBit = 1;
				}
