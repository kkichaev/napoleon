...
-(BOOL) write:(GRSObject*) data toStmt:(sqlite3_stmt*) stmt;
-(BOOL) read:(sqlite3_stmt*) stmt toObject:(GRSObject*) data;
@end
...
- (int) putItem: (GRSObject*)item : (NSOutputStream*) stream;
- (void) setItem: (NSInputStream*)stream : (GRSObject*)item ;
...
- (void)setItem:(NSInputStream *)stream :(GRSObject *)item {
    ...
    for(int i=0; i<count; i++) {
        GRSObject* obj = [[itemClass alloc] init];
...
- (const void*) write:(GRSObject*) data : (int*) outBytes;
-(void) read:(GRSObject*)data fromBuffer: (const void*) buffer count: (int) cb;
...
-(BOOL) write:(GRSObject*) data toStmt:(sqlite3_stmt*) stmt { return TRUE; }
-(BOOL) read:(sqlite3_stmt*) stmt toObject:(GRSObject*) data { return TRUE; }
...
+ (BOOL) recreateTable:(Class) dataClass {
    NSString* stmt = [NSString stringWithFormat:@"DROP TABLE IF EXISTS [%@]", [dataClass getTableName]];
    return [GRSDBManager directExecute: stmt] && CreateTable(dataClass);
}
@end
...
-(BOOL) prepareToWrite:(GRSObject*) data {
   [self closeStmt];
...
-(NSMutableString*) prepareForSelect:(GRSObject*)data {
   [self closeStmt];
...
-(BOOL) prepareReadByKey:(GRSObject *)data {
...
- (BOOL) prepareRead:(GRSObject*) data where:(NSString *)where orderBy:(NSString *)orderBy {
   NSMutableString *sql = [self prepareForSelect:data];
...
- (NSArray<GRSObject *> *)fetch:(Class)dataObject {
    return [self fetch:dataObject where:@"" order:@""];
}

- (NSArray<GRSObject *> *)fetch:(Class)dataObject where:(NSString *)filter {
    return [self fetch:dataObject where:filter order:@""];
}

- (NSArray<GRSObject *> *)fetch:(Class)dataObject where:(NSString *)filter order:(NSString *)order {
    NSMutableArray<GRSObject*>* res = [[NSMutableArray alloc] init];
    
    GRSObject* obj = [[dataObject alloc] init];
    BOOL bdo = [self read:obj where:filter orderBy:order];
    while(bdo) {
        [res addObject:obj];
        obj = [[dataObject alloc] init];
        bdo = [self readNext:obj];
    }
    [self close];
    return res;
}
...
- (NSDictionary *)fetchDic:(Class)dataObject key:(NSString *)key {
    return [self fetchDic:dataObject key:key where:@""];
}
...
- (NSDictionary *)fetchDic:(Class)dataObject key:(NSString *)key where:(NSString *)filter {
    ...
    GRSObject* obj = [[dataObject alloc] init];
    BOOL bdo = [self read:obj where:filter orderBy:@""];
    while(bdo) {
        
        id value = [obj valueForKey:key];
        
        [res setObject:obj forKey:value];
