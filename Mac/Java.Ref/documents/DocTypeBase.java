...
/**
 * Чтобы не заводить отдельный класс для DocTypeManager - статические методы
 * класса - используются как Manager Классы типов документа - singletion Лучше
 * всего их получать так DocType.getDocType("Заявки");
 * 
 * @author 1111
 * 
 */
public abstract class DocTypeBase {
	...
	protected DocTypeBase(String name, Class<? extends Document<?>> docClass) {
		this.name = name;
		this.docClass = docClass;
	}

	protected DocTypeBase(String name, String objName,
	...
	public boolean equals(Object o) {
		return o instanceof DocTypeBase && objName.equals(((DocTypeBase)o).objName);
	}
	...
	public boolean isCreatable() {
		return CreatableDocument.class.isAssignableFrom(docClass);
	}
	...
	public DocList docList(String orgId) {
		return docList(orgId, null);
	}

	public DocList docList(String orgId, String order) {
		return docList(orgId, order, "");
	}
	...
	public DocList docList(String orgId, String order, DatePeriod selection) {
		return docList(orgId, order, selection, null);
	}
	...
	public DocList docList(String orgId, String order, DatePeriod selection,
		...
		if (selection != null) {
			...
			where = String.format(" %s >= %s and %s <= %s", selectionField,
					Long.toString(selection.begin.getTime()), selectionField,
					Long.toString(selection.end.getTime()));
		}
	...
	public static void addType(DocTypeBase dt) {		
		if(docTypes.contains(dt) == false) {
			Log.d("DocTypeBase", dt.objName);
			docTypes.add(dt);
		}
	}
	...
	public static void setCurDoc(DocTypeBase docType) {
		setCurDoc(docType, false);
	}
	...
	public static List<CreateDocDataObject> getPhotoDocs() {
		List<CreateDocDataObject> ret = new ArrayList<CreateDocDataObject>();
		
		for (DocTypeBase dt : docTypes) {
			List<CreateDocDataObject> cdocs = dt.getDirtyPhotos();
			if(cdocs != null && cdocs.size() > 0)
				ret.addAll(cdocs);
		}		
		return ret;
	}
	...
	public boolean photoDoc() { return false; }
	public List<CreateDocDataObject> getDirtyPhotos() { return null; }
	...
	public DocExportListener getDirtyDocuments() {
		...
		if (d instanceof CreatableDocument) {
			return new DocSendListner(getObjectName(), (Class<? extends CreatableDocument<?>>) d.getClass(), "params", getExportFlag());
		} else
			return null;
	}

	protected int getExportFlag() { return ParamState.ofExported; }
	...
	protected ArrayList<String> todays = new ArrayList<String>();
	...
	protected void updateTodayDocs() {
		synchronized (DocTypeBase.class) {
			...
			String where = String
					.format(" %s >= %s and %s <= %s", selectionField,
							Long.toString(now.getTime()), selectionField,
			...
			Document<?> doc = create();
			if (doc != null) {
				String tableName = DataObjectInfo.getInstance().getTableName(
						doc.getData().getClass());

				if (tableName != null && DbWriter.isTableExists(tableName)) {
					try {
						Cursor c = DataBaseManager.getDataBase().query(
								tableName, new String[] { "id" }, where, null,
								null, null, null);

						while (c.moveToNext())
							todays.add(c.getString(c
	...
	public Class<? extends Document<?>> getDocClass() {
		return docClass;
	}
