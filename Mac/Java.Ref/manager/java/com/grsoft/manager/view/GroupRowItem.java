package com.grsoft.manager.view;

import java.util.ArrayList;
import java.util.Date;
import java.util.List;

import android.app.Activity;
import android.content.Context;
import android.view.ContextMenu;
import android.view.View;
import android.widget.TextView;

import com.grsoft.database.DbReader;
import com.grsoft.database.DbWriter;
import com.grsoft.dataobjects.DataObjectInfo;
import com.grsoft.dataobjects.Division;
import com.grsoft.dataobjects.DivisionAgent;
import com.grsoft.manager.AgentData;
import com.grsoft.manager.Manager;
import com.grsoft.manager.R;
import com.grsoft.manager.ReportData;
import com.grsoft.napoleon.util.ProgressDrawable;
import com.grsoft.util.Consts;
import com.grsoft.util.Util;

public class GroupRowItem extends RowItem {
	public Division division;
	public String description;
	
	public List<DivisionAgent> agents;
	public List<DivisionAgent> allDivisionAgents;
	
	public static List<GroupRowItem> loadDivisions() {
		List<GroupRowItem> ret = new ArrayList<GroupRowItem>();	
		
		Division div = new Division();
		String table = DataObjectInfo.getInstance().getTableName(div.getClass());
		DbWriter.checkDBTable(div.getClass());
		DbReader r = new DbReader();
		boolean bdo = r.select(div, table, null);
		while(bdo) {
			GroupRowItem div_info = new GroupRowItem();
			div_info.division = (Division) div.clone();
			ret.add(div_info);
			div_info.agents = div.agents;
			div_info.allDivisionAgents = new ArrayList<DivisionAgent>();
			div_info.allDivisionAgents.addAll(div_info.agents);
			div = new Division();
			bdo = r.selectNext(div);
		}
		r.close();
		
		return ret;
	}	
	
	public static List<GroupRowItem> getHierarchicalDivisionList() {
		List<GroupRowItem> div_list = loadDivisions();
		
		GroupRowItem root = null;
		
		for(int i = 0; i <div_list.size(); i++) {
			if(div_list.get(i).division.parent == -1) {
				root = div_list.get(i);
				div_list.remove(root);
				break;
			}
		}
		
		List<GroupRowItem> ret = hierList(div_list, root == null || root.division == null ? "-1" : Integer.toString(root.division.id), 1);
		
		for(int i = 0; i < ret.size(); i++ ) 
			root.allDivisionAgents.addAll(ret.get(i).allDivisionAgents);
		
		if(root != null){
			root.setLevel(0);
			ret.add(0, root);
		}
		
		return ret;		
	}
	
	protected static List<GroupRowItem> hierList(List<GroupRowItem> lst, String start_with, int level) {
		
		List<GroupRowItem> subdivs = new ArrayList<GroupRowItem>();		
		
		for(int i = 0; i <lst.size(); i++) {
			GroupRowItem div = lst.get(i); 
			if(div.division.parent == (Integer.parseInt(start_with))) {
				div.setLevel(level);
				subdivs.add(div);
			}
		}
		
		List<GroupRowItem> ret = new ArrayList<GroupRowItem>();
		
		for(int i = 0; i < subdivs.size(); i++) {
			GroupRowItem div = subdivs.get(i);
			ret.add(div);
			lst.remove(div);
			List<GroupRowItem> hiersubdivs = hierList(lst,Integer.toString(div.division.id), level+1);
			
			for(int j = 0; j < hiersubdivs.size(); j++) 
				div.allDivisionAgents.addAll(hiersubdivs.get(j).allDivisionAgents);
			
			ret.addAll(hiersubdivs);
		}
		
		return ret;	
	}
	
	@Override
	protected int getViewId() {
		return R.layout.division_row;
	}
	
	@Override
	protected void adjustView(Context context, ReportData data, View view) {
		TextView tv;
		tv = (TextView)view.findViewById(R.id.tvName);
		tv.setText(division.name);			
		
		AgentData ad = data.getDivisionData(Integer.toString(division.id));
		
		String distance = "", orders = "", visits = "", sum = "", progress = "";
		if( ad != null ) {
			String km = context.getResources().getString(R.string.km);
			distance = Util.IntToScaleStr(ad.distance, 1) + " " + km;
			
			orders = Util.IntToScaleStr(ad.orders, 1);
			visits = Util.IntToScaleStr(ad.visits, 1);
			sum = Util.IntToScaleStr(ad.sum, Consts.SUM_SCALE, Util.DEC_DELIM, false);
			
			progress = ad.progress + "%";
		}
		
		((TextView)view.findViewById(R.id.tvDistance)).setText(distance);
		((TextView)view.findViewById(R.id.tvOrders)).setText(orders);
		((TextView)view.findViewById(R.id.tvVisits)).setText(visits);
		((TextView)view.findViewById(R.id.tvSum)).setText(sum);
		
		tv = (TextView)view.findViewById(R.id.tvProgress); 
		tv.setText(progress);
		if( ad != null )
			tv.setBackgroundDrawable(new ProgressDrawable(ad.progress));
		else
			tv.setBackgroundDrawable(null);
	}
	
	@Override
	public void runNapoleon(Context context) {}
	
	@Override
	public void adjustMenu(ContextMenu menu) {
		menu.findItem(R.id.itNapoleon).setVisible(false);
	}
	
	@Override
	public List<DivisionAgent> getAgents() {
		return allDivisionAgents;
	}
	
	@Override
	public void open(Activity activity, Date date) {
		Manager manager = (Manager) activity;
		manager.expandGroup(Integer.toString(division.id));
	}
	
	@Override
	protected void setBackground(int pos, View view) {
		view.setBackgroundResource(R.drawable.list_grey_selector );
	}
	
	@Override
	public String getTitle() {
		return division.name;
	}
}
