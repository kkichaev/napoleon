...
//#endif
      ...
      {
         const wchar_t* id = holder.Add(di->id);
         int index = dog.AddString(di->name);
         dog.SetItemData(index, (int)id);
         if( wcscmp(order->dogovor, id) == 0 )
            dog.SetCurSel(index);
      }
      ...
      {
         CutomCostImpl cc;
         SQLTable table(cc.Name());
         canChange = table.Select(&cc, L"id = userid");
      }
   ...
   LRESULT Closing(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
   {
      ...
      if( wID == IDOK )
      {
         ...
         int cs = dogs.GetCurSel();
         if( cs >= 0 )
            order->dogovor = order->holder.Add((const wchar_t*)dogs.GetItemData(cs));
