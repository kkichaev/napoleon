...
def loadData(server, params) -> Agents:
    ...
    select d.*, di.date_i as date_i from 
        (select distinct sd.userid, sd.created, sd.id, o.name, o.address as address2 from 
            ScriptDoc sd left join (select id, name, address from Org o union all select id, name, address from PotenzialOrg) o
			on sd.id = o.id ) d,
        (select di.ScriptDoc$userid as userid, di.ScriptDoc$created as created, di.state as state, 
            isnull(vi.date, di.date) date_i from ScriptDoc$items di left join Visit$items vi 
    ...
    for k, v in data.items():
        ...
        for d in v:
            if cdate == None or cdate != d.doc.created.date():
                ...
                head = ['Дата', 'Название ТТ', "Адрес ТТ", "Время создания сценария", "Время начала визвта в ТТ", 
