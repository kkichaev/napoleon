...
def printOut(params):
    ...
    row += 2
    xlb.makeHead(sheet, row, ['Подразделение / агент', 'визиты','заявки','сумма','сумма ПКО','Кол-во документов ПКО','процент заявок','прогресс'], True)
    
    for item in params.items:
        row += 1
        values = [item.name, item.visit, item.orders, item.sum, item.pkoSum, item.pkoCnt, calcOrderPercent(item.visit, item.orders), item.progress]
        xlb.makeCells(sheet, row, values)
        
        iprg = int(item.progress + 0.5)
        if iprg > 0 :
            color = Color.GREEN if iprg >= 67 else Color.YELLOW if iprg > 33 else Color.RED
    ...
    if len(params.items) > 0:
        row += 1
        item = params.items[0]
        values = ["Итого", item.visit, item.orders, item.sum, item.pkoSum, item.pkoCnt,  calcOrderPercent(item.visit, item.orders), item.progress] 
