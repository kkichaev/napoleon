...
class Item:
    ...
    def getData(self, row):
      ...
      return [self.aid, self.username, self.org_in_route, self.visit_in_route, self.visit_out_route, "=G{0}+H{0}".format(row), 
        self.incass_count, self.visit_doc_count, self.order_in_route, self.order_out_route,  self.order_sum, self.incass_sum,
         "=IFERROR((L{0} + M{0})/I{0},0)".format(row), 
...
def loadData(params, server):
    ...
    for u in params.userids:
      ...
      item.distance = computeDist(server, params, aid)
      if len(day_start) != 0:
         sumH = 0
         for val in day_start:
           sumH += val.hour * 3600 + val.minute * 60 + val.second
         item.start = datetime.datetime(2019, 1, 1, (int)((sumH / 3600) / len(day_start)), (int)((sumH % 3600 / 60)/ len(day_start)), (int)((sumH % 60)/len(day_start)))

         sumH = 0
         for val in day_finish:
           sumH += val.hour * 3600 + val.minute * 60 + val.second
         item.finish = datetime.datetime(2019, 1, 1, (int)((sumH / 3600) / len(day_finish)), (int)((sumH % 3600 / 60)/ len(day_finish)), (int)((sumH % 60)/len(day_finish)))
...
def printOut(data, params):
    ...
    head = ['Дата начала', 'Дата конца', 'филиал', 'ИД', 'ТП', 'Итого точек в маршруте', 'По маршруту посещений', 'Не по маршруту посещений', 'Итого посещений', 
      'ПКО', 'Фотоотчеты', 'Заявок по маршруту', 'Заявок не по маршруту', "Сумма заявок", "Сумма ПКО", "% заявок", 
    ...
    for item in data.data:
      values = [params.start.strftime("%d.%m.%Y"), params.finish.strftime("%d.%m.%Y"), data.rootDivision]
    ...
    cc = 1
    SZ = 12
    for w in [SZ,SZ,SZ,SZ,26,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ,SZ]:
...
def run(server):
    ...
    XLBuilderCommon().workbookToObject(wb, "work_report.xlsx", server)                
    logging.info('end')
