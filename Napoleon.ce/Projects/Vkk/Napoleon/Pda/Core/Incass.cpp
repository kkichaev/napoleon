...
LRESULT IncassForm::Sending(WORD nCode, WORD id, HWND hWnd, BOOL &bHandled)
{
   ...
   if( SendDocument(data->doc, docTypeManager.GetDocType(dtIncass), L"Документ отправлен") )
   {
...
void IncassForm::UpdateLayout(bool forceRecalc)
{
   ...
   CWindow btn(GetDlgItem(IDC_DIG_1));
   btn.GetWindowRect(bounds);
   ScreenToClient(bounds);

   int btnWdh = bounds.Width();
   int btnLeft = rc.right - 4 - btnWdh*3;
   int btnTop = top;

   int digs[4][3] = { 
      { IDC_DIG_1, IDC_DIG_2, IDC_DIG_3}, 
      { IDC_DIG_4, IDC_DIG_5, IDC_DIG_6},
      { IDC_DIG_7, IDC_DIG_8, IDC_DIG_9},
      { IDC_DIG_PT, IDC_DIG_0, IDC_DIG_BS},
   };
   for( int i=0; i<4; i++ )
   {
      int l = btnLeft;
      for( int j=0; j<3; j++ )
      {
         CWindow b(GetDlgItem(digs[i][j]));
         b.MoveWindow(l, btnTop, btnWdh, btnWdh);
         l += btnWdh-1;
      }
      btnTop += btnWdh-1;
   }
   ...
   w.MoveWindow(2, top, btnLeft - 4, rc.bottom - 2 - top, FALSE);
