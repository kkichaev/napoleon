...
bool OrderImpl::Init(const ROWID &orgID)
{
...
#ifdef BLOCK_IN_STOP_LIST
   if((org.flags & ofStopList) != 0)
   {
      MessageBox(NULL, L"Клиент находится в стоп-листе, заявка не будет обработана", L"Ошибка", MB_OK | MB_ICONEXCLAMATION);
      return false;
   }
#elif STOP_LIST
   if((org.flags & ofStopList) != 0)
   ...
   *(__int64*)&date += (__int64)10000000 * 3600 * 24;
   //if( st.wDayOfWeek == 5 )
   //   *(__int64*)&date += (__int64)10000000 * 3600 * 48;
   //if( st.wDayOfWeek == 6 )
   //   *(__int64*)&date += (__int64)10000000 * 3600 * 24;
