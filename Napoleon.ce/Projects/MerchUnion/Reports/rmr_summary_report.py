...
class Item:
  __slots__ = ['id', 'name', 'visit', 'fake', 'order', 'sum', 'dist', 'progress', 'order_progress']
  ...
  def __str__(self):
    return 'id: {0}, visit: {1}, order: {2}, sum: {3}, dist: {4}, progress: {5}, order_progress: {6}, fake: {7}'.format(self.id, self.visit, self.order, self.sum, self.dist, self.progress, self.order_progress, self.fake)
    
  def getData(self):
    return [self.name, self.visit, self.fake, self.order, self.sum, self.order_progress, self.progress]
...
def loadData(params, server):
  ...
  while curDate < endDate: 
    ...
    for key, value in dayRep.items():
      ...
      q = '"fake" = 1 and "created" >= ToDate("{0}") and "created" <= ToDate("{1}") and "userid" = "{2}"'.format(
        curDate.strftime("%d/%m/%Y 0:0:0"),
        curDate.strftime("%d/%m/%Y 23:59:59"),
        key)
      
      scripts = server.Get('ScriptDoc',q)
...
def printOut(data, params):
  ...
  head = ['Подразделение / агент', 'визиты', 'на основании', 'заявки', 'сумма', 'процент заявок', 'прогресс']
