...
class Item:
  __slots__ = ["address", "org", "created", "user", "items", "orgcode", "orgData", "formatTT"]
  ...
  def values(self):
    ...
    res = [self.org, self.orgcode, coord, self.address, self.formatTT, self.created.strftime('%d.%m.%Y'), self.user]
  ...
  def loadDocs(self, docs, pics, href):
    for d in docs:
      ...
      if d.id in self.orgs:
        ...
        i.formatTT = self.orgs[d.id].formatTT
...
def printOut(data, params):
    ...
    r = 2
    arr = ["Наименование", "Код ТТ", "Координаты", "Адрес", "Формат ТТ", "Дата", "Торговый представитель"]
