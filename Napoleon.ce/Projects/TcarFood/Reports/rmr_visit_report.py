    ...
    while date <= params.finish:
      ...
      for dt in docTypes:
        ...
        for d in docs:
          ...
          item.orgname = "{0}, {1}".format(orgs[d.id].name, orgs[d.id].address) if d.id in orgs else "Контрагент с кодом <{0}>".format(d.id)
          ...
          if dt.objectName == "Incass":
            incass_remakr = ""

            for i in d.items:
              if len(incass_remakr) > 0:
                incass_remakr = incass_remakr + ", "

              incass_remakr = incass_remakr + i.number
              
            if len(incass_remakr) > 0:
               incass_remakr = incass_remakr + " "

            incass_remakr = incass_remakr + d.remark
               
            item.remark = incass_remakr
