...
{
   virtual IDocument* Create() const { return new ControlDocImpl(); }
   virtual void Free(IDocument* document) const { delete (ControlDocImpl*)document; }
} orderFactory;
...
public:
   ...
   virtual const wchar_t* Name() const { return L"ControlDoc"; }
...
SQLTable* OrderPHandler::Prepare(const OrderProceeded& data)
{
   ...
   std::wstring tname(ControlDocImpl().Name());
