...
def loadData(server, params) -> Agents:
    ...
    select d.*, di.date_i as date_i from 
        (select distinct d.userid, d.created, d.name as agent, o.name, o.city, o.address2 from 
            (ScriptDoc left join Agents on scriptdoc.userid = agents.id) d, Org o where d.id = o.id) d,
    ...
    docs = server.Query(stmt, "Docs[name:s,agent:s,created:dt,city:s,address2:s,docs(userid,created)[date@date_i:dt]]")
