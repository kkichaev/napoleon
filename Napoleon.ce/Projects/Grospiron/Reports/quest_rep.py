...
class Item:
  ...
  def values(self):
    ...
    if self.orgData != None :
      city = self.orgData.city
      self.address = self.orgData.address2
      brand = self.orgData.brand
    ...
    res = [self.org, city, brand, self.address, self.created.strftime('%d.%m.%Y'), self.user]
...
def printOut(data, params):
    ...
    r = 2
    arr = ["Наименование", "Город", "Сеть", "Адрес", "Дата", "Торговый представитель"]
