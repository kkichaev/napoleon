...
bool SyncOrgRemnants::SetToDB(DataForm *db, const IReflectableData &data) const
{
   ...
   char buf[200];
   const OrgRemnantsSend &remnants = (const OrgRemnantsSend &)data;
   SYSTEMTIME st;
   ...
   const char *flags = (remnants.flags & orfCall) ? "внеплановый звонок" : 
      (remnants.flags & orfMeet) ? "внеплановая встреча" :
      (remnants.flags & orfOther) ? "иное" :
      "";
   ...
   {
      vector_t<OrgRemnantsItemSend>::const_iterator i = remnants.items.begin();
