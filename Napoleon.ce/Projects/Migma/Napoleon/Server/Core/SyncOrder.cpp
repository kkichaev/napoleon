...
bool SyncOrder::SetToDB(DataForm *db, const IReflectableData &_data) const
{
...
#ifdef GPS_POS
   db->Fill("LNG", (double)data.longitude / GPS_SCALE);
   db->Fill("LAT", (double)data.latitude / GPS_SCALE);
#endif

   {
      ...
      if( data.params & ofBankPay ) bn = 'Ѓ';

      char ttbuf[100];
      wsprintf(ttbuf, "%d%c%02d.%d=%c=%02d.%02d.%d=%02d:%02d", data.sumType + 1, sign, discount/DISCOUNT_SCALE, 
         discount % DISCOUNT_SCALE, bn, st.wDay, st.wMonth, st.wYear, st.wHour, st.wMinute);

      db->Fill("COSTTYPE", ttbuf);
   }
