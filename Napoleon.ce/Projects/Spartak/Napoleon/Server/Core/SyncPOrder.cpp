...
bool SyncOrder::SetToDB(DataForm *db, const IReflectableData &_data) const
{
   ...
   db->Fill("SUM", sum);
   db->Fill("ORDTYPE", ((data.params & ofCash) != 0) ? 1 : 0);
   db->Fill("REMARK", W2A_CP(data.remark, CP_OEMCP));

   db->Fill("FIRMA", W2A_CP(data.supplCode, CP_OEMCP));
   db->Fill("CTYPE", W2A_CP(data.costCode, CP_OEMCP));
   db->Fill("DOGOVOR", W2A_CP(data.dogovor, CP_OEMCP));
