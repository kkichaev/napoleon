...
struct PriceFormDataAdd : public PriceFormData
{
   ...
   ~PriceFormDataAdd() { if( order ) order->whIndex = currentWh; }
   ...
   {
      saveWh = newWh;
      PriceFormData::SetWarehous(newWh);
      if( order )
         order->whIndex = newWh;
   }
   ...
   {
      wchar_t buf[200];
      wsprintf(buf, L" AND (collectionValue(qty, 'QtyItem', %d, 'qty') > 0 OR (flags & 1) = 0)", currentWh);
      *result = whereStr + buf;
   }
...
bool PriceFormDataAdd::FindItemsByCode(const wchar_t *text)
{
   ...
   searchCode.Search(&leafs, text, &wstr);
   return true;
}
