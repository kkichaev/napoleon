...
def printOut(params):
    ...
    row += 2
    xlb.makeHead(sheet, row, ['Подразделение / агент', 'визиты','заявки','сумма','прогресс'])
    for item in params.items:
        row += 1
        values = [item.name, item.visit, item.orders, item.sum, item.progress]
        xlb.makeCells(sheet, row, values)

    if len(params.items) > 0:
        row += 1
        item = params.items[0]
        values = ["Итого", item.visit, item.orders, item.sum, item.progress] 
