...
from manager.document import Order

import tempfile
import io
import manager.coordutils
...
def getDivisionAgents(server, division, agents, divisions):
    ...
    for d in division :
        if divisions != None :
            dd = DivisionData()
            dd.load(d)
            # пїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅпїЅ пїЅпїЅпїЅпїЅпїЅпїЅпїЅ parent
            if len(divisions) == 0 :
                dd.parent = -1
            divisions.append(dd)
...
def makeQuery(agentQuery, for_date, suppl):
    filter = '"created" in (select sdi."date" from "ScriptDoc$items" sdi INNER JOIN "ScriptDoc" sd on '\
             'sdi."ScriptDoc$userid" = sd."userid" and sdi."ScriptDoc$created" = sd."created" where sd."created" >= ToDate("{0}") and '\
             'sd."created" < ToDate("{1}") and sd."scriptId" in (select "id" from "ScriptDef" where "suppl" = \'{2}\')) and {3}'.format(
                for_date.strftime("%d/%m/%Y 0:0:0"), 
                for_date.strftime("%d/%m/%Y 23:59:59"), suppl, agentQuery);

    return filter;

def dailyReport(server, agents_list, for_date, suppl):
    ...
    where = makeQuery(agentQuery,for_date,suppl)
    
    for dt in docTypes:
        
        docs = dt.docList(server, where)
        
        #print 'docType:', dt.objectName
...
def computeDist(server, curdate, userid):
    ...
    for pos in gpspos:
        if lastpos == None:
            lastpos = pos
            continue
        distance += manager.coordutils.distance(lastpos.latitude, lastpos.longitude, pos.latitude, pos.longitude)
...
def run(server):
   ...
   #curDate = datetime.strptime('01032015', "%d%m%Y")
    ...
    while curDate < endDate: 
        dayRep = dailyReport(server, divAgents, curDate, divMgr[0].suppl)
        for key, value in dayRep.items():
            ...
            if len(value.plan) > 0:
                ...
                obj.order_progress = (value.plannedOrders / float(len(value.plan)) * 100)
            else:
                obj.progress = 0
