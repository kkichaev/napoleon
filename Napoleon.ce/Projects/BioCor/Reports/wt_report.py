...
def doReportMin(sheet, start, stop, agents):
    ...
    sheet.cell(row=sr, column=4).value = "время работы"
    ...
    for a in agents:
        ...
        if rd.fullTime != None:
            sheet.cell(row=sr, column=4).value = timeDeltaToHuman(rd.fullTime)    
        
        if rd.orgTime != None:
            sheet.cell(row=sr, column=5).value = timeDeltaToHuman(rd.orgTime)
    ...
    if fullWt != None:
        sheet.cell(row=sr, column=4).value = timeDeltaToHuman(fullWt)
        
    if orgTime != None:    
        sheet.cell(row=sr, column=5).value = timeDeltaToHuman(orgTime)
...
def doReportByDay(sheet, start, stop, agents):
    ...
    while start <= stop:
        ...
        sheet.cell(row=sr, column=6).value = "время работы"
        ...
        for a in agents:
            ...
            if rd.fullTime:
                sheet.cell(row=sr, column=6).value = timeDeltaToHuman(rd.fullTime)    
            
            if rd.orgTime != None:
                sheet.cell(row=sr, column=7).value = timeDeltaToHuman(rd.orgTime)
        ...
        if fullWt != None:
            sheet.cell(row=sr, column=6).value = timeDeltaToHuman(fullWt)
            
        if orgTime != None:    
            sheet.cell(row=sr, column=7).value = timeDeltaToHuman(orgTime)
...
def timeDeltaToHuman(tf):
    h = tf.total_seconds() / 3600
    m = (tf.total_seconds() % 3600) / 60
    return '{0:0>2} : {1:0>2}'.format(int(h),int(m))      
...
def run(server):
    ...
    if param.divisionID != 0 :
        ...
        col_letter = get_column_letter(4) 
        sheet.column_dimensions[col_letter].width = 12 
        sheet.cell(row=sr, column=5).value = "в первой точке"
        col_letter = get_column_letter(6) 
        sheet.column_dimensions[col_letter].width = 17
        sheet.cell(row=sr, column=6).style.alignment.wrap_text = True
        sheet.cell(row=sr, column=6).value = "в последней точке"
        col_letter = get_column_letter(7) 
        sheet.column_dimensions[col_letter].width = 17
        sheet.column_dimensions[col_letter].width = 17 
        sheet.cell(row=sr, column=7).style.alignment.wrap_text = True
        sheet.cell(row=sr, column=7).value = "время работы"
        col_letter = get_column_letter(8) 
        sheet.column_dimensions[col_letter].width = 11
        sheet.cell(row=sr, column=8).style.alignment.wrap_text = True
        sheet.cell(row=sr, column=8).value = "время в точках"
        ...
        for a in al:
            ...
            while(b <= param.end):
                ...
                if start != None:
                    ...
                    sheet.cell(row=sr, column=5).value = start.strftime("%d/%m/%Y %H:%M")
                      
                if stop != None:
                    sheet.cell(row=sr, column=6).style.font.color.index = color
                    sheet.cell(row=sr, column=6).value = stop.strftime("%d/%m/%Y %H:%M")
                     
                if start != None and stop != None:
                    sheet.cell(row=sr, column=7).style.font.color.index = color
                    sheet.cell(row=sr, column=7).value = timeDeltaToHuman(fwt)
                    
                if tio != None:
                    sheet.cell(row=sr, column=8).style.font.color.index = color
                    sheet.cell(row=sr, column=8).value = timeDeltaToHuman(tio)
                    
                borders(sheet, sr, 0, 9)  
