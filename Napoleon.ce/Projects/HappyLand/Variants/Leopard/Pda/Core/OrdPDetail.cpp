...
protected:
   LRESULT OnInitDialog(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
   {
      ...
      SQLTable ft(f.Name());
      bdo = ft.Select(&f, L"ORDER BY name");
      while( bdo )
      {
      ...
      config.ReadValue(&val, L"ТипЗаявки");
      LoadComboboxWithCode(val, IDC_ORDER_INFO, order->ordType, L'\t', &holder);

      SkladImpl sklad;
      CComboBox scb(GetDlgItem(IDC_WAREHOUS));
      SQLTable skt(sklad.Name());
      bdo = skt.Select(&sklad, L"ORDER BY name");
      while( bdo )
      {
         int index = scb.AddString(sklad.name);
         scb.SetItemDataPtr(index, holder.Add(sklad.id));
         if( !wcscmp(order->whCode, sklad.id) )
            scb.SetCurSel(index);
         bdo = skt.SelectNext(&sklad);
      }
   ...
   LRESULT OnSizeChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
   {
      ...
      GetDlgItemRect(bounds2, IDC_WAREHOUS);
      GetDlgItem(IDC_WAREHOUS).MoveWindow(bounds.left, bounds2.top, wdh - bounds.left - offset, bounds.Height());

      GetDlgItemRect(bounds2, IDC_ORDER_INFO);
      GetDlgItem(IDC_ORDER_INFO).MoveWindow(bounds.left, bounds2.top, wdh - bounds.left - offset, bounds.Height());
      return 0;
   }
   ...
   LRESULT Closing(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
   {
      ...
      if( wID == IDOK )
      {
         ...
         if( cs >= 0 )
         {
            const wchar_t* code = (const wchar_t*)fcb.GetItemData(cs);
         ...
         CComboBox otcb(GetDlgItem(IDC_ORDER_INFO));
         cs = otcb.GetCurSel();
         if( cs >= 0 )
         {
            const wchar_t* code = (const wchar_t*)otcb.GetItemData(cs);
            if( wcscmp(code, order->ordType) )
               order->ordType = order->holder.Add(code);
         }

         CComboBox wcb(GetDlgItem(IDC_WAREHOUS));
         cs = wcb.GetCurSel();
         if( cs >= 0 )
         {
            const wchar_t* code = (const wchar_t*)wcb.GetItemData(cs);
            if( wcscmp(code, order->whCode) )
               order->whCode = order->holder.Add(code);
         }
