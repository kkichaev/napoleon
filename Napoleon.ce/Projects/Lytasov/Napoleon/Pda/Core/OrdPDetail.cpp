...
//#endif
      ...
      val.clear();
      config.ReadValue(&val, L"Склады");
      LoadCombobox(val, IDC_MULTIWH, order->whIndex);
   ...
   LRESULT OnSizeChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
   {
      ...
      GetDlgItemRect(bounds, IDC_MULTIWH);
      GetDlgItem(IDC_MULTIWH).MoveWindow(bounds.left, bounds.top, wdh - bounds.left - offset, bounds.Height());
   ...
   LRESULT Closing(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL& bHandled)
   {
      ...
      if( wID == IDOK )
      {
         ...
         order->whIndex = ((CComboBox)GetDlgItem(IDC_MULTIWH)).GetCurSel();
