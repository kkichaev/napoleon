...
def loadData(data, params, server):
  if len(params.userids) > 0:
    ...
    for ido in orgs:
      ...
      item.faceAll = o.faceAll
      item.faceCoordiant = o.faceCoordiant
      item.contactName = ''
      item.staffPosition = ''
      item.contactPhone = ''
      ...
      if len(o.contacts) > 0:
        ...
        item.staffPosition = staffPosition[c.staffPositionID].name if c.staffPositionID in staffPosition else "Должность с кодом <{0}>".format(c.staffPositionID)
...
def printOut(data, params):
    ...
    r = 0
    arr = ["Код РТТ","Юр. название РТТ", "Факт. название РТТ", "Область / край", "Город / поселок", "Улица, дом", "Телефон РТТ", "Сайт", "Тип РТТ", "Принадлежность РТТ",
      "Средний объем продаж РТТ в год, шт", "Доля Cordiant в общем объеме, %", "Общее кол-во фейсов на основном месте продаж, шт", "Фейсинг Cordiant, шт", "Контактное лицо ФИО",
      "Должность", "Телефон"]
