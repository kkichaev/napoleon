      ...
      for di in d.items:
        if di.id in price:
          ...
          if p.docFilter == 1 and p.season == 2: # Лето
            putRemnant(data, 0, d, di)
            
          if p.docFilter == 1 and p.season == 2 and p.keySKU == 1: # KEY Лето
            putRemnant(data, 1, d, di)  
            
          if p.docFilter == 1 and p.season == 2 and (p.diameter == 17 or p.diameter == 18): # 17-18 дюйм Лето 
            putRemnant(data, 2, d, di)   

          if p.docFilter == 1 and p.season == 0: # Зима
            putRemnant(data, 3, d, di)
            
          if p.docFilter == 1 and p.season == 0 and p.keySKU == 1: # KEY Зима
            putRemnant(data, 4, d, di)  
            
          if p.docFilter == 1 and p.season == 0 and (p.diameter == 17 or p.diameter == 18): # 17-18 дюйм Зима
            putRemnant(data, 5, d, di)
              
          if p.docFilter == 1 and p.autoType == 'ЛГШ': # ЛГШ
            putRemnant(data, 6, d, di)    
  return data
  
def putRemnant(data, idx, d, di):
  monthFields = ["month1", "month2", "month2"]
  
  ai = data.items[idx].items[d.userid]
  m = (d.created.month - 1) % 3
  ...
  om[di.id].append(di.qty + di.face)
