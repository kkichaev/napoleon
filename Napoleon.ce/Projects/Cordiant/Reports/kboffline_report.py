...
def loadData(data, params, server):
  ...
  for u in params.userids:
      ...
      if r.id in orgs:
        ...
        for ri in r.items:
          if ri.id in price:
            ...
            if (ri.qty > 0 or ri.face > 0) and p.docFilter == 1 and not r.id in akb[org.typepttID]:
              akb[org.typepttID].append(r.id)
              
            if (ri.qty > 0 or ri.face > 0) and p.docFilter == 1 and p.diameter in [17,18] and not r.id in d17_18[org.typepttID]:
              d17_18[org.typepttID].append(r.id)
              
            if (ri.qty > 0 or ri.face > 0) and p.docFilter == 1 and p.season == 2 and not r.id in akb_summer[org.typepttID]:
              akb_summer[org.typepttID].append(r.id)  
              
            if (ri.qty > 0 or ri.face > 0) and p.docFilter == 1 and p.season == 2 and p.diameter in [17,18] and not r.id in d17_18_summer[org.typepttID]:
              d17_18_summer[org.typepttID].append(r.id)  
  
            if (ri.qty > 0 or ri.face > 0) and p.docFilter == 1 and p.season == 0 and not r.id in akb_winter[org.typepttID]:
              akb_winter[org.typepttID].append(r.id)  
            
            if (ri.qty > 0 or ri.face > 0) and p.docFilter == 1 and p.season == 0 and p.diameter in [17,18] and not r.id in d17_18_winter[org.typepttID]:
              d17_18_winter[org.typepttID].append(r.id)
              
            if (ri.qty > 0 or ri.face > 0) and p.docFilter == 1 and p.autoType == 'ЛГШ' and not r.id in lgsh[org.typepttID]:
    ...
    for pt in data.items:
      ...
      if aid in pti.items:
        ai = pti.items[aid]
        ai.kb = countByType(pt, orgs.values())
        ai.akb = 0
