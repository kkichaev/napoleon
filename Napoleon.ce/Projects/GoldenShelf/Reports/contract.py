...
class PageItem:
    ...
    def post_process(self, c, report):
        self.partshelf = report.getPartShelf(c.id)
        ...
        all = my + other
        onepercent  = all / 100
        self.fact = "{:0.0f}".format(my / onepercent if onepercent != 0 else 0 )
...
class Report:
    ...
    def __init__(self, server):
        ...
        self.photo = server.Params[0].photo
        ...
        if self.photo == 1:
            self.visit = server.Get("Visit", where)
        else:
            self.visit = server.Get("VisitInfo", where)
