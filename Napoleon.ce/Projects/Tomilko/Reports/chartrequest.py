...
def run(server):
    ...
    data = sorted(map.values(), cmp=lambda x,y: cmp(y.qty,x.qty))
    data = data[:5]
    ...
    data = sorted(mapSum.values(), cmp=lambda x,y: cmp(y.date,x.date))
    ...
    obj = output.New()
    obj.id = "1"
    obj.alldoc = (odcnt / len(orgdict)) * 100 if len(orgdict) > 0 else 0
    obj.inroute = (rtcnt / len(routedict)) * 100 if len(routedict) > 0 else 0
