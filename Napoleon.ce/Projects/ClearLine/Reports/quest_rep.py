...
class ReportData:
  __slots__ = ['items', 'quests', 'orgs', 'agents', "cellidx", 'photos', 'orgLocation']
  KEY_FMT =  "{0}\t{1}"
  ITEM_KEY_FMT = "{0}\t{1}\t{2}"
  ...
  def loadDocs(self, docs, pics, href):
    for d in docs:
      ...
      if org != None:
        ...
        if d.id in self.orgLocation:
          loc = self.orgLocation[d.id]
          i.orgData.latitude = loc.latitude
          i.orgData.longitude = loc.longitude
...
def loadData(data, params, server):
    ...
    data.orgLocation = server.Get('OrgLocation', '', 'id')
