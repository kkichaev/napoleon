...
static long WriteBillRecord(DataForm &bill, long faceRecNo, long emplRecNo, const OrderSend &item, 
                            int orderNo, const NapoleonConfig &config, int code, const char *userName, const char *userID)
{
   USES_CONVERSION;
...
bool SyncOrder::SetToDB(DataForm *db, const IReflectableData &_data) const
{
   ...
   int orderRec = WriteBillRecord(*db, orgRec, emplRec, order, db->GetRecNo(), config, code, UserName(ep, userID), userID);
