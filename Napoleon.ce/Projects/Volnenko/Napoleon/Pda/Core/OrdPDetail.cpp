...
//#ifdef COST_MANAGER
//      const CostList& cl = CostManager::CostTypes();
//      CComboBox ccb(GetDlgItem(COST_TYPE));
//      CostList::const_iterator ci = cl.begin();
//      for( ; ci != cl.end(); ci++ )
//      {
//         int index = COST_TYPE.AddString(ci->name.c_str());
//         fcb.GetItemDataPtr(index, holder.Add(ci->id));
//         if( !wcscmp(order->supplyer, f.id) )
//            fcb.SetCurSel(index);
//         bdo = ft.SelectNext(&f);
//      }
//#else
...
#endif
      // убираем себестоимость из типа цены
      CComboBox cbBox(GetDlgItem(IDC_COST_TYPE));
      cbBox.DeleteString(0);
      if( order->sumType == 0 )
         cbBox.SetCurSel(0);
   ...
   LRESULT OnSizeChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
   {
      ...
      GetDlgItem(IDC_COST_TYPE).MoveWindow(bounds.left, bounds.top, wdh - bounds.left - 2 * offset, bounds.Height());
      return 0;
   }
