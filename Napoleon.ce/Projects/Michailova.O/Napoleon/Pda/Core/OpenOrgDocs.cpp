...
static ListFormData::Header header[] = 
{
   ...
   { ListFormData::Header::Right, L"Дней", L"date", 50 },
...
BEGIN_TYPE_REFLECTION(PayItem)
   ...
   REGISTER_LONG_SCALE_MEMBER2(PayItem, date, 100, true)
...
COLORREF PayData::GetItemColor(int index, COLORREF defaultColor) const
{
   ...
   //   {
   //      const Delivery* dlv = (Delivery*)((BalanceDoc*)doc)->Data();
...
bool PayData::Get(IReflectableData* data, int index) const
{
   ...
   if( ((BalanceDoc*)doc)->isDelivery )
   {
      const DeliveryImpl* dlv = (DeliveryImpl*)((BalanceDoc*)doc)->Data();
      ...
      {
         __int64 res = *(__int64*)&dlv->payDate - *(__int64*)&ft;
         res /= ((__int64)10000000 * 3600 * 24);
         date = (int) res;
      }
