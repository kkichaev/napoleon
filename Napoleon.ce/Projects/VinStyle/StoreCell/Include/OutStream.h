...
public:
   ...
   void Append(const wchar_t* tstr) { str.append(tstr); }
   void Append(wchar_t tsym) { str.append(1, tsym); }
   ...
   const std::wstring& ToString() const { return str; }

   void Clear() { str.clear(); needCompress = true; }

   DWORD Size() const { return str.size() * sizeof(*str.c_str()); }
