...
//----------------------------------- OrderListData ----------------------------------
//
OrderListData::OrderListData() : current(NULL), inputRack(false), blocked(false), qtyDlg(NULL), selectedItem(-1)
{
...
bool OrderListData::NewItem()
{
   ...
   //if( curItem.compare(code) != 0 && !CheckCurItem() )
   //   return false;
...
bool OrderListData::IsHavePallet(const std::wstring& barCode) const
{
	...
	for( ; i != current->items.end(); i++ )
	{
		if( wcscmp(i->barcode, barCode.c_str()) == 0 )
			return true;
	}
...
bool OrderListData::Get(IReflectableData* data, int index) const
{
	...
	((OrderListItem*)data)->item = price.name;
   ((OrderListItem*)data)->qty = oi->qty;
...
void OrderListForm::OnBarcode(const wchar_t* _barcode)
{
   ...
   bool refresh = false;
	...
	{
		if( ((OrderListData*)data)->CheckPallet(barcode) )
		{
			Refresh();
		} else {
