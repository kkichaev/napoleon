...
int Application::ReceiveData(std::wstring *answer, IProgressIndicator *pi, bool clearBase, bool fullExchange)
{
   ...
   ConfigRcvr ci;
	...
	SyncConfigImpl syncConfig;
	syncConfig.key = (fullExchange) ? L"ПолныйОбмен" : L"ЧастичныйОбмен";
	bool readConfig = syncConfig.Read();

	DBObjAliasRcvr<SyncConfigImpl> synci(IDS_PROCESS_SETTINGS, false, L"SyncConfig");
   params.objects.push_back(&synci);
	DBObjAliasRcvr<ConfigImpl> sci(IDS_PROCESS_SETTINGS, false, L"ServerConfig");
   params.objects.push_back(&sci);
   DBObjAliasRcvr<ConfigImpl> uci(IDS_PROCESS_SETTINGS, false, L"UserConfig");
   params.objects.push_back(&uci);

	DBObjAliasRcvr<WhAgentsImpl> agents(IDS_AGENTS, true, L"WhAgents");
   params.objects.push_back(&agents);

	DBObjAliasRcvr<DocTypeImpl> doctypes(IDS_DOC_TYPES, true, L"WhDocType");
   params.objects.push_back(&doctypes);
	
	DBObjAliasRcvr<WhSkladImpl> whsmi(IDS_PRICE, true, L"WhSklad");
	if(!readConfig || wcsstr(syncConfig.value, L"SKLADS") != NULL)
	{
	//MessageBox(NULL, syncConfig.value, syncConfig.key, MB_OK);
		params.objects.push_back(&whsmi);
	}

	DBObjAliasRcvr<WhSkaldRestImpl> whskRest(IDS_PRICE, true, L"WhSkladRest");
	if(!readConfig || wcsstr(syncConfig.value, L"SKLREST") != NULL)
	{
		params.objects.push_back(&whskRest);
	}

	DBObjAliasRcvr<SkMarksImpl> smi(IDS_PRICE, false, L"SkMarks");
	if(!readConfig || wcsstr(syncConfig.value, L"MARKS") != NULL)
	{
		params.objects.push_back(&smi);
	}

	DBObjAliasRcvr<PriceImpl> pirce(IDS_PRICE, false, L"WhCellPrice");
	if(!readConfig || wcsstr(syncConfig.value, L"PRICE") != NULL)
	{
		params.objects.push_back(&pirce);
	}

	DBObjAliasRcvr<ControlDocImpl> ctrlDocs(IDS_CTRL_DOCS, false, L"ControlDoc");
	if(!readConfig || wcsstr(syncConfig.value, L"CONTROL") != NULL)
	{
		params.objects.push_back(&ctrlDocs);
	}

	DBObjAliasRcvr<PalletsImpl> pallets(IDS_PALLETS, true, L"Pallets");
	if(!readConfig || wcsstr(syncConfig.value, L"PALLETS") != NULL)
	{
		params.objects.push_back(&pallets);
	}	

	DBObjAliasRcvr<PartCodeImpl> parties(IDS_PALLETS, true, L"PartCode");
	if(!readConfig || wcsstr(syncConfig.value, L"PARTIES") != NULL)
	{
		params.objects.push_back(&parties);
	}	

	DBObjAliasRcvr<ItemGroupsImpl> groups(IDS_PALLETS, true, L"Groups");
	if(!readConfig || wcsstr(syncConfig.value, L"GROUPS") != NULL)
	{
		params.objects.push_back(&groups);
	}	
