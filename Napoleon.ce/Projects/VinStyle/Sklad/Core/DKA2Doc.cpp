...
struct DKA2DocItem : public IReflectableData
{
   wchar_t* item;
   DWORD qty;
   DECLARE_TYPE_REFLECTION(DKA2DocItem)
};

BEGIN_TYPE_REFLECTION(DKA2DocItem)
   REGISTER_STRING_MEMBER(DKA2DocItem, item)
   REGISTER_ULONG_SCALE_MEMBER2(DKA2DocItem, qty, QTY_SCALE, true)
END_TYPE_REFLECTION(DKA2DocItem)
...
#if ZEBEX
class DKA2DocForm : public ListForm, public BarcodeHandler//, public CCustomDraw<MovmentDocForm>
#else
class DKA2DocForm : public ListForm//, public CCustomDraw<DKA2DocForm>
#endif
{
public:
	DKA2DocForm() {}
   ...
   DECLARE_FORM(DKA2DocForm, IDD_DKA2_DOC)
...
IMPLEMENT_FORM(DKA2DocForm);

//
//----------------------------------- MovmentDocData ----------------------------------
//
DKA2DocData::DKA2DocData(const wchar_t *doc) : current(NULL)
{
...
void DKA2DocData::RefreshItems()
{
   ...
   {
		std::vector<std::wstring> grpItems;

		ItemGroupsImpl igi;
		igi.LoadPriceItems(&grpItems, i->id);

		std::vector<std::wstring>::const_iterator gi = grpItems.begin();
		for( ; gi != grpItems.end(); gi++ )
		{
			std::map<std::wstring, Data*>::iterator fnd = imap.find(*gi);
			if( fnd == imap.end() )
			{
				Data d;
				d.id = *gi;
				...
				imap.insert(std::map<std::wstring, Data*>::value_type(*gi, src));
			} else {
				fnd->second->qty++;
			}
		}
...
bool DKA2DocData::NewItem(const wchar_t* code)
{
	if( current->HaveItem(code) )
		return false;
	...
	ItemGroupsImpl ig;
	ig.barcode = (wchar_t*)code;
	if( !ig.Read() )
		return false;

	OrderItem oi;
	oi.id = current->holder.Add(code);
	oi.qty = QTY_SCALE;
...
bool DKA2DocData::New(const wchar_t* doc, DKA2DocData::ItemType type)
{
   bool ret = false;
...
bool DKA2DocData::NewDoc(const wchar_t* doc)
{
   if( current == NULL )
   {
      current = new DKA2Impl();
   } else
   {
...
void DKA2DocData::GetDocTitle(std::wstring* text) const
{
   text->clear();
   if( current != NULL )
...
void DKA2DocData::GetPackTitle(std::wstring* text) const
{
   text->clear();
   if( current != NULL )
...
bool DKA2DocData::Get(IReflectableData* data, int index) const
{
	if( index < 0 || (unsigned)index >= items.size() )
      return false;

	const Data* oi = &items.at(index);
	price.id = (wchar_t*)oi->id.c_str();
	price.Read();

	((DKA2DocItem*)data)->item = price.name;
   ((DKA2DocItem*)data)->qty = oi->qty;
...
//-------------------------------- MovmentDocForm --------------------------------
//
bool DKA2DocForm::SetDataEx(IFormData *_data, int scale)
{
   ...
   wchar_t text[MAX_PATH];
	((DKA2DocData*)data)->GetDocTitle(&tstr);
	wsprintf(text, L"Документ: %s", tstr.c_str());
	SetDlgItemText(IDC_DOC, text);

	((DKA2DocData*)data)->GetPackTitle(&tstr);
...
LRESULT DKA2DocForm::OnInputBC(WORD nCode, WORD id, HWND hWnd, BOOL &bHandled)
{
   BCDialog dlg;
...
void DKA2DocForm::OnBarcode(const wchar_t* _barcode)
{
	if( readBarcode )
		return;
   ...
   bool refresh = false;
	...
	{
		DKA2DocData::ItemType itemType = DKA2DocData::Item;
		if( !isItem && *barcode == L'9' && *(barcode+1) == L'1' )
		{
			itemType = DKA2DocData::Document;
		}

		if( ((DKA2DocData*)data)->New(barcode, itemType) )
		{
			std::wstring tstr;
			((DKA2DocData*)data)->GetDocTitle(&tstr);
			wsprintf(text, L"Документ: %s", tstr.c_str());
...
LRESULT DKA2DocForm::ScanData(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM lParam, BOOL& bHandled)
{
   std::wstring data;
...
LRESULT DKA2DocForm::Remove(WORD nCode, WORD id, HWND hWnd, BOOL &bHandled)
{
   if( data->Count() > 0 )
   {
      if( MessageBox(L"Удалить документ?", L"Вопрос", MB_YESNO | MB_ICONQUESTION) == IDYES )
      {
         ((DKA2DocData*)data)->RemoveDoc();
...
LRESULT DKA2DocForm::Backing(WORD nCode, WORD id, HWND hWnd, BOOL &bHandled)
{
#ifdef ZEBEX
...
void DKA2DocForm::UpdateLayout(const RECT& bounds, bool forceRecalc)
{
   CRect rc, rc2;
...
{
   DKA2DocData *pfd = new DKA2DocData(doc);
	_Module.GetFrame()->Load(IDD_DKA2_DOC, pfd);
}
