...
public:
   ...
   BEGIN_MSG_MAP(NetworkProperties)
		COMMAND_ID_HANDLER(IDC_PLAY_GOOD, PlayWavFile)
		COMMAND_ID_HANDLER(IDC_PLAY_BAD, PlayWavFile)
...
protected:
   LRESULT PlayWavFile(WORD nCode, WORD id, HWND hWnd, BOOL &bHandled);

	wchar_t goodSnd[MAX_PATH], badSnd[MAX_PATH];
};
...
LRESULT NetworkProperties::PlayWavFile(WORD nCode, WORD id, HWND hWnd, BOOL &bHandled)
{
	CComboBox cb(GetDlgItem((id == IDC_PLAY_GOOD) ? IDC_GOOD_SND : IDC_BAD_SND));

	wchar_t buf[MAX_PATH];
	cb.GetWindowText(buf, sizeof(buf) / sizeof(buf[0]));

	std::wstring fn;
	_Module.MakeFileName(&fn, L"Sounds\\");
	fn += buf;
	fn += L".wav";

	PlaySound(fn.c_str(), NULL, SND_SYNC | SND_NODEFAULT);
	return 0;
}
...
void NetworkProperties::Init()
{
   ...
   CComboBox cbGood(GetDlgItem(IDC_GOOD_SND));
   CComboBox cbBad(GetDlgItem(IDC_BAD_SND));

	std::wstring ffiles;
	_Module.MakeFileName(&ffiles, L"Sounds\\*.wav");

   WIN32_FIND_DATA data;
   HANDLE h = FindFirstFile(ffiles.c_str(), &data);
   if( h != INVALID_HANDLE_VALUE )
   {
      do
      {
			wchar_t* sp = wcsrchr(data.cFileName, L'\\');
			if(sp == NULL)
				sp = data.cFileName;
			wchar_t* ep = wcsrchr(sp, L'.');
			*ep = L'\0';

			int sel = cbGood.AddString(sp);
			if( wcscmp(sp, preference.goodItem) == 0)
				cbGood.SetCurSel(sel);
			sel = cbBad.AddString(sp);
			if( wcscmp(sp, preference.badItem) == 0)
				cbBad.SetCurSel(sel);
...
void NetworkProperties::Save(Preference *preference)
{
   ...
   preference->port = GetDlgItemInt(IDC_PORT, NULL, FALSE);
	...
	cbGood.GetWindowText(preference->goodItem, sizeof(preference->goodItem) / sizeof(wchar_t));
	cbBad.GetWindowText(preference->badItem, sizeof(preference->badItem) / sizeof(wchar_t));
}
