...
class RowData:
   ...
   def avgInterval(self) -> timedelta:
      ...
      for di in self.docs:
         print('before', di.created, cur)
         ret += di.created - cur
         cur = self.docLastTime(di)
         print('doclasttime', cur)
...
def loadData(params, server) -> ReportData:
   ...
   stmt = '''
    select s.*, si.date as d_created, d.name as division
    from ScriptDoc s, ScriptDoc$items si, Division d, Division$agents da
    where s.userid = si.ScriptDoc$userid and s.created = si.ScriptDoc$created  
   ...
   # print(stmt)
   docs = server.Query(stmt, 'Docs[created:dt,userid:s,agent:s,division:s,items(userid,created)[created@d_created:dt]]')
