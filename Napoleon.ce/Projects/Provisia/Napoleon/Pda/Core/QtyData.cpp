...
void PriceFormData::SetQTYDataFromOrder(QTYData *qd, const Price& p, std::vector<OrderItem>::iterator item)
{
   ...
   {
#ifdef SAVE_IN_PACK
      qd->flags = (saveInPack & (~oiHideRemnants));
#endif
      qd->dcost = 0;
      qd->cost = (order->params & ofNetCost) ? p.cost[2] : qd->cost = p.cost[0];
   } else
   {
      qd->qty =  item->qty;
      qd->flags = item->flags;
      ...
      qd->dcost = item->cost;
      qd->cost = (order->params & ofNetCost) ? p.cost[2] : p.cost[0];
   }
   qd->discount = order->discount;

#ifdef MULTI_WH
   whIndex = order->WarehouseIndex();
#endif
...
#ifdef SAVE_IN_PACK
   saveInPack = (qd.flags & (~oiHideRemnants));
#endif
...
#ifdef Provisia
DWORD GetPriceCost(const wchar_t* id, bool netCost)
{
   PriceImpl p;
   p.id = (wchar_t*)id;
   p.Read();
   DWORD cost = (netCost) ? p.cost[2] : p.cost[0];
...
bool InvoiceData::Selecting(int index)
{
   ...
   qd.discount = order->discount;
   qd.dcost = i->cost;
   qd.cost = GetPriceCost(i->id, ((order->params & ofNetCost)!=0));
#endif
