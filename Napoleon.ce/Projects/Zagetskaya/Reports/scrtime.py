...
class Item:
    ...
    def getData(self, row):
      return [self.data.strftime("%d.%m.%Y"), self.org, self.address, self.data.strftime("%H:%M") if self.data != None else "" , self.finish.strftime("%H:%M") if self.finish != None else "",
        "=E{0}-D{0}".format(row+1), "=D{0}-E{1}".format(row+2, row+1)]
...
def inflateParams(server):
    return server.Params[0].start, server.Params[0].finish, unpack(server.Params[0].userids)
...
def ptintSheet(xlb, sh, data, agent):
    ...
    head = ["Дата", "Название РТТ", "Адрес РТТ", "Время начала визита в РТТ", "Время окончания визита в РТТ", "Итого время в РТТ", "Итого время передвижения от РТТ до РТТ"]
