...
class DailyDataItem:
    ...
    def addDlvDoc(self, doc, lastDate):
        ...
        self.dlvCount += 1
        self.dlvSum += sum
        if doc.incass != None: self.paySym += doc.incass
    ...
    if orgDailyData != None:
        values.extend(
           ['X' if orgDailyData.haveDoc else '',
            ...
            orgDailyData.paySym if orgDailyData.paySym > 0 else ''])
...
def createDailyReport(sheet, data, param):
    ...
    for agent in data.agents:
        ...
        for cdate in data.commonData.days:
            index = 0
            route = agent.route.getDayRoute(datetime(cdate.year, cdate.month, cdate.day))
            ...
            # out of route orgs 
            for org,orgdata in agent.orgData.data.items():
            ...
            for org in route:
                ...
                for cdate in data.commonData.days:
                    if org in agent.orgData.data and cdate in agent.orgData.data[org].data :
                        ...
                        valuepay.append(orgDailyData.paySym if orgDailyData.paySym > 0 else '')
