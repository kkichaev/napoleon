...
#     type = "OrderResult[created:s,orgid:s,balance:n(2),ordnumber:s,ordstatus:s,dlvstatus:s,dlvnumber:s,message:s]"
    ...
    if conn == None :
        err = 'Ошибка при подключении к 1с'
        objRes = objList.New()
        objRes.ordstatus = 'fail'
        objRes.message = err
        server.Put(objList)
...
#         objRes.balance = src.balance
        ...
        if src.dlvstatus == 'handled' and src.dlvitems != None and src.dlvitems.Count() > 0 :
