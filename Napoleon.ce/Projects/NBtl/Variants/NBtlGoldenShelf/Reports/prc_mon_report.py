...
class ItemData :
    __slots__ = ['categ', 'mfr', 'name', 'cost', 'weight', 'costWt']
    ...
    def __init__(self, item, groups, price):
        ...
        if item.id in price:
            ...
            self.weight = prc.weight
            self.costWt = 0 if prc.weight == 0 else item.cost / prc.weight
...
def printOut(data, params):
    ...
    head = ['Город', 'Мерчендайзер', 'Наименование торговой сети', 'Наименование торговой точки', 
            'Адрес', 'Категория', 'Производитель', 'Дата заполнения отчета', 'SKU', 'Вес, кг','Цена, за единицу, руб.', 'Цена за кг']
    ...
    for item in data.data:
        ...
        for oi in item.items:
            ...
            values.extend([oi.categ, oi.mfr, item.date, oi.name, oi.weight, oi.cost, oi.costWt])
    ...
    cc = 1
    for w in [20,20,20,20,20,20,20,20,20,20,20,20]:
