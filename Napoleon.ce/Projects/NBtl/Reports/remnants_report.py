...
class ItemData :
    __slots__ = ['categ', 'mfr', 'name', 'qty', 'qtyFlag', 'qtyFact', 'factFlag', 'qtyInput', 'qtyUnload']
...
def printOut(data, params):
    ...
    head = ['Город', 'Мерчендайзер', 'Наименование торговой сети', 'Наименование торговой точки', 'Адрес', 'Категория', 
            'Производитель', 'Дата заполнения отчета', 'SKU','Остаток товара', 'Кол-во фейсов', 'Виртуальный остаток','Наличие ценника','Остаток на выходе']
    ...
    for item in data.data:
        ...
        for oi in item.items:
            ...
            values.extend([oi.categ, oi.mfr, item.date, oi.name, oi.qty, oi.qtyFact, oi.qtyInput, oi.qtyUnload, qtyExit])
    ...
    cc = 1
    for w in [20,20,20,20,20,20,20,20,20,20,20,20,20,20]:
