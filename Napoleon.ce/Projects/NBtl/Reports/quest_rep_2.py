...
class Item:  
  ...
  def values(self):
    res = [self.org, self.code, self.city, self.address, self.created.strftime('%d.%m.%Y'), self.user]
...
class ReportData:
  ...
  def loadDocs(self, docs, pics, href, orgLocation, cities):
...
def loadData(data, params, server):
    ...
    cities = server.Get("City","","id")
    data.prepare(answers, quests)

    href = params.hrefBase
    data.loadDocs(answers, pictures, href, OrgLocation(server), cities)
...
def printOut(data, params):
    ...
    r = 2
    arr = ["Наименование", "Код ТТ", "Город", "Адрес", "Дата", "Мерчандайзер"]
