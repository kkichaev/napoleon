...
QuestItem = NewType('QuestItem', object)
...
class Quest:
  def __init__(self, quest):
    ...
    self.orgs = list()
    self.orgids = []
    self.questItems: dict[str, QuestItem] = {}
    
    idx = 0
    quest.items.sort(key= lambda x: x.number)

    for i in quest.items:
      self.questItems[i.iditem] = i
  ...
  def addanswer(self, d, r, pics, href):
    if d.id in self.orgids:
      return
    
    self.orgids.append(d.id)

    sz = len(self.rowidx)
    a = [""] * sz

    for qi in self.questItems.values():
      altText: str = None

      if qi and len(qi.clients) > 0:
        clients:list[str] = qi.clients.split(',')
        altText = qi.altText if not d.id in clients else None
        if altText : self.putItemValue(a, ReportData.KEY_FMT.format(self.quest.idquest, qi.iditem), altText)
...
def getOrgName(curOrg) -> str:
  return " г. {0}, {1} {2}".format(curOrg.cid, curOrg.name, curOrg.address)
...
class ReportData:
  ...
  def loadDocs(self, docs, pics, href, orgLocation):
    docs = sorted(docs, key=lambda lhs: lhs.created, reverse=True)

    for d in docs:
      if d.question in self.quests:
        self.quests[d.question].addanswer(d, self, pics, href)  

    for org in self.orgs.orgs:
      for q in self.quests:
        if not org in self.quests[q].orgids:
...
def run(server):
    ...
    data = ReportData()
    data.server = server
    data = quest_rep.loadData(data, params, server)
