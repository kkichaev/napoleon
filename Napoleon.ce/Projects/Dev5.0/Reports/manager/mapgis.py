...
class MapGis(RObj):
    ...
    def __init__(self, server, orglist):
    ...
    def postProcess(self, server, mapHint):  
        ...
        if self.startRoute != None:
            dist = " пробег: %.01f" % self.distance
            self.obj.title = "Трек начало: " + self.startRoute.strftime("%H:%M") + " окончание: " + self.endRoute.strftime("%H:%M") + dist + " км."
        else:    
