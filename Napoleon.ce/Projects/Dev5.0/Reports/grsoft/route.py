...
class AgentRoute:
    ...
    def __init__(self, server, userid:str):
        self.userid = userid
        ...
        if cfg != None and len(cfg) > 0 and len(cfg[0].value) > 0:
            self.sheduleStart = datetime(*(time.strptime(cfg[0].value, '%Y-%m-%d')[0:6])).date()
    ...
    def __weekIndex(self, day):
        if self.sheduleStart != None:
            ...
            return (int)((d.days / 7) % 4) + 1
        return 0
    
    def getDailyOrgList(self, day):
        curDay = AgentRoute.days[day.weekday()]
        if curDay in self.route:
            return self.route[curDay]
    ...
    def getDayRoute(self, day):
        rlist = self.getDailyOrgList(day)
        if rlist != None:
            return self.__makeList(rlist)
