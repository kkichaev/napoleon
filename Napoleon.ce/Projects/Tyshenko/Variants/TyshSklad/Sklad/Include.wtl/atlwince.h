...
public:
	...
	static HRESULT ActivatePreviousInstance(HINSTANCE hInstance, LPCTSTR  lpstrCmdLine, bool bDialog, HANDLE* hOutMutex)
	{
      ...
      if( hOutMutex != NULL ) *hOutMutex = NULL;
		...
		for(int i = 0; i < iMaxRetries; ++i)
		{
			...
			{
            if( hOutMutex != NULL ) *hOutMutex = hMutex;
				return S_OK;
			}
		}
		return S_OK;
...
public:
	...
	static HRESULT ActivatePreviousInstance(HINSTANCE hInstance, LPCTSTR  lpstrCmdLine, HANDLE *hMutex = NULL)
	{
		return CAppWindowBase< T >::ActivatePreviousInstance(hInstance, lpstrCmdLine, false, hMutex);
	}
};
