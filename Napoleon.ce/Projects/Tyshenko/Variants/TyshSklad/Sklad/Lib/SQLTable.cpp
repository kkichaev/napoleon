...
void SQLTable::ResultBinder::Prepare(const IReflectableData* data, std::vector<std::wstring> *tableFields, bool addRID)
{
   ...
   if( data != NULL )
   {
      ...
      while( i != tableFields->end() )
      {
         int fld;
         const std::wstring& field = (*i);
         if( field.at(0) == L'\"' ) fld = r.Find(field.substr(1, field.size()-2).c_str(), true);
         else fld = r.Find((*i).c_str(), true);
...
bool SQLTable::Select(const wchar_t *stmt, IReflectableData *data)
{
   if( db == NULL ) return false;
	...
	if(!PrepareStmt(stmt, opSelect, (DWORD)(&data->GetType()), L""))
		return false;

   vector<wstring> fields;
	int count = sql_column_count(opStmt);
	for(int i=0; i<count; i++)
	{
		fields.push_back((const wchar_t*)sql_column_name16(opStmt, i));
	}
	result.Prepare(data, &fields, false);
