...
LRESULT ListViewMultiLine::OnChar(UINT /*uMsg*/, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	bHandled = FALSE;
	if(owner && owner->OnChar(this, wParam))
	{
		bHandled = TRUE;
	}
	return 0;
...
void ListViewMultiLine::LoadImageList(int resID)
{
   ...
   if( bmp ) 
   {
      ...
      {
         BITMAP bmi;
         GetObject(bmp, sizeof(bmi), &bmi);
         images.Create(bmi.bmHeight, bmi.bmHeight, ILC_COLORDDB, 1, 0);
         images.Add(bmp);
      } else
      {
         CImageList il;
...
void ListViewMultiLine::Setup(int scale, ListFormData *listData, int imageList)
{
   ...
   while( ctr < listData->ColumnsCount() )
   {
      ...
      const wchar_t *pTitle = header[ctr].wTitle;
      std::wstring title;
      if( pTitle == NULL )
      {
         _Module.LoadString(&title, header[ctr].title);
         pTitle = title.c_str();
      }
      InsertColumn(ctr, pTitle, fmt, header[ctr].startWidth);
...
ListForm::ListForm() : data(NULL), element(NULL), curIndex(-1), cmdButtons(NULL)
   //, reverse(false), sortedColumn(-1)
{
}

ListForm::ListForm(int sumScale) : data(NULL), element(NULL), curIndex(-1), sumLabel(sumScale), cmdButtons(NULL)
   //, reverse(false), sortedColumn(-1)
{
}
...
{ 
   ...
   hotKeysMaps[0] = IDC_BACK;
   hotKeysMaps[1] = 0;
   hotKeysMaps[2] = 0;

   RegisterHotKeys(m_hWnd, false);

   if( cmdButtons != NULL )
   {
      for( int i=0; cmdButtons[i]; i++ )
      {
         PictButton *pb = new PictButton(cmdButtons[i]);
         CWindow wnd(GetDlgItem(cmdButtons[i]));
         pb->SubclassWindow(wnd.m_hWnd);
...
void ListForm::SetListLayout(bool forceRecalc, int listTop, int height, ListViewMultiLine *list, ListFormData *listData)
{
   ...
   if( cmdButtons != NULL && cmdButtons[0] )
   {
      CRect btnBnds;
      GetDlgItem(cmdButtons[0]).GetWindowRect(btnBnds);

      height -= (btnBnds.Height() + 2 * 2);

      int wdh = btnBnds.Width();
      int hgh = btnBnds.Height();
      int count = 0, offset;
      
      while(cmdButtons[count])
         count++;

      if( count < 3 )
         count = 3;
      offset = (rc.Width() / count - wdh) / 2;
      if( offset == 0 )
         offset = 2;

      btnBnds.top = rc.bottom - 4 - hgh;
      btnBnds.left = offset;
      btnBnds.bottom = btnBnds.top + hgh;
      btnBnds.right = btnBnds.left + wdh;

      for( int i=0; cmdButtons[i]; i++ )
      {
         CWindow btn(GetDlgItem(cmdButtons[i]));
         btn.MoveWindow(btnBnds);
         btnBnds.OffsetRect(btnBnds.Width() + 2 * offset, 0);
      }
   }
...
LRESULT ListForm::OnHotKey(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
   int index = wParam - SWK1;
   if( index >= 0 && index < sizeof(hotKeysMaps)/sizeof(hotKeysMaps[0]) && hotKeysMaps[index] != 0 )
      PostMessage(WM_COMMAND, hotKeysMaps[index], 0);
   return 0;
}

void ListForm::Destroy()
{
   UnregisterHotKeys(m_hWnd, false);

   std::vector<PictButton*>::const_iterator i = buttons.begin();
   for( ; i != buttons.end(); i++ )
   {
      (*i)->m_hWnd = 0;
      delete (*i);
...
void ListForm::UpdateLayout(const RECT& bounds, bool forceRecalc)
{
   SetListLayout(forceRecalc, bounds.top, bounds.bottom - bounds.top);
