...
static Session::AckReturn DoReceive(Socket* socket, const char *param, Binary *packet, Session* session, const wchar_t* objName, bool compressed)
{
   ...
   if( ReceiveData(socket, buf, size, packet, session->EvStop()) )
   {
      ...
      SessionObject* dest = session->Build(objName, false);
      const DataReflector* type = FindTypeReflector(objName);
      ...
      {
         if( dest->CreateWriter(NULL) && dest->Write() )
...
static bool SendObject(Socket* socket, Session* session, const wchar_t* objName, const wchar_t* command, bool sendFail, int dbVer, bool sendVer)
{
   ...
   const SessionObject *so = session->LoadObject(srcname.c_str(), NULL);
...
Session::AckReturn Session::ReadPreviousVersion(Binary* packet)
{
   ...
   if( !strncmp(cmd, SND_ORDER_W, sizeof(SND_ORDER_W)-1) )
      ret = DoReceive(socket, cmd + sizeof(SND_ORDER_W)-1, packet, this, L"Order", true);
   ...
   {
      SendList sl;
      sl.push_back(ObjSendData(L"Config", SND_CONFIG_W, false, false));
   ...
   {
      ret = DoReceive(socket, cmd + sizeof(SND_PROXY)-1, packet, this, L"Proxy", true);
   }
#endif
   ...
   {
      ret = DoReceive(socket, cmd + sizeof(SND_ORG_DO)-1, packet, this, L"Task", true);
   }
#endif
