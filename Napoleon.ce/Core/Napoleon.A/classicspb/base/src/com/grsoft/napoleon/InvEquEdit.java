...
public class InvEquEdit extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnInputBarcode = findViewById(R.id.btnInputBarcode);
		...
		btnInputBarcode.setOnClickListener(new OnClickListener() {
			
			@Override
			public void onClick(View v) {
				showDialog(R.id.input_barcode_dlg);
			}
		});
		...
		list.setOnItemLongClickListener(new AdapterView.OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view, int position, long id) {
				InvEquItem f = (InvEquItem) parent.getItemAtPosition(position);
		...
		list.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				InvEquItem f = (InvEquItem) parent.getItemAtPosition(position);
				InvEquRemark.open(InvEquEdit.this, doc.getRowid(), f.id);
			}
		});
	...
	private Dialog createBarcodeDlg() {
		...
		builder.setPositiveButton(R.string.ok, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				String bc = ed.getText().toString().trim();
				bc = setValidBarcode(bc);
				barcodeCheck(bc);
				((Adapter)list.getAdapter()).buildData();
	...
	protected void barcodeCheck(String bc) {
		...
		if (item.newItem == 1) {
			InvEquRemark.open(this, doc.getRowid(), item.id);
		}
	}
	...
	private class Adapter extends BaseAdapter implements FilterAdapter {
		...
		public View getView(int position, View view, ViewGroup parent) {
			...
			tv.setText(i.barcode);
			
			tv = (TextView) view.findViewById(R.id.tvRemark);
			if (i.remark.trim().length() == 0)
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (scanResult != null) {
		     ...
		     if (bc != null) { //http://212.232.41.126/bugzilla/show_bug.cgi?id=2880
		    	 bc = setValidBarcode(bc);
	...
	protected String setValidBarcode(String bc) {
