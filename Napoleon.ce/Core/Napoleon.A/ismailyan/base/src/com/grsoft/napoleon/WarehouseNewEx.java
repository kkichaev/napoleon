...
public class WarehouseNewEx extends WarehouseNew {
	...
	protected void setTextColumnValue(TextView textView, int type, Price price) {
		...
		if (type == COLUMN_COST) {
			String c = Util.IntToScaleStr(getCost(price), Consts.SUM_SCALE, Util.DEC_DELIM, false);
			int ac = ((PriceEx)price).acost;
			
			if (ac > 0) {
				String acs = Util.IntToScaleStr(ac, Consts.SUM_SCALE, Util.DEC_DELIM, false);
				c = "<font color='red'>" + acs + "</font>&nbsp;" + c;
