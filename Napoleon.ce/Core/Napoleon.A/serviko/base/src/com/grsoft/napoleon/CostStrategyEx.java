...
public class CostStrategyEx extends CostStrategy {
	...
	static boolean canAdd(List<ServikoAction> dest, ServikoAction action) {
		for(ServikoAction check : dest) {
			//есть другая промоакция с полным вытеснением не по приоритету
			if(check.sameValue(-2, 1))
				return false;
			
			//есть другая промоакция с вытеснением по номенклатуре
			if(check.sameValue(-1, -1))
				return false;
			
			//есть другая промоакция с вытеснением по приоритету в строке
			if(check.sameValue(action.priority, -1))
				return false;

			//есть другая промоакция с вытеснением по приоритету в документе
			if(check.sameValue(action.priority, 1))
				return false;
		}
...
//				}
	...
	public int getOrgCost(Price p, Document<?> doc) {
	...
	public int getItemCost(Price p, Document<?> doc) {
		...
		if(doc instanceof OrderImpl) {
			...
			if(iad != null) {
				OrderItemEx oie = (OrderItemEx) ((OrderImpl)doc).findItem(p.id);
				cost = iad.count(cost, (oie != null) ? oie.promoId : "").cost;
			}
			cost = costWithDiscount(cost, -((OrderEx)doc.getData()).nac, Consts.SUM_SCALE);
		}
