...
public class PriceCountEx extends PriceCount implements OrderImplBase.UpdateQtyHandler {
	...
	protected void refreshData() {
		...
		if(document instanceof OrderImpl) {
			...
			orgCost = ((CostStrategyEx)CostStrategy.defaultInstance).getOrgCost(p, document);
			priceCost = ((CostStrategyEx)CostStrategy.defaultInstance).getPriceCost(p, document);
	...
	CompoundButton.OnCheckedChangeListener actionChecked = new CompoundButton.OnCheckedChangeListener() {
		...
		public void onCheckedChanged(CompoundButton arg0, boolean arg1) {
			...
			if(document instanceof OrderImpl)
				newCost = CostStrategy.costWithDiscount(newCost, -((OrderEx)document.getData()).nac, Consts.SUM_SCALE);
