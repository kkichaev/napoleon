...
public abstract class DWaybillEdit extends FragmentActivity {
	...
	OnItemClickListener itemClick = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			DialogFragment dlg = createItemEditDialog();
			Bundle args = new Bundle();
			args.putInt(DWaybillItemEdit.ITEM_POS, position);
	...
	class Adapter extends BaseAdapter implements FilterAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if(convertView == null)
				convertView = View.inflate(DWaybillEdit.this, R.layout.dlvitemrow, null);
			
			DWaybillDocumentItem i = (DWaybillDocumentItem) getItem(position);
			
			int color = i.inqty != i.outqty ? getResources().getColor(R.color.toxic_green) : getResources().getColor(R.color.black);
