...
public class MovementEdit extends BaseActivity{
	private MovementImpl movement = new MovementImpl();
	...
	public static void open(Context context, MovementImpl order) { 
		open(context, order, true); 
	}
	
	public static void open(Context context, MovementImpl order, boolean editOldOrder) {
		Intent i = new Intent(context, MovementEdit.class);
	...
	private void init() {
		...
		movement.read(orderRowId);
		Order o = movement.getData();
		...
		btnOK.setEnabled(!movement.isExported());
	...
	class DelayClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			tvCounter.setText(Integer.toString(movement.getData().delay));
			...
			btnCounterOK.setOnClickListener(new OnClickListenerToNotify() {
				...
				public void onClick(View v) {
					super.onClick(v);
					movement.getData().delay = Integer.parseInt(tvCounter.getText().toString());
					updateDisplayDelay();
					dialog.hide();
				}
			});
	...
	private void deleteEmptyOrder() {
		if(!editMode) {
			if( movement.getData().items == null || movement.getData().items.size() == 0 )
				movement.delete();
		}
	}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			if (editMode && (movement.getSumType() != costType && costType >= 0))
		...
		private void okDone(boolean updateSumType) {
			...
			if (updateSumType)
				movement.updateItemsCost(o.sumType);
			else
				movement.write();
			
			if(!editMode)
				Warehouse.open(MovementEdit.this, movement, false);
