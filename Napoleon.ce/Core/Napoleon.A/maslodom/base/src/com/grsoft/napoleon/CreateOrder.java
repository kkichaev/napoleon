...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		if(!editMode){
			btnOK.setText(R.string.create_order);
			Button btnAutoorder = (Button) findViewById(R.id.btnAutoorder);
			btnAutoorder.setVisibility(View.VISIBLE);
			btnAutoorder.setOnClickListener(new OnClickListener() {
				
				@Override
				public void onClick(View v) {
					Cursor c = null;
					try{
						Date created = order.getData().created;
						Date date = order.getDate();
						String table = order.getTableName();
						order.delete();
						
						StringBuilder sql = new StringBuilder();
						sql.append("created = (select max(created) from ").append(table).append(") and id=?");
						c = DataBaseManager.getDataBase().query(order.getTableName(), new String[]{"created"}, sql.toString(), 
								new String[]{order.getId()}, null, null, null);
						
						if(c.moveToFirst()){
							order.delete();
							OrderImplEx copy = (OrderImplEx) OrderDoc.instance().create();
							copy.read(c.getLong(0));
							copy.close();
							copy.getData().created = created;
							copy.getData().date = date;
							copy.getData().params = 0;
							copy.getData().number = "";
							copy.getData().podRemark = "";
							copy.insert();
							
							copy.open(v.getContext());
							finish();
						}else
							Toast.makeText(v.getContext(), R.string.orders_not_found, Toast.LENGTH_SHORT).show();
					}catch(Exception e){
						e.printStackTrace();
					}finally{
						if(c != null)
