...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		PayTypeImpl pti = new PayTypeImpl();
		PayType pt = pti.getData();
		pt.category = oe.category;
		pt.id = oe.paytype;
		
		boolean canCreate = pti.read();
		pti.close();
		
		((TextView)findViewById(R.id.tvPayType)).setText("Вариант оплаты: " + pt.name);
		
		View btnOK = findViewById(R.id.btnOK);
		btnOK.setEnabled(order.isEditable() && canCreate);
