	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			if (editMode && ((order.getSumType() != costType && costType >= 0)) ||
					discount != ((IOrder)(order.getData())).getDisc() )
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			((IOrder)o).setDisc(discount);
			
			updateOrder();
			
			if (updateSumType){
				order.updateItemsCost(o.sumType);
				if(((OrderImplEx)order).minCostError)
					Toast.makeText(CreateOrder.this, 
