...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKey(config, "—клады", new ArrayList<KeyValue>(), spWh, o.whCode);
		
		Spinner spPay = (Spinner) findViewById(R.id.spPayType);
		DialogHelper.loadSpinnerFromConfig(config, "“ипќплаты", new ArrayList<CharSequence>(), spPay, o.payType);
		
		Spinner spPayMode = (Spinner) findViewById(R.id.spPayMode);
		DialogHelper.loadSpinnerFromConfig(config, "—пособќплаты", new ArrayList<CharSequence>(), spPayMode, o.payMode);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.payMode = ((Spinner)findViewById(R.id.spPayMode)).getSelectedItemPosition();
	...
	private Dialog createOrgDueDlg() {
		...
		builder.setMessage(getString(R.string.alert_due_msg_new, 
