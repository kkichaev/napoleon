...
public class NapoleonApp extends Application {
	...
	private void initDocTypes() {		
		...
		DocType.addType(DiscountType.instance());
		...
		Features.PRICE_MOVE_INSIDE_FOLDER = true;
		Features.LOAD_FULL_PRICE = true;
		
		Features.DELIVERY_ADDRESS = true;
		Features.PACK_INPUT = true;
		Features.REPLACE_COMMA_TO_PLUS = true;
		
		StringBuilder value = new StringBuilder();
		ConfigImpl cfg = new ConfigImpl();
		if(cfg.getValue(value, "Range")){
			try{
				AssortmentMatrixAdapter.PERIOD_IN_MONTH = Integer.parseInt(value.toString());
			}catch(Exception e){
				e.printStackTrace();
			}
		}
