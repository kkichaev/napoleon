...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		if(oe.orgAddress != null){
			for(OrgAddress addr : oe.orgAddress) {
			...
			ArrayAdapter<KeyValue> aa = new ArrayAdapter<KeyValue>(this, 
					R.layout.simple_spinner_layout, addresses);
		...
		DialogHelper.loadSpinnerFromConfig(c, "Склады", new ArrayList<CharSequence>(), (Spinner)findViewById(R.id.spWh), o.whIndex);
		c.close();
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			int pos = ((Spinner)findViewById(R.id.spWh)).getSelectedItemPosition();
			if( pos >= 0 )
				o.whIndex = pos;
			
//			CostStrategy cs = CostStrategy.getInstance(order.getClass());
...
//					prc.id = oi.id;
//					if( p.read() )
//						oi.cost = cs.getItemCost(prc, order);
//				}
//			}
