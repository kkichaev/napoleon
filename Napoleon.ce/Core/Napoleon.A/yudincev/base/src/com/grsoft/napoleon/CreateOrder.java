...
public class CreateOrder extends BaseActivity {
	...
	private void init() {
		...
		DialogHelper.loadSpinnerFromConfig(config, "ТипДоставки", new ArrayList<CharSequence>(),
				(Spinner)findViewById(R.id.spDlvType), o.dlvType);

		DialogHelper.loadSpinnerFromConfig(config, "ВидДоставки", new ArrayList<CharSequence>(),
				(Spinner)findViewById(R.id.spDlvKind), o.dlvKind);
		...
		if (maxDiscount == 0)
			...
			tv.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					...
					}, Consts.SUM_SCALE, false, getString(R.string.max_discount, 
							Util.IntToScaleStr(maxDiscount, Consts.SUM_SCALE)),  DiscountInputDlg.Type.OnlyDiscount);
				}
			});
		discount = ((OrderEx)o).discount;
		updateDisplayDelay();
		refreshDate();
		updateDicsount();
	}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			if (editMode && ((order.getSumType() != costType && costType >= 0) || 
					((OrderEx)order.getData()).discount != discount))
		...
		private void okDone(boolean updateSumType) {
			...
			o.dlvKind = ((Spinner)findViewById(R.id.spDlvKind)).getSelectedItemPosition();
			o.dlvType = ((Spinner)findViewById(R.id.spDlvType)).getSelectedItemPosition();
