...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		Spinner route = (Spinner)findViewById(R.id.spChangeRoute);
		DialogHelper.loadSpinnerFromDataObject(route, DogovorRoute.class, new DialogHelper.Selected<DogovorRoute>() {
			@Override
			public boolean isSelected(DogovorRoute object) {
				return o.idRoute.equals(object.id);
			}
		}, true, "name");
		...
		((CheckBox)findViewById(R.id.cbChangeRoute)).setChecked(oe.changeRoute != 0);
		
		route.setEnabled(false);
		if(order.isEditable() && oe.changeRoute != 0) {
			route.setEnabled(true);
			route.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {

				@Override
				public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
					DogovorRoute sel = (DogovorRoute) arg0.getItemAtPosition(arg2);
					if(sel != null)
						btnOK.setEnabled(sel.id.length() > 0);
				}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			DogovorRoute ds = (DogovorRoute) ((Spinner)findViewById(R.id.spChangeRoute)).getSelectedItem();
			if(ds != null) {
				o.idRoute = ds.id;
			}
