...
public class CreateOrder extends BaseActivity
{
	...
	private boolean editMode = false, starting = true;;
	
	ArrayList<KeyValue> priceTypes = new ArrayList<KeyValue>();
...
//		DialogHelper.loadSpinnerWithKey(config, "Организация", new ArrayList<KeyValue>(), spFirma, o.supplCode);

		Spinner spPrices = (Spinner) findViewById(R.id.spPrices);
		DialogHelper.loadSpinnerWithKey(config, "ВидЦены", priceTypes, spPrices, o.costCode);
		...
		while(bdo) {
			if( od.id.equals(o.dogCode) )
				selected = dogovors.size();
			dogovors.add(od);
			od = new OrgDogovor();
			bdo = r.selectNext(od);
		}
		ArrayAdapter<OrgDogovor> da = new ArrayAdapter<OrgDogovor>(this, R.layout.simple_spinner_layout, dogovors);
		...
		spDog.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				setNewDogovor((OrderEx)order.getData(), dog.id);
			}
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	void setNewDogovor(OrderEx o, String dogCode) {
		o.dogCode = dogCode;
		
		if( !editMode || !starting ) {
			for( OrgDogovor dog : dogovors ) {
				if( dog.id.equals(dogCode) ) {
					o.sumType = dog.costType;
					if( o.sumType < priceTypes.size() ) {
						o.costCode = priceTypes.get(o.sumType).key.toString();
						((Spinner)findViewById(R.id.spPrices)).setSelection(o.sumType);
					}
					
					o.supplCode = dog.firm;
					break;
				}
			}
		}		
...
//				return dateHandler.createDialog();
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			if (editMode && (selectedPriceIndex != costType && costType >= 0))
