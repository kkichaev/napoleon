...
public class DebetView extends DocumentsBase {
	...
	class DebtAdapter extends BaseExpandableListAdapter {
		Map<String, Sklad> sklads = new HashMap<String, Sklad>();
		...
		public void load(String orgId) {
			...
			DataTraveler.travel(Sklad.class, new DataTraveler.Travel<Sklad>(true) {
				...
				public boolean travel(DataTraveler<Sklad> item) {
					if(!sklads.containsKey(item.data.id))
						sklads.put(item.data.id, item.data);
					return true;
				}}, null);
		...
		public View getGroupView(int groupPosition, boolean isExpanded, View convertView, ViewGroup parent) {
			...
			if( pd != null ) {
				...
				String name = getString(R.string.sklad_noname, pd.name);
				
				if(sklads.containsKey(pd.name))
					name= sklads.get(pd.name).name;
