...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		if( oe.dogovors != null ) {
			...
			for(OrgDogovor od : oe.dogovors) {
				...
				if( od.id.equals(o.idDogovor) )
					selDog = values.size();
				values.add(kv);
			}
			ArrayAdapter<KeyValue> aa = new ArrayAdapter<KeyValue>(this, R.layout.simple_spinner_layout, values);
			spDog.setAdapter(aa);
			if( selDog >= 0 && selDog < spDog.getCount())
				spDog.setSelection(selDog);
			
			if( oe.dogovors.size() <= 1 )
				findViewById(R.id.trDogovor).setVisibility(View.GONE);
		}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			Spinner spDog = (Spinner) findViewById(R.id.spDogovors);
			KeyValue kv = (KeyValue) spDog.getSelectedItem();
			if( kv != null )
				o.idDogovor = kv.key.toString();
