...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		createDogovorSpinner(this, (OrderExtended) o, (OrgExtended) oe, dgvCache);
...
//		}
	...
	public static Spinner createDogovorSpinner(Activity activity, OrderExtended o, 
			OrgExtended oe, Map<String, OrgDogovor> dc) {
		int selected = -1;
		ArrayList<KeyValue> v = new ArrayList<KeyValue>();
		int index = 0;
		
		List<OrgDogovor> dgvs = oe.getDogovors();
		
		if(dgvs != null)
			for(OrgDogovor od : dgvs) {
				KeyValue kv = new KeyValue(od.id, od.name);
				if(selected < 0 &&  kv.key.equals(o.getDogCode())) {
					selected = index;
				}
		...
		if (o.getDogCode().length() == 0 && v.size() > 0)
			selected = 0;
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			if( sel != null )
				((OrderExtended)o).setFirmCode(sel.key.toString());
			if( costType >= 0 )
				o.sumType = costType;
