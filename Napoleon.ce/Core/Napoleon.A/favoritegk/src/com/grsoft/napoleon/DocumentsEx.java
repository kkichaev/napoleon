...
public class DocumentsEx extends Documents {
	...
	protected Dialog onCreateDialog(int id) {
		if(id == R.id.visit_info_dlg) {
			AlertDialog.Builder b = new AlertDialog.Builder(this);
			b.setTitle("Информация");
			b.setMessage(Html.fromHtml(((OrgEx)org.getData()).visit));
	...
	protected void docCreating() {
		if(DocType.getCurDoc() == ScriptDoc.instance() && ((OrgEx)org.getData()).visit.length() > 0) {
			showDialog(R.id.visit_info_dlg);
			return;
		}
		super.docCreating();
	}
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		if(id == R.id.visit_info_dlg) {
			((AlertDialog)dialog).setMessage(Html.fromHtml(((OrgEx)org.getData()).visit));
			return;
		}
		switch (id) {
		case FIND_ITEM_DLG:
