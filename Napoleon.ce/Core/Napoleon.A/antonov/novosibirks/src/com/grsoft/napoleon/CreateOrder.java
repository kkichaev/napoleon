...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		rgWParam = (RadioGroup) findViewById(R.id.rgWParam);
		rgWParam.setOnCheckedChangeListener(new OnCheckedChangeListener() {
		...
		switch(o.wparam){
		case Order2Ex.NAKL: 
			rgWParam.check(R.id.rbNakl);
			break;
		case Order2Ex.CH_FACT:
			rgWParam.check(R.id.rbShFact);
			break;
		default:
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			CheckBox cbInvoiceType = (CheckBox) findViewById(R.id.cbInvoiceType);
			int invoiceType = cbInvoiceType.isChecked() ? 1 : 0;
			...
			int rbid = rgWParam.getCheckedRadioButtonId(); 
			
			if(rbid == R.id.rbNakl)
				o.wparam = Order2Ex.NAKL;
			else if(rbid == R.id.rbShFact)
				o.wparam = Order2Ex.CH_FACT;
