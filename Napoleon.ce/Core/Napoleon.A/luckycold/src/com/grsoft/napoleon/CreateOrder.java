...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKey(config, "ВидЦены", new ArrayList<KeyValue>(), spPrices, o.prcCode);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spFirma = (Spinner) findViewById(R.id.spFirma);
			KeyValue sv = (KeyValue)spFirma.getSelectedItem();

			Spinner spPrices = (Spinner) findViewById(R.id.spPrices);
			KeyValue pv = (KeyValue)spPrices.getSelectedItem();

			if( sv != null ) {
				o.firmCode = sv.key.toString();
				o.supplyer = spFirma.getSelectedItemPosition();
			}
			if( pv != null  ) {
				o.prcCode = pv.key.toString();
