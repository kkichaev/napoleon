...
{
	private OrderImplBase<?> order = (OrderImplBase<?>)OrderDoc.instance().create();
...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		List<KeyValue> costs = new ArrayList<KeyValue>();
		DialogHelper.loadSpinnerWithKey(config, "ВидЦены", costs, spPrices, o.prcCode);
		if( o.prcCode.length() == 0 && costs.size() > o.sumType ) {
			o.prcCode = costs.get(o.sumType).key.toString();
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			if( costType >= 0 ) {
				o.sumType = costType;
				o.prcCode = ((KeyValue)spPrices.getSelectedItem()).key.toString();
			}
