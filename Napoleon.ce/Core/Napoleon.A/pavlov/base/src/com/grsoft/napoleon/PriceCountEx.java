...
public class PriceCountEx extends PriceCount {
	...
	protected long getSumValue() {
		...
		int count =  text.length() == 0 ? 0 : Util.StrToScale(text, Consts.QTY_SCALE);
		qtyItems = count;
		...
		count =  text.length() == 0 ? 0 : Util.StrToScale(text, Consts.QTY_SCALE);
		qtyItems += count;
	...
	protected boolean isInputValid(Runnable r) {
		...
		if (cf.getValue(sb, "MinCost")){
			...
			if (v.length() > 0) {
				agentmincost = v;
				int i = Util.StrToScale(v, Consts.SUM_SCALE);
	...
	protected boolean updateOrder() {
		...
		if( document != null ) {
			int qty1 = Util.StrToScale(edQty1.getText().toString().trim(), Consts.QTY_SCALE);
			
			if (cbPack1.isChecked())
				qty1 = (int)FPOperation.itemMul(qty1, qtyInPack, Consts.QTY_SCALE);
			
			int qty2 = Util.StrToScale(edQty2.getText().toString().trim(), Consts.QTY_SCALE);
	...
	protected boolean updateQty(boolean inPack1, int qty1, boolean inPack2, int qty2) {
		return !((OrderImplEx)document).updateQty(price, qty1, qty2, getInputCost(price.getData()), inPack1, inPack2);
	}
}
