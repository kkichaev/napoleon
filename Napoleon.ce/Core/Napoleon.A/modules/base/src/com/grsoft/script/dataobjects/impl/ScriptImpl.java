...
public class ScriptImpl extends CreatableDocument<Script> {
	...
	static public boolean containsDocument(String docType, Date created, String id) {
		boolean res = false;

		DocList dl = ScriptDoc.instance().docList(id);
		for( int i=0; i<dl.getCount(); i++ ) {
			ScriptImpl si = (ScriptImpl) dl.get(i);
			if( si.containsDoc(docType, created) ) {
				res = true;
				break;
			}
		}
...
//		scriptListProcess(context, orgId);
	...
	public boolean isComplete() {
		for(ScriptItem item : data.items){
			if( item.state != ScriptItem.DOC_INITED && item.state != ScriptItem.DOC_SKIPPED)
				return false;
		
			if (Features.DEL_VISIT_WITHOUT_PHOTO && item.type.equals(VisitDoc.instance().getObjectName())){
				...
				if(vi.read(item.date.getTime())){
					...
					if(v.items == null || v.items.size() == 0){
						vi.delete();
						vi.close();
						return false;
					}
	...
	public static boolean hasUncomplete() {
		...
		boolean sv = Features.DONT_SEND_UNCOMPLETE_SCRIPTS;
		Features.DONT_SEND_UNCOMPLETE_SCRIPTS = false;
		DocExportListener e =  ScriptDoc.instance().getDirtyDocuments();
		Features.DONT_SEND_UNCOMPLETE_SCRIPTS = sv;
