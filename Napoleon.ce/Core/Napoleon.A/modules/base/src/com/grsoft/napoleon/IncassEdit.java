...
public class IncassEdit extends RegDurationActivity implements SendResultListener {
...
//	protected IncassImpl doc;
	...
	protected void init(Bundle bundle) {
		...
		if( Features.CANT_SEND_SCRIPT_PART ) {
			if(ScriptImpl.containsDocument(IncassDoc.instance().getObjectName(), doc.getData().created, doc.getId()))
	...
	void checkObjectSendResult(OrderImplBase<? extends Order> object, String response, int result) {
		if( result <  0 ) {
		...
		if( result == ObjectExchange.RESULT_FAIL ) {
			...
			if( result == ObjectExchange.RESULT_SAVE ) {
				syncMsg = getString(R.string.doc_save_error);
		...
		this.runOnUiThread(new Runnable() {			
			@Override public void run() { 
				AlertDialog.Builder b = new AlertDialog.Builder(IncassEdit.this);
				b.setTitle(syncTitle);
				b.setMessage(syncMsg);
				b.create().show();
			}
		});
	}
	...
	private void objectExchange() {
		new ObjectExchange(IncassEdit.this, findViewById(R.id.btnSend), 
				DocType.getCurDoc().getObjectName(), 
				ObjectExchange.WRITE_OBJECTS, doc, new ObjectExchange.ObjectSendedHandler() {						
					@SuppressWarnings("unchecked")
					@Override
