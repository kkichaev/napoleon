...
public class DiscountInputDlg {
	...
	private static String getCostText(int priceCost, int value, int scale) {
		int newCost = CostStrategy.costWithDiscount(priceCost, value, scale);
	...
	public static void open(final Context context, final InputNumber inputNumber, final int scale, 
		...
		if(inputNumber._priceCost != 0) {
			final TextView dscView = (TextView) panel.findViewById(R.id.tvDiscountInfo);
			dscView.setVisibility(View.VISIBLE);
			dscView.setText(getCostText(inputNumber._priceCost, inputNumber.getValue(), scale));
			edCount.addTextChangedListener(new TextWatcher() {
				
				@Override
				public void onTextChanged(CharSequence arg0, int arg1, int arg2, int arg3) {
					int newdsc = Util.StrToScale(arg0.toString(), scale);
					if(sp.getSelectedItemPosition() == 1)
						newdsc= -newdsc;
					String dscText = getCostText(inputNumber._priceCost, newdsc, scale);
			...
			sp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
					int newdsc = Util.StrToScale(edCount.getText().toString(), scale);
					if(arg2 == 1)
						newdsc= -newdsc;
					String dscText = getCostText(inputNumber._priceCost, newdsc, scale);
					dscView.setText(dscText);
