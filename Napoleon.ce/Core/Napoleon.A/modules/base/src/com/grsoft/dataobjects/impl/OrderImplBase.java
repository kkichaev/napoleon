...
//						String sql = "UPDATE \"" + tableName + "\" SET qty = qty + ? where id = ?";
//						SQLiteStatement stmt = DataBaseManager.getDataBase().compileStatement(sql);
//						
//						for(OrderItem item : data.items) {
//							stmt.clearBindings();
//							stmt.bindLong(1, item.qty);
//							stmt.bindString(2, item.id);
//							stmt.execute();
//						}
//						stmt.close();
//					}
	...
	public boolean initSilent(String orgId, GpsCoord coord) {
		...
		return super.init(null, orgId, coord);
	}
	...
	public void autoorder(String orgId, GpsCoord coord, HashMap<String, Integer> items, boolean inited) {
		...
		for(Entry<String, Integer> se : items.entrySet()) {
			...
			if( qty > 0 ) {
				prc.id = se.getKey();
				if( p.read() ) {
					int cost = cs.getItemCost(prc, this);
