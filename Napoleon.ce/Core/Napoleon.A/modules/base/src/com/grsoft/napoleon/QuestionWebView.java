...
public class QuestionWebView extends BaseActivity {
...
/*		try {
		...
		if (answerrowid != ExtrasConst.INVALID_ID){
        	AnswerImpl answerImpl = (AnswerImpl) QuestionDoc.instance().create();
        	if (answerImpl.read(answerrowid))
        		answer = answerImpl.getData();
        	answerImpl.close();
        	isEditable = answerImpl.isEditable();
		}
		
        String html = AnswerHTML.makeHTML(questionImpl.getData(), answer, isEditable, this);
        
        webView.loadDataWithBaseURL(null, html, "text/html", null, null);
        webView.getSettings().setBuiltInZoomControls(true);
	}
	
	private String createInit(Answer answer) {
	...
	private Object makeFunctionForNumber(String id, String answer) {
	...
	public void handle(String answer){
			...
			if (nameVal.length == 2){
				...
				for(QuestionItem qitem: questionImpl.getData().items){
					if (qitem.iditem.equals(id)){
						...
						if (qitem.type == QuestionItem.NUMBER) 
							answerItem.answer = answerItem.answer.replace(DOT_SEP, COMMA_SEP);
					}
				}
		...
		if (ids.size() > 0){
			...
			DocType.getCurDoc().refreshDocSum(orgid);
	...
	protected void fillNecessaryQuestion(HashMap<String, String> ids) {
		for (QuestionItem qi : questionImpl.getData().items) {
			QuestionItem qie = (QuestionItem) qi;
