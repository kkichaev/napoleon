...
//	protected SalesImpl salesImpl = new SalesImpl();
	...
	protected FirmAdapter createFirmAdapter() {
		try {
			return new FirmAdapter(this);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	protected void setFirmSelection() {
		try{
			if(spFirma != null){
				BaseAdapter adapter = createFirmAdapter(); 
	...
	protected void saveFirm() {
		...
		if( firmImpl != null && firmImpl.getData() != null)
			sales.supplyercode = firmImpl.getData().id;
	}
...
class FirmAdapter extends DataBaseAdapter<Firm>{
	...
	public FirmAdapter(Context context, String order) {
		super(context, new FirmImpl(), "", order);
	}
