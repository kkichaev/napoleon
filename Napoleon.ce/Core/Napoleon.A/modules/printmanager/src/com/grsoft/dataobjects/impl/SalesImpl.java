...
public class SalesImpl extends OrderImplBase<Sales> implements Itemsable{
...
//			return (int)((long)((PricePrint)item).vanQty * Consts.QTY_SCALE / item.qtyInPack);
	...
	protected SalesItem createNewItem(PricePrint price, int qty, int cost, boolean inPack) {
		SalesItem item = null;
...
//	}
	...
	public void updateItemsCost(int sumType) {
		...
		PricePrint p = (PricePrint)priceImpl.getData();
		...
		try{
			for(OrderItem it : doc.items){
				...
				p.id = item.id;
				if (priceImpl.read()) {
					item.cost = CostStrategy.getInstance((Class<? extends Document<?>>) this.getClass()).getItemCost(p, this);
//					countTaxSum(p.tax1, item);
	...
	public void initFromOrder(OrderImplBase<?> src, GpsCoord location) {
		OrderPrint s = (OrderPrint) src.getData();
