...
public class Documents extends DocumentsW implements OrgSelectDialog.OrgSelect {
	...
	protected void adjustViewForDocType(DocType docType) {
		if( docType == TaskDoneDocW.instance()) {
	...
	protected void onResume() {
		...
		if( v != null ) {
			...
			v.setVisibility(haveOutstandingTask() ? View.VISIBLE : View.GONE);
		}
	}
	
	private boolean haveOutstandingTask() {
		boolean res = false;
		...
		String sql= "select count(*) from " + new OrgTask().getTableName() + " where not id in (select idTask from " + 
				new TaskDone().getTableName() + " where id = '" + orgId + "') and orgid = '" + orgId + "'";
		Cursor c = null;
		try {
	...
	protected void doCreate() {
		if (!ConfigHelper.isValidOrgTime(this, org.getData().id) && 
				DispositionActivity.isNeedDisposition(this, org.getData().id, DocType.getCurDoc().getObjectName()))
