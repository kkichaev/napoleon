...
//            }
    ...
    String evaluate(String input) throws SyntaxException {
        ...
        String result = new DecimalFormat("#.##").format(mSymbols.eval(input));
        
        if (result.equals(NAN)) { // treat NaN as Error
            mIsError = true;
            return mErrorString;
        }
        return result.replace('-', MINUS).replace(INFINITY, INFINITY_UNICODE).replace(',', '.');
    }
    ...
    public void applyResult(){
    	...
    	if (resultListener != null)
    		resultListener.applyResult(mResult);
    }
