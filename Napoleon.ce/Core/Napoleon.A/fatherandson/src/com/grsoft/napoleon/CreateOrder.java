...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKey(config, "ВидЦены", priceType, spPrices, o.costCode);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	 ...
	 * @param o
	 */
	private void initOrder(OrderEx ord, Org o) {
		ConfigImpl config = new ConfigImpl();
		...
		if(o.costype >= 0 && o.costype < kv.size()) {
			ord.prcType = kv.get(o.costype).key.toString();
			ord.costCode = ord.prcType;
			ord.sumType = o.costype;
		}
	}
