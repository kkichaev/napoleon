...
//	DateHandler dateHandler;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		spSklad = (Spinner) findViewById(R.id.spSklad);
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKey(config, "ВидЦены", priceType, spPrices, o.prcType);
		DialogHelper.loadSpinnerWithKey(config, "Склады", new ArrayList<KeyValue>(), spSklad, o.whCode);
		
		spSklad.setOnItemSelectedListener(new OnItemSelectedListener() {
		...
		spDog.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
				String prc = ((OrgDog)parent.getItemAtPosition(position)).prcType;
				for(int i=0;i<priceType.size(); i++) {
					KeyValue kv = priceType.get(i);
					if(kv.key.toString().equals(prc)) {
						spPrices.setSelection(i, true);
						break;
					}
				}
			}
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	private void loadFirms(final OrderEx order) {
		...
		DataTraveler.travel(Firm.class, new DataTraveler.Travel<Firm>() {
			...
			public boolean travel(DataTraveler<Firm> item) {
				if(order.firmCode.equals(item.data.id))
					selectedFirm = firms.size();
				firms.add(new KeyValue(item.data.id, item.data.name));
				return true;
			}
		}, "");
		ArrayAdapter<KeyValue> fa = new ArrayAdapter<KeyValue>(this, R.layout.simple_spinner_layout, firms);
		spFirma.setAdapter(fa);
		if(selectedFirm >= 0)
			spFirma.setSelection(selectedFirm);
	}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			if( costType >= 0 ) {
				o.sumType = costType;
				o.prcType = priceType.get(costType).key.toString();
			}
			
			OrgDog selDog = (OrgDog)((Spinner)findViewById(R.id.spDogovor)).getSelectedItem();
			if( selDog != null)
				o.dogovor = selDog.id;
