...
import com.grsoft.view.DropDownListAdapter;

public class CreateOrder extends Activity
{
	private OrderImplBase<? extends Order> order;
	...
	protected Spinner spPrices;
	private EditText edNumZak;
	...
	protected void init()
	{
		...
		edNumZak = (EditText) findViewById(R.id.edNumZak);
		...
		DropDownListAdapter<CharSequence> firmaAdapter = 
				new DropDownListAdapter<CharSequence>(this, firms);
				
		spFirma.setAdapter(firmaAdapter);
		
		DropDownListAdapter<CharSequence> pricesAdapter = 
				new DropDownListAdapter<CharSequence>(this, priceType);
		...
		org.read();
		org.close();
		tvOrgName.setText(od.name);
		

		if(!editMode)
			o.sumType = od.costype;
		...
		LinearLayout llDebt = (LinearLayout) findViewById(R.id.llDebt);
		
		OrgEx oe = (OrgEx)org.getData();
		if(oe.balance > 0){
			llDebt.setVisibility(View.VISIBLE);
			
			((TextView)findViewById(R.id.tvDebt)).setText(
					Util.IntToScaleStr(oe.balance, Consts.SUM_SCALE));
		}else{
			llDebt.setVisibility(View.GONE);
		}
		
		edNumZak.setText(o.numzak);
	...
	protected void updateDisplayDate()
	{
	...
	class OKClickListener extends OnClickListenerToNotify
	{
		...
		public void onClick(View v)
		{
			...
			if (editMode && (order.getSumType() != getSumType())){
					askToApplyNewSumType(v.getContext(), getSumType());
			}else 
				okDone(false);
		}
		...
		private void okDone(boolean updateSumType){
		    ...
		    o.numzak = edNumZak.getText().toString();
			order.write();
