...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		String orderPayType = ((OrgEx)oi.getData()).payType;
		final String TPKEY = "ТипОплаты";
		
		config.getData().key = TPKEY;
		boolean configIsContains = false;
		
		if(config.read()){
			ArrayList<CharSequence> pt = new ArrayList<CharSequence>();
			DialogHelper.makeList(config.getData().value, pt);
			
			for(CharSequence val : pt)
				if(val.equals(orderPayType)){
					configIsContains = true;
					break;
				}
		}
		...
		if(!configIsContains && orderPayType != null && orderPayType.trim().length() > 0)
			priceType.add(orderPayType);
		
		if(o.payType.length() == 0)
			o.payType = orderPayType;
		
		DialogHelper.loadSpinnerFromConfig(config, TPKEY, priceType, spPrices, o.payType);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.payType = (String) spPayType.getSelectedItem();
