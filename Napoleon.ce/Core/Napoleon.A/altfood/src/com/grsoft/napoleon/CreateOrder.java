...
//	DateHandler dateHandler;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cbDover = (CheckBox) findViewById(R.id.cbDover);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		int sel = -1;
		for(int i = 0; i < org.dogovors.size(); i++) {
			OrgDog od  = org.dogovors.get(i);
			KeyValue kv = new KeyValue(od.id, od.name);
			values.add(kv);
			
			if (sel == -1 && o.dogovor.equals(od.id))
				sel = i;
		}
		...
		if (spDogs.getCount() > 0)
			spDogs.setSelection(sel == - 1 ? 0 : sel);

		cbDover.setChecked(o.dover != 0);
