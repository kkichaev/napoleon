...
public class CreateOrder extends BaseActivity
{
	...
	private static final int DIALOG_DISCOUNT_ID = 2;
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
        ...
        findViewById(R.id.tvDiscount).setOnClickListener(new View.OnClickListener() {			
			@Override public void onClick(View v) { changeDiscount(); }
		});
        updateDiscount();
        refreshDate();
	}

	protected void changeDiscount() { showDialog(DIALOG_DISCOUNT_ID); }

	private void updateDiscount() {
		int dsc = ((OrderEx)order.getData()).discount;
		
		String label = (dsc <= 0) ? "Скидка:" : "Наценка:";
		TextView tv = (TextView)findViewById(R.id.tvDiscountLabel);
		tv.setText(label);
		
		tv = (TextView)findViewById(R.id.tvDiscount);
		tv.setText(Util.IntToScaleStr(Math.abs(dsc), Consts.DISCOUNT_SCALE, Util.DEC_DELIM, false) + " %");
	}
	...
	private void initOrder(OrderEx o, Org org) {
		...
		o.whIndex = ((org.flags & Org.FL_CUSTOM_FIZ) == 0) ? 0 : 1;
	}
	
	private void updateDisplayDelay() {
		((TextView)findViewById(R.id.tvDelay)).setText(Integer.toString(order.getData().delay));
	}
	...
	protected Dialog onCreateDialog(int id) {
		switch(id) {
			...
			case DIALOG_DISCOUNT_ID:
				AlertDialog.Builder b = new AlertDialog.Builder(this);
				b.setTitle("Введите скидку");
				if( panel == null ) {
					panel = View.inflate(this, R.layout.input_discount, null);
					Spinner sp = (Spinner)panel.findViewById(R.id.spDiscount);
				    ArrayAdapter<CharSequence> adapter = ArrayAdapter.createFromResource(
				            this, R.array.discounts, android.R.layout.simple_spinner_item);
				...
				panel.findViewById(R.id.btnOK).setOnClickListener(new View.OnClickListener() {					
					@Override public void onClick(View v) { discountChanged(d); d.dismiss(); }
				});
				return d;
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		if( id == DIALOG_DISCOUNT_ID ) {
			Spinner sp = (Spinner) dialog.findViewById(R.id.spDiscount);
			EditText ed = (EditText) dialog.findViewById(R.id.edCount);
			int dsc = ((OrderEx)order.getData()).discount;
			
			sp.setSelection((dsc<=0) ? 0 : 1);
			ed.setText(Util.IntToScaleStr(Math.abs(dsc), Consts.DISCOUNT_SCALE, Util.DEC_DELIM, false));
			ed.selectAll();
		} else
			super.onPrepareDialog(id, dialog);
	}
	
	protected void discountChanged(Dialog dialog) {
		Spinner sp = (Spinner) dialog.findViewById(R.id.spDiscount);
		EditText ed = (EditText) dialog.findViewById(R.id.edCount);
		
		int dsc = Util.StrToScale(ed.getText().toString(), Consts.DISCOUNT_SCALE);
		if( sp.getSelectedItemPosition() == 0 )
			dsc = -dsc;
		((OrderEx)order.getData()).discount = dsc;
		updateDiscount();
	}
