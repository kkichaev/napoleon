...
public class ReturnPriceCount extends PriceCount implements OrderImplBase.UpdateQtyHandler {
	...
	protected void refreshData() {
		...
		DialogHelper.loadSpinnerWithKey(ci, "ПричиныВозврата", new ArrayList<KeyValue>(), sp, re == null ? "" : re.cause);
		ci.close();
		
		adapter.refresh(document.getId(), itemId, document.getDate(), (((ReturnEx)document.getData()).dogovor));
		updateQty();
	}
	...
	class DlvAdapter extends BaseAdapter {
		...
		public void refresh(String orgId, String itemId, Date bestBfr, String dogovor) {
...
class DlvData {
	...
	public DlvData(Delivery doc, DeliveryItem item) {
		int sum = item.qty == 0 ? 0 : (int)((long)item.sum * Consts.QTY_SCALE / item.qty);
