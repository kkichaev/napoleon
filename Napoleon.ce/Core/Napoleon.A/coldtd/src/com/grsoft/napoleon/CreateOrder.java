...
{
	private OrderImplBase<?> order = (OrderImplBase<?>) DocType.getCurDoc().create();
	
	private static final int DIALOG_DATE_PICKER_ID = 0;
	private static final int DIALOG_TIME_PICKER_ID = 1;

	private static final int ORD_DATE_PICKER_ID = 2;
...
//		config.close();
		...
		ordDateHandler = new DateHandler((TextView)findViewById(R.id.tvOrdDate), o.orderDate, ORD_DATE_PICKER_ID);
		
		EditText ed = (EditText)findViewById(R.id.edOrdNumber);
		ed.setText(o.orderNumber);
	...
	private void loadFirms() {
		...
		int selected = -1;
		String firmCode = ((OrderPrint)order.getData()).firmCode;
		while( bdo ) {
			KeyValue kv = new KeyValue(fi.id, fi.name);
			if( firmCode.equals(fi.id) )
				selected = firms.size();
			firms.add(kv);
			bdo = r.selectNext(fi);
		}
	...
	protected Dialog onCreateDialog(int id) {
		switch(id) {
			...
			case ORD_DATE_PICKER_ID:
				return ordDateHandler.createDialog();
		}
...
//			int costType = spPrices.getSelectedItemPosition();
			...
			if( suppl >= 0 )
				o.firmCode = firms.get(suppl).key.toString();
//			if( costType >= 0 )
//				o.sumType = costType;
			
			EditText ed = (EditText)findViewById(R.id.edOrdNumber);
			o.orderNumber = ed.getText().toString();
