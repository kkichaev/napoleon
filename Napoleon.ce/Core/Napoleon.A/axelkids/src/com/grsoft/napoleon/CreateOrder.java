...
public class CreateOrder extends BaseActivity
{
	...
	private CostStrategyEx costStrategy;
...
//	DateHandler dateHandler;
	...
	private void init() {
		costStrategy = (CostStrategyEx) CostStrategy.getInstance(order.getClass());
		...
		if (!editMode || order.getData().items.size() == 0)
			tv.setEnabled(false);
		else
					...
					DiscountInputDlg.open(CreateOrder.this, new InputNumber() {
						...
						public void applayInput(int value, Object... params) {
								...
								for(OrderItem i : order.getData().items){
									...
									int priceCost  = costStrategy.getBaseCost(price.getData(), order);
									i.cost = CostStrategyEx.calcDiscount(
											-discount+costStrategy.getDiscount(
													price.getData(), order),
									priceCost);
									((OrderItemEx)i).discount = discount;
								}
								
								updateDicsount();
						}
					}, Consts.SUM_SCALE, false, getString(R.string.discount_percent),  DiscountInputDlg.Type.Both);
				}
			});
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			OrderDoc.instance().refreshDocSum(order.getId());
