...
public class WarehouseEx extends WarehouseNew {
	...
	public void sortingPriceList(ArrayList<TreeNode> price) {
		Collections.sort(price, new GroupComparator(true));
	}
	...
	class GroupComparator extends TreeNodeCmp {
		...
		public int compare(TreeNode object1, TreeNode object2) {
			if (object1 instanceof PriceTreeNode
				...
				PriceTreeNode ptn1 = (PriceTreeNode)object1;
				PriceTreeNode ptn2 = (PriceTreeNode)object2;
				if(upReds) {
					if(redids.contains(ptn1.getId())) {
						return redids.contains(ptn2.getId()) ? super.compare(object1, object2) : -1;
					}
					if(redids.contains(ptn2.getId()))
						return 1;
				}
				...
				try {
					...
					lhs = db.rawQuery(
							...
							new String[] { Long.toString(ptn1.getRowid()) });

					rhs = db.rawQuery(
							"select name from folder where id = (select folderid from price where rowid = ?)",
							new String[] { Long.toString(ptn2.getRowid()) });

					if (lhs.moveToFirst() && rhs.moveToFirst()) {
						String f1 = lhs.getString(0);
						String f2 = rhs.getString(0);
