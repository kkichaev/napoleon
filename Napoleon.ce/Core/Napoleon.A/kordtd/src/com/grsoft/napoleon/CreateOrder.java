...
public class CreateOrder extends BaseActivity {
	...
	private void init() {
		...
		Spinner spDlv = (Spinner) findViewById(R.id.spDlv);
		DialogHelper.loadSpinnerWithKey(config, "Доставка", new ArrayList<KeyValue>(), spDlv, o.deliveryType);

		if (((OrgEx) oi.getData()).nodisc == 0) {
			Spinner spDsc = (Spinner) findViewById(R.id.spDsc);
			DialogHelper.loadSpinnerWithKey(config, "Скидки", new ArrayList<KeyValue>(), spDsc, o.discount);
		} else
			findViewById(R.id.trDsc).setVisibility(View.GONE);
		...
		if (Features.DELIVERY_ADDRESS) {
			...
			if (v != null) {
				...
				if (spAddress != null) {
					...
					ArrayAdapter<KeyValue> aa = new ArrayAdapter<KeyValue>(this, R.layout.simple_spinner_layout,
							addresses);
		...
		// dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date,
		// DIALOG_DATE_PICKER_ID);
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			Spinner spDlv = (Spinner) findViewById(R.id.spDlv);
			KeyValue sel = (KeyValue) spDlv.getSelectedItem();
			if (sel != null) {
				o.deliveryType = sel.key.toString();
			}

			sel = (KeyValue) ((Spinner) findViewById(R.id.spDsc)).getSelectedItem();
			if (sel != null) {
				o.discount = sel.key.toString();
				o.dscValue = Util.StrToScale(sel.value.toString(), Consts.SUM_SCALE);
			}
