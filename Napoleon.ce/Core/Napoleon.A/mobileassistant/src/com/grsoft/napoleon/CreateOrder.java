...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	public void initDogovor() {
		spDogovor = (Spinner) findViewById(R.id.spDogovor);
		Cursor c = null;
		
		try{
			final String IDCLMN = "id";
			final String NAMECLMN = "name";
			
			c = DataBaseManager.getDataBase().query(DataObjectInfo.getInstance().getTableName(Dogovor.class), 
					new String[]{IDCLMN, NAMECLMN}, null, null, null, null, null);
		
			ArrayList<KeyValue> dgv = new ArrayList<KeyValue>();
			while(c.moveToNext()){
				KeyValue kv = new KeyValue(c.getString(c.getColumnIndex(IDCLMN)), c.getString(c.getColumnIndex(NAMECLMN)));
				dgv.add(kv);
			}
			
			ArrayAdapter<KeyValue> aa = new ArrayAdapter<KeyValue>(this, R.layout.simple_spinner_layout, dgv);
			...
			for(int i = 0; i < aa.getCount(); i++){
				...
				if (kv.key.equals(o.dgv)){
					selected = i;
					break;
				}
			}
			
			spDogovor.setSelection(selected, true);
		}catch(Exception e){
	...
	private void updateDisplayDelay() {
		((TextView)findViewById(R.id.tvDelay)).setText("Поставка через: " + 
				order.getData().delay);
	}
...
//				return dateHandler.createDialog();
	...
	class DelayClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			...
			builder.setTitle("Поставка через");
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			KeyValue d = (KeyValue) spDogovor.getSelectedItem();
			if(d != null)
				o.dgv = d.key.toString();
