...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		((TextView)findViewById(R.id.tvDiscount)).setText(
				getString(R.string.discount_val,
						Util.IntToScaleStr(((OrgEx)oi.getData()).discount, Consts.SUM_SCALE)));
		...
		final int priceIdx = oi.getData().costype;
		
		spWh.setOnItemSelectedListener(new OnItemSelectedListener() {

			@Override
			public void onItemSelected(AdapterView<?> arg0, View arg1,
					int pos, long arg3) {
				if(pos == 0)
					spPrices.setSelection(priceIdx, true);
				else if(pos == 1)
					spPrices.setSelection(0, true);
			}
