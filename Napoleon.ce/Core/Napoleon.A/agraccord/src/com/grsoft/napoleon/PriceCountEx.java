...
public class PriceCountEx extends PriceCount {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(document != null && document instanceof OrderImpl && document.getRowid() != ExtrasConst.INVALID_ID ) {
			...
			if( oe != null ) {
				...
				String table = DataObjectInfo.getInstance().getTableName(OrgDiscount.class); 
				String where = "id='" + document.getId() + "'";
				DbReader r = new DbReader();
				boolean bdo = r.select(od, table, where);
				if(bdo) {
	...
	void inputSumDD() {
		InputNumberDlg.open(PriceCountEx.this, new InputNumber() {
			...
			@Override
			public void applayInput(final int value, Object... params) {
				int sv = value * Consts.SUM_SCALE / getSumValue();
				if( sv <= maxPrcDD ) {
					sumDD = value;
					refreshSumDD();
					refreshPrcDD();
