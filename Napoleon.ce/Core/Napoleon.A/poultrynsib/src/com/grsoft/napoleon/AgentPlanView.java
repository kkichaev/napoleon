...
public class AgentPlanView extends Activity{
	...
	private Map<String, Integer> itemsWeight = new HashMap<String, Integer>(); 
	private Map<Integer, Integer> groupsWeight = new HashMap<Integer, Integer>();
			...
			private void collectOrdersWeight(Date begin, Date end) {
				...
				while(bdo){
					for(OrderItem item : data.items){
						...
						if(itemsWeight.containsKey(item.id))
							w = itemsWeight.get(item.id);
						
						w += FPOperation.itemMul(priceImpl.getData().weight, item.qty, Consts.QTY_SCALE);
						
						itemsWeight.put(item.id, w);
						
						int gw = 0;
						int fid = priceImpl.getData().folderID;
						
						if(groupsWeight.containsKey(fid))
							gw = groupsWeight.get(fid);
						
						gw += w;
						
						groupsWeight.put(fid, gw);
					}
	...
	private int getItemFact(String id) {
		...
		if(itemsWeight.containsKey(id))
			result = itemsWeight.get(id);;
	...
	private int getGroupFact(int id) {
		...
		if(groupsWeight.containsKey(id))
			result = groupsWeight.get(id); 
