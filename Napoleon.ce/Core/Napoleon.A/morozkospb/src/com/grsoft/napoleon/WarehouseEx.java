...
public class WarehouseEx extends WarehouseNew implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (dt == MerchDoc.instance() || dt == MonitoringDoc.instance() || DocType.getCurDoc() == RivalMntrDoc.instance()) {
		...
		else if (dt == MonitoringDoc.instance() || DocType.getCurDoc() == RivalMntrDoc.instance())
	...
	protected void postAdapterInit() {
		...
		if (adapterInited)
			...
			if (document.getRowid() != ExtrasConst.INVALID_ROWID && (dt == MerchDoc.instance()
					|| dt == MonitoringDoc.instance() || dt == RivalMntrDoc.instance())) {
	...
	protected boolean inheritedApplayMatrix(String matrixName) {
		...
		if (matrixName.equals(OrgMatrixAdapter.NAME)) {
			String suppl = ((ISuppl)document).getSuppl();
			applayAdapter(new OrgMatrixAdapter(this, document.getId(), 
	...
	protected int getItemLayoutId() {
		...
		else if (DocType.getCurDoc() == MonitoringDoc.instance() || DocType.getCurDoc() == RivalMntrDoc.instance())
	...
	public View getPriceView(PriceTreeNode node, View convertView) {
		...
		if (DocType.getCurDoc() == MerchDoc.instance()) {
			...
			MonitoringItem i = (MonitoringItem) ((MonitoringImplBase<?>) document).findItem(node.getId());
	...
	protected void updateTotalSum() {
		if (DocType.getCurDoc() == MerchDoc.instance() || DocType.getCurDoc() == MonitoringDoc.instance() || 
	...
	protected boolean isAllowAllPrice() {
		DocType dt = DocType.getCurDoc();
		return !(dt == MerchDoc.instance() || dt == MonitoringDoc.instance() || DocType.getCurDoc() == RivalMntrDoc.instance());
	}
