...
public class AgentPlanView extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new AsyncTask<Void, Void, String>() {
			...
			protected String doInBackground(Void... params) {
				...
				Iterator it = addedItems.entrySet().iterator();
				 while (it.hasNext()) {
					 ...
					 collectFromNode((Item)pairs.getValue(), html);
				...
				if (reader.select(data, orderTable, where)) {
					do {
						for (OrderItem item : data.items) {
							...
							int qty = item.qty;
							long sum = FPOperation.itemMul(item.cost, item.qty, Consts.QTY_SCALE);
			...
			private void bubbleUpValues(Item item, int qty, long sum)
			{
				if (null == item)
					return;
