...
public class CreateOrder extends BaseActivity
{
	...
	private ArrayList<CharSequence> dost = new ArrayList<CharSequence>();
...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		Spinner spDost = (Spinner) findViewById(R.id.spDost);
		DialogHelper.loadSpinnerFromConfig(config, "ИнтервалДоставки", dost, spDost, ((OrderEx)o).dst);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spDost = (Spinner) findViewById(R.id.spDost);
			int dostIdx = spDost.getSelectedItemPosition();
			
			if(dostIdx >= 0)
				((OrderEx)o).dst = dostIdx;
