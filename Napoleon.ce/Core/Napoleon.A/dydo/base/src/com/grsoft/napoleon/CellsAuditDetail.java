...
public class CellsAuditDetail extends BaseActivity implements SendResultListener {
	...
	protected static final int SELECT_CELL_NUMBER_OR_TYPE = 2;
	...
	View.OnLongClickListener changeCell = new OnLongClickListener() {
		...
		public boolean onLongClick(View v) {
			...
			showDialog(SELECT_CELL_NUMBER_OR_TYPE);
			return true;
		}
	};
	...
	protected android.app.Dialog onCreateDialog(int id) {
		...
		case SELECT_CELL_NUMBER_OR_TYPE:
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		if(id == SELECT_CELL_NUMBER_OR_TYPE) {
			((AlertDialog)dialog).getListView().setItemChecked(changedItem == null ? -1 : changedItem.cellType, true);
			return;
		}
	...
	private Dialog createCellDialog() {
		...
		builder.setSingleChoiceItems(R.array.cell_type_number, -1, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if( which < 3) {
					changedItem.cellType = which;
					adapter.notifyDataSetChanged();
				} else {
					InputNumberDlg.open(CellsAuditDetail.this, new InputNumber() {
						@Override public int getValue() { return changedItem.cell; }
						
						@Override
						public void applayInput(int value, Object... params) {
							changedItem.cell = value;
							doc.write();
							adapter.notifyDataSetChanged();
						}
						...
						public boolean isValid(int value, Object... params) {
							...
							for(CellsAuditItem ci : doc.getData().items) {
								if( ci.cell == value && ci != changedItem) {
									canDo = false;
	...
	class Adapter extends BaseAdapter {
		...
		public View getView(int pos, View view, ViewGroup arg2) {
			...
			if( item != null ) {
				...
				tv.setBackgroundColor(CellTypes.getBackColor(item.cellType));
