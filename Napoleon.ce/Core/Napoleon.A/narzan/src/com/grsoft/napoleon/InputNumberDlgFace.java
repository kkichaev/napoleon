...
public class InputNumberDlgFace {
	...
	public static void open(final Context context, final InputNumberEx inputNumber, int iscale, 
		...
		final EditText edFace = (EditText) panel.findViewById(R.id.edFace);
		...
		edFace.setText(Util.IntToScaleStr(values[1], scale, Util.DEC_DELIM, hideRest));
		edFace.setInputType(InputType.TYPE_NULL);
		
		InputNumberHelper nh = new InputNumberHelper((EditText)panel.findViewById(R.id.edFace));
		nh.makeNumericKeypad(panel);
		
		((TextView)panel.findViewById(R.id.tvQty)).setText(Util.IntToScaleStr(values[2], scale, Util.DEC_DELIM, hideRest));
		((TextView)panel.findViewById(R.id.tvFace)).setText(Util.IntToScaleStr(values[3], scale, Util.DEC_DELIM, hideRest));
			...
			public void onClick(View v) {
				...
				try{
					int[] value = new int[]{
							...
							Util.StrToScale(edFace.getText().toString(), scale)
					};
		...
		edFace.setOnFocusChangeListener(new OnFocusChangeListener() {
