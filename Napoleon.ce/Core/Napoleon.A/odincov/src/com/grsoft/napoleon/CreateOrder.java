...
public class CreateOrder extends BaseActivity
{
	...
	public static class CostTypeEx extends CostManager.CostType {
	...
	private void loadCostTypes() {
		...
		if( ctypes != null ) {
			...
			for( CostManager.CostType ct : ctypes ) {
				...
				costTypes.add(ctx);
				if( ct.id.compareTo(ord.costType) == 0 )
					selected = ctx;
			}
		}
...
//		DialogHelper.loadSpinnerFromConfig(config, "Организация", firms, spFirma, o.supplyer);
		...
		CheckBox cb;
		cb = (CheckBox)findViewById(R.id.cbSelfDlv);
		cb.setChecked(o.selfDlv > 0);
		cb = (CheckBox)findViewById(R.id.cbCert);
		cb.setChecked(o.cert > 0);
		cb = (CheckBox)findViewById(R.id.cbQuality);
		cb.setChecked(o.quality > 0);
		cb = (CheckBox)findViewById(R.id.cbA4);
		cb.setChecked(o.a4 > 0);
		
		cbBonus = (CheckBox)findViewById(R.id.cbBonus);
...
//				o.sumType = costType;
			...
			CheckBox cb;
			cb = (CheckBox)findViewById(R.id.cbSelfDlv);
			o.selfDlv = (cb.isChecked()) ? 1 : 0;
			cb = (CheckBox)findViewById(R.id.cbCert);
			o.cert = (cb.isChecked()) ? 1 : 0;
			cb = (CheckBox)findViewById(R.id.cbQuality);
			o.quality = (cb.isChecked()) ? 1 : 0;
			cb = (CheckBox)findViewById(R.id.cbA4);
			o.a4 = (cb.isChecked()) ? 1 : 0;
			
			o.bonus = cbBonus.isChecked() ? 1 : 0;
