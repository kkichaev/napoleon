...
public class PriceCountEx extends PriceCount {
	...
	void updatePackPrice() {
		...
		tv.setText(Util.IntToScaleStr(pc, Consts.SUM_SCALE, Util.DEC_DELIM,
				false));
	}
	...
	protected void makeSaleHistory(Price p) {
		...
		if (isComplexSalesHistory()) {
			...
			String history = ((OrderDoc) DocType.getCurDoc()).getHistory(
					document.getData().id, p.id);
			...
			for (int i = 0; i < historyItems.length - 1; i += 2) {
				...
				try {
					val = Integer.parseInt(historyItems[i + 1])
							* Consts.QTY_SCALE / qtyInPack;
				} catch (Exception e) {
					e.printStackTrace();
				}

				TextView tvSaleItem = new TextView(this);
				tvSaleItem.setText(Html.fromHtml(String.format("%s<br>%s",
						historyItems[i], Integer.toString(val))));
	...
	protected void refreshQty(Editable txt, boolean updateQty) {
		...
		if (firstView != null) {
			...
			OffTakeHistory.Item item = history.updateRest(price.getData().id,
					rest, prv);

			String qtxt = edCount.getText().toString();
			int count = qtxt.length() == 0 ? 0 : Util.StrToScale(qtxt,
					Consts.QTY_SCALE);

			int qty = (!prv.prevItem.empty()) ? item.qty / qtyInPack
					* Consts.QTY_SCALE : count;
