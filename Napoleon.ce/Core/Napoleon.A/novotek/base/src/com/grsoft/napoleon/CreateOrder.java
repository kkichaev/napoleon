...
{
	public static Class<? extends Activity> activity = CreateOrder.class;
	
	protected OrderImpl order = (OrderImpl)OrderDoc.instance().create();
	...
	private ArrayList<KeyValue> payType = new ArrayList<KeyValue>();
	private ArrayList<KeyValue> actions = new ArrayList<KeyValue>();
	...
	private void init() {
		...
		String firmid = o.firmCode;
		
		if(firmid.trim().length() == 0)
			firmid = ((OrgEx)oi.getData()).firmCode;
		
		DialogHelper.loadSpinnerWithKey(config, "Организация", firms, spFirma, firmid);
		
		Spinner spPayType = (Spinner) findViewById(R.id.spPayType);
		DialogHelper.loadSpinnerWithKey(config, "ФормаОплат", payType, spPayType, ((OrderEx)o).paytype);
		...
		CheckBox cb;
		cb = (CheckBox)findViewById(R.id.chItem);
		cb.setChecked(o.chitem > 0);

		cb = (CheckBox)findViewById(R.id.chPickUp);
		cb.setChecked(o.pickup > 0);
        ...
        Cursor<Action> c = new Cursor<Action>(new ActionImpl());
        actions.add(new KeyValue("",""));
        int selActPos = -1;
        int i = 0;
        
        Spinner spWh = (Spinner) findViewById(R.id.spWh);
		DialogHelper.loadSpinnerFromDataObject(spWh, Sklad.class, new DialogHelper.Selected<Sklad>() {
			@Override public boolean isSelected(Sklad object) { return object.key.equals(o.whCode); }
		}, false);

        
        while(c.moveNext()){
        	ActionImpl actionImpl = (ActionImpl) c.current();
        	
        	if(actionImpl != null){
	        	Action action = actionImpl.getData();
	        	
	        	if(action != null){
	        		if(((OrderEx)o).action.equals(action.id) && selActPos == -1)
	        			selActPos = i;
	        		actions.add(new KeyValue(action.id, action.name));
	        	}
        	}
        ...
        if(spAction != null){
        	spAction.setAdapter(new ArrayAdapter<KeyValue>(this, R.layout.simple_spinner_layout, actions));
        	
        	if(selActPos != -1)
        		spAction.setSelection(selActPos + 1, true);
        }
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spPayType = (Spinner) findViewById(R.id.spPayType);
			KeyValue payType = (KeyValue) spPayType.getSelectedItem();
			
			if(payType != null)
				o.paytype = payType.key.toString();
			
			Spinner spWh = (Spinner) findViewById(R.id.spWh);
			Sklad  kv = (Sklad)spWh.getSelectedItem();
			if( kv != null ) {
				o.whCode = kv.key;
				o.canDiv = kv.canDiv;
			...
			o.chitem = ((CheckBox)findViewById(R.id.chItem)).isChecked() ? 1 : 0;
			o.pickup = ((CheckBox)findViewById(R.id.chPickUp)).isChecked() ? 1 : 0;

			EditText remark = (EditText)findViewById(R.id.edCreateOrderNotes);
			o.remark = remark.getText().toString();
			
			if(spAction != null){
				KeyValue action = (KeyValue) spAction.getSelectedItem();
				
				if(action != null)
					((OrderEx)o).action = action.key.toString();
			}
