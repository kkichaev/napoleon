...
public class OrgBalanceHitching extends Hitching {
	...
	public void onStart() {
		try{
			...
			statement = database.compileStatement("UPDATE " + dlvTable + " SET sumd = ? WHERE number=? AND id=?");
			orgBalance.clear();
		}catch(Exception e){}
	}
	...
	public void onRead(RawObject rawObject) throws RuntimeException {
		...
		if (statement != null && dobj.num.length() > 0){
			dobj.num += DeliveryHitchingEx.sd.format(dobj.date);
			
			statement.clearBindings();
			statement.bindLong(1, dobj.sumd);
			statement.bindString(2, dobj.num);
			statement.bindString(3, dobj.id);
