...
//			return true;
	...
	public static String makeOrgInfo(OrgEx o, OrderImpl doc) {
		List<KeyValue> costTypes = new ArrayList<KeyValue>();
		OrderEx order = null;
		if( doc != null)
		...
		ci.read();
		ci.close();
		DialogHelper.makeListWithKey(c.value, costTypes, null);
		
		String costType = (costTypes.size() > o.costype) ? costTypes.get(o.costype).value.toString() : "";
		...
		ret += "<br/>код ТТ:" + o.codeTT;// + " категория:" + o.category + "," + o.segment; 
		...
		if(order != null) {
			limitFree +=  order.incass - doc.sum();
		}
		if( limitFree > o.debt)
			limitFree = o.debt;
