...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		List<CostItem> priceValues = CostItem.getItems(true);
		ArrayAdapter<CostItem> pa = new ArrayAdapter<CostItem>(this, R.layout.simple_spinner_layout, priceValues);
		pa.setDropDownViewResource(R.layout.simple_spinner_layout_drop_down);
		spPrices.setAdapter(pa);
		int sel = CostItem.getSelected(priceValues, o.sumType);
		if( sel >= 0)
			spPrices.setSelection(sel);
		...
		if( (o.params & BONUS) != 0 )
			((CheckBox)findViewById(R.id.cbBonus)).setChecked(true);
		...
		oldDisc = ((OrderEx)o).discount;
		findViewById(R.id.tvDiscount).setOnClickListener(new OnClickListener() { @Override public void onClick(View v) { discountClick(); }});
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	protected void discountClick() {
		DiscountInputDlg.open(CreateOrder.this, new InputNumber() {
			@Override public int getValue() { return -oldDisc; }
			
			@Override
			public void applayInput(int value, Object... params) {
				((OrderEx)order.getData()).discount = -value;
				updateDicsount();
			}}, Consts.SUM_SCALE, false, getString(R.string.cost_changing),DiscountInputDlg.Type.OnlyNac);
	}
	
	private void updateDicsount() {
		int val = ((OrderEx)order.getData()).discount;
		String label = "наценка,%";
...
//				return dateHandler.createDialog();
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			Spinner spPrices = (Spinner) findViewById(R.id.spPrices);
			CostItem ci = (CostItem)spPrices.getSelectedItem();
			
			int costType = ci.index;
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			if( ((CheckBox)findViewById(R.id.cbBonus)).isChecked() ) o.params |= BONUS;
			else o.params &= (~BONUS);
			...
			if(!updateSumType)
				updateSumType = (oldDisc == 0 && o.discount != 0) || (oldDisc != 0 && oldDisc != o.discount);
			...
			if(!editMode) {
				if (FocusedItemsImpl.getUnsettedItems(order).size() > 0)
					FocusItemEditorEx.open(CreateOrder.this, order, false);
				else
