...
public class NapoleonEx extends Napoleon {
	...
	protected BaseAdapter getMainOrgAdapter() throws IllegalAccessException, InstantiationException {
		return new MainOrgsEx(this);
	}
	
	@Override
	protected void adjustViewForDocType(DocType docType) {
		((MainOrgsEx)mainOrgsAdapter).setDebetFilter(docType == DebtDoc.instance());
	...
	class MainOrgsEx extends MainOrgsAdapter {

		public MainOrgsEx(Context context) throws IllegalAccessException, InstantiationException {
			super(context);
		}
		
		void setDebetFilter(boolean setFilter) {
			String filter = "";
			if(setFilter) {
				filter = " id in (select id from " + (new OrgSum()).getTableName() + " where \"type\"='Долги' and sum > 0 )";
			}
			cursor.setCondition(filter);
		}
	}
}
