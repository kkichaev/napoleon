...
public class TaskEdit extends TaskPreview {
	...
	protected void initView() {
		super.initView();
		((TextView)btnOK).setText(R.string.Finish);
				...
				try {
					if (Environment.getExternalStorageState().equals(
						...
						if (Build.VERSION.SDK_INT >= 24) {
							...
							uri = FileProvider.getUriForFile(c,"com.grsoft.ads.fileprovider", file); 
						}else
	...
	private OnClickListener finishClick() {
		return new OnClickListener() {
			...
			public void onClick(View view) {
				...
				task.getData().solution = TaskQuery.RESOLVED;
				task.write();
				task.close();
