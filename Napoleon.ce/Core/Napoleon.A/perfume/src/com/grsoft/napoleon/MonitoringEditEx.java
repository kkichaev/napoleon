...
import com.grsoft.dataobjects.MonitoringDocItemEx;
...
public class MonitoringEditEx extends MonitoringEdit {
	...
	protected void init(MonitoringDocImpl d) {
		...
		while(bdo) {
			MonitoringDocItem mdi = new MonitoringDocItemEx();
	...
	class Adapter extends BaseAdapter{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if( i != null ) {
				...
				text = "Цена:" + Util.IntToScaleStr(
						((MonitoringDocItemEx)i.item).cost,Consts.SUM_SCALE);
				tv.setText(text);
				tv.setTag(i);
				tv.setOnClickListener(new View.OnClickListener() {
		...
		protected void setCost(final TextView tv, final Item item) {
			InputNumberDlg.open(MonitoringEditEx.this, new InputNumber() {				
				@Override public int getValue() { return ((MonitoringDocItemEx)item.item).cost; }
				@Override public void applayInput(int value, Object... params) {
					((MonitoringDocItemEx)item.item).cost = value;
					String text = "Цена:" + Util.IntToScaleStr(
							((MonitoringDocItemEx)item.item).cost,Consts.SUM_SCALE);
					tv.setText(text);
				}
