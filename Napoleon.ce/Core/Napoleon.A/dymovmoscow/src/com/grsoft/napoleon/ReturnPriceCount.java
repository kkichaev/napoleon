...
public class ReturnPriceCount extends PriceCount implements OrderImplBase.UpdateQtyHandler, EventHandler {
	
	ImagesItemsAdapter photoAdapter; 
	...
	private void openAssociatedVisit() {
		...
		Return o = (Return) document.getData();
		if( !visit.openAssociatedVisit(o) ) {
...
//			adapter = new ImagesItemsAdapter(this, visit);
	...
	protected void onDestroy() {
		super.onDestroy();
		Visit v = visit.getData();
		boolean canDelete = (v.items.size() == 0);
		if( canDelete )
			visit.delete();
		visit.close();
	}
	...
	protected void onResume() {
		...
		List<VisitItem> phItems = visit.getItems(price.getData().id);
		photoAdapter = new ImagesItemsAdapter(this, phItems);
		HorizontalListView g = (HorizontalListView)findViewById(R.id.gvItems);
		g.setAdapter(photoAdapter);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if(requestCode == PhotoClickHandler.CAMERA_ACTIVITY && resultCode == RESULT_OK){
			if(picPath != null && picPath.trim().length() > 0) {
				visit.addPhoto(picPath.getBytes(), price.getData().id, document.getId());
				List<VisitItem> phItems = visit.getItems(price.getData().id);
				photoAdapter = new ImagesItemsAdapter(this, phItems);
				HorizontalListView g = (HorizontalListView)findViewById(R.id.gvItems);
				g.setAdapter(photoAdapter);
	...
	@Override public void prepareBoforeClick() {
		VisitImplEx.setPhotoTag(price.getData().id, document.getId());
	}
