...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		boolean bdo = r.select(f, table, "", "name");
		while(bdo) {
			if(f.id.equals(ob.getFirm()))
				selected = values.size();
			values.add(new KeyValue(f.id, f.name));
			bdo = r.selectNext(f);
		}
		r.close();
		...
		DialogHelper.loadSpinnerWithKey(config, "¬идыќплат", values, sp, ob.getPayType());
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	private void refreshDate() {
		((TextView)findViewById(R.id.tvDate)).setText(Util.simpleDateFormat.format(order.getDate()));		
	}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spFirma = (Spinner) findViewById(R.id.spFirma);
			KeyValue value = (KeyValue)spFirma.getSelectedItem();
			if( value != null ) {
				o.supplyer = spFirma.getSelectedItemPosition();
				ob.setFirm(value.key.toString());
			}
			Spinner spPrices = (Spinner) findViewById(R.id.spPrices);
			int costType = spPrices.getSelectedItemPosition();
			...
			value = (KeyValue)((Spinner)findViewById(R.id.spPayType)).getSelectedItem();
			if( value != null )
				ob.setPayType(value.key.toString());
