...
public class QtyEditor extends InputNumber {
	...
	public boolean isPackCanChange() {
		return (((PriceEx)p).pack == 0);
	}

	@Override
	public boolean isInpack() {
		return item == null ?
				(((PriceEx)p).pack > 0) || ((CfgNpl)ConfigManager.getConfig()).isPackView :
				item.inPack();
	}
	
	@Override public int getValue() { 
		return item == null ? 
				Consts.QTY_SCALE : item.inPack() ?
				(int)((long)item.qty * Consts.QTY_SCALE) / p.qtyInPack : 
				item.qty; 
	}
