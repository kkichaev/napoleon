...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKey(config, "Склады", new ArrayList<KeyValue>(), spWh, o.whId);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	private void initOrder(Order o, Org org) {
		int idx = Features.COST_MANAGER.getCostIndex(o.id);
		if(idx >= 0)
			o.sumType = idx;
		else
...
//				o.sumType = costType;
			...
			KeyValue sel = (KeyValue) ((Spinner)findViewById(R.id.spWh)).getSelectedItem();
			if( sel != null ) {
				o.whId = sel.key.toString();
			}
