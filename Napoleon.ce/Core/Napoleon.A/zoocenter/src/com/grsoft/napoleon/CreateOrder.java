...
//	TimeHandler timeHandler;
	...
	private void init() {
		cbTransport = (CheckBox) findViewById(R.id.cbTransport);
		
		cbTransport.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				edNumTransport.setEnabled(isChecked);
			}
		});
		
		edNumTransport = (EditText) findViewById(R.id.edNumTransport);
...
//		timeHandler = new TimeHandler((TextView)findViewById(R.id.tvTime), o.date, DIALOG_TIME_PICKER_ID);
		...
		cbTransport.setChecked(oe.transport > 0);
		
		if(oe.transport > 0){
			edNumTransport.setText(oe.numTransport);
			edNumTransport.setEnabled(true);
		}else
			edNumTransport.setEnabled(false);
	}
	...
	private void initOrder(Order o, Org org) {
		...
		o.date = calendar.getTime();
		
		ConfigImpl config = new ConfigImpl();
		
		config.getData().key = "Транспорт";
		try {
			if (config.read())
				((OrderEx)o).transport = Integer.parseInt(config.getData().value);
			else
				((OrderEx)o).transport = 1;
		} catch (Exception e) {
			e.printStackTrace();
		}
		config.close();
...
//			if( cash.isChecked() ) o.params |= ParamState.ofCash;
			...
			OrderEx oe = (OrderEx)o;
			oe.transport = cbTransport.isChecked() ? 1 : 0;
			
			if(cbTransport.isChecked()){
				oe.numTransport = edNumTransport.getText().toString().trim();
			}
