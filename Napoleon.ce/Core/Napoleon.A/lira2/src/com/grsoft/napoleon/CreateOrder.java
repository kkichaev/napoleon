...
public class CreateOrder extends Activity
{
	...
	protected void onResume() {
		...
		if( starting ) {
			...
			tvOrgName.setText(oe.name);

			if( oe.units != null ) {
				if( !editMode && oe.units.size() > 0 )
					o.unitCode = oe.units.get(0).id;
				
				tv.setOnClickListener(new View.OnClickListener() {					
					@Override public void onClick(View v) { UnitList.open(CreateOrder.this, order); };
				});
			}
		}
		
		if( oe.units != null ) {
			for( UnitItem ui : oe.units ) {
				if( ui.id == o.unitCode ) {
					tv.setText("Торг.точка: " + ui.name);
					break;
				}
			}
		}
