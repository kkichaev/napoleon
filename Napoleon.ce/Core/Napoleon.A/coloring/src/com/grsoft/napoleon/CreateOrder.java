...
public class CreateOrder extends BaseActivity
{
	...
	private ArrayList<CharSequence> stockNumber = new ArrayList<CharSequence>();
	private ArrayList<CharSequence> contracts = new ArrayList<CharSequence>();
...
//	DateHandler dateHandler;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cbDlvNote = (CheckBox) findViewById(R.id.cbDlvNote);
		init();
	}
	...
	private void init() {
		...
		Spinner spStock = (Spinner) findViewById(R.id.spStock);
		DialogHelper.loadSpinnerWithKey(config, "Склады", new ArrayList<KeyValue>(), spStock, o.whCode);
		
		int selected=0;
		
		final ArrayList<OrgDog> values = new ArrayList<OrgDog>();
		Spinner spDogs = (Spinner) findViewById(R.id.spDogovors);
		for(OrgDog od : oe.dogovors) {
			OrgDog kv = (OrgDog) od.clone();
			if(od.id.equals(o.dogovor))
				selected = values.size();
			values.add(kv);
		}
		ArrayAdapter<OrgDog> adapterDog = new ArrayAdapter<OrgDog>(this, R.layout.simple_spinner_layout, values);
		spDogs.setAdapter(adapterDog);
		if( selected < spDogs.getCount() )
			spDogs.setSelection(selected);
		
		spDogs.setOnItemSelectedListener(new OnItemSelectedListener() {

			@Override
			public void onItemSelected(AdapterView<?> parentView, View selectedView, int position, long id) {
				OrgDog od = (OrgDog)values.get(position);
				if( od != null ) {
					if( od.costype < spPrices.getCount() )
						spPrices.setSelection(od.costype);
				}
			}
			@Override
			...
			if( v != null ) {
				...
				if( spAddress != null ) {
					...
					if( selected >= 0 && selected < spAddress.getCount())
						spAddress.setSelection(dogSelected);
				}
			}
		}
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		cbDlvNote.setChecked(((OrderEx)order.getData()).dlvnote > 0);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner stockSp = (Spinner) findViewById(R.id.spStock);
			KeyValue kv = (KeyValue)stockSp.getSelectedItem();
			if( kv != null ) {
				o.whIndex = stockSp.getSelectedItemPosition();
				o.whCode = kv.key.toString();
			}

			Spinner spDogs = (Spinner) findViewById(R.id.spDogovors);
			OrgDog orgDog = (OrgDog)spDogs.getSelectedItem();
			if( orgDog != null )
				o.dogovor = orgDog.id;
			...
			((OrderEx)o).dlvnote = cbDlvNote.isChecked() ? 1 : 0;
