...
public class DebetView extends DocumentsBase {
	...
	class DebtAdapter extends BaseExpandableListAdapter {
		...
		private HashMap<String, String> loadDogovors(String orgId) {
			HashMap<String, String> dogovors = new HashMap<String, String>();
			OrgImpl oi = new OrgImpl();
			OrgEx o = (OrgEx)oi.getData();
			o.id = orgId;
			if( oi.read() )
				for(OrgDogovor dg : o.dogovors )
					dogovors.put(dg.id, dg.name);
			oi.close();
		...
		public void load(String orgId) {
			...
			HashMap<String, String> dogovors = loadDogovors(orgId);			
			data.clear();
			...
			for(int i=0; i<dl.getCount(); i++ ) {
				...
				payType = dogovors.get(dogId);
				if( payType == null )
					payType = "код договора'" + dogId + "'";
		...
		public View getChildView(int groupPosition, int childPosition, boolean isLastChild, View convertView, ViewGroup parent) {
			...
			if( dd != null ) {
				...
				TextView tv;
				tv = (TextView)convertView.findViewById(R.id.tvText);
				str = dd.number + "<br>" + Util.simpleDateFormat.format(dd.date);
				tv.setText(Html.fromHtml(str));
				tv.setTextColor(color);

				tv = (TextView)convertView.findViewById(R.id.tvDate);
				str = Util.IntToScaleStr(dd.sum, Consts.SUM_SCALE, Util.DEC_DELIM, false);
				tv.setText(str);
				tv.setTextColor(color);
			
				tv = (TextView)convertView.findViewById(R.id.tvSum);
				str = Util.IntToScaleStr(dd.exp, Consts.SUM_SCALE, Util.DEC_DELIM, false);
				tv.setText(str);
				tv.setTextColor(color);
			}
			return convertView;
