...
public class CreateOrder extends BaseActivity {
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKey(config, "Склады", new ArrayList<KeyValue>(), 
				(Spinner)findViewById(R.id.spSklads), ((OrderEx)o).sklad);
		...
		if (Features.DELIVERY_ADDRESS) {
			...
			if (v != null) {
				...
				if (spAddress != null) {
					...
					ArrayAdapter<KeyValue> aa = new ArrayAdapter<KeyValue>(
							this, R.layout.simple_spinner_layout, addresses);
		...
		findViewById(R.id.tvDate).setOnClickListener(
				new View.OnClickListener() {
					...
					public void onClick(View v) {
						Intent i = new Intent(CreateOrder.this,
		...
		Org oe = oi.getData();
		OrderEx ob = (OrderEx)o;
		
		DocHelper.loadSpinner(ob.dlvman, (Spinner) findViewById(R.id.spDlvMan), DeliveryMan.class);
		
		DocHelper.prepareSpinners((Spinner) findViewById(R.id.spDogovor),
				(Spinner) findViewById(R.id.spDiscount), 
				((OrgEx)oe).dogovors, ob.iddog, ob.discid);

		timeHandler = new TimeHandler((TextView) findViewById(R.id.tvTime),
				o.date, DIALOG_TIME_PICKER_ID);
	...
	private void refreshDate() {
		...
		((TextView) findViewById(R.id.tvDate)).setText(sd.format(order
				.getDate()));
	}
...
//	27.08.2014 Рамиль просил не прибавлять день(skype переговоры). kki		
...
//		o.date = c.getTime();
	...
	private void updateDisplayDelay() {
		((TextView) findViewById(R.id.tvDelay))
				.setText(getString(R.string.delay) + ": "
						+ order.getData().delay);
	}
	...
	class DelayClickListener implements OnClickListener {
		...
		public void onClick(View v) {
			AlertDialog.Builder builder = new AlertDialog.Builder(
					v.getContext());
			builder.setTitle(R.string.delay);
			View dialogView = View.inflate(v.getContext(), R.layout.counter,
					null);

			builder.setView(dialogView);
			final AlertDialog dialog = builder.create();

			Button btnCounterUp = (Button) dialogView
					.findViewById(R.id.btnCounterUp);
			Button btnCounterDown = (Button) dialogView
					.findViewById(R.id.btnCounterDown);
			Button btnCounterOK = (Button) dialogView
					.findViewById(R.id.btnCounterOk);
			Button btnCounterCancel = (Button) dialogView
					.findViewById(R.id.btnCounterCancel);
			final TextView tvCounter = (TextView) dialogView
					.findViewById(R.id.edCounter);
			...
			btnCounterOK.setOnClickListener(new OnClickListenerToNotify() {
				...
				public void onClick(View v) {
					super.onClick(v);
					order.getData().delay = Integer.parseInt(tvCounter
							.getText().toString());
					updateDisplayDelay();
					dialog.hide();
				}
	...
	class OKClickListener extends OnClickListenerToNotify {
			...
			if (editMode && (order.getSumType() != costType && costType >= 0))
				...
				if (!editMode) {
					AlertDialog.Builder b = new AlertDialog.Builder(
							v.getContext());
					b.setTitle(R.string.select_item);
					CharSequence[] items = new CharSequence[] { getString(R.string.autoorder) ,
							getString(R.string.planeorder)};
		...
		private void okDone(boolean updateSumType, boolean openDetail) {
			Order o = (Order) order.getData();
			OrderEx ob = (OrderEx)o;
			...
			sp = (Spinner)findViewById(R.id.spSklads);
			val = (KeyValue)sp.getSelectedItem();
			if (val != null) {
				ob.sklad = val.key.toString();
				ob.skladIndex = sp.getSelectedItemPosition();
		...
		private void askToApplyNewSumType(Context context, final int newSumType) {
			...
			builder.setMessage(R.string.type_cost_are_changed);

			builder.setPositiveButton(R.string.recount,
					new DialogInterface.OnClickListener() {
			...
			builder.setNegativeButton(R.string.leave,
					new DialogInterface.OnClickListener() {
