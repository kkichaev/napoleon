...
public class DebetView extends DocumentsBase {
	...
	HashMap<String, String> payTypes = new HashMap<String, String>();

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		
		for(OrgDogovors od : ((OrgEx)org.getData()).dogovors)
			payTypes.put(od.id, od.name);
		...
		public void load(String orgId) {
			...
			for(int i=0; i<dl.getCount(); i++ ) {
				...
				if( doc instanceof DeliveryImpl ) {
					...
					payType = payTypes.get(d.idDog);
					dd = new DocData(d);
				} else {
					PaymentEx p = (PaymentEx) doc.getData();
					payType = payTypes.get(p.idDog);
					dd = new DocData(p);
				}
				if( payType == null )
					payType = "";
