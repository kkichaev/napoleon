...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		Spinner sp = (Spinner) findViewById(R.id.spPayType);
		DialogHelper.loadSpinnerFromConfig(config, "ВидОплаты", new ArrayList<CharSequence>(), sp, o.payType);
		
		sp = (Spinner) findViewById(R.id.spDocs);
		DialogHelper.loadSpinnerFromConfig(config, "Документы", new ArrayList<CharSequence>(), sp, o.docType);
		...
		((CheckBox)findViewById(R.id.cbDover)).setChecked(o.dover > 0);
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			o.dover = ((CheckBox)findViewById(R.id.cbDover)).isChecked() ? 1 : 0;
			o.pickUp = ((CheckBox)findViewById(R.id.cbPickUp)).isChecked() ? 1 : 0;
			
			Spinner sp = (Spinner)findViewById(R.id.spPayType);
			String val = (String)sp.getSelectedItem();
			if(val != null)
				o.payType = val;

			sp = (Spinner)findViewById(R.id.spDocs);
			val = (String)sp.getSelectedItem();
			if(val != null)
				o.docType = val;
