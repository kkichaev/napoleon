...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		selected = -1;
		sp = (Spinner) findViewById(R.id.spAgent);
		ExecAgent a = new ExecAgent();
		DbReader r = new DbReader();
		values = new ArrayList<KeyValue>();
		boolean bdo = r.select(a, DataObjectInfo.getInstance().getTableName(a.getClass()), null, "name");
		while(bdo) {
			if(o.idAgent.length() == 0)
				o.idAgent = a.userid;
			
			KeyValue kv = new KeyValue(a.id, a.name);
			if( kv.key.equals(o.idAgent))
				selected = values.size();
			values.add(kv);
			bdo = r.selectNext(a);
		}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			if( val != null )
				o.dogId = val.key.toString();

			sp = (Spinner) findViewById(R.id.spAgent);
			val = (KeyValue) sp.getSelectedItem();
			if( val != null )
				o.idAgent = val.key.toString();
