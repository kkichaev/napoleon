...
public class OrderDetailEx extends OrderDetail implements SendResultListener {
	AgentNeedSell needMoreSell = new AgentNeedSell();
	
	boolean doBack = false, needCheckPlan = false;
	EditText editOutRemark;
	DailyPlanImpl dpi = null;
	Map<Folder, Integer> underPlan;
	ListView underPlanList;
	...
	public void updateTotalSum(long sum, int weight, int count) {
		...
		String text = "<b>" + DocType.SumConverter.toString(sum) + "</b>";
		if(weight != 0) {
			text += "<br/>";
			long avgCost = sum * Consts.WEIGHT_SCALE / weight;
			text += "<i>" + OrderDoc.instance().weightToString(weight, getString(R.string.kg)) + "</i>&nbsp;&nbsp;&nbsp;";
			text += "<b>" + Util.IntToScaleStr(avgCost, Consts.SUM_SCALE, Util.DEC_DELIM, false) + "<b/>";
		}
		tv.setText(Html.fromHtml(text));
	}
	
	public static void updatePrcInfo(Activity a, OrderEx o) {
		final double DEFAULE_VALUE = 5;
		final double ADD_SUM_VALUE = 50;
		final double HALF_GROSH = 0.005;
		double ap = AgentProcent.getProcent();
		
		if (ap == 0)
			ap = DEFAULE_VALUE;
		
		double prc = ap / (100.0 * Consts.SUM_SCALE);
		double sum = o.sum() * prc + ADD_SUM_VALUE + HALF_GROSH;
	...
	boolean checkPlan() {
		boolean ret = true;
		if(dpi == null) {
			dpi = new DailyPlanImpl();
			long rid = DailyPlanImpl.getPlan(doc.getId(), doc.getData().created);
			if(rid != ExtrasConst.INVALID_ROWID) {
				dpi.read(rid);
			}
		}
		
		underPlan = dpi.checkDoc((OrderImpl)doc);
		if(underPlan.size() > 0) {
			ret = false;
			showDialog(R.id.order_below_plan_dlg);
		}
	...
	protected Dialog onCreateDialog(int id) {
		...
		if(id == R.id.order_below_plan_dlg) {
			AlertDialog.Builder b = new AlertDialog.Builder(this);
			b.setTitle("Вы не заказали по плану");
			View v = View.inflate(this, R.layout.below_plan, null);
			underPlanList = (ListView)v.findViewById(R.id.lvItems);
			b.setView(v);
			b.setNegativeButton(android.R.string.no, new DialogInterface.OnClickListener() {
				@Override public void onClick(DialogInterface arg0, int arg1) {
					arg0.dismiss();
	...
	protected void onPrepareDialog(int id, Dialog dialog) {
		...
		} else if( id == R.id.order_below_plan_dlg) {
			underPlanList.setAdapter(new PlanAdapter(underPlan));
		} else
	...
	class PlanAdapter extends BaseAdapter {
		...
		public PlanAdapter(Map<Folder, Integer> underPlan) {
			this.underPlan = underPlan;
			items = new ArrayList<Folder>(underPlan.keySet());
			
			Collections.sort(items, new Comparator<Folder>() {
				@Override public int compare(Folder arg0, Folder arg1) { return arg0.name.compareTo(arg1.name); }
			});
		}
		...
		public View getView(int arg0, View view, ViewGroup arg2) {
			if(view == null) {
				view = View.inflate(OrderDetailEx.this, R.layout.belos_plan_row, null);
			}
			Folder f = (Folder)getItem(arg0);
			TextView tv;
			tv = (TextView)view.findViewById(R.id.tvName);
			tv.setText(f.name);
			Integer val = underPlan.get(f);
			tv = (TextView)view.findViewById(R.id.tvQty);
			tv.setText(val == null ? "" : Util.IntToScaleStr(val, Consts.WEIGHT_SCALE, Util.DEC_DELIM, false));
			return view;
		}
