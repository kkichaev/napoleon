...
public class WarehouseEx extends WarehouseNew {
	...
	protected void onResume() {
		...
		if(document.getRowid() != ExtrasConst.INVALID_ROWID && document instanceof OrderImpl) {
			long rid = DailyPlanImpl.getPlan(document.getId(), ((OrderImpl)document).getData().created);
			if(rid != ExtrasConst.INVALID_ROWID) {
				DailyPlanImpl dpi = new DailyPlanImpl();
				dpi.read(rid);
				plan = dpi.getData();
				
				DailySalesData dsd = DailySalesData.load(document.getId(), ((OrderImpl)document).getData().created);
	...
	public View getFolderView(FolderTreeNode node, View convertView) {
		...
		TextView tvSales = (TextView)v.findViewById(R.id.tvSales);
		if(salesData != null) {
			long wp = plan.getPlan(fe);
			String text = "";
			if(wp > 0) {
				long wf = salesData.get(fe);
				text = Util.IntToScaleStr(wp, Consts.WEIGHT_SCALE, Util.DEC_DELIM, false) + "/" + Util.IntToScaleStr(wf, Consts.WEIGHT_SCALE, Util.DEC_DELIM, false);
			}
			tvSales.setText(text);
