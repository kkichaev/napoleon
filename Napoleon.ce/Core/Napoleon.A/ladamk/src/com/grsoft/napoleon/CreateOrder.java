...
public class CreateOrder extends BaseActivity
{
	...
	private static final int DOVER_PAY = 0x10;
	private static final int DOVER_RET = 0x20;

	protected static final int WH_REMARK_LIST = 10;
	protected static final int OTHER_REMARK_LIST = 11;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKey(config, "ВидЦены", values, spPrices, o.priceType);
		...
		spSklad.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			@Override public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) { FoldersAdapter.resetCache(); }
		...
		ed.setText(o.whRemark);
		if( order.isEditable() )
			ed.setOnLongClickListener(new View.OnLongClickListener() {
				@Override
				public boolean onLongClick(View v) {
					showDialog(WH_REMARK_LIST);
					return true;
				}
			});
		
		ed = (EditText)findViewById(R.id.edOthRemark);
		ed.setText(o.otherRemark);
		if( order.isEditable() )
			ed.setOnLongClickListener(new View.OnLongClickListener() {
				@Override
				public boolean onLongClick(View v) {
					showDialog(OTHER_REMARK_LIST);
					return true;
				}
			});
		...
		if( (o.params & DOVER_PAY) != 0 )
			((CheckBox)findViewById(R.id.cbDoverPay)).setChecked(true);

		if( (o.params & DOVER_RET) != 0 )
			((CheckBox)findViewById(R.id.cbDoverRet)).setChecked(true);
		
		Spinner spOrg = (Spinner) findViewById(R.id.spOrg);
		
		final ArrayList<KeyValue> orgs = new ArrayList<KeyValue>();
		int selOrg = -1;
		...
		Collections.sort(orgs, new Comparator<KeyValue>() {
		...
		for (int i = 0; i < orgs.size(); i++)
			if( orgs.get(i).key.toString().equals(o.id2)) {
				selOrg = i;
				break;
			}
		
		ArrayAdapter<KeyValue> aa = new ArrayAdapter<KeyValue>(this, R.layout.simple_spinner_layout, orgs);
		spOrg.setAdapter(aa);
		if( selOrg >= 0 && selOrg < spOrg.getCount())
			spOrg.setSelection(selOrg, true);
		...
		dateHandler.setHandler(new DateHandler.Handler() {
			...
			public boolean canSetDate(Date newDate) {
				return (newDate.compareTo(Util.getDate()) >= 0);
			}
		});
	...
	private void initOrder(OrderEx o, OrgEx org) {
		o.priceType = org.priceType;
		Calendar c = Calendar.getInstance();
		c.setTime(o.date);
		c.add(Calendar.DAY_OF_MONTH, 1);
		o.date = c.getTime();
		
		if (o.id2.trim().length() == 0)
			o.id2 = o.id;
	}
	...
	protected Dialog onCreateDialog(int id) {
		switch(id) {
			...
			case WH_REMARK_LIST:
			case OTHER_REMARK_LIST:
				return createListDialog(id);
		}
		return super.onCreateDialog(id);
	}
	
	private Dialog createListDialog(final int id) {
		ConfigImpl ci = new ConfigImpl();
		Config c = ci.getData();
		c.key = (id == WH_REMARK_LIST) ? "СписокСклад" : "СписокПрочее";
		ci.read();
		ci.close();
		...
		b.setTitle("Выберите примечание");
		b.setItems(values.toArray(new CharSequence[values.size()]), new DialogInterface.OnClickListener() {
			
			@Override
			public void onClick(DialogInterface dialog, int which) {
				EditText ed = (EditText)findViewById(id == WH_REMARK_LIST ? R.id.edWhRemark : R.id.edOthRemark);
				ed.setText(values.get(which));
			}
		});
		return b.create();
	}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			if( costType >= 0 ) {
				...
				o.priceType = v.key.toString();
			}
			

			KeyValue value;
			Spinner spWh = (Spinner)findViewById(R.id.spSklad);
			value = (KeyValue)spWh.getSelectedItem();
			if( value != null ){
			...
			Spinner spOrg = (Spinner)findViewById(R.id.spOrg);
			value = (KeyValue)spOrg.getSelectedItem();
			
			if( value != null ){
				o.id2 = value.key.toString();
			}
			...
			CheckBox cb = (CheckBox)findViewById(R.id.cbDoverPay);			
			if( cb.isChecked() ) o.params |= DOVER_PAY;
			else o.params &= (~DOVER_PAY);
			
			cb = (CheckBox)findViewById(R.id.cbDoverRet);			
			if( cb.isChecked() ) o.params |= DOVER_RET;
			else o.params &= (~DOVER_RET);
			...
			EditText ed;
			ed = (EditText)findViewById(R.id.edWhRemark);
			o.whRemark = ed.getText().toString();
			
			ed = (EditText)findViewById(R.id.edOthRemark);
			o.otherRemark = ed.getText().toString();
		...
		private void askToApplyNewSumType(Context context, final int newSumType){
			...
			builder.setPositiveButton("Пересчитать", new DialogInterface.OnClickListener() {
				@Override public void onClick(DialogInterface dialog, int which) { okDone(true); }
			});
			
			builder.setNegativeButton("Оставить", new DialogInterface.OnClickListener() {
				@Override public void onClick(DialogInterface dialog, int which) { okDone(false); }
			});
