...
//			ad.getWindow().setLayout(ViewGroup.LayoutParams.FILL_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT);
	...
	class PriceAdapter extends BaseAdapter {
		...
		public View getView(int arg0, View arg1, ViewGroup arg2) {
			if( arg1 == null )
				arg1 = View.inflate(DocListEx.this, R.layout.price_filter_row, null);
	...
	protected String docSumText(Document<?> doc) {
		if( docsData.size() > 0 && DocType.getCurDoc() == OrderDoc.instance()) {
			String text = "";
			DocRowData drd = docsData.get(doc.getRowid());
			if( drd != null ) {
				text += Util.IntToScaleStr(drd.sum, Consts.SUM_SCALE, Util.DEC_DELIM, false);
				text += "<br/><i>(";

				boolean packView = ((CfgNplW)ConfigManager.getConfig()).isPackView; 
				text += Util.IntToScaleStr(packView ? drd.packQty : drd.qty, Consts.QTY_SCALE) + (packView ? "ÛÔ." : "¯Ú.");
				text += ", " + Util.IntToScaleStr(drd.weight / Consts.WEIGHT_SCALE, 1);
				text += " Í„)</i>";
			}
			return text;
		} else
	...
	protected void refreshTotalSum(boolean useFilter) {
		...
		if( tv != null ) {
			...
			String sumstr = DocType.SumConverter.toString(drd.sum);
			String text = "<b>" + sumstr + "</b>"; 
			text += "<br>" + Util.IntToScaleStr(drd.weight / Consts.WEIGHT_SCALE, 1) + "Í„";
			if( Features.COUNT_DOCS_IN_DOCSLIST ) {
				text += ", " + Integer.toString(countDocs(adapter));
			}
