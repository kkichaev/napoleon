...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		if( config.read()) {
			int idx = DialogHelper.makeListWithKey(c.value, firms, o.firmCode);
		
			String[] arr = new String[firms.size()];
			for (int i = 0; i < firms.size(); i++)
				arr[i] = firms.get(i).key.toString();
		
			spFirma.setAdapter(new ArrayAdapter<String>(this, R.layout.simple_spinner_layout, arr));
			spFirma.setSelection(idx, true);
		}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			KeyValue kv = firms.get(suppl);
			
			if (kv != null) {
				o.firmCode = kv.key.toString();
				try {
					o.whIndex = Integer.parseInt(kv.value.toString());
				}catch (Exception e) {
					e.printStackTrace();
				}
			}
