...
public class WarehouseEx extends WarehouseNew {
	...
	protected void applayMatrix(String matrixName) {
		...
		if(document instanceof OrderImplEx) {
			...
			adapter.putFilter(new OrgItemFilter(oe, document.getDate()));
		}
	}
	
	@Override
	protected BaseAdapter createListAdapter() {
		WarehouseAdapter wa = (WarehouseAdapter) super.createListAdapter(); 
		if(document instanceof OrderImplEx) {
			OrgImpl oi = new OrgImpl();
			OrgEx oe = (OrgEx) oi.getData();
			oe.id = document.getId();
			...
			wa.putFilter(new OrgItemFilter(oe, document.getDate()));
		}
		return wa;
	}
	...
	class OrgItemFilter extends Filter {
		Set<String> used = new HashSet<String>();
		Set<String> reject = new HashSet<String>();
		
		public OrgItemFilter(OrgEx o, Date orderDate) {
			super("OrgFilter" + o.id);
			for(MatrixItem mi : o.matrix)
				used.add(mi.id);
			for(MatrixItem mi : o.reject)
				reject.add(mi.id);

			if(o.noLicense(orderDate))
				where = "license = 0";
		}
