...
import com.grsoft.dataobjects.OrgUnit;
	...
	private void init() {
		...
		if( v != null ) {
			...
			if( spAddress != null ) {
				...
				int selected = -1;
				for(OrgUnit addr : ((OrgEx)oi.getData()).units) {
					KeyValue kv = new KeyValue(addr.id, addr.name);
					if( kv.key.toString().equals(o.unitCode))
						...
						if (view instanceof TextView) {
							...
							tv.post(new Runnable() {
								...
								public void run() {
									...
									tv.setPadding(0, 0, 0, (int)getResources().getDimension(R.dimen.spinner_bottom_pd));
								}
							});
