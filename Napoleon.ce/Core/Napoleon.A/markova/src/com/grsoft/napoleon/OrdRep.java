...
public class OrdRep extends WarehouseNew {
	...
	public static void open(Context ctx){
		Intent i = new Intent(ctx, OrdRep.class);
		ctx.startActivity(i);
	}
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		spOrg.setAdapter(new OrgAdapter(orgid));
		
		if(orgid.trim().length() > 0)
			for(int i = 0; i < spOrg.getCount(); i++){
				Org o = (Org) spOrg.getItemAtPosition(i);
				if(o != null && o.id.equals(orgid)){
					spOrg.setSelection(i, true);
					break;
				}
			}
	...
	private OnClickListener reportClick() {
		return new OnClickListener() {
			...
			public void onClick(View v) {
				...
				Intent i = new Intent(v.getContext(), OrdView.class);
				...
				Org o = (Org) spOrg.getSelectedItem();
				
				if(o != null){
					i.putExtra(ORGID, o.id);
	...
	private class OrgAdapter implements SpinnerAdapter{
		private List<Org> data = new ArrayList<Org>();
		
		public OrgAdapter(String orgid) {
		...
		public View getView(int position, View view, ViewGroup parent) {
			if (view == null)
				view = View.inflate(OrdRep.this, R.layout.simple_spinner_layout, null);
			
			TextView tv = (TextView) view.findViewById(R.id.tvFirmaName);
			tv.setText(((Org)getItem(position)).name);
