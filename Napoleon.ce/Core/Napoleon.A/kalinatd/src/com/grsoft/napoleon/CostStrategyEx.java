...
import com.grsoft.dataobjects.OrgCostModifyItem;
...
public class CostStrategyEx extends CostStrategy {
	...
	public static void refreshCash() {
		...
		while (bdo) {
			...
			for (OrgCostModifyItem i : org.costmodify)
	...
	public int getItemCost(Price p, Document<?> doc) {
		...
		if (doc != null && ((PriceEx)p).noDiscount == 0) {
			if(doc.getId().trim().length() == 0)
		...
		if( discount != 0 )
			result = costWithDiscount(result, discount, Consts.SUM_SCALE); 
...
//					* Consts.SUM_SCALE / 2) / (Consts.SUM_SCALE * Consts.SUM_SCALE));
	...
	public static int findDiscount(OrgEx org, int fid) {
		...
		while (fld != null) {
			for (OrgCostModifyItem od : org.costmodify)
