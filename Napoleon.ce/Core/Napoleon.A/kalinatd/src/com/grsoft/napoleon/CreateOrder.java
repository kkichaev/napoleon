...
//	DateHandler dateHandler;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		cbSelfPickup = (CheckBox) findViewById(R.id.cbSelfPickup);
		btnRemark = findViewById(R.id.btnRemark);
		btnRemark.setOnClickListener(btnRemarkClick);
		edRemark = (EditText)findViewById(R.id.edCreateOrderNotes);
	...
	protected void loadRemarks() {
		final List<OrderRemark> list = new ArrayList<OrderRemark>();
		DataTraveler.travel(OrderRemark.class, new DataTraveler.Travel<OrderRemark>(true) {
			@Override public boolean travel(DataTraveler<OrderRemark> item) {
				list.add(item.data);
				return true;
			}}, null);
		
		Collections.sort(list, new Comparator<OrderRemark>() {@Override public int compare(OrderRemark lhs, OrderRemark rhs) { return lhs.pos - rhs.pos; }});
		
		for(OrderRemark o : list)
			remarks.add(o.text);
	}
	
	OnClickListener btnRemarkClick = new OnClickListener() { @Override public void onClick(View v) { showDialog(R.id.select_remark_dlg); } };
	...
	private void init() {
		...
		tv.setText(Html.fromHtml(((OrgEx)org).info));
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		DialogHelper.loadSpinnerFromConfig(config, "ТипОплаты", new ArrayList<CharSequence>(), spPayType, ((OrderEx)o).paytype);
		cbSelfPickup.setChecked(((OrderEx)o).selfpickup == 1);
	...
	private void initOrder(Order o, Org org) {
		...
		((OrderEx)o).paytype = ((OrgEx)org).paytype;
	}
...
//				return dateHandler.createDialog();
			...
			case R.id.select_remark_dlg:
				return createSelectRemarkDlg();
		}
		return super.onCreateDialog(id);
	}
	
	private Dialog createSelectRemarkDlg() {
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setTitle(R.string.select_remark_title);
		builder.setItems(remarks.toArray(new String[remarks.size()]), onSelectRemark);
		return builder.create();
	}

	DialogInterface.OnClickListener onSelectRemark = new DialogInterface.OnClickListener(){
		@Override public void onClick(DialogInterface dialog, int which) {
			...
			edRemark.setText(text + remarks.get(which)); 
		}};
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.remark = edRemark.getText().toString();
			...
			OrderEx e = (OrderEx) o;
			e.paytype = spPayType.getSelectedItemPosition();
			e.selfpickup = cbSelfPickup.isChecked() ? 1 : 0;
