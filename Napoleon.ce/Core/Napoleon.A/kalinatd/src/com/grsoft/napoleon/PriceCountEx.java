...
public class PriceCountEx extends PriceCount {
	...
	@Override protected boolean canChangeCost() { return canDoDiscount(); }
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if( getIntent().getLongExtra(ExtrasConst.DOC_ROW_ID_STR, ExtrasConst.INVALID_ID) == ExtrasConst.INVALID_ROWID){
			...
			priceCost = costStrategy.getCostInt(price.getData(), document, 0);
		}else
			priceCost = priceVal;
		...
		if( canDoDiscount() ) {
				...
				@Override public void onClick(View v) { 
					DiscountInputDlg.open(PriceCountEx.this, new InputNumber() {
						...
						public void applayInput(int value, Object... params) {
							...
							int min = Math.min(((PriceEx)price.getData()).minCost, priceCost);
							if(newPreciVal >= min)
							{
			...
			tv.setEnabled(false);
			findViewById(R.id.tvPrice).setEnabled(false);
		}
	...
	private boolean canDoDiscount() {
		boolean result = false;
		
		if (((PriceEx)price.getData()).minCost > 0){
			ConfigImpl config = new ConfigImpl();
			config.getData().key = "МожноДелатьНаценку";
