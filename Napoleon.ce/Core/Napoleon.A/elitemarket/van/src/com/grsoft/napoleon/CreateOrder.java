...
//	DateHandler dateHandler;
	...
	public static void open(Context context, DbObject<?> order, boolean editOldOrder) {
	...
	protected void init() {
		...
		OrderPrint o = (OrderPrint) order.getData();
		...
		String table = DataObjectInfo.getInstance().getTableName(f.getClass());
		DbReader r = new DbReader();
		int sel = -1;
		boolean bdo = r.select(f, table, null, "name");
		while(bdo) {
			if( f.id.equals(o.firmCode) )
				sel = firms.size();
