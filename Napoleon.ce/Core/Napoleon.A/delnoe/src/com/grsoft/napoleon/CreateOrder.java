...
public class CreateOrder extends BaseActivity
{
	...
	public static final int ASK_REMOVE_ITEMS = 10;
...
//				return dateHandler.createDialog();
			...
			case ASK_REMOVE_ITEMS:
				return createAskRemoveDlg();
		}
		return super.onCreateDialog(id);
	}
	
	private Dialog createAskRemoveDlg() {
		AlertDialog.Builder b = new AlertDialog.Builder(this);
		b.setTitle("Внимание!");
		
		b.setMessage("При смене фирмы будут удалены все скидки. Удалить скидки?");
		b.setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface arg0, int arg1) {
				((OrderImplEx)order).removeDiscounts();
				arg0.dismiss();
			}
		});
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			boolean needRemoveDiscount = false;
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			if( suppl >= 0 ) {
				...
				o.supplyer = suppl;
				o.firmCode = fe.id;
				needRemoveDiscount = o.items.size() > 0 && o.canDiscount > 0 && fe.canDiscount == 0;
				o.canDiscount = fe.canDiscount;
			}
			if( costType >= 0 )
				o.sumType = costType;
			...
			if(needRemoveDiscount) {
				showDialog(ASK_REMOVE_ITEMS);
				return;
			}
