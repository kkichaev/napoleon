...
//------------------------------- IDataSource ------------------------------------
//
IDataSource::IReader* DataSource::CreateReader(const SourceDefList& srcList, const SessionObject& object, std::wstring *name)
{
   SourceType t = stInternal;
   IReader* reader = (IReader*)srcList.CreateSource(SourceDefList::skReader, NULL, object, name, &t);
   if( reader != NULL )
      return reader;

   return (IReader*)srcList.CreateSource(SourceDefList::skReader, NULL, object, name, NULL);
}
