...
bool Image::ReadJPEG(BYTE* src, size_t size, size_t maxSize)
{
	...
	int scale = 1;
	unsigned wdh = max(cinfo.image_width, cinfo.image_height);
	while (true)
	{
		size_t newW = wdh / 2;
		if (newW < maxSize)
...
IBinary* Image::MakePNG()
{
	...
	png_set_IHDR(p, info_ptr, (size_t)m_size.width, (size_t)m_size.height, 8,
		PNG_COLOR_TYPE_RGBA,
		PNG_INTERLACE_NONE,
	...
	memcpy(wrb->Alloc(out.size()), &(*out.begin()), out.size());
...
IBinary* Image::MakeJPEG()
{
	...
	Binary *wrb = new Binary();
	memcpy(wrb->Alloc(bytes), out_buffer, bytes);
	MemoryBinary *mb = new MemoryBinary(wrb);
