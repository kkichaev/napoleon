...
void FieldLoader::SetData(ObjectDef::ObjectSet* objectSet, ObjectDef::Field* field, const Attributes& atts, const std::wstring& objectName)
{
   state = stInitial;
   objectLoader = NULL;
   ...
   } else if( type.compare(L"timestamp") == 0 )
   {
      field->format.type = MemberFormat::mtDateTime;
...
//---------------------------------------- FieldsLoader ---------------------------------------------
//
SourceLoader::SourceLoader(ObjectDef* object, SourceDef* source, IXmlHandler* prevHandler, const Attributes& atts) : ILoader(prevHandler)
{
   this->source = source;
