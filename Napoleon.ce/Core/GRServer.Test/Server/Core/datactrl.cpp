...
const wchar_t AUTH_ERROR[] = L"Пользователь неопределен";
const wchar_t AUTH_ERROR_NO_USERS[] = L"Нет информации о пользователях";
const wchar_t AUTH_LICENSE[] = L"Демо-период закончился";
const wchar_t AUTH_MGR_ERROR[] = L"Сейчас войти не удастся. Превышено число подключений оговоренных в лицензионном соглашении";
const wchar_t AUTH_NO_ID[] = L"Нет ID у пользователя";
const wchar_t AUTH_MONITOR_ERROR[] = L"Вход в монитор некорректной программой";
const wchar_t AUTH_MONITOR_LOGIN_ERROR[] = L"Неверный логин для программы монитор";

const wchar_t AUTH_COM_LOGIN_ERROR[] = L"Неверный пароль для входа";

const wchar_t AUTH_ADMIN_ERR[] = L"Неправильный пароль администратора";
const wchar_t AGENTS[] = L"Agents";
const wchar_t ADMIN_USER[] = L"admin";

const DWORD SESSION_TIME = 15 * 60; // 15 минут
         ...
         } else
         {
            ...
            } else
            {
               ...
               *user = new User(session);
               ...
               const Member* mver = data[VERSION_MEMBER];
               (*user)->Assign(login, division, duration, (const std::wstring&)*mver->str);
            }
         }
      }
   }
...
#if defined(NBtl)
	if( !ret && category.compare(L"btlViewer") == 0 )
	{
		const ObjectDef* omgr = ObjectDef::Get(L"NBTLViewer");
		if( omgr )
		{
			std::wstring filter(L"\"id\"='"); filter += name; filter += L"'";

	      SessionObject managers(omgr, session);
			managers.CreateReader(filter.c_str());
			managers.Load(NULL);
			managers.CloseReader();
			...
			if( managers.size() > 0 )
			{
				...
				{
					std::wstring login((const std::wstring&)*m[L"id"]->str);
					...
					} else
					{
						...
						*user = new User(session);
						...
						const Member* mver = data[VERSION_MEMBER];
						int division = 0;
						const Member* mdivision = data[L"division"];
						if( mdivision != NULL )
							division = (int)(mdivision->number + 0.005);
						if( division > 0 )
							(*user)->Assign(login, division, duration, (const std::wstring&)*mver->str);
						else
...
User* DataController::CheckUser(std::wstring* message, const std::wstring& name, const std::wstring& passwd, Session* session, const Object& data)
{
   ...
   SessionObject agents(a, session);
...
User* DataController::GetUser(const Object& data, Session* session)
{
   ...
   {
      session->AddAnswer(false, L"Сервер не зарегистрирован, проведите, пожалуйста, регистрацию сервера");
      return false;
   }
#endif
...
#endif
   {
      ...
      } else
      {
		...
		session->AddAnswer(false, L"Обнаружен незаполненный логин");
...
User* DataController::GetUser(const std::wstring& id, Session* session)
{
   ...
   SessionObject agents(ObjectDef::Get(L"Agents"),session);
   if( agents.CreateReader() )
   {
      agents.Load(NULL);
      agents.CloseReader();
