...
struct Command
{
	...
	{
		return Send(socket.GetSocket(), command, id, (unsigned char*)message.c_str(), message.size());
	}
	static bool Send(SOCKET socket, DWORD command, DWORD id, const std::string& message)
	{
	...
	static bool Send(Socket& socket, DWORD command, DWORD id, unsigned char* data, DWORD dataLen)
	{
		return Send(socket.GetSocket(), command, id, data, dataLen);
	}
};
...
bool ClientHandler::Read(SOCKET s)
{
	...
	DWORD cb = recv(s, buf, MAX_PACKET_LEN, 0);
	if ((int)cb > 0)
	{
...
bool Run(Socket &mainSock, HANDLE hStop, WORD port)
{
	...
	do
	{
		...
		int rc = select(maxSock + 1, &workingSet, NULL, NULL, &timeout);
...
DWORD JoinServer::Register(std::string* error, const std::string& login, const std::string& pwd, const char* project)
{
	DWORD res = 0;

	char *ubuf = (char*)malloc(200 + login.size() + pwd.size() + strlen(project));
	wsprintfA(ubuf, "https://grsoft.ru/grjs/grjs.php?register&login=%s&password=%s&project=%s",
		login.c_str(), pwd.c_str(), project);
...
static bool GetAddress(std::string* serverAddr, WORD* serverPort, DWORD *id, const std::string& login, const std::string& pwd, const char* project)
{
	bool ret = false;
...
static DWORD JSThread(JSThreadParam* _param)
{
	...
	while (running)
	{
		...
		while (!GetAddress(&jsAddr, &jsPort, &serverID, login, pwd, project.c_str()) || !Connect(mainSock, serverID, jsAddr.c_str(), jsPort, evStop))
		{
			mainSock.Close();
