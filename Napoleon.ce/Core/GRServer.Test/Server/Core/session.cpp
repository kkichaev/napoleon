...
Session::Session(Dispatcher* _d) :
   ...
   response(&formats), trash(&formats), answer(NULL), socket(NULL), flags(0)
{
   user = NULL;
}
...
bool Session::SendStreamPart()
{
	...
	return (SendStream() && ReadAck(WAIT_CLIENT_TIMEOUT) && AckIs(SERVER_COMMAND) && CommandIs(DONE_COMMAND));
}
...
Session::AckReturn Session::ReadAck(DWORD timeout)
{
   ...
   try
   {
      retVal = ReadPreviousVersion(&buf);
   } catch(...)
   {
...
{
   return internalDataSource->Query(stmt, name, groupExpr, this);
}
