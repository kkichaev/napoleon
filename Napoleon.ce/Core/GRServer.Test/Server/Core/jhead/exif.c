...
//--------------------------------------------------------------------------
void create_EXIF(const char* comment)
{
    char Buffer[1024];
...

    {
        ...
        if( comment && * comment )
        {
           ...
           wcomment = (wchar_t*)alloca((commentLen) * sizeof(wchar_t));
#ifdef UNIX
           ConvHelper(comment, (char*)wcomment, commentLen, commentLen * sizeof(wchar_t), "CP1251", "UTF16");
#else
           MultiByteToWideChar(CP_ACP, 0, comment, commentLen, wcomment, commentLen);
#endif
        }
    ...
    if( comment && * comment ) {
          DataWriteIndex = DirIndex;

          Put16u(Buffer+commentOffset, TAG_WINXP_COMMENT);      // Tag
         Put16u(Buffer+commentOffset + 2, 1);        // Format
         commentLen *= 2;
         Put32u(Buffer+commentOffset + 4, commentLen);                // Components
         Put32u(Buffer+commentOffset + 8, DataWriteIndex-8); // Pointer or value.

          memcpy(Buffer + DataWriteIndex, wcomment, commentLen);
          DataWriteIndex += commentLen;

        Put32u(Buffer+DataWriteIndex, 0);
    }
