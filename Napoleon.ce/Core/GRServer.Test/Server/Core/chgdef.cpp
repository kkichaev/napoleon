...
public:
		...
		const IXmlHandler::Attributes& atts, IXmlHandler* prevHandler) : IChanger(L"change")
	{
		...
		{
			changedTags.push_back(Data);
		}

		if (atts.Find(&tstr, L"width"))
		{
			changedTags.push_back(Width);
...
{
protected:
   enum State { stStart, stAdd, stRemove, stChange };
...
public:
   ModifyObject(ObjectDef::ObjectSet* _objects) : state(stStart), objects(_objects) {}
...
void ModifyObject::StartElement(const std::wstring& name, const Attributes& atts)
{
   ...
   else
   {
      if( name.compare(L"field") == 0 )
      {
         ...
         } else if( state == stChange || state == stAdd )
         {
