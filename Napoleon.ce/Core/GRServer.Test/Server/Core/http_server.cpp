...
const DWORD HTTP_READ_TIMEOUT = 10 * 1000; // 30 sec
...
static void URLDecode(std::string* dest, const std::string& src)
{
...
static void MakeStatusLine(std::string* stLine, int code, const std::string& msg)
{
	...
	stLine->assign(HTTP_VERSION).append(1, ' ').append(buf).append(1, ' ').append(msg).append(CRLF);
}
...
static bool SendErrorResponse(Socket& socket, int error, const std::string& msg, GRServer::Dispatcher* dispatcher, bool keepAlive)
{
	...
	return socket.Write((const BYTE*)stline.c_str(), stline.size());
}

static bool Send(Socket& socket, const std::string& targetURL, GRServer::Dispatcher* dispatcher, bool onlyHeaders, bool keepAlive)
{
	...
	USES_CONVERSION;
	wchar_t* wfn = A2W_CP(target.c_str(), CP_UTF8);
	...
	else
	{
		...
		if (mime.empty() == false)
		{
			...
			headers.AddHeader(HEADER_CONTENT_LENGTH, fileLength);
			headers.AddHeader(HEADER_CONNECTION, keepAlive ? "keep-alive" : "close");
			headers.AddHeader(HEADER_SERVER, HTTP_SERVER_VERSION);

			headers.PutHeaders(&msg);
			socket.Write((const BYTE*)msg.c_str(), msg.size());

			if (!onlyHeaders)
				socket.Write(cp, fileLength);
		}
	}
...
bool Headers::Read(HttpSockStream& ss)
{
	...
	while (ss.ReadLine(&line, HTTP_READ_TIMEOUT))
	{
		...
		unsigned cp = pos + 1, count = line.size() - pos - 1;
