...
static bool ConvertToDate(CString* dst, const wchar_t *src)
{
...
{
   return ConvertToDate(dest, src.c_str());
}
...
bool SessionObject::CreateReader(const wchar_t* filter, bool forceCreate, bool clearObject)
{
   ...
   //Log(objDef, "open");
	...
	source.reader = DataSource::CreateReader(objDef->GetSource(), *this, (std::wstring*)&source.readerName);
...
void SessionObject::FireEvent(IEvent::Type eventType, Object* o)
{
	if (objDef != NULL && objDef->events.HaveEvent(eventType))
	{
		...
		curObjIndex = 0;
		fireEvent = true;
		const_cast<ObjectDef*>(objDef)->events.Fire(eventType, session, this);
		fireEvent = false;
...
{
   return (objDef != NULL) ? objDef->sendLimit : 0;
}
      ...
      if( currentIndex < sz )
      {
         ...
         if( objDef )
         {
            curObjIndex = currentIndex;
				fireEvent = true;
				const_cast<ObjectDef*>(objDef)->events.Fire(Event::Get, session, this);
				fireEvent = false;
         }
      }
      return (currentIndex < sz);
...
bool SessionObject::Write(bool updateExecutable, RID_LIST *ids)
{
   ...
   if( source.writer != NULL )
   {
		...
		{
			USES_CONVERSION;
			gServer->AddLog(IErrorLogger::Full, "Start write object (%X): %s, %s", (unsigned)this, W2A(this->format->name.c_str()), ((fireEvent) ? "fireEvent" : "no fireEvent"));
		}
      ...
      if( ids != NULL && clearIDS )
		...
		{
			USES_CONVERSION;
			gServer->AddLog(IErrorLogger::Full, "Done write object (%X): %s", (unsigned)this, ((fireEvent) ? "fireEvent" : "no fireEvent"));
		}
		...
		if (objDef && !fireEvent && updateExecutable)
      {
         ...
         SaveWriter(&writers);
			...
			{
				USES_CONVERSION;
				gServer->AddLog(IErrorLogger::Full, "Start fire put event %X", (unsigned)this);
			}
         ...
         RestoreWriter(writers.begin());
			...
			{
				USES_CONVERSION;
				gServer->AddLog(IErrorLogger::Full, "End fire put event %X", (unsigned)this);
			}
