	...
	while (true)
	{
		...
		try
		{
			...
			if (file != INVALID_HANDLE_VALUE)
			{
				...
				WriteFile(file, logEntry->buffer, logEntry->cb, &writed, NULL);
				WriteFile(file, "\r\n", 2, &writed, NULL);
...
static void WriteLog(const char* prefix, const char* msg, bool printOut, va_list args)
{
	...
	try
	{
		...
		int bytes = cb + prefixLength + TIME_LENGTH + 200;
		...
		int wr = vsnprintf(p, cb, msg, args);
		if (wr > 0)
			p[wr] = '\0';
...
void Dispatcher::AddError(bool critical, const char* msg, ... )
{
   ...
   va_start(args, msg);
	...
	if (critical)
	{
		WriteLog("Критическая ошибка: ", msg, true, args);
