...
//
// возвращает тип дату, а с параметром тип строка. Параметр - формат даты/времени
//
...
//
// параметры объект, формат
//  формат пока только даты D[D] M[M] YY[YY] h[h] H[H] m[m] S[S]
//
      ...
      *result = L"";
      ...
      if( params.size() == 3 )
      {
         ...
         if( cnt == 2 && params[2].type == Token::ttString )
         {
            ...
            {
               FileTimeToSystemTime(&ft, &st);
               int newMonth = (st.wMonth + value - 1); // приведем начало к 0
               int addYear = 0;
      ...
      *result = L"";
      ...
      if( params.size() == 1 && params[0].type == Token::ttServObject )
      {
         const SessionObject *so = params[0].value.object;
			if( so->CurObjectIndex() == 0 ) // если нет объекта вернем 1
            *result = L"1";
      }
      return true;
   }
...
//
// 1 - если объекта нет в базе (проверяет только в internal)
//
class IsNew : public IFunction
{
public:
   virtual void Close();
   virtual const wchar_t* Name() const { return L"IsNew"; }
   virtual bool Do(Token* result, const std::vector<Token>& params, Session* session, const SessionObject *thisObject);

protected:
   bool MakeAndRunSelector(const SessionObject *thisObject);

   typedef std::map<std::wstring, IDataSource::ISelector*> Selectors;
   Selectors selectors;
} isNew;
...
void IsNew::Close()
{
   Selectors::iterator i = selectors.begin();
...
struct MRunner
{
   ...
   MRunner(std::wstring& ss, std::wstring& ws, std::vector<int>& si, std::vector<int>& wi, GRServer::Format* f) :
      selStr(ss),
      whereStr(ws),
      selIdx(si),
      whereIdx(wi),
   ...
   void Do(std::wstring& f)
   {
      if( !f.empty() )
      {
         size_t start = f.find_first_not_of(L' ');
         f = f.substr(start, f.size() - start);
         int idx = format->FindMember(f.c_str());
         if( idx >= 0 )
         {
            if( begin )
            {
...
bool IsNew::MakeAndRunSelector(const SessionObject *thisObject)
{
   const IObjectData* objDef = thisObject->GetObjectDef();
   if( objDef == NULL )
      return false;
   ...
   IDataSource::ISelector *sel = DataSource::CreateSelector(((ObjectDef*)objDef)->GetSource(), *thisObject);
   if( sel == NULL )
      return false;

   std::wstring selStr, whereStr;
   std::vector<int> selIdx, whereIdx;
//   bool begin = true;

   MRunner runner(selStr, whereStr, selIdx, whereIdx, thisObject->format);
   ...
   selectors[thisObject->format->name] = sel;

   Object* obj = Object::Create(*thisObject->format);
   bool ret = sel->Select(obj, thisObject->at(thisObject->CurObjectIndex()),
      selStr, whereStr, NULL, *thisObject->format, *thisObject->format, selIdx, whereIdx);
   delete obj;

   return ret;
}

bool IsNew::Do(GRServer::Token *result, const std::vector<Token> &params, GRServer::Session *session, const GRServer::SessionObject *thisObject)
{
   bool res = true;
   int iresult = 1;
   try
   {
      int idx = thisObject->CurObjectIndex();
      if( idx >= 0 && idx < (int)thisObject->size() )
      {
         Selectors::iterator fnd = selectors.find(thisObject->format->name);
         if( fnd != selectors.end() )
         {
            IDataSource::ISelector *sel = fnd->second;
            Object* obj = Object::Create(*thisObject->format);

            if( sel->SelectNext(obj, thisObject->at(idx)) )
               iresult = 0;
            delete obj;
         } else
         {
            if( MakeAndRunSelector(thisObject) )
               iresult = 0;
         }
      }
   } catch(...)
   {
      gServer->AddError(false, "isNew exception");
   }
   (*result) = iresult;
   return res;
}
