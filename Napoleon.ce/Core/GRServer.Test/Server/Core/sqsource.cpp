...
public:
   ...
   {
      return creator.CreateSelector(parameters, object);
   }
...
public:
   ...
   {
      USES_WCONVERSION;
      value.assign(W16_32((const wchar_t*)sqlite3_column_text16(stmt, index)));
      m->str = (CString*)&value;
   }
...
{
public:
	SQLiteReader(const SessionObject &object, const std::wstring& tableName, ParamHelper *defaults, const wchar_t* filter = NULL, bool debug = false);
...
protected:
   ...
   std::wstring sqlStr, rdFilter, tableName;
   Binder binder;
   sqlite3_stmt *stmt;
	bool debug;

	DWORD commitId;
	ParamHelper *defaults;
	ParamHelper params;
};
...
protected:
	SQLiteQuery(const ISessionObject& _object) : stmt(NULL), object(*(const SessionObject*)_object.Self()), nextObject(NULL), bof(false),defaults(NULL) { }
...
class SQLiteSelector : public IDataSource::ISelector
{
public:
   SQLiteSelector(const std::wstring& tableName);
   ~SQLiteSelector();
   ...
   virtual bool Select(Object* selObject, const Object* whereObject,
      ...
      const GRServer::Format& selFmt, const GRServer::Format& whereFormat,
...
//sqlite3* GRServer::SQLite::SQLiteBase()
//{
//   return db;
//}
//
...
bool Execute(const CString& sql)
{
   ...
   int rc = sqlite3_prepare16_v2(db,  W32_16(sql.c_str()), -1, &stmt, NULL);
   if( rc == SQLITE_OK )
...
bool IsTableExists(const std::wstring& name)
{
   ...
   int rc = sqlite3_prepare16_v2(db, W32_16(str.c_str()), (int)(str.size() * sizeof(unsigned short)), &stmt, NULL);
   if( rc == SQLITE_OK )
      rc = sqlite3_step(stmt);
   sqlite3_finalize(stmt);
...
//static void AddField(vector<MemberFormat> *fields, const ObjectDef& objDef, const std::wstring& field)
//{
//   ObjectDef::Field f;
//   f.format.name = field;
//
//   ObjectDef::Fields::const_iterator fnd = objDef.fields.find(f);
//   if( fnd != objDef.fields.end() )
//   {
//      MemberFormat mf = fnd->format;
//      mf.name.insert(0, objDef.tableName + L"$");
//      fields->push_back(mf);
//   }
//}
//
//// имена таких полей ParentName$field
//static bool LoadFK(vector<MemberFormat> *fields, const ObjectDef& objDef)
//{
//   if( objDef.parent.empty() ) return false;
//
//   const ObjectDef* parent = ObjectDef::Get(objDef.parent);
//
//   ObjectDef::Members::const_iterator keyI = parent->members.find(PRIMARY_KEY_STR);
//   if( keyI == parent->members.end() ) return false;
//
//   wstring::const_iterator si = keyI->second.begin(), ei = keyI->second.end();
//   if( *si == L'"' ) si++;
//
//   wstring f;
//   for( ; si != ei; si++ )
...
//static void AddFKConstraint(std::wstring* text, std::wstring *indexText, const std::wstring &name, const vector<MemberFormat> &fkFields)
//{
//   if( fkFields.size() == 0 ) return;
//
//   indexText->assign(L"CREATE INDEX fki_");
//   indexText->append(name);
...
//   wstring fTable;
//
//   text->append(L", CONSTRAINT fk_");
//   text->append(name);
//   text->append(L" FOREIGN KEY (");
//
//   vector<MemberFormat>::const_iterator ki = fkFields.begin();
...
//      else
//      {
//         int pos = ki->name.find_last_of(L'$');
...
//static bool CanCreateField(const ObjectDef::Field& field)
//{
...
//      return false;
//
//   if( format.type == MemberFormat::mtBinary && (field.flags & ObjectDef::Field::File) != 0 )
//      return false;
//
//   return true;
//}
...
bool TableDef::AddFields(const std::vector<ObjectDef::Field>& fields)
{
   ...
   for( ; i != fields.end(); i++ )
   {
      ...
      sqlite3_stmt *stmt;
      USES_WCONVERSION;
      int rc = sqlite3_prepare16_v2(db,  W32_16(text.c_str()), -1, &stmt, NULL);
      if( rc == SQLITE_OK )
         sqlite3_step(stmt);
      sqlite3_finalize(stmt);
      ...
      {
         USES_CONVERSION;
         gServer->AddError(true, "Ошибка изменения таблицы %s: %s", W2A(name.c_str()), sqlite3_errmsg(db));
         break;
      }
   }
...
static bool CheckDBFormat()
{
   ...
   for( ; res && i != names->end(); i++ )
   {
      ...
      if( !res )
         gServer->AddError(false, "SQLite ошибка при создании объекта %s", W2A(i->c_str()));
		else
		{
...
bool SQLiteInternal::Init(GRServer::IObjectDef* objDef, const GRServer::ServerConfig& config)
{
   ...
   {
      std::string fileName;
      FullFileName(&fileName, config.serverBase.c_str());

      USES_CONVERSION;
      const wchar_t *fn = A2W(fileName.c_str());
   ...
   if( !res )
   {
      gServer->AddError(true, "Ошибка при инициализации источника данных '%s'", config.serverBase.c_str());
   }
   return res;
}
...
public:
   ...
   struct KeyText : public Key
   {
      ...
      virtual bool IsNew(sqlite3_stmt *stmt)
      {
         ...
         bool ret = (value.empty() || value.compare(v) != NULL);
         value = v;
         return ret;
      }
...
static void ReadColumns(std::vector<std::wstring> *columns, sqlite3_stmt *stmt)
{
   ...
   {
      const wchar_t* name = W32_16((const wchar_t*)sqlite3_column_name16(stmt, i));
                             ...
                             ISession *session, wchar_t* groupExpr, sqlite3_stmt* stmt)
{
   ...
   for( ; fi != format->end(); fi++, index++ )
   {
      if( fi->type == MemberFormat::mtObject )
      {
         ...
         ret = PrepareRecWriter(child, columns, chf, session, (p != NULL) ? p+1 : L"", stmt);
      }
   }
...
ISessionObject* SQLiteInternal::Query(const wchar_t* wstmt, const wchar_t* typeDef, const wchar_t* groupExpr, ISession* session)
{
   ...
   USES_WCONVERSION;
   sqlite3_stmt *stmt;
   int rc = sqlite3_prepare16_v2(db, W32_16(wstmt), -1, &stmt, NULL);
   if( rc == SQLITE_OK )
   {
...
static void GetTableNameW(std::wstring* tableName, const ParamList& parameters, const SessionObject& object)
{
   ...
   //const Parameter *tname = parameters.Find(L"tableName", -1);
   //if( tname != NULL )
   //{
   //   Token tName;
   //   if( object.GetSession().Parse(&tName, tname->value, &object) && tName.type == Token::ttString )
   //      tableName->assign(tName.value.str->c_str());
   //}
}
...
IDataSource::IReader* SQLiteSourceCreator::CreateReader(const ParamList& parameters, const ISessionObject& iobject) const
{
   ...
   if( filterP != NULL )
      ((Session&)iobject.GetSession()).Parse(&filter, filterP->value, &object);

	const Parameter *dbg = parameters.Find(L"debug", -1);
	ParamHelper *defaults = new ParamHelper();
	defaults->Read(parameters, &object.GetSession(), &object, gServer);
   return new SQLiteReader(object, tableName, defaults, filter.c_str(), (dbg != NULL));
}
...
IDataSource::ISelector* SQLiteSourceCreator::CreateSelector(const ParamList& parameters, const ISessionObject& iobject) const
{
   const SessionObject& object = *(const SessionObject*)iobject.Self();
   std::wstring tableName;
   GetTableNameW(&tableName, parameters, object);

   return new SQLiteSelector(tableName);
}
//
//------------------------------------------ Binder ----------------------------------------------------
//
...
bool Binder::Prepare(sqlite3_stmt* stmt, const IObjectData* od, const GRServer::Format& format)
{
   USES_WCONVERSION;
	...
	for( int i=0; i<sqlite3_column_count(stmt); i++ )
	{
		...
		const wchar_t *colName = W16_32((const wchar_t *)sqlite3_column_name16(stmt, i));
   ...
   for( ; fi != od->fields.end(); fi++ )
   {
      ...
      } else if( (fi->flags & ObjectDef::Field::File) != 0 && !fi->src.empty() )
      {
         ...
         if( srcidx >= 0 && meidx >= 0 && object.format->at(srcidx).type == MemberFormat::mtString )
         {
				...
				//std::string folder(gServer->GetConfig().ExchangeFolder());
    //        if( !fi->baseFolder.empty() )
    //        {
    //           USES_CONVERSION;
    //           const char* bf = W2A(fi->baseFolder.c_str());
    //           if( IsLocalName(bf) )
    //              folder += bf;
    //           else
    //              folder = bf;
    //        }
            files.push_back(new FileField(srcidx, meidx, folder.c_str(), gServer));
         }
      }
   }
...
bool SQLiteWriter::Prepare(const ISessionObject &iobject)
{
   ...
   if( binder.Prepare(object) )
   {
      ...
      ret = (sqlite3_prepare16_v2(db, W32_16(sql.c_str()), (int)(sql.size() * sizeof(unsigned short)), &stmt, NULL) == SQLITE_OK);
      if( ret )
         Execute("BEGIN;");
      else
      {
         USES_CONVERSION;
         gServer->AddError(false, "Ошибка при записи объекта %s: %s", W2A(object.Name().c_str()), sqlite3_errmsg(db));
      }
   }
...
bool SQLiteWriter::Write(const Object &o, RowID *rid)
{
   ...
   if( stmt != NULL && binder.Write(stmt, o) )
   {
      ...
      if( ret )
      {
		...
		{
			USES_CONVERSION;
			gServer->AddError(false, "Ошибка при записи объекта %s: %s", W2A(o.format.name.c_str()), sqlite3_errmsg(db));
		}
...
//------------------------------------------ SQLiteChildWriter ----------------------------------------------------
//
SQLiteChildWriter::SQLiteChildWriter(const SessionObject& object, const std::wstring& tableName) : SQLiteWriter(tableName), rmvStmt(NULL)
{
   const ISessionObject* parentObjI = object.Parent();
   const SessionObject* parentObj = (parentObjI == NULL) ? NULL : (const SessionObject*)parentObjI->Self();
   if( parentObj != NULL )
...
//         const IObjectData* od = object.GetObjectDef();
         ...
         USES_WCONVERSION;
         sqlite3_prepare16_v2(db, W32_16(sql.c_str()), (int)(sql.size() * sizeof(unsigned short)), &stmt, NULL);
         sqlite3_prepare16_v2(db, W32_16(rmvSql.c_str()), (int)(rmvSql.size() * sizeof(unsigned short)), &rmvStmt, NULL);
      }
   }
}
...
//------------------------------------------ SQLiteReader ----------------------------------------------------
//
SQLiteReader::SQLiteReader(const SessionObject &object, const std::wstring& tableName, ParamHelper *defaults, const wchar_t* filter, bool debug) :
   sessionObject(object), stmt(NULL)
{
	this->debug = debug;
   this->tableName = tableName;
   ...
   {
      CString dest, src(filter);

      object.PrepareFilterStr(&dest, src);

      //rdFilter = L" WHERE ";
      rdFilter.append((const std::wstring&)dest);
   }
}
   ...
   if( binder.Prepare(object) )
   {
      ...
      sqlStr += L"\" ";
		if (*whereStmt)
		{
			...
			CString* res = params.Substitute(whereStmt, defaults);
			sqlStr += res->c_str();
			delete res;
      ...
      USES_WCONVERSION;
      ret = (sqlite3_prepare16_v2(db, W32_16(sqlStr.c_str()), (int)(sqlStr.size() * sizeof(unsigned short)), &stmt, NULL) == SQLITE_OK);

		if (debug)
		{
			USES_CONVERSION;
			gServer->AddLog(IErrorLogger::Full, "preparing stmt %s", W2A(sqlStr.c_str()));
			if (!ret)
			{
      ...
      if( (od->flags & IObjectDef::RemoveOnCommit) != 0 )
      {
			...
			wsprintf(buf, L"%d", commitId);
         wstring stmt(L"UPDATE \""); stmt += od->tableName; stmt += L"\" SET \""; stmt += SENDED_FIELDS; stmt += L"\" = "; stmt += buf;
         if( *whereStmt != L'\0' )
			{
				stmt += L" ";
            stmt += whereStmt;
         ...
         char *errMsg = NULL;
			sqlite3_exec(db, W2A(stmt.c_str()), NULL, NULL, &errMsg);
         sqlite3_free(errMsg);
		}
	}
...
bool SQLiteReader::SetFilter(const wchar_t* filter, const ISessionObject& object)
{
   ...
   if( *filter != L'\0' )
   {
		...
		{
			CString tfilter, src(filter);
			if (!object.PrepareFilterStr(&tfilter, src))
				return false;

			//dst.append(L" WHERE ");
			dst.append((const std::wstring&)tfilter);
		}
   }
...
bool SQLiteReader::MoveNext(Object *parentObject)
{
   if( stmt == NULL )
   {
      if( !Prepare(sessionObject, rdFilter.c_str()) )
         return false;
   }
...
SQLiteChildReader::SQLiteChildReader(const SessionObject& object, const std::wstring& tableName ) :
   SQLiteReader(object, tableName, NULL), childPrepared(false), childIndex(0)
{
   const ISessionObject* parentObjI = object.Parent();
   const SessionObject* parentObj = (parentObjI == NULL) ? NULL : (const SessionObject*)parentObjI->Self();
   if( parentObj != NULL )
...
//------------------------------------------ SQLiteRemover ----------------------------------------------------
//
SQLiteSelector::SQLiteSelector(const std::wstring& tableName)
{
   this->tableName = tableName;
   stmt = NULL;
}
...
bool SQLiteSelector::Select(Object* selObject, const Object* whereObject,
   const std::wstring& selStr, const std::wstring& whereStr, const std::wstring* orderStr,
   const GRServer::Format& selFmt, const GRServer::Format& whereFmt,
   const std::vector<int>& selIdx, const std::vector<int>& whereIdx)
{
   ...
   for( ; i != selIdx.end(); i++ )
   {
      FieldBinder* fb = FieldBinder::Create((*i), selFmt);
      if( fb != NULL )
         selBinder.Add(fb);
   }
   ...
   {
      FieldBinder* fb = FieldBinder::Create((*i), whereFmt);
      if( fb != NULL )
         whereBinder.Add(fb);
   }

   USES_WCONVERSION;
   if(sqlite3_prepare16_v2(db, W32_16(sql.c_str()), (int)(sql.size() * sizeof(unsigned short)), &stmt, NULL) == SQLITE_OK)
      return SelectNext(selObject, whereObject);
   return false;
}

bool SQLiteSelector::SelectNext(Object* selObject, const Object* whereObject)
{
   bool ret = false;
   if( stmt == NULL )
      return ret;
   ...
   {
      sqlite3_reset(stmt);
      whereBinder.Write(stmt, *whereObject);
   }

   if(sqlite3_step(stmt) == SQLITE_ROW)
      ret = selBinder.Read(selObject, stmt);
...
SQLiteQuery::SQLiteQuery(const CString& query, const SessionObject& _object, bool debug, int rowCount, ParamHelper *defaults) : 
	stmt(NULL), object(_object), nextObject(NULL), bof(false)
{
	this->debug = debug;
	this->query = query;
...
bool SQLiteQuery::SetFilter(const wchar_t* filter, const ISessionObject& object)
{
 ...
 //     return true;
	...
	//CString tfilter(filter);

 //  std::wstring str((const std::wstring&)query);
 //  std::vector<std::wstring> params;
 //  std::vector<std::wstring>::const_iterator i;
 //  int cnt = 1;

 //  PKToList(&params, (const std::wstring&)tfilter, L';', true);
...
bool SQLiteQuery::Prepare(const SessionObject& object)
{
   ...
   USES_WCONVERSION;
	CString str(query);
	query.clear();
	if (!object.PrepareFilterStr(&query, str))
		return false;

	CString* res = params.Substitute(query.c_str(), defaults);
	query.assign(res->c_str());
	delete res;

	USES_CONVERSION;
	if (debug)
	{
		gServer->AddLog(IErrorLogger::Full, "Do Query (%d): %s", ((const Session&)object.GetSession()).GetSocket(),  W2A(query.c_str()));
	}
	ret = (sqlite3_prepare16_v2(db, W32_16(query.c_str()), (int)(query.size() * sizeof(unsigned short)), &stmt, NULL) == SQLITE_OK);
	if( !ret )
	{
		gServer->AddLog(IErrorLogger::Short, "Ошибка в выражении (%d) %s: %s", ((const Session&)object.GetSession()).GetSocket(), W2A(query.c_str()), sqlite3_errmsg(db));
	} else
	{
...
{
public:
	SQLFolderReader(const ISessionObject& object, const std::wstring& tableName, const CString& parentF, const CString& childF,
		const CString* ridF, const CString* stmt, const CString* filter, bool debug);
...
SQLFolderReader::SQLFolderReader(const ISessionObject& _object, const std::wstring& tableName, const CString& parentF, const CString& childF,
	const CString* ridF, const CString* stmt, const CString* filter, bool debug) :
		readed(false), object(_object)
{
	const SessionObject& so = (const SessionObject&)(*_object.Self());
	if (stmt == NULL)
		reader = new SQLiteReader(so, tableName, NULL);
	else
		reader = new SQLiteQuery(*stmt, so, debug, 0, NULL);
...
IDataSource::IReader* SQLFolderCreator::CreateReader(const ParamList& parameters, const ISessionObject& iobject) const
{
	...
	if (pf != NULL)
	{
		...
		else
		{
			...
			{
				ret = new SQLFolderReader(object, tableName, *spf, *scf, sridf, stmt, filter, (pdebug == NULL));
			}
			delete spf;
			delete scf;
			delete sridf;
...
class SQLFolderWriter : public SQLiteWriter
{
public:
	SQLFolderWriter(const std::wstring& tableName, GRServer::Format *format, const CString& parentF, const CString& childF);
...
//-------------------------------------- FolderTreeWriter ----------------------------------------------
//
SQLFolderWriter::SQLFolderWriter(const std::wstring& tableName, GRServer::Format *format, const CString& parentF, const CString& childF) : SQLiteWriter(tableName)
{
	parentIndex = format->FindMember(parentF.c_str());
...
bool SQLFolderWriter::Write(const Object& o, RowID *rid)
{
	...
	return SQLiteWriter::Write(o, rid);
}
...
IDataSource::IWriter* SQLFolderCreator::CreateWriter(IDataSource::IWriter* parent, const ParamList& parameters, const ISessionObject& iobject) const
{
	...
	if (session.Parse(&spf, pf->value, &object) && session.Parse(&scf, cf->value, &object))
	{
		ret = new SQLFolderWriter(tableName, format, *spf, *scf);
	}
	delete spf;
	delete scf;
...
class SQLExecutor : public IDataSource::IWriter
{
public:
	SQLExecutor(IInternalDataSource *source, const std::wstring& _stmt, bool debug) : stmt(_stmt)
	{
		this->source = source;
		this->debug = debug;
	}
...
select * into temp_table from OrgFolder where userid = @src;

update temp_table set userid = @dst where userid = @src;

insert into OrgFolder select * from temp_table;

update OrgFolder$items set OrgFolder$userid= @dst where OrgFolder$userid = @src;

delete from OrgFolder where userid = @src;

commit;
*/
bool SQLExecutor::Write(const Object& o, RowID *rid)
{
	const GRServer::Format &fmt = o.format;

	std::wstring dest, name;
	wchar_t commit[7];
	int cp = 0;
	bool inName = false;
	commit[6] = L'\0';

	std::wstring::const_iterator i = stmt.begin();
	for (; i != stmt.end(); i++)
	{
		wchar_t sym = (*i);
		//if (sym == L';')
		//{
		//	source->Execute(dest.c_str(), session);
		//	dest.clear();
		//	continue;
		//}
		...
		if (sym == L'%')
		{
			if (inName)
			{
				int idx = fmt.FindMember(name.c_str());
				if (idx >= 0)
				{
					const Member& m = o.at(idx);
					MemberFormat::MemberType t = fmt.at(idx).type;
					if (t == MemberFormat::mtString)
						dest.append((const std::wstring&)*m.str);
					else if(t == MemberFormat::mtNumber)
					{
						wchar_t buf[100];
						WORD fraction = fmt.at(idx).format.fraction;
						double value = m.number;
						if (fraction == 0)
						{
						...
						{
							_swprintf(buf, L"%.*f", fraction, value);
							// brute force
		...
		{
			dest.append(1, sym);
			if (cp >= sizeof(commit)/sizeof(commit[0]) - 1)
			{
				memmove(commit, commit + 1, sizeof(commit) - sizeof(wchar_t));
				commit[cp-1] = toupper(sym);
				if (wcscmp(commit, L"COMMIT") == 0)
				{
					cp = 0;
					source->Execute(dest.c_str(), session);
					dest.clear();
	...
	if (!dest.empty())
	{
		bool ret = source->Execute(dest.c_str(), session);
		//if (debug)
		//{
		//	USES_CONVERSION;
		//	gServer->AddLog(IErrorLogger::Full, "execute(%s) %s", (ret ? "true" : "false"), W2A(dest.c_str()));
		//}
	}
...
IDataSource::IWriter* SQLExecutorCreator::CreateWriter(IDataSource::IWriter* parent, const ParamList& parameters, const ISessionObject& object) const
{
	const Parameter* p = parameters.Find(L"stmt", -1);
	if (p == NULL)
	{
		gServer->AddError(false, "SQLExecutor нет параметра stmt");
		return NULL;
	}
	CString *stmt = NULL;
	if (!object.GetSession().Parse(&stmt, p->value, &object))
	{
		gServer->AddError(false, "SQLExecutor не правильный параметр stmt");
		delete stmt;
		return NULL;
	}
	const Parameter *dbg = parameters.Find(L"debug", -1);

	SQLExecutor* ret = new SQLExecutor(dataSource, (const std::wstring&)*stmt, (dbg != NULL));
	delete stmt;
	return ret;
}
...
bool SQLiteInternal::BackupBase(const char* backupFileName)
{
	...
	if (rc == SQLITE_OK)
	{
		...
		if(rc != SQLITE_OK)
		{
			gServer->AddLog("Backup error %d: %s", rc, sqlite3_errmsg(pFile));
			ret = false;
		}
	}
