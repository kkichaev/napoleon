...
public:
	...
	virtual void AddMimeFileData(const char* name, const char* data, size_t dataSize, const char* fileName);
...
CurlHandler::CurlHandler() : mime(NULL), method(Get), headers(NULL)
{
	...
	curl_easy_setopt(handle, CURLOPT_WRITEDATA, (long)this);

	curl_easy_setopt(handle, CURLOPT_HEADERFUNCTION, WriteOut);
	curl_easy_setopt(handle, CURLOPT_HEADERDATA, (long)this);
...
void CurlHandler::AddMimeFileData(const char* name, const char* data, size_t dataSize, const char* fileName)
{
	if (mime == NULL)
		mime = curl_mime_init(handle);
