...
def make_token():
    data = { 
        "sub": current_app.config['MOY_SKLAD_APPUID'],
        "iat" : time(),
        "exp" : time() + 300,
...
def make_answer(answer:dict[str,Any]) -> Response:
    ...
    response.headers['Authorization'] = 'Bearer ' + make_token()
    return response
...
def getAppStatus(account:Account) -> str:
    try:
        token = make_token()
...
def setAppStatus(status:str, account:Account) -> None:
    try:
        token = make_token()
        headers = {'Content-Type':'application/json'
                   ,'Accept':'application/json'
...
def getUserContext(contextKey:str) ->dict[str,Any]:
    token = make_token()
