...
#ifdef Python3
	const wchar_t* sname;
	if (strObj == NULL || (sname = PyUnicode_AS_UNICODE(strObj)) == NULL)
...
static int SetFromPy(const MemberFormat& format, Member& value, PyObject* obj)
{
   ...
   case MemberFormat::mtString:
   {
      USES_CONVERSION;
		...
		} else 
		{
			...
			if (po != NULL)
			{
				const wchar_t* ptr = PyUnicode_AsUnicode(po);
				if (ptr != NULL)
				{
   ...
   case MemberFormat::mtBinary:
		...
		if (PyUnicode_Check(obj))
		{
			DWORD len = (DWORD)PyUnicode_GET_SIZE(obj);
			SetBinary(value, len, (const char*)PyUnicode_AsUnicode(obj));
		}
