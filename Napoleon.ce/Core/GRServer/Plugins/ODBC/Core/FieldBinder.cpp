...
void StringBinder::Read(Object* obj)
{
   ...
   {
      if (ind > (SQLINTEGER)(length * sizeof(wchar_t)))
      {
         ind = (SQLINTEGER)(length * sizeof(wchar_t));
      }
      src.assign(buf, ind / 2);
      size_t fnd = src.find(L'\0');
      if (fnd != wstring::npos) {
         src = src.substr(0, fnd);
      }
   }
   ...
   GRServer::Format *format = obj.Self()->format;
	...
	for (int pass = 0; pass < 2; pass++)
	{
		...
		for (; fi != od->fields.end(); fi++)
		{
			...
			//       {
			//          std::string folder;
			//          SetFileFieldBaseFolder(&folder, *fi, gServer->GetConfig().ExchangeFolder());
