...
class QueryBinder : public BinderBase
{
public:
   HRESULT Prepare(CSession& session, const std::wstring& stmt, const std::vector<const ISessionObject*>& objects);
...
{
public:
   QueryReader(CDataConnection& c, const CString& stmt, const ISessionObject& object);
   ~QueryReader();
