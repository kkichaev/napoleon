...
class SelectBinder : public ParamBinder
{
public:
   SelectBinder();
   ...
   {
      reparse = true;
      return ParamBinder::WriteParams(obj);
   }
   ...
   { 
      if( reparse )
         reader.Open();
      return ParamBinder::MoveNext(parentObject);
   }
         ...
         const Format& selFmt, const Format& whereFormat, const std::vector<int>& selIdx, const std::vector<int>& whereIdx)
{
   ...
   for( ; i != selIdx.end(); i++ )
   {
      ...
      FieldBinder* fb = FieldBinder::Create(mf, (*i));
      if( fb != NULL )
      {
         fields.push_back(fb);
...
{
   return new OleSelector(object, connection);
}