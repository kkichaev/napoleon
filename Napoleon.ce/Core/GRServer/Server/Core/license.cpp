...
#ifdef ___RT_TEST
			USES_CONVERSION;
			gServer->AddLog("___RT_TEST Load Demo lic '%s' allow %d, timespan %d ", W2A(lt.type.c_str()), lt.allowCount, lt.timeSpan);
#endif
		}
	}
	if( pb >= ep )
...
#ifdef ___RT_TEST
			USES_CONVERSION;
			gServer->AddLog("___RT_TEST Receive Demo lic '%s' allow %d, timespan %d ", W2A(dd.type.c_str()), dd.allowCount, dd.timeSpan);
#endif
			break;
		}
		}
...
#ifdef ___RT_TEST
		USES_CONVERSION;
		gServer->AddLog("___RT_TEST Test req send - allow %s", W2A(PROJECT_NAME));
#endif
		return true;
	}
	return false;
