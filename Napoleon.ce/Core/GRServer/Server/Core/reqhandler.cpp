...
DWORD Dispatcher::RequestHandler::Execute()
{
	...
	{
#ifdef VERSION_5
		return DoHTTPRequest(socket, dispatcher);
#else
   ...
   if( session.AckIs(UPDATE_COMMAND) )
   {
	...
	bool stdObjectSended = false, putLicenseRequest = false;
	while (retVal)
	{
		bool canRepeat = true;
		...
		if (retVal)
			retVal = sv;
		// Если нет команды для сервера, вываливаемся 
		...
		if (!retVal)
		{
			dispatcher->AddLog(IErrorLogger::Short, "session broken (%d) no bye close session", (int)socket.GetSocket());
			break;
		}
		// Что-то клиент ответил - обрабатываем ответ
	}
