...
void ServerConfig::UpdateFullNames()
{
   ...
   FullFileName(&fuFolder, updateFolder.c_str());
	...
	if (IsLocalName(imageFolder.c_str())) {
		fiFolder = feFolder + imageFolder;
	}
	else {
		fiFolder = imageFolder;
	}
}
...
static IErrorLogger::DebugLevel GetDebugLevel(const std::string& value, bool *printLog)
{
	*printLog = false;
   ...
   if( _stricmp(str, "FULL") == 0 ) return IErrorLogger::Full;
	if (_stricmp(str, "CONSOLE") == 0) {
		*printLog = true;
		return IErrorLogger::Full;
	}
...
bool ServerConfig::SetValue(const std::string& key, const std::string& value)
{
   ...
   if( strcmp(k, CONFIG_FILE_PARAM) == 0 ) configFile = value;
   ...
   else if( strcmp(k, DEBUG_KEY_NAME) == 0 ) debugLevel = GetDebugLevel(value, &printLog);
	...
	else if (strcmp(k, SEND_LIMIT) == 0) sendObjectSizeLimit = strtoul(value.c_str(), &ep, 10) * 1024 * 1024;
	else if (strcmp(k, SESSION_LIMIT) == 0) sessionMemoryLimit = ((size_t)strtoul(value.c_str(), &ep, 10));// *1024 * 1024;
	else if (strcmp(k, MEMORY_LIMIT) == 0) memoryLimit = ((size_t)strtoul(value.c_str(), &ep, 10));// * 1024 * 1024;
	else
   {
   ...
   if( wr )
   {
      ...
      if( DBF_CODE_PAGE == CP_ACP )
		...
		if(sendObjectSizeLimit)
			fprintf(wr, "%s = %u\n", SEND_LIMIT, sendObjectSizeLimit / (1024*1024));

		if(sessionMemoryLimit)
			fprintf(wr, "%s = %u\n", SESSION_LIMIT, sessionMemoryLimit);
