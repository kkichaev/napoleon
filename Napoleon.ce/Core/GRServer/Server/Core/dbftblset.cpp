...
public:
   ...
   {
      USES_CONVERSION;
      dest->assign(W2A(src.at(index).str->c_str()));
   }
};
   ...
   if( agents.CreateReader() )
   {
      ...
      if( idi >= 0 && idx >= 0 )
      {
         ...
         if( reader != NULL )
         {
            ...
            for( ; i != agents.end(); i++ )
            {
               std::string id = W2A((*i)->at(idi).str->c_str());
               std::string val;
               reader->Read(&val, *(*i));
               Trunc(&val);
               (*this)[id] = val;
...
void DBFUserSet::BaseFolderChanged(const std::string& baseFolder,  const ISessionObject& object)
{
   ...
   USES_CONVERSION;
   const char* userA = W2A(((Session&)object.GetSession()).GetUser().ID());
