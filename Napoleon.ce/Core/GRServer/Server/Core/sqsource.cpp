...
ISessionObject* SQLiteInternal::Query(const wchar_t* wstmt, const wchar_t* typeDef, const wchar_t* groupExpr, ISession* session)
{
   ...
   ISessionObject* so = NULL;
	...
	sqlite3_stmt *stmt;
   int rc = sqlite3_prepare16_v2(db, W32_16(dest.c_str()), -1, &stmt, NULL);
   if( rc == SQLITE_OK )
   {
	...
	else
	{
		...
		gServer->AddError(false, "Query is %s", W2A(dest.c_str()));
	}
   ...
   parameters.Load(&filters, L"readFilter", iobject);
	...
	if (whF != NULL)
	{
		...
		if (needAssign)
		{
			...
			{
				USES_CONVERSION;
				gServer->AddLog("Error while parse whereFilter of %s", W2A(object.Name().c_str()));
			}
		}
	}
...
bool SQLiteWriter::Prepare(const ISessionObject &iobject)
{
   ...
   if( binder.Prepare(object) )
   {
      ...
      ret = (sqlite3_prepare16_v2(db, W32_16(sql.c_str()), (int)(sql.size() * sizeof(unsigned short)), &stmt, NULL) == SQLITE_OK);
		...
		{
			USES_CONVERSION;
			gServer->AddLog(IErrorLogger::Full, "Run stmt %s", W2A(sql.c_str()));
...
bool SQLiteWriter::Write(const Object &o, RowID *rid)
{
   ...
   if( stmt != NULL && binder.Write(stmt, o) )
   {
      ...
      if( ret )
      {
		...
		{
			USES_CONVERSION;
			gServer->AddError(false, "write sqlite3_step %s: %s", W2A(o.format.name.c_str()), sqlite3_errmsg(db));
		}
...
void SQLiteReader::Remove()
{
   ...
   char *errMsg = NULL;
	const char* pstmt = W2A(stmt.c_str());
...
bool SQLiteRemover::Remove(const wchar_t* filter)
{
   ...
   if( ret )
   {
      ...
      if( sqlite3_prepare16_v2(db, W32_16(dst.c_str()), (int)(dst.size() * sizeof(unsigned short)), &stmt, NULL) == SQLITE_OK )
		...
		USES_CONVERSION;
		gServer->AddLog(IErrorLogger::Full, "Run stmt %s", W2A(dst.c_str()));
...
bool SQLiteQuery::Prepare(const SessionObject& object)
{
   ...
   //USES_WCONVERSION;
	...
	if (!ret)
	{
		gServer->AddLog(IErrorLogger::None, "Error parse todate in query %s", W2A(res->c_str()));
		delete res;
		return false;
	}
	delete res;
...
public:
	SQLFolderReader(const ISessionObject& object, const std::wstring& tableName, const CString& parentF, const CString& childF,
		const CString* ridF, const CString* stmt, const std::vector<wstring>& filter, bool debug);
		...
		readed(false), object(_object)
{
	...
	if (stmt == NULL)
		reader = new SQLiteReader(so, tableName, NULL, NULL, filters);
	else
...
private:
	...
	//int curLevel, levelIndex;
	//int childIndex, parentIndex, idIndex;
};
