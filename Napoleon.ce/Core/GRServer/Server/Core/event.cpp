	...
	try 
	{
		...
		for (; i != data.end(); i++)
		{
			...
			if (pr.Do(i->param))
			{
				...
				if (a == NULL)
				{
					gServer->AddError(false, "Нет action %s", W2A(i->action.c_str()));
				}
				else
				{
			...
			else
			{
				...
				{
					USES_CONVERSION;
					gServer->AddLog(IErrorLogger::Full, "action error param resolve %s", W2A(i->param.c_str()));
				}
			}
		}
	}
...
{
   if( type.compare(L"login") == 0 ) return Login;
   if( type.compare(L"failLogin") == 0 ) return FailLogin;
   if( type.compare(L"error") == 0 ) return Error;
   if( type.compare(L"warning") == 0 ) return Warning;
   if( type.compare(L"get") == 0 ) return Get;
   if( type.compare(L"remove") == 0 ) return Remove;
   if( type.compare(L"userChange") == 0 ) return Impersonate;
   if( type.compare(L"writeCommit") == 0 ) return WriteCommit;
   if( type.compare(L"beforePut") == 0 ) return BeforePut;
   if( type.compare(L"readCommit") == 0 ) return ReadCommit;
	if (type.compare(L"resolveObjects") == 0) return ResolveObjects;
	if (type.compare(L"beforeRead") == 0) return BeforeRead;
	if (type.compare(L"onLoad") == 0) return OnLoad;
