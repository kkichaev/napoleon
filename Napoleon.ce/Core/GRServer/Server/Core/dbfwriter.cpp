...
bool DBFWriter::CreateBase(const SessionObject& object, const std::vector<DBRec>& dbFields)
{
   const IObjectData* od = object.GetObjectDef();
...
bool DBFWriter::PrepareFields(const SessionObject& object, std::vector<DBRec>* dbFields)
{
	const IObjectData* od = object.GetObjectDef();
	if (od == NULL) return false;
	...
	{
		char buf[20];
		memcpy(buf, (*i).name, sizeof(((DBRec*)0)->name));
		buf[(*i).name, sizeof(((DBRec*)0)->name)] = 0;
		std::string name(buf);

		if (fieldName.find(name) != fieldName.end())
		{
			ret = false;
			break;
		}
...
bool DBFWriter::Prepare(const ISessionObject& iobject)
{
   ...
   if( !base.Open(fileName.c_str()) )
   {
		...
		if (!PrepareFields(object, &dbFields))
		{
			gServer->AddError(false, "Ошибка в описании полей файла '%s'", fileName.c_str());
			return false;
		}
		if( !CreateBase(object, dbFields) )
      {
         gServer->AddError(false, "Не могу открыть файл '%s' для записи ошибка %d", fileName.c_str(), GetLastError());
         return false;
      }
   }
