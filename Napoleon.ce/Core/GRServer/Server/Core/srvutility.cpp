   ...
   if( !IsLocalName(fileName) )
	...
	} else
	{
      ...
      GetModuleFileNameA(NULL, path, MAX_PATH);
		...
		//USES_CONVERSION;
		//MessageBox(NULL, wpath, L"fileW", MB_OK);

		//char* path = W2A(wpath);
		//MessageBoxA(NULL, path, "fileA", MB_OK);
...
static bool ReadUserCodes(std::vector<std::string>* users, const wchar_t** sp)
{
   ...
   if( EatWhite(sp) && **sp == L'(' )
   {
      while( true )
      {
         ...
         if( ReadCode(&code, sp) )
         {
            users->push_back(W2A(code.c_str()));
            if( EatWhite(sp) )
            {
               wchar_t sym = **sp;
               if( sym == L',' )
...
// функция даты вида 20.04.2010 20-04-2010 10:20:15 20/04/2010 10:20:15
