...
ReceivedStream* NetworkExchange::Receive(const IPAddress* addr1, const IPAddress* addr2, OutStream& stream, IProgressIndicator *pf)
{
   ReceivedStream *rcvd = NULL;
   if( pf != NULL ) pf->SetText(L"Соединение...");

   if( Connecting(addr1, addr2, stream, timeout, true) )
   {
      NetStream stream(*this);
      std::vector<PacketOperator> operations;
      rcvd = new ReceivedStream();
