...
{
public:
   DBObjectRcvr(const wchar_t* _progressText, bool clearTable, const wchar_t* excluded = L"") : 
      progressText(_progressText), clearBefore(clearTable), reader(NULL), table(NULL), excludedFields(excluded)
   {
   }
   ...
   virtual const wchar_t* ProgressText() const { return progressText; }
...
{
public:
   DBObjAliasRcvr(const wchar_t* progressText, bool clearTable, const wchar_t* alias) : 
      DBObjectRcvr<Impl>(progressText, clearTable)
      {
         this->alias = alias;
      }
