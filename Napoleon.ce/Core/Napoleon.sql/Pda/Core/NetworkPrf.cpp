...
void NetworkProperties::Init()
{
   ...
   if( preference.flags & npfConfirmOrderSend )
      CheckDlgButton(IDC_CONFIRM_SEND, BST_CHECKED);

   LoadAccessPoint();

#ifdef TOP_APP
   if( (preference.flags & apfTopApp) != 0 ) CheckDlgButton(IDC_TOP_APP, BST_CHECKED);
#else
   GetDlgItem(IDC_TOP_APP).ShowWindow(SW_HIDE);
#endif
}
...
void NetworkProperties::Save(Preference *preference)
{
   ...
   if( IsDlgButtonChecked(IDC_CONFIRM_SEND) == BST_CHECKED ) preference->flags |= npfConfirmOrderSend;
   else preference->flags &= (~npfConfirmOrderSend);

   SaveAccessPoint();
#ifdef TOP_APP
   if( IsDlgButtonChecked(IDC_TOP_APP) == BST_CHECKED ) preference->flags |= apfTopApp;
   else preference->flags &= (~apfTopApp);
#else
