...
struct OSRid : public OrgSumImpl
{
   ROWID rowid;
   DECLARE_TYPE_REFLECTION(OSRid);
};

BEGIN_TYPE_REFLECTION(OSRid)
   REGISTER_INT64_MEMBER(OSRid, rowid)
   CHAIN_REFLECTION(OSRid, OrgSums)
END_TYPE_REFLECTION(OSRid)

BEGIN_TYPE_REFLECTION(OrgSums)
   REGISTER_STRING_MEMBER(OrgSums, id)
   REGISTER_ULONG_MEMBER(OrgSums, sum)
   REGISTER_STRING_MEMBER(OrgSums, type)
END_TYPE_REFLECTION(OrgSums)

BEGIN_TYPE_REFLECTION(Sum)
   REGISTER_ULONG_MEMBER(Sum, sum)
END_TYPE_REFLECTION(Sum)

static const wchar_t* nextDocType = NULL;
...
bool CreateNextDoc(const wchar_t* orgID)
{
   ...
   const DocType* dt = docTypeManager.GetDocType(nextDocType);
   nextDocType = NULL;
   if( dt != NULL )
   {
      OrgImpl org;
      org.id = (wchar_t*)orgID;
      org.Read();

      ret = dt->CreateDocument(org.rid);
   }
