...
{
public:
   Binary(BYTE *buf, DWORD cb) :freeBuf(true) { this->buf = buf; this->cb = cb; totalSize = cb; }
   Binary(const std::string& src) :freeBuf(false)
   {
      this->buf = (BYTE*)src.c_str();
      this->cb = (DWORD)src.size();
      this->totalSize = this->cb;
   }

   Binary() : buf(NULL), cb(0), totalSize(0), freeBuf(true) {}
