...
public class DialogHelper {
	...
	public interface Filter {
		boolean contains(KeyValue value);
	}

	public static void loadSpinnerFromConfig(ConfigImpl config, String key, List<CharSequence> values, Spinner s, int selected) {
		loadSpinnerFromConfig(config, key, values, s, selected, false);
	}

	public static void loadSpinnerFromConfig(ConfigImpl config, String key, List<CharSequence> values, Spinner s, int selected, boolean addEmpty) {
	...
	public static void loadSpinnerFromConfig(ConfigImpl config, String key, List<CharSequence> values, Spinner s, String selected, boolean addEmpty) {
		Config c = config.getData();
		c.key = key;
		int sel = -1;
		if( config.read())
	...
	public static int makeListWithKey(String value, List<KeyValue> values, String selected) {
		return makeListWithKeyFilter(value, values, selected, null);
	}

	public static int makeListWithKeyFilter(String value, List<KeyValue> values, String selected, Filter filter) {
		int sel = -1;
		...
		while(pos != -1) {
			...
			if(filter == null || filter.contains((kv))) {
