...
public class DataObjectUtils {
	...
	@SuppressWarnings("unchecked")
	static public <T extends DataObject> List<T> readList(Class<T> dataType, byte[] stream, StreamPos sp)
	{
//		Debug.startMethodTracing("read_list_bytes3");
		...
		if (listHasCounter(stream, sp)){
			...
			sp.addPos(4); // Counter
			version = getInt(stream, sp.getPos());
			sp.addPos(4); // Version 
		}
		...
		try
		{
			while (stream != null && stream.length > sp.getPos())
			{
				...
				T instance = itemType.newInstance();
				assignFieldsByType(stream, instance, sp, fields, version);
				list.add(instance);
...
//			Debug.stopMethodTracing();
	...
	static Map<Field, Integer> verFields = new HashMap<Field, Integer>();
	
	static private int assignFieldsByType(byte[] stream, DataObject instance, StreamPos sp, Field[] fields, int streamVersion) 
			throws IllegalAccessException, TypeNotImplemented
	{
		int idx = sp.getPos();
		for(Field field: fields)
		{
			int iver = 0;
			Integer ver = verFields.get(field);
			if(ver == null) {
				if(verFields.size() > 500)
					verFields.clear();
				
				FieldVersion fc = field.getAnnotation(FieldVersion.class);
				if(fc != null)
					iver = fc.version();
				verFields.put(field, iver);
			} else
				iver = ver;
			if(iver > streamVersion)
				continue;
			...
			if (fieldType == int.class) {
				...
				long val = Util.bytesToLong(stream, idx);
				idx += 8;
				field.set(instance, val);
			} else if (fieldType == byte[].class) {
				int val = getInt(stream, idx);//((stream[idx++] << 8) & 0xFF00) | (stream[idx++] & 0xFF);
				idx += 4;
				if(val > stream.length - idx)
					val = stream.length - idx;
