...
public class SingleTaskResetHelper implements Runnable {
    ...
    public SingleTaskResetHelper(Context context, boolean afterException, String uid) {
    ...
    public void run() {
        try {
            //Utils.timeChecker("setMaximumOrder");
            setMaximumOrder();
        } catch (Exception e) {
            toLog(e);
        } finally {
        ...
        //
        try {
            //Utils.timeChecker("setUserOrder");
            setUserOrder();
        } catch (Exception e) {
            toLog(e);
        } finally {
        ...
        //
        try {
            //Utils.timeChecker("setMarkerOrder");
            setMarkerOrder();
        } catch (Exception e) {
            toLog(e);
        } finally {
        ...
        //
        try {
            //Utils.timeChecker("setIsTaskUseTerm1");
        ...
        //
        try {
            //Utils.timeChecker("setIsTaskUseTerm2");
        ...
        //
        try {
            //Utils.timeChecker("resetUnknownFields");
            resetUnknownFields();
        } catch (Exception e) {
            toLog(e);
        } finally {
        ...
        //
        try {
            //Utils.timeChecker("VerticalDepthHelper");
            new VerticalDepthHelper(mContext, mTaskUUID);
        } catch (Exception e) {
            toLog(e);
        } finally {
            //Utils.timeChecker("VerticalDepthHelper");
        ...
        //
        try {
            //Utils.timeChecker("getCompletedTasks");
            getCompletedTasks();
        } catch (Exception e) {
            toLog(e);
        } finally {
        ...
        //
        try {
            Utils.timeChecker("SingleTaskLinkReset");
            new SingleTaskLinkReset(mContext, mTaskUUID).runAll();
    ...
    private void setUserOrder() {
        ...
        try {
            c = db.rawQuery("UPDATE LionTask SET UserOrder = " +
                    ...
                    " END )" +
                    " WHERE uid='"+mTaskUUID+"' AND ( UserOrder = 0 OR UserOrder IS NULL )", null);
            c.moveToFirst();
    ...
    private void setMarkerOrder() {
        ...
        try {
            c = db.rawQuery("UPDATE LionTask SET MarkerOrder = " +
                    ...
                    "END ) " +
                    "WHERE uid='"+mTaskUUID+"' AND ( MarkerOrder = 0 OR MarkerOrder IS NULL )", null);
            c.moveToFirst();
    ...
    private void setIsTaskUseTerm(String termColumnName, String isUseTermColumnName) {
        ...
        try {
            c = db.rawQuery("UPDATE LionTask SET "+isUseTermColumnName+" =  \n" +
                    ...
                    "                    END ) \n" +
                    "                    WHERE uid='"+mTaskUUID+"' AND ( "+isUseTermColumnName+" = 0 OR "+isUseTermColumnName+" IS NULL )", null);
            c.moveToFirst();
    ...
    private void getCompletedTasks() {
        ...
        Utils.clearStringBuilder(mSb);
        String selection = SelectionKeeper.equals(mSb, CompletedTaskContract.Uid, mTaskUUID);
        mCr.delete(CompletedTaskContract.CONTENT_URI, selection, null);
        mCr.bulkInsert(CompletedTaskContract.CONTENT_URI, cvs);
        if (tasks.size() == 0) {
            CompletedTask completedTask = CompletedCache.getInstance(mContext).find(mTaskUUID);
            if (completedTask != null) {
                CompletedCache.getInstance(mContext).remove(completedTask);
            }
        } else {
    ...
    private void getCompletedTasks(Map<String, CompletedTask> tasks, boolean withParrent) {
        Utils.clearStringBuilder(mSb);
        String selection = mSelectionBuilder.getCompletedTasksWithParent(withParrent, mTaskUUID).build();
        Cursor c = null;
