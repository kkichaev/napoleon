...
public class EmpChanger extends BaseTaskChanger {

    public EmpChanger(Context context, LTask taskNew, LTask taskOld, VerticalDepthTask verticalDepth) {
        super(context, taskNew, taskOld, verticalDepth);
    }

    @Override
    public boolean equalsTasks(LTask taskNew, LTask taskOld) {
        return taskOld != null && (possiblyHasLink(taskNew) == possiblyHasLink(taskOld)) && (taskNew.getEmailPerformer().equals(taskOld.getEmailPerformer()));
    }

    @Override
    public boolean possiblyHasLink(LTask task) {
        return task.getEmailPerformer() != null && task.getStatus() !=  1 && task.getStatus() != 7 && task.getStatus() != 5 && task.getStatus() != 8;
    }
    ...
    public boolean removeLinks(LTask task) {
        ...
        addRawUpdate(updateTotalLinkCounter(EmpTotalLinkContract.TABLE_NAME, uid, task, false));

        /** удаляем связь задачи с uid */
        addOperation(ContentProviderOperation.newDelete(EmpLinkContract.CONTENT_URI).//
    ...
    public boolean increaseLinksCounter() {
        ...
        /** уменьшаем счетчики связи в соответствии с задачей */
        if (totalLinkExists(EmpTotalLinkContract.CONTENT_URI, uid)) {
            /** обновляем если связь существует */
            addRawUpdate(updateTotalLinkCounter(EmpTotalLinkContract.TABLE_NAME, uid, task, true));
    ...
    public boolean createRelatedLinks() {
        ...
        sb.getTaskEqualsCustomerEmp(uid);
    ...
    public void notifyChanges() {
        getContext().getContentResolver().notifyChange(EmpTotalLinkContract.CONTENT_URI, null);
    }
}