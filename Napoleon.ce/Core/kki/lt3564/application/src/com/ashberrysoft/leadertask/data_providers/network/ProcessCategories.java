...
public class ProcessCategories extends BaseTimeSOAP<Serializable> implements ProcessSOAPRequestConstants {

    private static final long serialVersionUID = 1L;
    protected static final String METHOD_NAME = "ProcessCategories";

    private List<Category> mListVerify;
    private List<UUID> mListSend;

    public ProcessCategories(Context context, LeaderTaskUser user, List<Category> verify) {
    ...
    protected void writeRequestSubXML(OutputStreamWriter writer) throws IOException {
        ...
        if (mListVerify != null && !mListVerify.isEmpty()) {
            for (Category entity : mListVerify) {
    ...
    protected Serializable parseResponse(Reader inputStream) throws Exception {
        ...
        try {
            ...
            /*final SwitchParseHandler<BaseLionProcessEntity<Category>> handler = SwitchParseHandler.newInstance(inputStream);
            final BaseLionProcessEntity<Category> entity = handler.getData();

            if (!entity.getListDelete().isEmpty()) {
                dbHelper.deleteCategories(convertStringsToUUIDs(entity.getListDelete()));
            }

            mListSend = convertStringsToUUIDs(entity.getListSend());

            if (!entity.getListProcess().isEmpty()) {
                UidToDelete.removeUidsFromTable(mContext, entity.getListProcess(), Category.SERVER_CLASS);
                dbHelper.deleteCategories(convertStringsToUUIDs(entity.getListProcess()));
            }

            if (!entity.getListAdd().isEmpty()) {
                dbHelper.updateCategories(entity.getListAdd());
            }*/
    ...
    public String getResultAction() {
        return ServiceConstants.ACTION_PROCESS_CATEGORIES;
    }
