...
public class CreateUser extends AbstractHTTPServiceRequest<Serializable, Void, R> {
    ...
    private CreateUserEnvelope mAnswer;

    public CreateUser(Context context, String email, String name/*, String pass, String phone*/) {
        super(context);
    ...
    protected R sendRequest(Void p) throws AbstractDataRequestException {
        try {
            ...
            mAnswer = new Persister().read(CreateUserEnvelope.class, reader, false);
            input.close();
            reader.close();
    ...
    public String getResultAction() {
        if (mSuccesful) {
            return ServiceConstants.ACTION_REGISTRATION;
        } else {
            return ServiceConstants.ACTION_REGISTRATION_NOT_SUCCESSFUL;
        }
    }
    ...
    private String writeSOAPRequest() {
        ...
        if (mPhoneNumber != null && !mPhoneNumber.isEmpty()) {
            request.append(BaseSOAP.getValueLine("phone", mPhoneNumber + " (TimeZone: " + zone + ")"));
        }*/
    ...
    public CreateUserEnvelope getAnswer() {
