...
public class MultiTasksTermDialog extends BaseDialog//
    ...
    public static final int CODE = R.id.multi_dialog_task_term;
    ...
    public static MultiTasksTermDialog newInstance(Fragment fragment, ArrayList <LTask> tasks) {
        final Bundle b = new Bundle(3);
        long temp = 0;
        b.putSerializable(EXTRA_TASKS, tasks);
        b.putSerializable(EXTRA_TERM_BEGIN, temp);
        b.putSerializable(EXTRA_TERM_END, temp);
        final MultiTasksTermDialog d = new MultiTasksTermDialog();
    ...
    public void onCreate(Bundle b) {
        ...
        mTasks = (ArrayList <LTask>) bundle.getSerializable(EXTRA_TASKS);
        mTermBegin = (long) bundle.getSerializable(EXTRA_TERM_BEGIN);
        mTermEnd = (long) bundle.getSerializable(EXTRA_TERM_END);
    ...
    public Dialog onCreateDialog(Bundle b) {
        ...
        mCalendarView = new LTCalendarView(getActivity(), new LTCalendarView.OnCalendarDateSelectedListener() {
            ...
            public void onDateSelected(Date date) {
                if (mTermBegin != 0 && !mTimeHelper.isWholeDayTask(mTermBegin, mTermEnd)) {
                    Calendar calendar = Calendar.getInstance(TimeHelper.DEFAULT_TIME_ZONE);
                    calendar.setTime(new Date(Long.valueOf(mTermBegin)));
                    ...
                    mTermBegin = mCalendar.getTimeInMillis();
                    mTermEnd = mCalendar.getTimeInMillis();
                }
                else {
                    mTermBegin = setTimeTo(Long.valueOf(date.getTime()), true);
                    mTermEnd = setTimeTo(Long.valueOf(date.getTime()), false);
                }
        ...
        if(mTermBegin != 0) {
            mCalendarView.setDate(0, new Date(Long.valueOf(mTermBegin)).getMonth(), mCalendar, -1);
            mCalendarView.setChosenDate(new Date(Long.valueOf(mTermBegin)));
        }
        else {
    ...
    public void onSaveInstanceState(Bundle b) {
        ...
        b.putSerializable(EXTRA_TASKS, mTasks);
        b.putSerializable(EXTRA_TERM_BEGIN, mTermBegin);
        b.putSerializable(EXTRA_TERM_END, mTermEnd);
    ...
    private void updateViews() {
        if (mTermBegin == 0) {
            ...
            mDate.setTime(mTermBegin);

            SimpleDateFormat format = new SimpleDateFormat("EEE");
            Date tmpmDate = mDate;
            tmpmDate.setHours(mDate.getHours());

            mStringBuilder.append(mTimeHelper.getCuteDateTitle(tmpmDate));

            final SimpleDateFormat sdf = DateFormat.is24HourFormat(getApp()) ? SDF_24H : SDF_12H;
            mDate.setTime(mTermBegin);
            if (mTimeHelper.isWholeDayTask(mTermBegin, mTermEnd)) {
                mSelectedTime.setTextColor(Color.GRAY);
                mSelectedTime.setText(R.string.select_time);
            }
            else {
    ...
    public void onClick(DialogInterface dialog, int which) {
        if (which == DialogInterface.BUTTON_POSITIVE) {
            for (LTask task : mTasks) {
                if (getSettings().getUserName().equals(task.getEmailCustomer())) {
                    task.setTermBeginCustomer(mTermBegin);
                    task.setTermEndCustomer(mTermEnd);
                    task.setUsnFieldCustomerTerm(task.getUsnFieldCustomerTerm() + 1);
                }
                task.setTermBegin(mTermBegin);
                task.setTermEnd(mTermEnd);
                task.setUsnFieldTerm(task.getUsnFieldTerm() + 1);
            }
            receiveObjects(CODE, mTasks);
        }
    }
    ...
    public void onClick(View v) {
        ...
        case R.id.btn_without_time:
            if (mTermBegin != 0) {
                mTermBegin = setTimeTo(mTermBegin, true);
                mTermEnd = setTimeTo(mTermEnd, false);
            }
            break;

        case R.id.btn_select_date:
            PickDateDialog.newInstance(this, mTermBegin).showDialog(getFragmentManager());
            return;

        case R.id.btn_select_time:
            if(mTermBegin != 0 ) {
                if (mTimeHelper.isWholeDayTask(mTermBegin, mTermEnd)) {
                    mCalendar.setTimeInMillis(mTermBegin);
                    ...
                    PickTimeDialog.newInstance(this, mTermBegin).showDialog(getFragmentManager());
                }
            }
            else {
    ...
    public void onReceivingObjects(int code, Object... objects) {
        ...
        case PickDateDialog.CODE: {
            ...
            if (mTermBegin != 0 && !mTimeHelper.isWholeDayTask(mTermBegin, mTermEnd)) {
                ...
                mTermBegin = date;
                mTermEnd = date;
                mTermBegin = setTimeTo(date, true);
                mTermEnd = setTimeTo(date, false);
            }
            mCalendarView.setDate(0, new Date(Long.valueOf(mTermBegin)).getMonth(), mCalendar, -1);
            mCalendarView.setChosenDate(new Date(Long.valueOf(mTermBegin)));
        }
        ...
        case PickTimeDialog.CODE: {
            final long date = (long) objects[0];
            if (date == mTermBegin && !mTimeHelper.isWholeDayTask(mTermBegin, mTermEnd)) {
                return;
            }

            mTermBegin = date;
            mTermEnd = date/*+30*60*1000*/;
