...
import static com.ashberrysoft.leadertask.day_calendar.custom_calendar.WeekViewUtil.*;
...
public class WeekView extends View {
    ...
    private int mMaxCountEventsInRow = 5;
    ...
    private int mTextSizeHeader = 16;
    ...
    private int mBorderColor = Color.rgb(167, 167, 167);
    private int mTodayBackgroundColor = Color.rgb(239, 247, 254);
    private int mHourSeparatorHeight = 2;
    private int mBorderHeight = 2;
    ...
    public WeekView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        try {
            ...
            mTextSizeHeader = a.getDimensionPixelSize(R.styleable.WeekView_textSize, (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, mTextSizeHeader, context.getResources().getDisplayMetrics()));
            ...
            MIN_HOUR = LTSettings.getInstance().getMinHour();
            MAX_HOUR = LTSettings.getInstance().getMaxHour();
        } finally {
            a.recycle();
        }
    ...
    private void init() {
        ...
        mHeaderTextPaint.setTextSize(mTextSizeHeader);
        ...
        // Border
        mEventBackGroundBorder = new Paint();
        mEventBackGroundBorder.setStyle(Paint.Style.STROKE);
        mEventBackGroundBorder.setStrokeWidth(mBorderHeight);
        mEventBackGroundBorder.setColor(mBorderColor);
        ...
        mTodayHeaderTextPaint.setTextSize(mTextSizeHeader);
    ...
    private void drawHeaderRowAndEvents(Canvas canvas) {
        ...
        if (mAreDimensionsInvalid) {
            mEffectiveMinHourHeight= Math.max(mMinHourHeight, (int) ((getHeight() - mHeaderHeight - mHeaderRowPadding * 2 - mHeaderMarginBottom) / MAX_HOUR));
        ...
        for (int dayNumber=leftDaysWithGaps+1; dayNumber <= leftDaysWithGaps + mNumberOfVisibleDays + 1; dayNumber++) {
            ...
            canvas.drawText(dayLabel, startPixel + mWidthPerDay / 2, mHeaderTextHeight + mHeaderRowPadding*1.330f, sameDay ? mTodayHeaderTextPaint : mHeaderTextPaint);
    ...
    public Calendar getCurDate(){
        int leftDaysWithGaps = (int) -(Math.ceil(mCurrentOrigin.x / (mWidthPerDay + mColumnGap)));
        int dayNumber = leftDaysWithGaps;
        Calendar res = today();
        res.add(Calendar.DATE, dayNumber);
    ...
    private void drawAllDayEvents(Calendar date, float startFromPixel, Canvas canvas) {
        if (mEventRects != null && mEventRects.size() > 0) {
            boolean needResetWidth =  false;
            int size = mEventRects.size();
            /*if (mEventRects.size() > mMaxCountEventsInRow) {  // todo Тут предпологалось сделать ограничение на количество задач в строке. Но работает с багами
                needResetWidth = true;
                size = mMaxCountEventsInRow;

            } else {
                needResetWidth = false;
                size = mEventRects.size();
            }*/
            float j = 0;
            for (int i = 0; i < size; i++) {
                if (isSameDay(mEventRects.get(i).event.getStartTime(), date) && mEventRects.get(i).event.isAllDay()){
                    ...
                    /*if (needResetWidth) {
                        ...
                        eventRect.width = 1f / size - 0.0140f;
                        eventRect.left = j / size + 0.0070f;
                    }*/

                    setEventTextColor(mEventRects.get(i).event.getTextColor());

                    // Calculate top.
                    float top = mHeaderRowPadding * 2*1.320f + mHeaderMarginBottom +  + mTimeTextHeight/2 + mEventMarginVertical;
                    ...
                    if (left < right &&
                        ...
                        canvas.drawRoundRect(mEventRects.get(i).rectF, mEventCornerRadius, mEventCornerRadius, mEventBackgroundPaint);
                        // border
                        canvas.drawRoundRect(mEventRects.get(i).rectF, mEventCornerRadius, mEventCornerRadius, mEventBackGroundBorder);
                        //
                        drawEventTitle(mEventRects.get(i).event, mEventRects.get(i).rectF, canvas, top, left);
                    }
                    else {
    ...
    private void drawEventTitle(WeekViewEvent event, RectF rect, Canvas canvas, float originalTop, float originalLeft) {
        ...
        name = name.replaceAll("\t", " "); // TODO: хуйня, это пиздец, надо менять
        name = name.replaceAll("\n", " ");
        name = name.replaceAll("\r", " ");
        if (name != null) {
            bob.append(name);
            bob.setSpan(new StyleSpan(Typeface.NORMAL), 0, bob.length(), 0); // TODO: 21.02.2018 жирность текста задачи
            bob.append(' ');
        }
    ...
    private void getMoreEvents(Calendar day) {
        ...
        if (mWeekViewLoader != null){
            ...
            if (!isInEditMode() && (mFetchedPeriod < 0 || mFetchedPeriod != periodToFetch || mRefreshEvents)) {
                ...
                } /*else  if (previousPeriodEvents == null) {
                    previousPeriodEvents = mWeekViewLoader.onLoad(periodToFetch - 1);
                } else  if (nextPeriodEvents == null) {
        ...
        for (int i = 0; i < maxRowCount; i++) {
            ...
            for (List<EventRect> column : columns) {
                if (column.size() >= i+1) {
                    ...
                    eventRect.width = 1f / columns.size() - 0.0140f;
                    eventRect.left = j / columns.size() + 0.0070f;
    ...
    public void setTextSize(int textSize, int textSizeHeader) {
        mTextSize = textSize;
        mTimeTextPaint.setTextSize(mTextSize);

        mTextSizeHeader = textSizeHeader;
        mTodayHeaderTextPaint.setTextSize(mTextSizeHeader);
        mHeaderTextPaint.setTextSize(mTextSizeHeader);
    ...
    public void goToDateAfterSave(Calendar day) {
        goToDate(day);
    }
    ...
    public void goToHour(double hour){
        ...
        if (hour > MAX_HOUR)
            verticalOffset = mHourHeight * MAX_HOUR;
        else if (hour > MIN_HOUR)
            verticalOffset = (int) (mHourHeight * hour);

        if (verticalOffset > mHourHeight * MAX_HOUR - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom)
            verticalOffset = (int)(mHourHeight * MAX_HOUR - getHeight() + mHeaderHeight + mHeaderRowPadding * 2 + mHeaderMarginBottom);
