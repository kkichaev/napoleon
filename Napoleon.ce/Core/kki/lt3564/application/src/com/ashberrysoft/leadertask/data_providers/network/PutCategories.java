...
public class PutCategories extends BaseTimeSOAP<Serializable> {

    private static final long serialVersionUID = 1L;
    private static final String METHOD_NAME = "PutCategories";
    private static final String LIST_ENTITIES = "tags";

    private List<Category> mEntities;

    public PutCategories(Context context, LeaderTaskUser user, List<Category> entities) {
    ...
    protected Serializable parseResponse(Reader inputStream) throws Exception {
        final SwitchParseHandler<BaseLionPutEntity<Category>> handler = SwitchParseHandler.newInstance(inputStream);

        final BaseLionPutEntity<Category> entity = handler.getData();
        final DbHelper dbHelper = DbHelper.getInstance(mContext);

        if (!entity.getListDelete().isEmpty()) {
            UidToDelete.removeUidsFromTable(mContext, entity.getListDelete(), Category.SERVER_CLASS);
            dbHelper.deleteListCategory(convertStringsToUUIDs(entity.getListDelete()));
        }

        if (!entity.getListChange().isEmpty()) {
            dbHelper.updateCategories(entity.getListChange());
        }
