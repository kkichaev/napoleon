   ...
   internal class DivisionList : DataSet<int, Division>
   {
      ...
      internal Division Root
      {
         get
         {
            ...
            return CurrentUser.user.Division;
         }
      }
      ...
      internal List<Agent> UnusedAgents()
      {
         ...
         if (agents != null)
         {
            foreach (KeyValuePair<string, Agent> av in agents)
            {
               ...
               if ((a.login == null || a.login.Length == 0) && (a.progid == null || a.progid.Length == 0)) continue;
#else
               if (a.login == null || a.login.Length == 0) continue;
#endif
      ...
      private void RemoveChildList(List<Division> childs, List<Division> rc)
      {
         foreach (Division d in childs)
         {
            RemoveChildList(d.Childs, rc);
