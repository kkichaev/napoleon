...
namespace GRSoft.NapoleonManager
{
   public partial class FmExportPhoto : Form
   {
      ...
      public FmExportPhoto()
      {
         ...
         dsScriptDoc = (DataSet<int, ScriptDoc>)DataModule.Get(ScriptDoc.OBJECT_NAME) ?? new DataSet<int, ScriptDoc>(ScriptDoc.OBJECT_NAME);
      ...
      private void btnStart_Click(object sender, EventArgs e)
      {
         ...
         dsScriptDoc.Filter = String.Format(COMMON_FILTER_STR, "created", dpv.Start.Date, dpv.Finish.Date);
         ...
         if (agentWhere.Length > 0)
         {
            ...
            dsScriptDoc.Filter += " and " + agentWhere;
         }
         dsScriptDef.Filter = "\"id\" <> '' ";
            ...
            if(v.items != null && v.items.Count > 0)
            {
               ...
               foreach (Visit.VisitItem item in v.items)
               {
                  using (MemoryStream stream = new MemoryStream(item.id))
                  {
                     ...
                     //25.08.2015 Артем Андреев просил убрать.
                     //
                     ...
                     try
                     {
                        ...
                        string dir = parent + "\\" + v.AgentName + slsNet;
