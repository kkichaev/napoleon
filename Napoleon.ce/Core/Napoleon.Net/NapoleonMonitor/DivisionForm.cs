...
namespace GRSoft.NapoleonManager
{
   ...
   public partial class DivisionForm : UserControl
   {
      ...
      protected class DataItem
      {
         ...
         {
            get { return GetTrackCaption((agent==null) ? "" : agent.id); }
         ...
         private string ToTrackingCode(string caption)
         {
         ...
         private string GetTrackCaption(string agentid)
         {
            string code = GetTrackingCode(agentid);
            if (code.Equals("none"))
               return "нет";
      ...
      {
         DivisionChief dc = new DivisionChief(division);
         dc.ShowDialog();
         ...
         //{
         //   Agent[] ch = sa.SelectedAgents;
         //   if (ch.Length == 1)
         //      SetCheif(ch[0], SetCheifAction.Ack);
         //}
      }

      internal void AddAgents(Agent[] agents, bool exclusive)
      {
         if (exclusive)
         {
      ...
      protected virtual bool IsAllowCommit(DataGridViewCell cell)
      {
         ...
         return cell != null && childUserList.Columns[cell.ColumnIndex].HeaderText == TRACKING_COLUMN_TEXT;
      }
