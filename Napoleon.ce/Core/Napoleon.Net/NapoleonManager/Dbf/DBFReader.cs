   ...
   public class DBFReader : DBFBase, IDisposable
   {
     ...
     */
      ...
      public DBFReader(string anIn)
      {
         try
         {
            FileStream fs = File.Open(anIn, FileMode.Open, FileAccess.Read, FileShare.Read);
            dataInputStream = new BinaryReader(fs);
            isClosed = false;
     ...
     */
      ...
      {
         int index = 0;
         foreach (DBFField field in header.FieldArray)
         {
            if (field.Name.Trim().CompareTo(fieldName) == 0)
               return index;
            index++;
         }
               ...
               switch (header.FieldArray[i].DataType)
               {
                  ...
                  case NativeDbType.Float:
                     ...
                     try
                     {
                        ...
                        if (decSep != ".")
                           tParsed = tParsed.Replace(".", decSep);
