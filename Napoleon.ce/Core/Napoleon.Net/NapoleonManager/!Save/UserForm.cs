...
namespace GRSoft.NapoleonManager
{
   public partial class UserForm : UserControl
   {
      ...
      private DataSet<int, AgentQuest> dsAgentQuest;
#endif
      ...
      private void InitDataSets()
      {
         ...
         dsAgentQuest = (DataSet<int, AgentQuest>)DataModule.Get(AgentQuest.OBJECT_NAME) ??
            new DataSet<int, AgentQuest>(AgentQuest.OBJECT_NAME);
#endif
      }
      ...
      private void tvQuest_AfterCheck(object sender, TreeViewEventArgs e)
      {
         ...
         foreach (TreeNode node in tvQuest.Nodes)
         {
            if (node.Checked)
            {
               ...
               quest.idquest = ((Question)node.Tag).idquest;
               quest.userid = Agent.id;
               dsAgentQuest.Add(index++, quest);
            }
         }
