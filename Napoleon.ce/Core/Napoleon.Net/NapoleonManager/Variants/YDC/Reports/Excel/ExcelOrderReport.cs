...
namespace GRSoft.NapoleonManager.Reports.Excel
{
   class ExcelOrderReport : Excel, IReportImplementation
   {
      ...
      public void Build(ReportData reportData)
      {
         ...
         const int COL_MANUF_INDEX = 3;
         const int ROW_ORGS_INDEX = 1;
         const int COL_ORGS_INDEX = 4;
            ...
            Dictionary<Org, Pair<double, double>>>> row in group)
         {
            ...
            foreach (KeyValuePair<Price, Dictionary<Org, Pair<double, double>>> rrow in row.Value)
            {
               ...
               SetShrinkToFit(row_index, COL_PRICE_INDEX, true);
               SetValue(row_index, COL_MANUF_INDEX, GetManufacturer(ordData.dsManagerFolder, row.Key));
               SetShrinkToFit(row_index, COL_MANUF_INDEX, true);
         ...
         SetValue(FIRST_ROW_POS, COL_MANUF_INDEX, "Производитель");
         SetCellBoldFont(FIRST_ROW_POS, COL_MANUF_INDEX, true);
         SetCellHorizontalAlign(FIRST_ROW_POS, COL_MANUF_INDEX, xlCenter);
         SetCellVerticalAlign(FIRST_ROW_POS, COL_MANUF_INDEX, xlCenter);

         const double FIRST_COL_WIDTH = 50.0;
         const double ARTICUL_COL_WIDTH = 7.5;
         const double MANUF_COL_WIDTH = 17.0;

         SetColumnWidth(FIRST_COL_POS, ARTICUL_COL_WIDTH);
         SetColumnWidth(FIRST_COL_POS + 1, FIRST_COL_WIDTH);
         SetColumnWidth(COL_MANUF_INDEX, MANUF_COL_WIDTH);
      ...
      private string GetManufacturer(DataSet<string, ManagerFolder> dsManagerFolder, 
         string fieldId)
      {
         string lastKnownManufacturer = null;
         const int MANUFACTURER_LEVEL = 1;

         foreach (ManagerFolder mf in dsManagerFolder.Data)
         {
            if (mf.level == MANUFACTURER_LEVEL)
               lastKnownManufacturer = mf.name;

            if (mf.id.Equals(fieldId) && lastKnownManufacturer != null)
               return lastKnownManufacturer;
         }
