...
namespace GRSoft.NapoleonManager
{
   class FmPriceEx : FmPrice
   {
      ...
      private DataSet<string, ManagerFolder> dsDelFolder = new DataSet<string, ManagerFolder>(ManagerFolder.OBJECT_NAME, false);
      ...
      public FmPriceEx()
      {
         ...
         // btnLoadPrice
         // 
         ToolStripButton btnLP = new ToolStripButton();
         btnLP.DisplayStyle = System.Windows.Forms.ToolStripItemDisplayStyle.Text;
         btnLP.ImageTransparentColor = System.Drawing.Color.Magenta;
         btnLP.Name = "btnLP";
         btnLP.Size = new System.Drawing.Size(23, 22);
         btnLP.Text = "Загрузить прайс";
         btnLP.Click += new EventHandler(btnLP_Click);
         //toolStrip1.Items.Add(btnLP);
      }
      ...
      private void btnLP_Click(object sender, EventArgs e)
      {
         ...
         if (dlg.ShowDialog() == DialogResult.OK)
            LoadPrice(dlg.FileName);
      }

      private void LoadPrice(string file)
      {
         dsPrice.Clear();

         DataSet<int, Folder> flds = new DataSet<int, Folder>(Folder.OBJECT_NAME, false);
         ...
         var adapter = new System.Data.OleDb.OleDbDataAdapter("SELECT * FROM [Лист2$]", connectionString);
         var ds = new System.Data.DataSet();

         adapter.Fill(ds, "name");

         System.Data.DataTable data = ds.Tables["name"];
         ...
         foreach (System.Data.DataRow row in data.Rows)
         {
            ...
            Price prc = new Price();
            prc.id = GRSoft.Network.DataObject.GenId();
            prc.name = r[1].ToString().Trim();
            prc.fid = f.id.ToString();
            prc.folderID = f.id;

            dsPrice.Add(prc.id, prc);
         }

         List<IDataSet> upd = new List<IDataSet>();
         upd.Add(flds);
         upd.Add(dsPrice);
         DataModule.UpdateDataSet(upd, null, null, Config.GetConfig().GetConnection());
      ...
      private void DeleteRecursive(TreeGridNode row)
      {
         if (row.Tag is ManagerFolder)
         {
            ...
            if(!dsDelFolder.ContainsKey(f.id))
               dsDelFolder.Add(f.id, f);
         }
         else if (row.Tag is Price)
         {
            ...
            if(!dsDelPrice.ContainsKey(p.id))
      ...
      protected override void BeforeWrite(List<IDataSet> wrSet, List<IDataSet> rmvSet, List<ReplacedSet> rpcSet)
      {
         ...
         if (dsDelFolder.Count > 0)
            rmvSet.Add(dsDelFolder);
      }
