...
namespace GRSoft.NapoleonManager
{
  [System.Security.Permissions.PermissionSet(System.Security.Permissions.SecurityAction.Demand, Name = "FullTrust")]
   ...
   class FmDetailEx : FmDetail
   {
      ...
      void RefreshDistribs(List<String> distribs)
      {
         if (distribs.Count > 0)
         {
            String where = "id in(";
            foreach (String id in distribs)
               where += "'" + id + "',";

            where = where.Remove(where.Length - 1) + ")";
            dsDistribMatrix.Filter = where;
            DataModule.RefreshDataSet(dsDistribMatrix, Config.GetConfig().GetConnection(), false, null);
         }
      }
      ...
      protected override void AfterRefreshData()
      {
         ...
         List<String> distribs = new List<string>();
         foreach (OrgDistribution od in dsDistrib.Data)
            if (distribs.Contains(od.id) == false)
               distribs.Add(od.id);

         RefreshDistribs(distribs);
         ...
         if (!((FmDetail)cond.fmDetail).IsScriptMode)
         {
            ...
            if (cond.OrderType != null ? cond.OrderType.Equals(ObjType.TObjType.Bonus) : true && cdata != null)
            {
               foreach (Bonus bonus in cdata.Data)
               {
                  ...
                  Add(new OrderDetailRepresentation(bonus, new ObjType(ObjType.TObjType.Bonus), oneDay));
               }
            }
            ...
            if (cond.OrderType != null ? cond.OrderType.Equals(ObjType.TObjType.OrgDistrib) : true && cdata != null)
            {
               foreach (OrgDistribution od in cdata.Data)
               {
                  ...
                  Add(new OrderDetailRepresentation(od, new ObjType(ObjType.TObjType.OrgDistrib), oneDay));
               }
            }
         }
      }
