...
namespace GRSoft.NapoleonManager
{
   public partial class FmCreateScript : Form
   {
      ...
      private void FmCreateScript_Load(object sender, EventArgs e)
      {
         ...
         foreach(KeyValuePair<string, ScriptDocItem> pair in items)
         {
            pos++;

            ScriptDocItem item = pair.Value as ScriptDocItem;
            ...
            {
               name = GetName(item) + " " + pos;
            }

            DocControl control = null;

            if (item.type.Equals("Visit"))
            {
               control = new VisitControl((item.Document as Visit).Clone());
            }
            else if (item.type.Equals("Answer"))
            {
               control = new AnswerControl((item.Document as Answer).Clone());
            }
            else if (item.type.Equals("OrgRemnants"))
            {
               control = new RemnantsControl((item.Document as OrgRemnants).Clone());
            }

            if (control != null)
            {
               listView.Items.Add(name).Tag = control;
               docHash[item.itemID] = control;
            }
         }
      ...
      private string GetName(ScriptDocItem item)
      {
