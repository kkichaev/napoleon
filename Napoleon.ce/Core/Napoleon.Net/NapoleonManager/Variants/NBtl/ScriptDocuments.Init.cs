...
namespace GRSoft.NapoleonManager
{
   partial class ScriptDocuments
   {
      static void InitDocuments()
      {
         ...
         //docs.Add(new ReturnOnDeliveryDoc());
   ...
   {
      private static readonly string TITLE = "Дистрибуция";

      internal DistribDoc()
         : base("Distrib", TITLE, Resources.distrib_doc)
      {
      }
   }

   internal class ReturnOnDeliveryDoc : ScriptDocument
   {
      private static readonly string TITLE = "Возврат при поставке";

      internal ReturnOnDeliveryDoc()
         : base("ReturnOnDelivery", TITLE, Resources.return_doc)
      {
      }
   }
}
