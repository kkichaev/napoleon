...
namespace GRSoft.NapoleonManager
{
   ...
   class RouteReport : HtmlReport
   {
      ...
      internal void Build(RouteReportData data, DataSet<int, OrgFolder> dsOrgFolder)
      {
         ...
         for (int p = 0; p < WEEK_LEN; p++)
         {
            ...
            foreach (OrgRouteQueueItem item in filter)
            {
               ...
               {
                  if (isWeekEnable(item.OrgID, 1, d, dsOrgFolder))
                     w1[p].Add(item.OrgName);
                  if (isWeekEnable(item.OrgID, 2, d, dsOrgFolder))
                     w2[p].Add(item.OrgName);
                  if (isWeekEnable(item.OrgID, 3, d, dsOrgFolder))
                     w3[p].Add(item.OrgName);
                  if (isWeekEnable(item.OrgID, 4, d, dsOrgFolder))
      ...
      public bool isWeekEnable(string id, int week, WeekDay d, DataSet<int, OrgFolder> dsOrgFolder)
      {
         bool result = false;
         //DataSet<int, OrgFolder> dsOrgFolder = DataModule.Get(OrgFolder.OBJECT_NAME) as DataSet<int, OrgFolder>;
            ...
            foreach (OrgFolder of in dsOrgFolder.Data)
            {
               ...
               foreach (OrgFolderItem i in of.items)
               {
                  if (i.org.id.Equals(id))
                  {
                     ...
                     if (!int.TryParse(new String(new char[] { of.name[0] }), out wi) || wi == week)
                     {
                        return true;
                     }
                  }
