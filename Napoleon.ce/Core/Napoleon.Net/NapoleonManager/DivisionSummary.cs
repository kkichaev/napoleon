...
namespace GRSoft.NapoleonManager
{
   public class DivisionSummary : Dictionary<string, SummaryData>
   {
      ...
      public SummaryDivisionData MakeDivisionSummary(Division division, DateTime begin, DateTime end, DataGridViewImageColumn progressColumn, TreeGridView treeView)
      {
...
#if Sibtrade
         cdata = DataModule.Get(Bonus.OBJECT_NAME);
         if (cdata != null)
            foreach (Bonus order in cdata.Data)
               this.Add(order);
#endif
         ...
         if (cdata != null)
            foreach (MoveDoc mve in cdata.Data)
               this.Add(mve);
#endif
         ...
         if (cdata != null)
            foreach (Monitoring monitoring in cdata.Data)
               this.Add(monitoring);
#endif
