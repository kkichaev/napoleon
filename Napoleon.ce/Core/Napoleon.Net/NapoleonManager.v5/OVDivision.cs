...
namespace GRSoft.NapoleonManager
{
      ...
      Dictionary<String, List<OrderDetailRepresentation>>
   {
      ...
      private DataSet<int, Schedule> dsSchedule = (DataSet<int, Schedule>)DataModule.Get(Schedule.OBJECT_NAME);
      ...
      private Dictionary<String, List<Org>> GetRoutePeriod(DateTime begin, DateTime end)
      {
         ...
         DataSet<int, Schedule> routes = (DataSet<int, Schedule>)DataModule.Get(Schedule.OBJECT_NAME);
         ...
         while (begin.Date < end.Date)
         {
            ...
            {
               foreach (Schedule orgFolder in routes.Data)
               {
                  if (orgFolder.date.Equals(begin.Date))
                  {
                     if (!result.ContainsKey(orgFolder.userid))
                        result.Add(orgFolder.userid, new List<Org>());

                     List<Org> listOrg = result[orgFolder.userid];


                     foreach (ScheduleItem item in orgFolder.items)
                     {
