...
namespace GRSoft.NapoleonManager.Utils
{
   ...
   class DataUtils
   {
      ...
      public static readonly string COMMON_PRICE_FILTER_STR = "";// + " and SetQtyFilter(False)";
      ...
      public static void beginDragAndDropOnDataGrid<T>(DataGridView dataGridView, MouseEventArgs e)
      {
         if (e.Button == MouseButtons.Left)
         {
            ...
            {
               T item = (T)dataGridView.Rows[info.RowIndex].DataBoundItem;
               if (item != null)
               {
   ...
   public class FindDataGridObject : FindObject
   {
      ...
      private DataGridViewRow Next()
      {
         ...
         return pos < 0 || pos >= dgv.Rows.Count ? null : dgv.Rows[pos];
      }
