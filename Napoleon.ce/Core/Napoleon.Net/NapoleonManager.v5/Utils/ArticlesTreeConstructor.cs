...
namespace GRSoft.NapoleonManager.Utils
{
   ...
   public class ArticlesTreeConstructor
   {
      ...
      public void MakeArticlesTree(int folderImageIndex, int priceImageIndex, IsPriceChecked isChecked, int folderSelectedIndex)
      {
         int lvl = -1;
         ...
         try
         {
            ...
            foreach (ManagerFolder mFolder in dsManagerFolder.Data)
            {
               try
               {
                  TreeNode node = new TreeNode(mFolder.name, folderImageIndex, folderSelectedIndex);
                  node.Tag = mFolder;
                  ...
                  if (dsPrice != null)
                     AddPriceItems(priceList, node, mFolder.fid, priceImageIndex, isChecked);
               }
               catch (Exception e)
               {
      ...
      {
         MakeArticlesTree(folderImageIndex, priceImageIndex, isChecked, folderImageIndex);
      }
