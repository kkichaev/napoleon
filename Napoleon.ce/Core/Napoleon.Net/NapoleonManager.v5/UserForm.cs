...
namespace GRSoft.NapoleonManager
{
   public partial class UserForm : UserControl
   {
      ...
      private DataSet<int, Schedule> mgrRoute;
      ...
      public DataSet<string, HiddenFolders> dsHiddenFolders;
        protected Button btnOrgLocation;
      ...
      public UserForm(Divisions owner)
      {
         ...
         dsHiddenFolders = new DataSet<string, HiddenFolders>(HiddenFolders.OBJECT_NAME, false);
#endif
         ...
         btnOrgLocation = new Button();
         btnOrgLocation.Anchor = ((System.Windows.Forms.AnchorStyles)((System.Windows.Forms.AnchorStyles.Bottom | System.Windows.Forms.AnchorStyles.Left)));
         Point p = new Point(btnEditRoute.Left, btnEditRoute.Top);
         p.Offset(btnEditRoute.Width + 2, 0);
         btnOrgLocation.Location = p;
         btnOrgLocation.Name = "mtxBtn";
         btnOrgLocation.Size = new System.Drawing.Size(150, btnEditRoute.Height);
         btnOrgLocation.TabIndex = 2;
         btnOrgLocation.Text = "Координаты клиента";
         btnOrgLocation.UseVisualStyleBackColor = true;
         btnOrgLocation.Click += new EventHandler((o, e) => {
            if (!WebViewWarning.IsWebViewExists())
            {
               WebViewWarning.Open();
               return;
            }
            SelectOrgLocation.Open(Agent);
         });

         panel3.Controls.Add(btnOrgLocation);
#endif 
      ...
      protected void GetDataForCurAgent(string AgentID)
      {
         ...
         mgrRoute = new DataSet<int,Schedule>(Schedule.OBJECT_NAME, false);
...
#if FOLDER_CONSTRUCTOR
         updSets.Add(dsHiddenFolders);
         dsHiddenFolders.Command = new ServerCommand(Commands.Impersonate(Commands.GET, AgentID), dsHiddenFolders.Name);
#else
...
#if SCRIPT_DOC
#if Discount
         dsCommonScriptDefs.Filter = filter;
#endif
      ...
      private void FillRoute()
      {
         ...
         //{
         //   foreach (KeyValuePair<int, Schedule> kv in mgrRoute)
         //   {
         //      Schedule of = kv.Value;
      ...
      void UncheckNodes(TreeNodeCollection nodes, DataSet<string, HiddenFolders> hidden)
      {
         foreach(TreeNode tn in nodes)
         {
            ManagerFolder mf = tn.Tag as ManagerFolder;
            tn.Checked = !(hidden.ContainsKey(mf.fid));
            if (tn.Nodes.Count > 0)
               UncheckNodes(tn.Nodes, hidden);
         }
      }
...
#if FOLDER_CONSTRUCTOR
         UncheckNodes(tvAccessibleArticles.Nodes, dsHiddenFolders);
#else
      ...
      {
         if ((parent.Tag as ManagerFolder).fid.Equals(id))
         {
            return parent;
         }
         ...
         foreach (TreeNode node in parent.Nodes)
         {
            ...
            if ((node.Tag as ManagerFolder).fid.Equals(id))
            {
               return node;
            }
         }
      ...
      {
         dsManagerFolder = new DataSet<string, ManagerFolder>(ManagerFolder.OBJECT_NAME, false, true);
         ...
         //dsCommonScriptDefs.Filter = "\"userid\" is null or \"userid\" is not null and rem != 1";
      ...
      private int UpdateDataAfterModifyTreeLow(TreeNode parent, int firstLevel, int levelShift, bool checkRoot)
      {
         if (parent.Checked)
         {
            ...
            dsManagerFolder.Add(dest.fid, dest);
         }
      ...
      void AddHidden(TreeNodeCollection nodes, DataSet<string, HiddenFolders> hidden)
      {
         foreach(TreeNode tn in nodes)
         {
            if(!tn.Checked)
            {
               HiddenFolders hf = new HiddenFolders();
               hf.userid = Agent.id;
               hf.id = ((ManagerFolder)tn.Tag).fid;

               hidden[hf.id] = hf;
            }
            if (tn.Nodes.Count > 0)
               AddHidden(tn.Nodes, hidden);
         }
      }
      ...
      private void tvAccessibleArticles_AfterCheck(object sender, TreeViewEventArgs e)
      {
         if (canCheckNode)
         {
            ...
            dsHiddenFolders.Clear();
            AddHidden(tvAccessibleArticles.Nodes, dsHiddenFolders);
      ...
      //public DataSet<int, Schedule> GetOrgFolderDataSet()
      //{
      //   DataSet<int, Schedule> result = new DataSet<int, Schedule>(Schedule.OBJECT_NAME, false);
      //   int index = 0;
