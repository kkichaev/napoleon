...
namespace GRSoft.NapoleonManager
{
   ...
   public class VisitQueueItem
   {
      ...
      public WorkTime wtime = null;
      public bool isMock = false;
      public DateTime stltime;

      public VisitQueueItem(DateTime dtVisit, Org org, double latitude, double longitude, VisitType objType, DateTime stltime)
      {
      ...
      {
         DataSet<int, WorkTime> dsWT = DataModule.Get(WorkTime.OBJECT_NAME) as DataSet<int, WorkTime>;

         if (dsWT != null)
            foreach (WorkTime w in dsWT.Data)
               if (w.start <= startTime && w.stop >= startTime)
               {
                  wtime = w;
                  break;
               }
      ...
      public String StlTime
      {
         get
         {
            ...
            if (!stltime.Equals(DateTime.MinValue) && stltime.Year != 1970)
            {
               result = stltime.ToString("HH:mm dd.MM.yy");
            }
