...
public class InputDlg extends RoundedDialog {
    public interface InputDlgControl {
        boolean valid(RowData data);
        void adjust(View view, RowData data);
    }
    ...
    public InputDlgControl control;

    public InputDlg(InputDlgParam[] params, String title) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        ((TextView) v.findViewById(R.id.title)).setText(b.getString(TITLE_KEY, ""));
        ...
        if (control != null && rows.size() > 0)
            control.adjust(v, rows.get(0));
    ...
    private void onOK() {
        ...
        for(RowData rd : rows) {
            ...
            if (rd.label == R.string.worth & val%(50*Consts.SUM_SCALE) > 0){
                Toast.makeText(getContext(), R.string.input_sum_error, Toast.LENGTH_LONG).show();
                selectRow(i);
                return;
            }
    ...
    private void selectRow(int i) {
        if(i != selected) {
            ...
            int idx = 0;
            for(RowData v : rows) {
                v.row.setBackgroundResource(idx++ == i ? R.color.purchase_back : R.color.white);
            }

            if (control != null)
                control.adjust(getView(), rows.get(i));
        }
    }
    ...
    public static String formatValue(long value, int scale, boolean hideRest) {
        return formatValue(Util.IntToScaleStr(value, scale, Util.DEC_DELIM, hideRest));
    }
