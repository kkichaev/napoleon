...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		Spinner spDoc = (Spinner) findViewById(R.id.spDocument);
		DialogHelper.loadSpinnerFromConfig(config, "Документ", new ArrayList<>(), spDoc, o.document, true);
		if(order.isEditable())
			spDoc.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
					...
					btnOK.setEnabled(val.isEmpty() == false);
				}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spDoc = (Spinner) findViewById(R.id.spDocument);
			String val = (String) spDoc.getSelectedItem();
			if(val != null)
				o.document = val;
