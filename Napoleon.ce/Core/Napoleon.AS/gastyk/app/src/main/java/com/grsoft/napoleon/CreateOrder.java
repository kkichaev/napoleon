...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		Spinner spPayV = (Spinner) findViewById(R.id.spPayType);
		DialogHelper.loadSpinnerWithKey(config, "ВариантРасчета", new ArrayList<>(), spPayV, o.payType);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spPayV = (Spinner) findViewById(R.id.spPayType);
			KeyValue sel = (KeyValue) spPayV.getSelectedItem();
			if(sel != null) {
				o.payType = sel.key.toString();
			}
			...
			if (Features.WH_QTY && sklads.size() > 0) {
				...
				sel = (KeyValue) spSklads.getSelectedItem();
