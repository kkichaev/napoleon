...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		((CheckBox)findViewById(R.id.cbBoy)).setChecked(o.boy > 0);
		((CheckBox)findViewById(R.id.cbBonus)).setChecked(o.bonus > 0);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		cbCheck.setChecked(o.check != 0);
		edDover.setText(Util.IntToScaleStr(o.dover_sum, Consts.SUM_SCALE));
		((TextView)findViewById(R.id.tvOrgInfo)).setText(Html.fromHtml(((OrgEx)org).info));
	}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.check = cbCheck.isChecked() ? 1 : 0;
			o.dover_sum = (int)Util.StrToScale(edDover.getText().toString().trim(), Consts.SUM_SCALE);
