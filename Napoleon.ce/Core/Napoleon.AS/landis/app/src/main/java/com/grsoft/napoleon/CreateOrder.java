...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		Spinner spdg = findViewById(R.id.spDog);
		ArrayAdapter<OrgDogovor> aod = new ArrayAdapter<>(this, R.layout.simple_spinner_layout, org.dogovors);
		spdg.setAdapter(aod);
		for(OrgDogovor od : org.dogovors) {
			if(od.id.equals(o.dogovor)) {
				spdg.setSelection(org.dogovors.indexOf(od));
				break;
			}
		}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spdg = findViewById(R.id.spDog);
			OrgDogovor dg = (OrgDogovor) spdg.getSelectedItem();
			if(dg != null) {
				o.dogovor = dg.id;
			}
