...
public class MonitoringItemEdit extends Activity {
	...
	TextView tvDate1;
	TextView tvDate2;
	MonitoringItemMSPB item = new MonitoringItemMSPB();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		item = (MonitoringItemMSPB) doc.findItem(getIntent().getStringExtra(ExtrasConst.ORG_ID_STR));
		
		spCatalog.setAdapter(new CatalogAdapter());
		
		if (item == null) {
			item = new MonitoringItemMSPB();
			item.id = getIntent().getStringExtra(ExtrasConst.ORG_ID_STR);
			item.date1 = Util.getDate();
			item.date2 = Util.getDate();
			
			((MonitoringMSPB)doc.getData()).items.add(item);
		}
