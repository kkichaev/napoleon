...
//	}
	...
	class SetQty implements View.OnClickListener {
		...
		public void onClick(View v) {
			final boolean useInpack = (((PriceEx) price.getData()).boxedReturn != 0);
			final int qip = price.getData().qtyInPack;

			InputNumberDlg.open(v.getContext(), new InputNumber() {

				@Override public boolean isInpack() { return useInpack; }
				@Override public boolean isPackCanChange() { return useInpack ? false : super.isPackCanChange(); }

				@Override
				public long getValue() {
					Integer value = dlvQtys.get(doc);
					int ret = value == null ? 0 : value;
					if(useInpack && qip > Consts.QTY_SCALE)
						ret = (int)((long)ret * Consts.QTY_SCALE / qip);
					return ret;
				}
			...
			}, Consts.QTY_SCALE, true, "¬ведите количество", useInpack);
		}
...
class DlvData {
	...
	public DlvData(Delivery doc, DeliveryItem item) {
		this.doc = new DlvKeySum(doc, item.qty, item.qty != 0 ? (int)((long)item.sum * Consts.QTY_SCALE / item.qty) : 0);
		this.item = item;
	}
}
