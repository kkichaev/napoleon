...
public class DocListEx extends DocList {
	...
	protected void drawData(View view, Document<?> doc, int position) {
		...
		if (dt == ReturnDoc.instance() || dt == OrderDoc.instance()) {
			DecisionImpl impl = new DecisionImpl();
			boolean res = impl.read(((CreateDocDataObject) doc.getData()).created.getTime());
			int decision = impl.getData().value;
			impl.close();

			if(dt == OrderDoc.instance() && ((OrderEx)doc.getData()).needDecision()) {
			}
			if (res && decision != 0) {
				String text = decision == Decision.APPROVED ? getString(R.string.approved) :
						dt == OrderDoc.instance() ? "Заказ не согласован" :
								getString(R.string.rejected);

				TextView tv = view.findViewById(R.id.tvStatus);
				tv.setText(Html.fromHtml(doc.getDescription(this) + "&nbsp;" + text));
