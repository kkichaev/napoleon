...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		Spinner sp = (Spinner)findViewById(R.id.spPayment);
		int spSel = o.payment - 1;
		if( spSel < 0 || spSel >= sp.getCount()) spSel = 0;
		sp.setSelection(spSel);
	...
	private void initOrder(OrderEx o, OrgEx org) {
		...
		ci.read();
		ci.close();
		o.whIndex = DialogHelper.makeListWithKey(c.value, new ArrayList<KeyValue>(), o.whCode); 
				
		CostType[] ct =  Features.COST_MANAGER.getCostTypes();
		if( ct != null && org.costype < ct.length )
			o.costType = ct[org.costype].id;
	}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			Spinner sp = (Spinner)findViewById(R.id.spPayment);
			o.payment = sp.getSelectedItemPosition() + 1;
