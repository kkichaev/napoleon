...
public class CreateOrder extends BaseActivity
{
	...
	private void init() {
		...
		String types = ((OrgEx)oi.getData()).ptypes;
		...
		if(types != null && types.trim().length() > 0){
			...
			for(int i = 0; i < arr.length; i++)
				if(arr[i].equals(o.payType)){
					sel = i;
					break;
				}
			
			ArrayAdapter<CharSequence> aa = new ArrayAdapter<CharSequence>
				(this, R.layout.simple_spinner_layout, arr);
			spPayType.setAdapter(aa);
			
			if(sel > -1)
				spPayType.setSelection(sel);
		}else
...
//		tvDelay.setOnClickListener(new DelayClickListener());
	...
	private void initOrder(OrderEx o, OrgEx org) {
		...
		o.delay = org.delay;
		o.payType = org.paytype;
	}
	
	private void updateDisplayDelay() {
		((TextView)findViewById(R.id.tvDelay)).setText("отсрочка: " + order.getData().delay);
	}
