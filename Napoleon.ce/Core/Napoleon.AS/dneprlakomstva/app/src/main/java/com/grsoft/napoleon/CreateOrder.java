...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		num.setText(Util.IntToScaleStr(o.willSum, Consts.SUM_SCALE, Util.DEC_DELIM, false));
		num.setOnFocusChangeListener(new View.OnFocusChangeListener() {
			...
			public void onFocusChange(View arg0, boolean arg1) {
				if(arg1) {
					willSumFocuesd = true;
					((CheckBox)findViewById(R.id.cbNeedIncass)).setChecked(true);
					willSumFocuesd = false;
				}
			}
		});
		
		CheckBox cb = (CheckBox)findViewById(R.id.cbNeedIncass);
		cb.setChecked(o.willPay != 0);
		cb.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
			
			@Override
			public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
				if(isChecked && !willSumFocuesd) {
					EditText ed = (EditText)findViewById(R.id.edWillSum);
					ed.selectAll();
					ed.requestFocus();
					InputMethodManager inputMethodManager=(InputMethodManager)getSystemService(Context.INPUT_METHOD_SERVICE);
				    inputMethodManager.toggleSoftInputFromWindow(buttonView.getWindowToken(), InputMethodManager.SHOW_FORCED, 0);					
				}
			}
		});
