...
public class WarehouseEx extends Warehouse {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lvItemSelect.setOnItemLongClickListener((p,v,x,i)->{
			PriceTreeNode n = (PriceTreeNode) adapter.getItem(x);

			if (n != null ){
				ActionPrice a = actions.get(n.getId());

				if (a != null){
					MessageBox.show(WarehouseEx.this, "Акционный товар", a.descr);
				}
			}
			return true;
		});
	...
	protected LinesOnClickListener createLinesOnClickListener() {
		return new LinesOnClickListener( lvItemSelect, btnLines, this, true){
			...
			public void onLinesCountChanged() {
				...
				if (getController().getLinesInRow() == 1) {
					((LinesOnClickListener.LinesController) getController()).circle();
					((LinesOnClickListener.LinesController) getController()).refreshImage();
				}
				...
				if (getController().getLinesInRow() == 3)
					btnLines.setImageResource(R.drawable.line_2);
			}
		};
	}
	...
	protected void postAdapterInit() {
		if(DocType.getCurDoc() == MerchEndDoc.instance()){
			applayAdapter(getMerchMatrix());
		}else
			super.postAdapterInit();
	}

	MerchMatrix getMerchMatrix() {
		if(merchMatrix == null)
	...
	protected boolean inheritedApplayMatrix(String matrixName) {
		if(matrixName.equals(getString(R.string.MerchMatrix)) && DocType.getCurDoc() == MerchEndDoc.instance()){
			applayAdapter(getMerchMatrix());
			return true;
		}else
...
class MerchMatrix extends MatrixBaseAdapter{
	...
	public MerchMatrix(Warehouse warehouse, MerchImpl doc) {
		super(warehouse);
		this.doc = doc; 
	}
