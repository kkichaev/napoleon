...
public class CreateOrder extends BaseActivity
{
	...
	void loadFirm(ConfigImpl config, OrgEx org, int fsel) {
		...
		ArrayAdapter<FirmData> aa = new ArrayAdapter<FirmData>(spFirma.getContext(), R.layout.simple_spinner_layout, values);
	...
	private void init() {
		...
		ArrayList<KeyValue> priceType = new ArrayList<KeyValue>();
		DialogHelper.loadSpinnerWithKey(config, "ЦеныСКодом", priceType, spPrices, o.costCode);

		Spinner spWh = (Spinner) findViewById(R.id.spWh);
		List<KeyValue> sklads = new ArrayList<KeyValue>();
		DialogHelper.loadSpinnerWithKey(config, "Склады", sklads, spWh, o.whCode);
		spWh.setEnabled(o.items.size()==0);
		
		config.getData().key = "МожноИзменятьЦену";
		try {
			boolean canChangeCost = true;
			if (config.read() && Integer.parseInt(config.getData().value) == 0) {
				DbReader r = new DbReader();
				CustomCost c = new CustomCost();
				String table = DataObjectInfo.getInstance().getTableName(c.getClass());
				canChangeCost = r.select(c, table, "id = userid");
				r.close();
			}
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			if( suppl >= 0 ) {
				FirmData fsel = (FirmData) spFirma.getSelectedItem();
				o.supplyer = fsel.index;
			}
			if( costType >= 0 ) {
				o.sumType = costType;
				o.costCode = ((KeyValue)spPrices.getSelectedItem()).key.toString();
			}
			...
			Spinner spDogs = (Spinner) findViewById(R.id.spDogovors);
			kv = (KeyValue)spDogs.getSelectedItem();
			if( kv != null )
				o.dogovor = kv.key.toString();
