...
public class CreateSalesEx extends BaseActivity {
	...
	private void init() {
		...
		CheckBox tabak = findViewById(R.id.cbTabak);
		tabak.setChecked(o.tabak != 0);
		tabak.setEnabled(o.items.size() == 0);
...
//			return;
		...
		String where = "companyid in (select idOrg from Sklads where van=1) and clientid='" + clientid + "'";
...
//		int suppl = spFirma.getSelectedItemPosition();
		...
		sales.tabak = ((CheckBox)findViewById(R.id.cbTabak)).isChecked() ? 1 : 0;
...
//		}
		...
		if(selDg != null) {
			...
			sales.supplyercode = selDg.companyid;
			if(selDg.black > 0)
				sales.useTax = 0;
		} else
			sales.contractid = "";
