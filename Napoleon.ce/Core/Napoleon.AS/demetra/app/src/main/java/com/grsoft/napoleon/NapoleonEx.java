...
public class NapoleonEx extends Napoleon {
	...
	private static final int DLG_WAIT = 300;
	...
	protected void showRouteMap() {
		OrgFoldersAdapter a = ((OrgFoldersAdapter)lvMainOrgs.getAdapter());
		OrgFolders of = a.currentFolder();

		if (of != null) {
			ArrayList<String> ids = new ArrayList<String>();

			for (OrgFolderItem i : a.currentFolder().items)
				ids.add(i.name);
	...
	public boolean isGlobusAvail() {
		return listViewMode == ListViewMode.ROUTE_LIST && ((OrgFoldersAdapter)lvMainOrgs.getAdapter()).isTopLevel();
	}
	...
	@Override
	protected int getDocButtonImage(DocType docType) {
		return docType.getResurce2Id();
	}
	...
	public void buldProcess(AsyncTask<?, ?, ?> task) {
		...
		if (!task.isCancelled()){
			if(!solidPrice) {
				...
				priceTop = createFoldersTreeNode(null);
				Collection<ArrayList<GroupInfo>> piListList = new ArrayList<ArrayList<GroupInfo>>(fprice.values());
				
				for(ArrayList<GroupInfo> val : piListList)
					for(GroupInfo pi : val)
							priceTop.getChilds().add(createTreeNode(priceTop, pi.rowid, pi.name));
