...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerWithKeyW(config, "СкладыNew", new ArrayList<KeyValue>(), spSklads, o.whCode, false);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	private void loadDogovors(OrderEx o, OrgEx org) {
		List<OrgDog> dogovors = new ArrayList<>();
		int sel = -1;
		for(OrgDog od : org.dogovors) {
			if(o.dogovor.equals(od.id) || (o.dogovor.length() == 0 && od.main == 1))
				sel = dogovors.size();
			dogovors.add(od);
		}

		Spinner s = findViewById(R.id.spDogovor);
		ArrayAdapter<OrgDog> aa = new ArrayAdapter<OrgDog>(this, R.layout.simple_spinner_layout, dogovors);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spd = findViewById(R.id.spDogovor);
			OrgDog selD = (OrgDog) spd.getSelectedItem();
			if(selD != null)
				o.dogovor = selD.id;
