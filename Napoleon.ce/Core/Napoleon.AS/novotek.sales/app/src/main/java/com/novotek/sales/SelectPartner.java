...
public class SelectPartner extends AppCompatActivity {
    ...
    static final String CAN_BACK_TAG = "can_back";

    boolean canBack = true;

    Partner cursel;
    public static boolean open(Activity context, boolean showList) {
        if(!showList && ProjectData.partners().size() == 1) {
            ProjectData.setCurrent(ProjectData.partners().get(0));
            return false;
        }

        Intent i = new Intent(context, SelectPartner.class);
        i.putExtra(CAN_BACK_TAG, showList);
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if(b != null) {
            canBack = b.getBoolean(CAN_BACK_TAG, true);
        }
        cursel = ProjectData.getCurrent();
        ...
        findViewById(R.id.doButton).setOnClickListener(v -> {
            if(cursel != null) {
                ProjectData.setCurrent(cursel);

                setResult(RESULT_OK, null);
                finish();
            } else {
                Toast.makeText(SelectPartner.this, R.string.choose_partner, Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    class Adapter extends BaseAdapter {
        ...
        @Override
        public int getCount() {
            return ProjectData.partners().size();
        }

        @Override
        public Object getItem(int i) {
            return ProjectData.partners().get(i);
        }
