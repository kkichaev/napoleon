...
public class CreateOrder extends BaseActivity
{
	...
	private static final int FLAGS_DIALOG = 2;
...
//	DateHandler dateHandler;
	...
	Map<Object, OrdFlag> flags = new HashMap<>();
	List<OrdFlag> flagsData = new ArrayList<>();
	...
	private void init() {
		...
		flagsData = DbReader.fetch(OrdFlag.class);
		for(OrdFlag fi : flagsData) {
			flags.put(fi.id, fi);
		}
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		updateFlags();
		if(order.isEditable()) {
			findViewById(R.id.tvFlags).setOnClickListener(v->{
				showDialog(FLAGS_DIALOG);
			});
		}
	}

	void updateFlags() {
		((TextView)findViewById(R.id.tvFlags)).setText(flagText());
	}

	String flagText() {
		String res = "";
		String[] cf = ((OrderEx)order.getData()).customFlags.split(DELIM);
		for(String fi : cf) {
			if(res.length() > 0)
				res += DELIM;
			OrdFlag val = flags.get(fi);
			if(val != null) res += val.name;
			else res += fi;
		}
...
//				return dateHandler.createDialog();
	...
	static class OrdFlagItem{
		OrdFlag flag;
		boolean checked;
		public OrdFlagItem(OrdFlag src) {
			flag = src;
		}
	}

	class OrdFlagAdapter extends BaseAdapter {
		List<OrdFlagItem> data = new ArrayList<>();

		public OrdFlagAdapter() {
			List<String> ids = Arrays.asList(((OrderEx) order.getData()).customFlags.split(DELIM));
			for(OrdFlag of: flagsData) {
				OrdFlagItem ofi = new OrdFlagItem(of);
				ofi.checked = ids.contains(of.id);
				data.add(ofi);
			}
		}

		OnClickListener togle = v -> {
