...
//	DateHandler dateHandler;
	...
	private void init() {
		cbRetDoc = findViewById(R.id.cbRetDoc);
		cbWriteOff = findViewById(R.id.cbWriteOff);

		cbRetDoc.setOnClickListener((v)->{
			if (cbRetDoc.isChecked())
				showDialog(R.id.return_alert_dlg);
		});

		cbWriteOff.setOnClickListener((v)->{
			if (cbWriteOff.isChecked())
				showDialog(R.id.writeoff_alert_dlg);
		});
		...
		if( (o.retdoc) != 0 )
			((CheckBox)findViewById(R.id.cbRetDoc)).setChecked(true);

		if( (o.writeoff) != 0 )
			((CheckBox)findViewById(R.id.cbWriteOff)).setChecked(true);
...
//				return dateHandler.createDialog();
			...
			case R.id.return_alert_dlg:
				return createReturnAlertDlg();
			case R.id.writeoff_alert_dlg:
				return createWriteOffAlertDlg();
		}
		return super.onCreateDialog(id);
	}

	private Dialog createWriteOffAlertDlg() {
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setTitle(R.string.alert);
		builder.setMessage("Вы собираетесь оформить Списание?");
		builder.setPositiveButton(R.string.ok, (v,d)->{cbRetDoc.setChecked(false);});
		builder.setNegativeButton(R.string.cancel, (v,d)->{cbWriteOff.setChecked(false);});
		builder.setCancelable(false);
		return builder.create();
	}

	private Dialog createReturnAlertDlg() {
		AlertDialog.Builder builder = new AlertDialog.Builder(this);
		builder.setTitle(R.string.alert);
		builder.setMessage("Вы собираетесь оформить Возврат?");
		builder.setPositiveButton(R.string.ok, (v,d)->{cbWriteOff.setChecked(false);});
		builder.setNegativeButton(R.string.cancel, (v,d)->{cbRetDoc.setChecked(false);});
		builder.setCancelable(false);
		return builder.create();
	}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.retdoc = ((CheckBox)findViewById(R.id.cbRetDoc)).isChecked() ? 1 : 0;
			o.writeoff = ((CheckBox)findViewById(R.id.cbWriteOff)).isChecked() ? 1 : 0;
