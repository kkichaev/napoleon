...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerFromConfig(config, "Склады", new ArrayList<CharSequence>(), spSklads, o.sklad);

		((CheckBox)findViewById(R.id.cbOrderOnly)).setChecked(o.orderOnly > 0);
		((CheckBox)findViewById(R.id.cbWithDocs)).setChecked(o.withDocs > 0);
		((CheckBox)findViewById(R.id.cbBeznal)).setChecked(o.beznal > 0);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.orderOnly = ((CheckBox)findViewById(R.id.cbOrderOnly)).isChecked() ? 1 : 0;
			o.withDocs = ((CheckBox)findViewById(R.id.cbWithDocs)).isChecked() ? 1 : 0;
			o.beznal = ((CheckBox)findViewById(R.id.cbBeznal)).isChecked() ? 1 : 0;

			Spinner spSklads = (Spinner) findViewById(R.id.spSklad);
			String selStr = (String) spSklads.getSelectedItem();
			if(selStr != null)
				o.sklad = selStr;
