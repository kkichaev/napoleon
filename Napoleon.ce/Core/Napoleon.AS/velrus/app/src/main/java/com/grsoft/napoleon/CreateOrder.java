...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerFromDataObject(spFirma, Firm.class, new DialogHelper.Selected<Firm>() {
			@Override public boolean isSelected(Firm object) { return o.firmCode.equals(object.id); }
		}, false);
...
//				return dateHandler.createDialog();
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		public void onClick(View v) {
			...
			if(spPrices.getSelectedItem() != null)
				costType = Features.COST_MANAGER.getCostIndex(((CostType)spPrices.getSelectedItem()).id);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spFirma = (Spinner) findViewById(R.id.spFirma);
			Firm f = (Firm) spFirma.getSelectedItem();
			...
			CostType ct = ((CostType)spPrices.getSelectedItem());

			if (ct != null) {
				o.sumType = Features.COST_MANAGER.getCostIndex(ct.id);
				o.costType = ct.id;
			}
