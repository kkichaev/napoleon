...
import com.grsoft.napoleon.printsources.SalesItemPrint;
...
public class PriceCountEx extends PriceCount {
	...
	protected boolean canChangeCost() {
		if(document instanceof SalesImpl) {
	...
	protected void refreshData() {
		super.refreshData();
		cbPackets.setEnabled(((PriceEx)price.getData()).unitType == PriceEx.UNIT_NONE);
		if(document instanceof SalesImpl) {
			View v = findViewById(R.id.trMinCost);
			if(minCost != 0) {
				v.setVisibility(View.VISIBLE);
				SalesItem sip = (SalesItem)((SalesImpl) document).findItem(price.getData().id);
	...
	protected void postOKProcess() {
		if (document instanceof OrderImpl){
			...
			if (item != null && cbAction.isChecked() && actionItem != null && actionItem.qty <= item.qty){
