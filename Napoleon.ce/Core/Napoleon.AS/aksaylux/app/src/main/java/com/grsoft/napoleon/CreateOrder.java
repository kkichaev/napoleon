...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerFromConfig(config, "ВариантОплаты", new ArrayList<CharSequence>(), spPayType, o.payType, true);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			String payType = (String) ((Spinner)findViewById(R.id.spPayType)).getSelectedItem();
			if(payType != null) {
				o.payType = payType;
			}
			if(o.payType.length() == 0) {
				Toast.makeText(CreateOrder.this, "Выберите вариант оплаты", Toast.LENGTH_LONG).show();
				return;
			}
