...
//			}
		...
		Size adjustToCameraParams(Config config, Camera.Parameters parameters) {
			List<Camera.Size> list = CameraHelper.getSupportedPictureSizes(parameters, true);

			Size ret = new Size(config.cameraWidth, config.cameraHeight);
			Size chkd = null;
		...
		public void surfaceChanged(SurfaceHolder holder, int format, int w, int h) {
			try {
				if (camera != null) {
					...
					Size setttingCameraSize = adjustToCameraParams(config, parameters);
