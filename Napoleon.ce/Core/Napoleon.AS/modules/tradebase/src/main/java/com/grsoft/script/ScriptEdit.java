...
public class ScriptEdit extends BaseActivity implements SendResultListener {
...
//	}
	...
	public void send() {
		if(Features.CANT_SEND_SCRIPT_PART && !doc.isComplete()){
	...
	protected DocumentSender createDocumentSender() {
		return new DocumentSender(this, findViewById(R.id.btnSend), doc.getSendedDocuments(), this);
	}
	...
	protected void onResume() {
		...
		if(DocType.getCurDoc() != ScriptDoc.instance()) {
	...
	public class ItemsAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			String text = sd.name;
			if(text.length() == 0 && dt != null) {
				text = getString(dt.getDocTitle());
			}
			if(text.length() == 0)
				text = "?";
			tv.setText(text);
