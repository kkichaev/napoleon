...
public class AssortmentMatrixAdapter extends MatrixBaseAdapter {
	...
	public List<MatrixItem> matrixItems = null;
	...
	public AssortmentMatrixAdapter(Warehouse warehouse, String id, List<MatrixItem> items) {
		super(warehouse);
	...
	protected List<MatrixItem> getMatrixItems() {
		...
		if(matrixItems == null) {
			matrixItems = new ArrayList<MatrixItem>();

			try {
				collectItems(id, matrixItems);
			} catch (Exception e) {

			}
		}

		for(MatrixItem mi : matrixItems)
	...
	public static void collectItems(String id, final List<MatrixItem> result, AssortimenMatrixDocIterator iterator) {
		...
		for (int i = 0; i < dl.getCount(); i++) {
			...
			iterator.iterItems(d, pi, id1 -> addItem(result, priceIds, id1, d));
		}
		pi.close();
	}
