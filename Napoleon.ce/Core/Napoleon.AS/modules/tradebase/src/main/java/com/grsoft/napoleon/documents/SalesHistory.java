...
public class SalesHistory  extends HashMap<Long, Integer> {
	...
	public static Class<? extends SalesHistory> HistClass = SalesHistory.class;
	...
	static public String[] getHistory(String orgId, String priceId, boolean fromOrders) {
		...
		SalesHistory history = null;
		try {
			history = HistClass.newInstance();
		} catch (Exception e) {
			e.printStackTrace();
		}
		return history == null? null : history.getHistoryInt(orgId, priceId, fromOrders);
	}
}
