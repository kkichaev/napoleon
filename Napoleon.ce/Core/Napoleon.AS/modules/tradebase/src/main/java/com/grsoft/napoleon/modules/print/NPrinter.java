...
class CellTextFormatter{
	...
	public static String getBCInfo(String text) {
		int offset = text.indexOf(NPrinter.BC_MARKER);
		String ret = "";
		if(offset >= 0) {
			for (Character c : text.substring(offset).toCharArray()) {
	...
	public static String removeBCInfo(String text) {
		String tt = getBCInfo(text);
		if(tt.length() == 0) {
			return text;
		}
		StringBuilder sb = new StringBuilder(text);
		int start = text.indexOf(NPrinter.BC_MARKER);
		sb.delete(start, start + tt.length());
		return sb.toString();
	}
	...
	public static Image getImage(String text, DataSource source, PdfWriter writer){
		...
		} else if(text.contains(NPrinter. BC_MARKER)) {
			text = getBCInfo(text);
	...
	public static String format(String text, DataSource source){
		if(text.startsWith(NPrinter.IMAGE_MARKER))
			return "";
		if(text.contains(NPrinter.BC_MARKER)) {
			text = removeBCInfo(text);
		}
...
class CellBorder{
	...
	public void setBorder(PdfPCell c, PdfPCell imgCell, int imgText) {
		...
		if(imgCell != null) {
			brd &= ~(PdfPCell.TOP | PdfPCell.BOTTOM);
			if(imgText > 0 && (value & BOTTOM) == BOTTOM) brd |= PdfPCell.BOTTOM;
			if(imgText < 0 && (value & TOP) == TOP) brd |= PdfPCell.TOP;
			imgCell.setBorder(brd);
		}
	}
}
