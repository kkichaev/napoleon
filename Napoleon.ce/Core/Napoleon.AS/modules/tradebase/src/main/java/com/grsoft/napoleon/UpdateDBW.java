...
public class UpdateDBW extends Activity implements ProgressHelper.ButtonAction{
	...
	protected boolean loadFullPrice = false;
			...
			cbPres.setOnCheckedChangeListener((buttonView, isChecked) -> {
				if(isChecked) {
					...
					if(c.loadPresentationByWiFi && ((CheckBox) findViewById(R.id.cbPresent)).isChecked()) {
						if(!isWiFiConnected(UpdateDBW.this)) {
							cbPres.setChecked(false);
							MessageBox.show(UpdateDBW.this, getString(R.string.warning),
	...
	boolean canRestoreDocuments() {
		...
		for(String tn : docTables) {
			String sql = String.format("select count(*) from [%s] where id <> ''", tn);
			try {
...
//									vsh.setActiveConnnection(activeCon);
					...
					if (errMessage == null && !isCancelled() && receiveGenData()) {
						...
						if (rcvHitch.size() > 0) {
							...
							if (!dataBaseUpdater.update(activity,
								...
								errKind = dataBaseUpdater.getKind();
					...
					if (!isCancelled()) {
						if (errMessage != null) {
							if(errKind.equals(UpdateMessageBox.UPDATE_KIND)) {
								UpdateMessageBox.show(errMessage, getString(R.string.error), activity);
							} else {
