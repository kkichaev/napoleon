...
public class ScriptImpl extends CreatableDocument<Script> {
	...
	public CreatableDocument<?>[] getDocuments() {
		...
		for(ScriptItem item : data.items) {
			if( item.state == ScriptItem.DOC_INITED || item.state == ScriptItem.DOC_PENDING ) {
	...
	public void openDoc(Context c, int index, ScriptDef defDoc) {
		...
		if( doc == null && isEditable()) {
			...
			boolean res = initChildDoc(c, index, doc, item);
			if( !res )
				return;
		}
	...
	public void updateDocument(int index, CreatableDocument<?> doc) {
		data.items.get(index).date = doc.getData().created;
		data.items.get(index).state = ScriptItem.DOC_INITED;
	}

	public boolean isDone(int position) {
		boolean res = (position < data.items.size()) ? (data.items.get(position).state == ScriptItem.DOC_INITED) : false;
	...
	public boolean isComplete() {
		if(Features.DEL_VISIT_WITHOUT_PHOTO) {
			...
			for(ScriptItem si : data.items) {
				if (si.state == ScriptItem.DOC_SKIPPED)
					continue;
		...
		CreatableDocument<?> doc[] = getDocuments();
		for(int i=0; i<doc.length; i++) {
			if (doc[i] != null && doc[i].isEmpty())
				return false;
		}
