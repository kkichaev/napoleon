...
public class StoreQtyHitching extends Hitching {
    ...
    public void onStart() {
        ...
        try{
            String stmt = "update [" + new PriceV5().getTableName() + "] set qty=0, whQty=null, whStates=0";
            DataBaseManager.getDataBase().execSQL(stmt);
        } catch (Exception e) {
            e.printStackTrace();
        }
        data = DbReader.fetchDic(PriceV5.class, "id");
    }
