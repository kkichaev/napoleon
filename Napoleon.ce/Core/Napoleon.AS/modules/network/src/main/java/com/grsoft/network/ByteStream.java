...
public class ByteStream
{
	...
	public static final String GET_SK_TAG = "GETSK";
	public static final String CRYPT_TAG = "CRPT";
	...
	public byte[] getData() { return bytes.getData(); }
	...
	public int send(OutputStream stream, String operations, Cipher cipher) throws RuntimeException
	{
		String head = "";
		if(cipher == null) {
			if (!operations.contains(GZIP_TAG) && !operations.contains(CRC_TAG))
				operations += CRC_TAG;
		}
...
//		}
		...
		if(cipher != null) {
			byte[] out = new byte[0];
			try {
				out = cipher.doFinal(bytes.getData());
			} catch (Exception e) {
				e.printStackTrace();
			}
			if(out != null) {
				head = CRYPT_TAG + ";" + head;
				bytes = new Buffer(out);
			}
		}


		String pktTag = PACKET_STR_TAG + "(" + Integer.toString(bytes.getLength()) + ");";
		head = pktTag + head + DATA_TAG + ";";
...
//		dbgWriteFileLn(bytes.getData(), true);
	...
	public int send(OutputStream stream, String operations) throws RuntimeException {
		return send(stream, operations, null);
	}

	public static ByteStream receive(InputStream stream, Context context, Cipher decoder) throws RuntimeException {
...
//		ByteStream res = decodePacket(packet, ops);
		...
		ByteStream res = decodePacket(inStr, ops, decoder);
		if( res != null )
			res.received = rcvd;
	...
	public static ByteStream receive(InputStream stream, Context context) throws RuntimeException {
		return receive(stream, context, null);
	}
...
//		if(BUF_SIZE > MAX_BUF_LENGTH)
	...
	private static ByteStream decodePacket(ByteStream packet, ArrayList<PacketOptions> ops, Cipher decoder) {
		for (PacketOptions op : ops) {
			...
			} else if(ByteStream.CRYPT_TAG.equals(opName)) {
				try {
					byte[] out = decoder.doFinal(packet.bytes.getData());
					packet = new ByteStream(out, packet.context);
				} catch (Exception e) {
					e.printStackTrace();
				}
			} else if(ByteStream.GET_SK_TAG.equals(opName)) {
				break;
			} else {
				packet = null;
			}
