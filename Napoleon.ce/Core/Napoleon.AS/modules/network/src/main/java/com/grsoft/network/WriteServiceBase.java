...
//					}
	...
	public boolean forcePut(Context context, UserInfo userInfo
			, List<ObjectListener> objects
			, List<Hitching> requests
			, SocketConnection conn, boolean needPackObjects) {
		boolean result = false;
		...
		try{
			...
			dataObjectPool.add(serverCommand);
			addObjectsToSend(dataObjectPool, userInfo, objects, requests);
			...
			ByteStream outStream = conn.receive(context);
	...
	public boolean forcePut(Context context, UserInfo userInfo, List<ObjectExportListener> objects, SocketConnection conn, boolean needPackObjects) {
		...
		for(ObjectExportListener oel : objects) {
			toSend.add(oel);
		}
		return forcePut(context, userInfo, toSend, new ArrayList<>(), conn, needPackObjects);
	}
...
//		ByteStream stream = winner.getByteStream();
		...
		if(Features.ENCODE_CONNECTION) {
			...
			if(winConnect != null) {
				return forcePut(context, userInfo, objectsToSend, requestHitchs, winConnect, true);
			}
		}
		...
		try
		{
			...
			if (!serverAnswerHitching.IsOK()){
				...
				kind = serverAnswerHitching.getKind();
				return false;
			}
	...
	protected void addObjectsToSend(DataObjectPool dataObjectPool
			, UserInfo userInfo
			, List<ObjectListener> toSend
			, List<Hitching> request) throws RuntimeException {
		for(ObjectListener ol : toSend) {
			if (ol instanceof DocExportListener){
				DocList list = ((DocExportListener)ol).getDocuments();
	...
	private DataObjectPool createDataObjectPool(UserInfo userInfo) throws RuntimeException {
		...
		addObjectsToSend(dataObjectPool, userInfo, objectsToSend, requestHitchs);
