...
public class ConnectionManager {
	...
	private synchronized void updatePool() {
		...
		if(GRJSHelper.IsJSAddress(config.address)) {
			connPool.add(createConnection(config.address, config.port));
		} else if(GRJSHelper.IsJSAddress(config.address2)) {
			connPool.add(createConnection(config.address2, config.port));
		} else {
			if(config.address != null && config.address.length() > 0)
				connPool.add(createConnection(config.address, config.port));
			if(config.address2 != null && config.address2.length() > 0)
				connPool.add(createConnection(config.address2, config.port2));
		}
	...
	SocketConnection createConnection(String address, int port) {
		if(Features.ENCODE_CONNECTION) {
			return new EncodableConnection(address, port);
		}
		return new SocketConnection(address, port);
	}
	
	public synchronized void createPool(UserInfo info) {
			...
			if(sz > 0){
				...
				for(int i = 0; i < sz; i++){
					...
					if(GRJSHelper.IsJSAddress(arg.address)) {
						connPool.clear();
						connPool.add(createConnection(arg.address, arg.port));
						break;
					}
					connPool.add(createConnection(arg.address, arg.port));
				}
