...
public class DbReader {
	private enum CursorType { none, read, select, selectStmt };
	...
	Map<Field, Integer> cursorFields = null;
	...
	private SQLiteCursor makeCursor(Class<? extends DataObject> dataType , String table, String pkStr, String[] keys, boolean addRID) {
		...
		return (SQLiteCursor) DataBaseManager.getDataBase().rawQuery(sb.toString(), pkStr.length() > 0 ? keys : null);
	}
	...
	public <T extends DataObject> boolean selectStmt(T data, String stmt) {
		close();

		ct = CursorType.selectStmt;
		try {
			cursor =  (SQLiteCursor) DataBaseManager.getDataBase().rawQuery(stmt, null);
		} catch (Exception e) {
			e.printStackTrace();
		}
		if(cursor == null)
	...
	public boolean selectPrev(DataObject data) {
		boolean ret = false;
	...
	static public <T extends  DataObject> List<T> fetch(Class<T> objType) {
		return fetch(objType, "", "");
	}

	static public <T extends  DataObject> List<T> fetch( Class<T> objType, String where) {
		return fetch(objType, where, "");
	}

	static public <T extends  DataObject> List<T> fetchStmt( Class<T> objType, String stmt) {
		DbReader r = new DbReader();
		List<T> ret = new ArrayList<>();

		try {
			T obj = objType.newInstance();
			boolean bdo = r.selectStmt(obj, stmt);
			while(bdo) {
				ret.add(obj);
				obj = objType.newInstance();
				bdo = r.selectNext(obj);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	static public <T extends  DataObject> List<T> fetch(Class<T> objType, String where, String order) {
		DbReader r = new DbReader();
		List<T> ret = new ArrayList<>();

		try {
			T obj = objType.newInstance();
			boolean bdo = r.select(obj, obj.getTableName(), where, order);
			while(bdo) {
				ret.add(obj);
				obj = objType.newInstance();
				bdo = r.selectNext(obj);
	...
	static public <T extends  DataObject> Map<Object, T> fetchDic(Class<T> objType, String key) {
		return fetchDic(objType, key, "");
	}

	static public <T extends  DataObject> Map<Object, T> fetchDic(Class<T> objType, String key, String where) {
		Map<Object, T> ret = new HashMap<>();

		DbReader r = new DbReader();
		try {
			Field f = objType.getField(key);
			T obj = objType.newInstance();
			boolean bdo = r.select(obj, obj.getTableName(), where, "");
			while(bdo) {
	...
	private void update(Cursor cursor, DataObject dataObject, Field[] fields) throws RuntimeException {
		...
		try {
			if(cursorFields == null) {
				...
				for(Field f : fields) {
					tmap.put(f.getName().toLowerCase(), f);
				}
				cursorFields = new HashMap<>();
				int idx = 0;
				for(String s : cursor.getColumnNames()) {
					Field f = tmap.get(s.toLowerCase());
					if(f != null)
						cursorFields.put(f, idx);
					idx++;
				}
			}
		...
		throws TypeNotImplemented, RuntimeException
	{
		...
		if (type == int.class)
			...
			if(ib.hasArray())
				return ib.array();
			int count = b.length / Integer.BYTES;
	...
	private Object readFromCursor(Cursor cursor, Field field, Class<? extends DataObject> dataObjectClass) 
		throws TypeNotImplemented, RuntimeException
	{
		Integer index = cursorFields.get(field);
		if(index == null)
			return null;
...
//		List<? extends DataObject> list = DataObjectUtils.readList(paramType, ByteBuffer.wrap(stream));
		if(ListTypeConvertor.class.isAssignableFrom(dataObjectClass)) {
			try {
				ListTypeConvertor ltc = (ListTypeConvertor) dataObjectClass.newInstance();
				stream = ltc.convert(stream, field.getName());
			} catch(Exception e) {
				e.printStackTrace();
			}
		}
