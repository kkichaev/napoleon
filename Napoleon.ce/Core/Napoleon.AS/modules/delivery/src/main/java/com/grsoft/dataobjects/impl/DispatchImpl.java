...
public class DispatchImpl extends CreatableDocument<Dispatch> {
	...
	public int isDocFinished(Context context) {
		for(int i=0; i<data.items.size(); i++) {
			DispatchItem di = data.items.get(i);
		...
		return DispatchItem.DOC_NOT_INITED;
	}


	public int isItemFinished(Context context, int pos) {
		int res = DispatchItem.DOC_NOT_INITED;
		...
		if(pos >= 0 && data.items.size() > pos){
			...
			if (i.state == DispatchItem.DOC_INITED) {
				...
				if(dt != null){
					...
					if (doc.isReadyToSend()){
						res = DispatchItem.DOC_COMPLETE;
					}
				}
			}
		}
