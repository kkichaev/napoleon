...
public class ReturnPriceCount extends PriceCount implements com.grsoft.dataobjects.impl.OrderImplBase.UpdateQtyHandler {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		ReturnEx ret = (ReturnEx) document.getData();

		if (ret.ordcrt.getTime() > 0){
			order = new OrderImpl();
			order.read(ret.ordcrt.getTime());
			order.close();
		}
	...
	private void okClick() {
		if (order != null) {
			...
			cfg.getValue(sb, "ПроцентЗаявкиДляВозврата");

			double maxPercent = 0;
			try {
				maxPercent = Double.parseDouble(sb.toString().replace(",", "."));
			}catch (Exception e){
				e.printStackTrace();
			}

			long limit = (int)(order.sum() * maxPercent / 100);
			long docSum =  ((ReturnImplEx)document).newSum(price.getData().id, getSumValue());
			if (limit < docSum){
				Toast.makeText(ReturnPriceCount.this,
						String.format("Сумма возврата %s не может превышать %.2f%%(%s) от суммы заявки %s",
								Util.IntToScaleStr(docSum, Consts.SUM_SCALE), maxPercent,
								Util.IntToScaleStr(limit, Consts.SUM_SCALE),
								Util.IntToScaleStr(order.sum(), Consts.SUM_SCALE)),
