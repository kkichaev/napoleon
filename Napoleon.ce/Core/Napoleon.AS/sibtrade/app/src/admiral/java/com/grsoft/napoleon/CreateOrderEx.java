...
public class CreateOrderEx extends BaseActivity
{
	protected OrderImplBase<? extends Order> order = null;
	...
	List<Integer> whDefault = new ArrayList<Integer>();
	
//	DateHandler dateHandler;
	TimeHandler timeHandler;
	
	protected int getLayoutId() { return R.layout.createorder; }
	...
	protected OrderImplBase<? extends Order> createDocument() {
		return (OrderImpl)OrderDoc.instance().create();
	}
	...
	public static void open(Context context, OrderImpl order, boolean editOldOrder) {
		Intent i = new Intent(context, CreateOrderEx.class);
	...
	private void init() {
		...
		List<CharSequence> srcWh = new ArrayList<CharSequence>();
		Config cfg = config.getData();
		cfg.key = "СкладОрганизации";
		config.read();
		DialogHelper.makeList(cfg.value, srcWh);
		for(CharSequence cs : srcWh) {
			int val = 0;
			try {
				val = Integer.parseInt(cs.toString().trim());
		...
		final Spinner spSklad = (Spinner) findViewById(R.id.spSklad);
		...
		spFirma.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				...
				int cs = whDefault.size() > arg2 ? whDefault.get(arg2) : 0;
				if( spSklad.getCount() > cs )
					spSklad.setSelection(cs);
			}
		...
		findViewById(R.id.tvDate).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				Intent i = new Intent(CreateOrderEx.this, CalendarActivity.class);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			if(!editMode)
				Warehouse.open(CreateOrderEx.this, order, false);
