...
public class ReturnPriceCount extends PriceCount {
	...
	private void collectRest() {
		rest.clear();
		String where = String.format("id='%s'and created != %d", document.getId(), document.getData().created.getTime());
		
		final String id = price.getData().id;
		...
		DataTraveler.travel(Return.class, new DataTraveler.Travel<Return>() {
			...
			public boolean travel(DataTraveler<Return> item) {
				for(OrderItem i : item.data.items)
					if (i.id.equals(id))
						for (ReturnItemDlv di : ((ReturnItem)i).items) {
							int qty = 0;
							
							if (rest.containsKey(di.number))
								qty = rest.get(di.number);
							
							qty = qty + di.qty;
							
							rest.put(di.number, qty);
						}
	...
	private UpdateQtyHandler updater = new UpdateQtyHandler() {
		...
		public void itemUpdated(OrderItem item, Order order, boolean isNewItem) {
			...
			rie.comment = edComment.getText().toString().trim();
...
//	}
	...
	class DlvAdapter extends BaseAdapter {
		...
		public View getView(int pos, View view, ViewGroup arg2) {
			...
			int res = rest.containsKey(dd.doc.number) ? rest.get(dd.doc.number) : 0;
			
			if (val != null)
				res += val;
			
			tv = (TextView)view.findViewById(R.id.tvMaxQty);
			tv.setText( String.format("%s / %s", Util.IntToScaleStr(dd.item.qty, Consts.QTY_SCALE), Util.IntToScaleStr(dd.item.qty - res, Consts.QTY_SCALE)));
		...
		public void onClick(View v) {
			InputNumberDlg.open(v.getContext(), new InputNumber() {
				...
				public void applayInput(int value, Object... params) {
					int res = rest.containsKey(doc.number) ? rest.get(doc.number) : 0;
					
					if( value + res > doc.qty) {
						Toast.makeText(ReturnPriceCount.this, R.string.out_of_limit, Toast.LENGTH_SHORT).show();
						return;
					}
	...
	@Override
	protected boolean isInputValid(Runnable r) {
		return spReturnCause.getSelectedItem().toString().trim().length() > 0;
	}
...
class DlvData {
	...
	public DlvData(Delivery doc, DeliveryItem item) {
		this.doc = new DlvKeySum(doc, item.qty, (int)((long)item.sum * Consts.QTY_SCALE / item.qty));
		this.item = item;
	}
}
