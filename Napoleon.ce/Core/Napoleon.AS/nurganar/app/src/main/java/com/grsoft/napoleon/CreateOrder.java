...
//	DateHandler dateHandler;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		spTareType = findViewById(R.id.spTareType);
	...
	private void init() {
		...
		((CheckBox)findViewById(R.id.cbSchFakt)).setChecked(o.schetFact != 0);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		spTareType.setSelection(o.tareType, true);

		spTareType.setEnabled(o.filter == 0);
		//DialogHelper.loadSpinnerFromDataObject(spDogovor, Dogovor.class, object -> object.id.equals(o.dogovor), true, String.format("ido='%s'", org.id));
	}
	...
	private void initOrder(Order o, Org org) {
		...
		if (((OrderEx)o).filter == 0)
			((OrderEx)o).tareType = 1;
...
//			if (spDogovor.getSelectedItemPosition() == 0 ) {
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.schetFact = ((CheckBox)findViewById(R.id.cbSchFakt)).isChecked() ? 1 : 0;
			...
			int tt = spTareType.getSelectedItemPosition();

			if (editMode && tt != o.tareType)
				order.deleteItems();

			o.tareType = tt;

			Dogovor d = (Dogovor)spDogovor.getSelectedItem();
			if (d != null)
				o.dogovor = d.id;
