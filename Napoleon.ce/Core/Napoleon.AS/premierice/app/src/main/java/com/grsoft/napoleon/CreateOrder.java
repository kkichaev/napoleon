...
//	DateHandler dateHandler;
	...
	void setInfo(OrgEx org) {
		String text = "";
		text = "Сумма кредита: " + Util.IntToScaleStr(org.creditLimit, Consts.SUM_SCALE);
		((TextView)findViewById(R.id.tvCredit)).setText(text);

		text = "Отсрочка: " + Util.IntToScaleStr(org.delay, 1);
		((TextView)findViewById(R.id.tvDelayInfo)).setText(text);

		text = "Отгр. в отсрочку: " + Util.IntToScaleStr(org.deliveryDelay, 1);
		((TextView)findViewById(R.id.tvDlvDelay)).setText(text);

		((CheckBox)findViewById(R.id.cbNalBuhF)).setChecked((org.workCondition & OrgEx.WRK_CND_NAL_BUH_FACT) != 0);
		((CheckBox)findViewById(R.id.cbNalBuhD)).setChecked((org.workCondition & OrgEx.WRK_CND_NAL_BUH_DELAY) != 0);
		((CheckBox)findViewById(R.id.cbNalTorgF)).setChecked((org.workCondition & OrgEx.WRK_CND_NAL_TORG_FACT) != 0);
		((CheckBox)findViewById(R.id.cbNalTorgD)).setChecked((org.workCondition & OrgEx.WRK_CND_NAL_TORG_DELAY) != 0);
		((CheckBox)findViewById(R.id.cbBeznal)).setChecked((org.workCondition & OrgEx.WRK_CND_BEZNAL_BUH_DELAY) != 0);
		((CheckBox)findViewById(R.id.cbBonusSeparate)).setChecked((org.workCondition & OrgEx.WRK_CND_BONUS_DOC) != 0);
	}
	...
	private void init() {
		...
		CheckBox cbBonus = findViewById(R.id.cbBonus);
		cbBonus.setChecked(o.bonus != 0);
		cbBonus.setEnabled((org.workCondition & OrgEx.WRK_CND_BONUS_DOC) != 0 && o.items.size() == 0);
		...
		spWh.setEnabled(o.items.size() == 0);
		...
		cb.setEnabled(org.canChangeNal());
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	private void initOrder(OrderEx o, OrgEx org) {
		Features.COST_MANAGER.getCostIndex(org.id);
