...
//	DateHandler dateHandler;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		spDlvGroup = findViewById(R.id.spDlgGroup);
		edAction = findViewById(R.id.edAction);
		init();
	}
	...
	private void init() {
		...
		if (Features.WH_QTY) {
			...
			spSklads.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
				...
				public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
					...
					List<DlvGroup> list = DbReader.fetch(DlvGroup.class, String.format("whCode='%s'", kv.key));

					int selIdx = -1;
					for(int i = 0; i< list.size(); i++){
						if (list.get(i).id.equals(o.dlgGroupID)) {
							selIdx = i;
							break;
						}
					}

					ArrayAdapter<DlvGroup> aa = new ArrayAdapter<>(CreateOrder.this, R.layout.simple_spinner_layout, list);
					aa.setDropDownViewResource(R.layout.simple_spinner_layout_drop_down);
					spDlvGroup.setAdapter(aa);

					if (selIdx > 0)
						spDlvGroup.setSelection(selIdx);
				}
		...
		List<CharSequence> pt = new ArrayList<CharSequence>();
		pt.add("");
		DialogHelper.loadSpinnerFromConfig(config, "ТипОплаты", pt, spPayType, o.payType);
		spPayType.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {

			@Override
			public void onItemSelected(AdapterView<?> arg0, View arg1, int arg2, long arg3) {
				if(order.isEditable()) {
					findViewById(R.id.btnOK).setEnabled(arg2 > 0);
				}
			}
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		edAction.setOnClickListener((v)->ActionView.openActionView(CreateOrder.this));
		edAction.setOnLongClickListener((v)->{
			edAction.setText("");
			o.actionID = "";
			return  true;
		});
		...
		action.read("id", o.actionID);

		edAction.setText(action.getData().name);
		refreshDate();
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		if( data != null && requestCode == DIALOG_DATE_PICKER_ID ) {
			...
			refreshDate();
		}else if (data != null && requestCode == ActionView.ACTION_VIEW_REQUEST){
			ActionImpl action = new ActionImpl();
			action.read("id", data.getStringExtra(ActionView.ACTION_ID));

			edAction.setText(action.getData().name);

			((OrderEx)order.getData()).actionID = action.getData().id;
		}
	}
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			DlvGroup selDG = (DlvGroup) spDlvGroup.getSelectedItem();

			if (selDG != null)
				o.dlgGroupID = selDG.id;

			CheckBox pickup = (CheckBox)findViewById(R.id.cbPickup);
			o.pickup = pickup.isChecked() ? 1 : 0;
