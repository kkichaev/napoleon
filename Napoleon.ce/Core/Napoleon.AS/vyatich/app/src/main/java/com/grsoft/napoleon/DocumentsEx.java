...
public class DocumentsEx extends Documents {
	...
	protected void createNewDoc() {
		PriceChecker.ActualState state = PriceChecker.getActual(this);
		if(state == PriceChecker.ActualState.NotActual) {
			Toast.makeText(this, "Прайс не актуальный, обновите прайс!", Toast.LENGTH_LONG).show();
			return;
		}
		if(state == PriceChecker.ActualState.Alert) {
			Toast.makeText(this, "Необходимо обновить цены на продукцию", Toast.LENGTH_LONG).show();
		}
		super.createNewDoc();
	}
