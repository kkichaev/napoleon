package com.serviko.view.numberpicker;
...
public class NumberPicker extends LinearLayout {
    ...
    public interface EventsHandler {
        int nextValue(int current);
        int prevValue(int current);

        String getText(int current);
        void onValueChanged(NumberPicker sender, int current);
    }
...
//    private NumberPickerListener mListener;
    ...
    public void setEventsHandler(EventsHandler h) { this.handler = h; }
    ...
    public void setValue(int value) {
        if(handler != null) {
            mValue = value;
            handler.onValueChanged(this, mValue);
        } else {
            if (value > mMaxValue) {
                value = mMaxValue;
            }
...
//        }
    ...
    public int getValueEms() { return valueEms; }
    public void setValueEms(int newValueEms) {
        valueEms = newValueEms;
        mValueTextView.setEms(valueEms);
    }
...
//        }
        ...
        mMinValue = typedArray.getInt(R.styleable.NumberPicker_snp_minValue,
            res.getInteger(R.integer.default_minValue));
        mMaxValue = typedArray.getInt(R.styleable.NumberPicker_snp_maxValue,
            res.getInteger(R.integer.default_maxValue));

        mStepSize = typedArray.getInt(R.styleable.NumberPicker_snp_stepSize,
            res.getInteger(R.integer.default_stepSize));

        mUpdateIntervalMillis = typedArray.getInt(R.styleable.NumberPicker_snp_updateInterval,
            res.getInteger(R.integer.default_updateInterval));

        mOrientation = typedArray.getInt(R.styleable.NumberPicker_snp_orientation,
            LinearLayout.HORIZONTAL);

        mValue = typedArray.getInt(R.styleable.NumberPicker_snp_value,
            res.getInteger(R.integer.default_value));

        mValueTextSize = typedArray.getDimension(R.styleable.NumberPicker_snp_value_text_size,
            INVALID_RES);
        mValueTextColor = typedArray.getColor(R.styleable.NumberPicker_snp_value_text_color,
            0);
        mValueTextAppearanceResId = typedArray.getResourceId(R.styleable.NumberPicker_snp_value_text_appearance, INVALID_RES);

        mScrollEnabled = typedArray.getBoolean(R.styleable.NumberPicker_snp_scrollEnabled,
            res.getBoolean(R.bool.default_scrollEnabled));

        mButtonColorStateList = ContextCompat.getColorStateList(context, typedArray.getResourceId(R.styleable.NumberPicker_snp_buttonBackgroundTintSelector, R.color.btn_tint_selector));
        ...
        valueEms = typedArray.getInt(R.styleable.NumberPicker_snp_value_ems, res.getInteger(R.integer.default_ems));
    ...
    private void tintButton(@NonNull ImageView button, ColorStateList colorStateList) {
        ImageViewCompat.setImageTintList(button, colorStateList);
    }
    ...
    private void increment() {
        if(handler != null) {
            int nv = handler.nextValue(mValue);
            if(nv != mValue)
                setValue(nv);
    ...
    private void decrement() {
        if(handler != null) {
            int nv = handler.prevValue(mValue);
            if(nv != mValue)
                setValue(nv);
