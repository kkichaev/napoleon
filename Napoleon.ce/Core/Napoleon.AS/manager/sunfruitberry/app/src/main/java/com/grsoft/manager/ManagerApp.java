...
import com.grsoft.manager.documents.AgentMemoDoc;
...
public class ManagerApp extends ManagerApplicationBase {
	...
	protected void init() {
		...
		DocTypeBase.addType(AgentMemoDoc.instance());
		...
		SyncDetail.handler = new SyncDetail.Events() {
			@Override
			public void startingSync(List<Hitching> ret, String userid, Date date) {
				ret.add(0, new PicsSrcHitching(userid, date));
			}
		};
	}
}
