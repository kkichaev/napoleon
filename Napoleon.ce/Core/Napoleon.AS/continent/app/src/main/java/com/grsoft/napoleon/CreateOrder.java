...
public class CreateOrder extends BaseActivity
{
	...
	private ArrayList<CharSequence> shipping = new ArrayList<CharSequence>();
...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		findViewById(R.id.trCost).setVisibility(View.GONE);
		...
		CheckBox cb;
		cb = (CheckBox)findViewById(R.id.cbCashToDriver);
		cb.setChecked(o.cashToDriver != 0);
		...
		DialogHelper.loadSpinnerFromConfig(config, "ВидДоставки", shipping, spShipping, o.shipping);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
	...
	private void initOrder(Order o, Org org) {
		int id = Features.COST_MANAGER.getCostIndex(org.id);
		if(id >= 0)
			o.sumType = id;
		else {
			o.sumType = org.costype;
			((OrderEx)o).priceCost = 1;
		}

		o.delay = ((OrgEx)org).delay;
	}
...
//				o.sumType = costType;
			...
			if( cb.isChecked() ) o.params |= ParamState.ofCash;
			else o.params &= (~ParamState.ofCash);
			
			cb = (CheckBox)findViewById(R.id.cbCashToDriver);
			o.cashToDriver = ( cb.isChecked() ) ? 1 : 0;
