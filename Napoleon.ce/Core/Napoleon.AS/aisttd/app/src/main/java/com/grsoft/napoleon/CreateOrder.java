...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		config.getData().key = "Организация";
		config.read();
		DialogHelper.makeList(config.getData().value, firms);
		List<FirmOrd> vals = new ArrayList<FirmOrd>();
		int sel = -1;
		for(String fidx: org.firms.split(",")) {
			try {
				int idx = Integer.parseInt(fidx.trim());
				if(firms.size() > idx) {
					if(idx == o.supplyer) {
						sel = vals.size();
					}
					String fname = firms.get(idx).toString();
					vals.add(new FirmOrd(fname, idx));
				}
			} catch (Exception e) {
			}
		}
		ArrayAdapter<FirmOrd> fa = new ArrayAdapter<FirmOrd>(this, R.layout.simple_spinner_layout, vals);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			Spinner spFirma = (Spinner) findViewById(R.id.spFirma);
			FirmOrd selfirm = (FirmOrd) spFirma.getSelectedItem();
			Spinner spPrices = (Spinner) findViewById(R.id.spPrices);
			int costType = spPrices.getSelectedItemPosition();

			if( selfirm != null )
				o.supplyer = selfirm.getIndex();
			if( costType >= 0 )
				o.sumType = costType;
...
class FirmOrd {
	...
	public FirmOrd(String name, int index) {
		this.name = name;
		this.index = index;
	}
