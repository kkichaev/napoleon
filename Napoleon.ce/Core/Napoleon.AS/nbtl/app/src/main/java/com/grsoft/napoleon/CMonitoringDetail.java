...
public class CMonitoringDetail extends BaseActivity implements DataSetNotify{
	public static Class<?> activity = CMonitoringDetail.class;
	...
	private CMonitoringImpl document = new CMonitoringImpl();
	...
	private void updateTotalSum(){ updateTotalSum(document.sum(), 0); }
	...
	private void intiView() {
		...
		if( Features.CANT_SEND_SCRIPT_PART ) {
			if(ScriptImpl.containsDocument( DocType.getCurDoc().getObjectName(), document.getData().created, document.getId()) != null)
	...
	private OnItemClickListener onItemClick() {
		return new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				CMonitoringItem item = (CMonitoringItem) parent.getItemAtPosition(position);
	...
	protected void send() {
		new DocumentSender(this, btnSend, CMonitoringDoc.instance().getObjectName(), document, document.getRowid()).execute((Void[])null);
	}
	...
	private int getContentViewId() { return R.layout.cmonitoringdetail; }
	...
	class Adapter extends BaseAdapter{
		...
		public View getView(int position, View view, ViewGroup parent) {
			if(view == null)
				view = View.inflate(CMonitoringDetail.this, R.layout.cmonitoringrow, null);
		
			CMonitoringItem item = (CMonitoringItem) getItem(position);
			if(item != null){
				...
				tv.setText(Util.IntToScaleStr(item.cost, Consts.SUM_SCALE));
			}
