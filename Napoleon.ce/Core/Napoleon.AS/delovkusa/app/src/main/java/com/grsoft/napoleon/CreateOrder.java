...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		List<OrgDogovor> dogs = new ArrayList<>();
		int selDog = -1;
		for(OrgDogovor od : org.dogovors) {
			if(o.dogovor.equals(od.id))
				selDog = dogs.size();
			dogs.add(od);
		}
		if(selDog < 0 && dogs.size() > 0)
			selDog = 0;

		Spinner spDog = findViewById(R.id.spDogovor);
		ArrayAdapter<OrgDogovor> aa = new ArrayAdapter<>(this, R.layout.simple_spinner_layout, dogs);
		aa.setDropDownViewResource(R.layout.simple_spinner_layout_drop_down);
		spDog.setAdapter(aa);
		if( selDog >= 0 && selDog < dogs.size())
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			OrgDogovor sel = (OrgDogovor) ((Spinner)findViewById(R.id.spDogovor)).getSelectedItem();
			if(sel != null) {
				o.dogovor = sel.id;
				o.firmCode = sel.firm;
				o.sumType = sel.costype;

				List<KeyValue> values = new ArrayList<>();
				ConfigImpl ci = new ConfigImpl();
				ci.read("key", "Склады");
				int shi = DialogHelper.makeListWithKey(ci.getData().value, values, sel.sklad);
				if(shi < 0)
					shi = 0;
				o.whCode = values.get(0).key.toString();
				o.whIndex = shi;

				ci.read("key", "ВидЦены");
				DialogHelper.makeListWithKey(ci.getData().value, values, "");
				o.prcType = values.get(sel.costype).key.toString();
			}
