...
public class SalesItemEx extends SalesItem {
	...
	@FieldVersion(version=3)
	@FieldOrder(order=USER_FIELDS+2)
	public List<BCItem> barcodes = new ArrayList<>();

	public boolean isScanned() { return barcodes.size() == qty / Consts.QTY_SCALE; }

	public void rejectMissing(SalesImplEx owner) {
		int newQty = barcodes.size() * Consts.QTY_SCALE;
		PriceImpl pi = new PriceImpl();
		if(pi.read("id", id)) {
			owner.updateQty(pi, newQty, cost, false);
		}
		pi.close();
	}
