...
public class TextPrinter extends GraphicPrinter{
	...
	public File print(Context context, String repName, List<DataSource> source) {
		...
		String[] reports = repName.split(FORM_DELIM_SIM);
		StringBuilder outStr = new StringBuilder();

		for(String r : reports) {
			if(NPrinter.forms.containsKey(r)){
				String form = getForm(context, r);
				StringBuilder output = makeContent(context, source.get(0), form);
				outStr.append(output);

				DataSource s = source.get(0);
				s.init(context, 0);
				s.startPage();
			}
		}

		result = savePrintResult(context, outStr);
		return result;
	}
