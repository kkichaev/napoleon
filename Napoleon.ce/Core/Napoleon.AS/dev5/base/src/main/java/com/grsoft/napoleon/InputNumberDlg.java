...
public class InputNumberDlg {
	...
	static String getDiscountText(long priceCost, long newCost) {
		String dscText = "";
		
		long dsc = 100 * Consts.SUM_SCALE - (int)(((float)newCost/(float)priceCost) * Consts.SUM_SCALE * 100 );
		...
		if(inputNumber.priceCost != 0) {
			...
			edCount.addTextChangedListener(new TextWatcher() {
				...
				public void onTextChanged(CharSequence arg0, int arg1, int arg2, int arg3) {
					int newCost = (int)Util.StrToScale(arg0.toString(), scale);
...
//		nh.makeNumericKeypad(panel);
	...
	protected static void applyInput(final InputNumber inputNumber, final View panel, final AlertDialog dialog) {
		...
		try{
			...
			int value = (int)Util.StrToScale(sval, scale);
