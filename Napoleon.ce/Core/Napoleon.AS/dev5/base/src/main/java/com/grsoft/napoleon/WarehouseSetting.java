 ...
 *
 */
public class WarehouseSetting extends SettingActivity {
	protected CfgNpl config;
	...
	protected void init() {
		...
		if( cb != null ) {
			if(Features.QTY_IN_PACK_IN_DOCS)
		...
		if(cbPriceImage != null){
			...
			cbPriceImage.setChecked(config.showImageInPriceCount);
		}

		if(spPriceImagePos != null)
			spPriceImagePos.setSelection(config.imagePosInPriceCount, true);

		CheckBox pm = (CheckBox)findViewById(R.id.cbPriceMover);

		if(pm != null)
			pm.setChecked(config.usePriceMover);

		if(cbShowDailySales != null)
			cbShowDailySales.setChecked(config.showDailySales);

		spLinesCount = (Spinner) findViewById(R.id.spLinesCount);

		if (config.linesCount > 0 && config.linesCount <= spLinesCount.getCount())
			spLinesCount.setSelection(config.linesCount, true);
			spLinesCount.setSelection(config.linesCount, true);

		edQty = findViewById(R.id.edQty);

		if (edQty != null)
			edQty.setText(Util.IntToScaleStr(config.priceQty, Consts.QTY_SCALE));
	}
	...
	public void save() {
		...
		cb = (CheckBox) findViewById(R.id.cbPackView);
		if( cb != null ) {
			config.isPackView = Features.QTY_IN_PACK_IN_DOCS ? cb.isChecked() : false;
		}
		...
		if(cbPriceImage != null)
			config.showImageInPriceCount = cbPriceImage.isChecked();

		if(spPriceImagePos != null)
			config.imagePosInPriceCount = spPriceImagePos.getSelectedItemPosition();

		if(cbShowDailySales != null)
			config.showDailySales = cbShowDailySales.isChecked();

		CheckBox pm = (CheckBox)findViewById(R.id.cbPriceMover);

		if(pm != null)
			config.usePriceMover = pm.isChecked();

		config.linesCount = spLinesCount.getSelectedItemPosition();

		if (edQty != null)
			config.priceQty = (int)Util.StrToScale(edQty.getText().toString(), Consts.QTY_SCALE);
