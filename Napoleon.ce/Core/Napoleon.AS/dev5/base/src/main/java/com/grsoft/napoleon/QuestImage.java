...
public class QuestImage extends QuestControl implements OnClickListener, OnLongClickListener {
	...
	public void onClick(View v) {
		if (getOwner() instanceof QuestPhoto)
			((QuestPhoto)getOwner()).doPhoto(this);
	}
	...
	protected void initPreview(Context context) {
		...
		for(int i = 0; i < list.size(); i++){
			...
			if (picStore.read("id", id)) {
				String p = new String(picStore.getData().getImageFileName());
				TextView t = new TextView(context);
				t.setCompoundDrawablesWithIntrinsicBounds(BitmapUtils.createBitmap(context, p, w, h), null, null, null);
				//t.setCompoundDrawables(BitmapUtils.createBitmap(context, p, w, h), null, null, null);
	...
	public boolean onLongClick(View v) {
		if (getOwner() instanceof QuestPhoto)
			((QuestPhoto)getOwner()).longClick(this, v.getTag().toString());
