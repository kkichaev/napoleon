...
//			return (int)((long)item.qty * Consts.QTY_SCALE / item.qtyInPack);
	...
	public static Map<String,Integer> getRemnants(String orgID){
		Map<String, Integer> res = new HashMap<>();

		for (DataObject dataObject :DbReader.fetch(DbObject.getDataType(Remnants.class), String.format("id='%s'", orgID)))
			for(RemnantItem item : ((Remnants) dataObject).items)
				res.put(item.id, item.qty);
