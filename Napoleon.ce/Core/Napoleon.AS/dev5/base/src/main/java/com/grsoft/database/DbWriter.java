...
public class DbWriter
{
	...
	private int bindStatement(SQLiteStatement statmenet, DataObject dataObject) throws RuntimeException
	{
		...
		{			
			for(Field field : DataObjectInfo.getInstance().getFields(dataObject.getClass()))
			{
				...
				{
					byte[] res = listToBlob((List<?>)field.get(dataObject), field, dataObject.getClass());
					...
					if(o != null)
						statmenet.bindBlob(idField, (byte[])o);
				}
				else if(fieldType == int[].class) {
					int[] o = (int[]) field.get(dataObject);
					if(o != null && o.length > 0) {
						ByteBuffer bf = ByteBuffer.allocate(o.length * Integer.BYTES);
						IntBuffer ibf = bf.asIntBuffer();
						ibf.put(o);
						statmenet.bindBlob(idField, bf.array());
					}
				}
				else 
	...
	private byte[] listToBlob(List<?> data, Field field, Class<? extends  DataObject> memberClass) throws Exception {
		
		final int ARRAY_SIZE = 128;
		ByteArrayOutputStream array = new ByteArrayOutputStream(ARRAY_SIZE);
		
		int version = 0;
		boolean versionInited = false;
		if (data != null) {
			Class<? extends DataObject> elClass = DataObjectInfo.getInstance().getListType(memberClass, field.getName());
			int count = data.size();
			...
			for (Object item: data) {
				for(Field ifield :  DataObjectInfo.getInstance().getFields(elClass)) {
					if(versionInited == false) {
						FieldVersion fc = ifield.getAnnotation(FieldVersion.class);
						if(fc != null) {
							int cv = fc.version();
							if(cv > version)
								version = cv;
					...
					if(fldType == int.class)
						...
						array.write(Util.longToBytes(ifield.getLong(item)));
					else if (fldType == String.class)
					{
						Object val =  ifield.get(item);
						if( val != null ) {
							String sval = val.toString().replaceAll("\0", "");
							array.write(UnicodUtils.toBytes(sval));
						}
					...
					else if (fldType == Date.class)
					{
						...
						byte[] fld = listToBlob((List<?>)ifield.get(item), ifield, elClass);
						array.write(fld);
					}
		...
		if(version != 0) {
			byte[] vbytes = Util.intToBytes(version);
			for(int i=0; i<4; i++) {
				res[i + 4 + DataObjectUtils.counterMarker.length] = vbytes[i];
			}
		}
		return res;
	}
...
//	}
	...
	static public void eraseTable(Class<? extends DataObject> dataType) {
		String tableName = DataObjectInfo.getInstance().getTableName(dataType);
		if( tableName != null ) {
			if (isTableExists(tableName)) {
				SQLiteDatabase dataBase = DataBaseManager.getDataBase();
