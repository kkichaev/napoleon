...
public abstract class MemberFormat 
{
	...
	public static MemberFormat createFormat(Field field, Class<? extends DataObject> dataType, UploadContext context) {
		Class<?> type = field.getType();
		String name = field.getName();

		if(field.getAnnotation(UploadSource.class) != null) {
			return null;
		}
		...
		if (type == List.class) {
			ListFormat result = new ListFormat(field.getName(), context);
...
//				((ParameterizedType) field.getGenericType()).getActualTypeArguments()[0];
			...
			for (Field listField: DataObjectInfo.getInstance().getFields(paramType)) {
				MemberFormat mf = createFormat(listField, paramType, context);
				if( mf != null) {
					result.add(mf);
				}
			}
