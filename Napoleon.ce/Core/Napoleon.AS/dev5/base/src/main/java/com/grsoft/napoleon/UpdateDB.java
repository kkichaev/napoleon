...
public class UpdateDB extends Activity implements ProgressHelper.ButtonAction{
	public static boolean CAN_EXPORT = true;
	public static Class<? extends Activity> activity = UpdateDB.class;
	...
	protected void initilizeUIComponent() {
		View button = findViewById(R.id.btnUpdate);
		...
		CheckBox cbPres = findViewById(R.id.cbPresent);
		if (Features.PRESENTATION_ON_SDCARD)
			cbPres.setVisibility(View.GONE);
		else {
			cbPres.setOnCheckedChangeListener((buttonView, isChecked) -> {
				if(isChecked) {
					CfgNpl c = (CfgNpl) ConfigManager.getConfig();
					if(c.loadPresentationByWiFi && ((CheckBox) findViewById(R.id.cbPresent)).isChecked()) {
						if(!isWiFiConnected(UpdateDB.this)) {
							cbPres.setChecked(false);
							MessageBox.show(UpdateDB.this, getString(R.string.warning),
									getString(R.string.need_on_wifi), true);
							return;
						}
					}
				}
		...
		boolean canRestore = canRestoreDocuments();
		CheckBox cb = findViewById(R.id.cbRecreateStory);
		cb.setEnabled(canRestore);
		if(!canRestore)
			cb.setChecked(false);
	...
	protected List<Hitching> getDebetHitching() {
		...
		debtHitchings.add(getDeliveryHitching());
		debtHitchings.add(new RcvNewHitching(Balance.class));
	...
	protected Hitching getPriceHitching() {
...
//			loadFullPrice = true;
	...
	protected List<Hitching> getGenDataHitchings() throws RuntimeException {
		...
		result.add(getServerInfoHitching());
		...
		if (Features.ORG_TASK)
			result.add(getOrgTaskHitching());

		// must be in this order
		result.add(new PriceTypeHitching());
		result.add(new PriceCostHitching());

		result.add(new ScheduleHitching());

		// must be in this order
		result.add(new StoreHitching());
		result.add(new StoreQtyHitching());

		result.add(new RcvNewHitching(Firm.class));
	...
	public Hitching getOrgTaskHitching() {
		return new Hitching(OrgTask.class, "OrgTask");
	}
	...
	protected String getSyncFinishMessage(int traffic) {
		return getString(R.string.sync_end_traffic) + Integer.toString((traffic + 512) / 1024) + " " + getString(R.string.kB);
	}
	...
	protected RestoreDocProceeded getRestoreDocProceeded() { return new RestoreDocProceeded(); }
	...
	protected List<Hitching> getRestoreHitching() {
		...
		result.add(getRestoreDocProceeded());
	...
	protected UserInfo getRcvUserInfo() {
		...
		return new LoginData(config.login, config.passw, config.impersonate, UpdateDB.this
	...
	class UpdateProcess extends NetworkAsyncTask {
		...
		protected UpdateProcess(ProgressHelper progressHelper, Activity context) {
			super(progressHelper);
			activity = context;
		}
		...
		String exportDocs(UserInfo ui, boolean exportVisits) {
			List<DocExportListener> exportedDocs;
			exportedDocs = getExportedDocs(true, exportVisits);
...
//				if( !vsh.send(UpdateDB.this, ui, docs, this) ) {
			...
			try {
				...
				if(CAN_EXPORT) {
					...
					if (!isCancelled()
						...
						errMessage = exportDocs(sndUserInfo, exportVisit);
					}
...
//                }
		...
		class EnableButton implements Runnable {
			...
			public EnableButton(View b, boolean e) {
				button = b;
				enabled = e;
			}
		...
		protected void onPostExecute(Boolean result) {
			...
			NetworkBroadcasts.sendSyncResult(UpdateDB.this, result);
			postSync(result);
		}
	...
	protected boolean receiveGenData() {
		return ((CheckBox) findViewById(R.id.cbGenData)).isChecked();
	}
	...
	protected List<Hitching> getPrezentHitching() {
		List<Hitching> result = new ArrayList<Hitching>();
		result.add(new PricePhotoHitching(this));
		return result;
	}
