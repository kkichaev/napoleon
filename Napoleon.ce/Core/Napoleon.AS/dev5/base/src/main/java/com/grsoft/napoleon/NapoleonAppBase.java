...
import com.grsoft.napmobile.base.MainExceptionHandler;
...
public class NapoleonAppBase extends Application {
	...
	public void onCreate() {
		...
		//
		// disable pack view
		// Features.QTY_IN_PACK_IN_DOCS
		CfgNpl config = createConfig();
		config.isPackView = false;
...
//		if (!Features.IS_MARKET_VERSION)
		...
		Main.Handler = new Main.Events() {
			@Override public void onStart(Activity sender) { startMainService(); }
			@Override public void setDefaultDocType(Activity sender) { setDefDocType(); }
			@Override public void onExit(Activity sender) { exit(); }
		};
		...
		DataBaseManager.dontDropTableNames.add(new SyncInfo().getTableName());
	}
...
//		Features.REPORT_REQUEST = true;
		...
		Features.ANDROID_CAMERA_PROHIBIT = true;
	...
	protected void initActivity() {
		...
		UpdateDB.activity = UpdateDB.class;
	...
	protected void initDocTypes(){
		...
		DebtDoc.init(getApplicationContext());
	...
	public static void initDemo(Context context) {
...
//		Features.CAN_CHANGE_COST = true;
		...
		if(!inited){
			createDemoDb(context);
			preferences.edit().putBoolean(INITED_KEY, true).commit();
		}
	}

	static void createDemoDb(Context context) {
