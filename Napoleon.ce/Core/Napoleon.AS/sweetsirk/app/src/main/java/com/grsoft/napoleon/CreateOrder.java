...
//	DateHandler dateHandler;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		edSumZabor = (EditText) findViewById(R.id.edSumZabor);
...
//		dateHandler = new DateHandler((TextView)findViewById(R.id.tvDate), o.date, DIALOG_DATE_PICKER_ID);
		...
		((TextView)findViewById(R.id.tvDiscount)).setText(getString(R.string.disc_value,
				Util.IntToScaleStr(((OrderEx)o).disc, Consts.SUM_SCALE)));
		
		Spinner spWh = (Spinner) findViewById(R.id.spWh);
		DialogHelper.loadSpinnerWithKey(config, "Склады", new ArrayList<KeyValue>(), spWh, ((OrderEx)o).whCode);

		
		if(editMode) {
			edTask.setText(((OrderEx)o).task);
			edSumZabor.setText(Util.IntToScaleStr(((OrderEx)o).zsum, Consts.SUM_SCALE));
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.task = edTask.getText().toString().trim();
			o.zsum = (int) Util.StrToScale(edSumZabor.getText().toString().trim(), Consts.SUM_SCALE);
