    ...
    public class BCAnalyzer implements ImageAnalysis.Analyzer {
        ...
        void onRead(List<Barcode> barcodes, long elapsedMs){
            ...
            for(Barcode b : barcodes) {
                ...
                if(barcodeHandler.onReadBarcode(bcValue, b.getFormat(), elapsedMs)) {
                    finish = true;
                    owner.finish();
                }
            }
...
//                Bitmap b = BitmapUtils.getBitmap(imageProxy);
                ...
                Bitmap b = Bitmap.createBitmap(src, cx - sw / 2, cy - sh / 2, sw, sh);

                InputImage ii = InputImage.fromBitmap(b, rotDeg);
