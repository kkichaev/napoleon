...
public class MainEx extends Main {
	...
	public DriverRouteActions prevActionPoint;
...
//			}
//		});
		h.add(new DocumentRestore((DocType)DShipmentDoc.instance(), "DShipment", months){
			@Override
			protected void beforeWrite(DataObject dobj) {
				super.beforeWrite(dobj);
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		if(item.getItemId() == R.id.itStartDoing) {
			...
			if(dimpl.readFromId(ritem.item.itemid)) {
				if(dimpl.isEditable() == false || dimpl.isDocFinished(this) == DispatchItem.DOC_COMPLETE)
					canStart = false;
			}
			...
			if(canStart) {
				...
				if(isDocsCompleete) {
					...
					if(!prevActionPoint.routeItemId.equals(ritem.item.itemid)) {
						...
						if( !prevActionPoint.isEmpty() && dimpl.readFromId(prevActionPoint.routeItemId) &&
								!(dimpl.isDocFinished(this) == DispatchItem.DOC_COMPLETE)) {
	...
	public static class AskAbandRoute extends DialogFragment implements DialogInterface.OnClickListener {
		...
		public void onClick(DialogInterface arg0, int arg1) {
			if(arg1 == Dialog.BUTTON_POSITIVE) {
				if(((MainEx)getActivity()).prevActionPoint != null && !((MainEx)getActivity()).prevActionPoint.isEmpty()) {
					DriverRouteActions.setStatus(((MainEx)getActivity()).prevActionPoint.routeItemId, "", DriverRouteActions.STAUS_CANCEL, "");
				}
				((MainEx)getActivity()).markNewPoint();
			} else if(arg1 == Dialog.BUTTON_NEGATIVE) {
				((MainEx)getActivity()).newActivePoint = null;
			}
		}
		
	}
	
	public static class SetDriverStatus extends DialogFragment implements DialogInterface.OnClickListener {
		...
		public void onClick(DialogInterface arg0, int arg1) {
			...
			((MainEx)getActivity()).updateStatusText(items[arg1]);
			((MainEx)getActivity()).sendStatus();
			dismiss();
		}		
	}
}
