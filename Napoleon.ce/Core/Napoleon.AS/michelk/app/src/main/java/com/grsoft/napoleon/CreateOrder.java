...
//	DateHandler dateHandler;
	...
	private void init() {
		...
		DialogHelper.loadSpinnerFromConfig(config, "Организация", firms, spFirma, o.supplyer, true);
		...
		DialogHelper.loadSpinnerWithKeyW(config, "Склады", new ArrayList<KeyValue>(), spWh, o.whCode, true );
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			if(suppl == 0) {
				Toast.makeText(CreateOrder.this, "Выберите организацию", Toast.LENGTH_LONG).show();
				return;
			}
			o.supplyer = suppl - 1;

			if (BuildConfig.FLAVOR.equals("nowh") ) {
				o.whCode = "";
			} else {
				KeyValue selWh = (KeyValue) ((Spinner) findViewById(R.id.spSklad)).getSelectedItem();
				if (selWh == null || selWh.key.length() == 0) {
					Toast.makeText(CreateOrder.this, "Выберите склад", Toast.LENGTH_LONG).show();
					return;
				}
				o.whCode = selWh.key.toString();
			}
			Spinner spPrices = (Spinner) findViewById(R.id.spPrices);
			int costType = spPrices.getSelectedItemPosition();
