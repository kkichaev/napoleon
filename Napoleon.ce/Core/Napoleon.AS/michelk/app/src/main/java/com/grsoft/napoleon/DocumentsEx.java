...
public class DocumentsEx extends Documents {
	...
	protected void createNewDoc() {
		...
		if(!hideDebtWarningMessage &&
				dt.equals(OrderDoc.instance()) && ((OrgEx)org.getData()).balance > 0)
	...
	protected Dialog onCreateDialog(int id) {
		if(id == ORG_DEBT_WARNING_DLG)
			return createOrgDebtWarningDlg();
		else
	...
	private Dialog createOrgDebtWarningDlg() {
		...
		builder.setMessage(getString(R.string.org_debt_warning, 
				Util.IntToScaleStr(((OrgEx)org.getData()).balance, Consts.SUM_SCALE)));
