...
public class CreateOrder extends BaseActivity
{
	...
	int selIndex = -1;
	void loadSpinner(Class<? extends IDNameBase> obj, Spinner sp, final String code) {
		
		selIndex = -1;
		final List<KeyValue> values = new ArrayList<KeyValue>();
		DataTraveler.travel(obj, new DataTraveler.Travel<IDNameBase>() {

			@Override
			public boolean travel(DataTraveler<IDNameBase> item) {
				KeyValue kv = new KeyValue(item.data.id, item.data.name);
				if(item.data.id.equals(code))
					selIndex = values.size();
				values.add(kv);
				return true;
			}
		}, "", "name");
	...
	private void init() {
		...
		loadSpinner(PriceTypes.class, spPrices, o.priceType);
...
//		DialogHelper.loadSpinnerWithKey(config, "ВидЦены", values, spPrices, o.priceType);
		...
		loadSpinner(Sklads.class, spSklad, o.whCode);
//		DialogHelper.loadSpinnerWithKey(config, "Склады", new ArrayList<KeyValue>(), spSklad, o.whCode);
		...
		((CheckBox)findViewById(R.id.cbDocPacket)).setChecked(o.docPacket > 0);
	...
	class OKClickListener extends OnClickListenerToNotify {
		...
		private void okDone(boolean updateSumType) {
			...
			o.docPacket = ((CheckBox)findViewById(R.id.cbDocPacket)).isChecked() ? 1 : 0;
