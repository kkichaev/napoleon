	...
	private void init() {
		...
		edRoute.setOnLongClickListener(new OnLongClickListener() {
			...
			public boolean onLongClick(View v) {
				RouteDlg dlg = new RouteDlg(ProcurationEdit.this);
		...
		edOrg.setOnLongClickListener(new OnLongClickListener() {
			@Override
			public boolean onLongClick(View v) {
				SelOrgDlg dlg = new SelOrgDlg(ProcurationEdit.this);
				dlg.show(getSupportFragmentManager(), dlg.getClass().toString());
				return true;
			}
		});
		...
		edRemark.setOnLongClickListener(new OnLongClickListener() {
			@Override
			public boolean onLongClick(View v) {
				RemarkDlg dlg = new RemarkDlg(ProcurationEdit.this);
				dlg.show(getSupportFragmentManager(), dlg.getClass().toString());
				return true;
			}
		});
		...
		edFio.setOnLongClickListener(new OnLongClickListener() {
			@Override
			public boolean onLongClick(View v) {
				FioDlg dlg = new FioDlg(ProcurationEdit.this);
				dlg.show(getSupportFragmentManager(), dlg.getClass().toString());
				return true;
			}
		});
		...
		edFirm.setOnLongClickListener(new OnLongClickListener() {
			@Override
			public boolean onLongClick(View v) {
				FirmDlg dlg = new FirmDlg(ProcurationEdit.this);
				dlg.show(getSupportFragmentManager(), dlg.getClass().toString());
				return true;
			}
		});
...
//				edOrg.getText().length() > 0;
	...
	static abstract class SelectFromListDlg extends DialogFragment {
		protected ProcurationEdit owner;

		SelectFromListDlg(ProcurationEdit owner) { this.owner = owner; }
	...
	public static class SelOrgDlg extends DialogFragment{

		protected ProcurationEdit owner;
		public SelOrgDlg(ProcurationEdit owner) { this.owner = owner; }
		...
		public Dialog onCreateDialog(Bundle savedInstanceState) {
			...
			lv.setOnItemClickListener(new OnItemClickListener() {
				@Override public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
					owner.edOrg.setText(((Org)parent.getItemAtPosition(position)).name);
					dismiss();
				}});
		...
		private ListAdapter createAdapter() {
			return new BaseAdapter() {
				...
				@Override public Object getItem(int position) { return owner.orgs.get(position);}
				@Override public int getCount() { return owner.orgs.size();	}
			};
		}
	}

	public static class FirmDlg extends SelectFromListDlg{
		public FirmDlg(ProcurationEdit owner) { super(owner);}

		@Override protected List<CharSequence> getTitles() {
			List<CharSequence> result = new ArrayList<CharSequence>();
			
			for(KeyValue kv : owner.firms)
				result.add(kv.value);
		...
		@Override protected void applayVal(String val) { owner.edFirm.setText(val); }
		@Override protected int getTitleId() { return R.string.firm_dlg_title;	}		
	}

	public static class FioDlg extends SelectFromListDlg{
		public FioDlg(ProcurationEdit owner) { super(owner);}

		@Override protected List<CharSequence> getTitles() { return owner.fio;	}
		@Override protected void applayVal(String val) { owner.edFio.setText(val); }
		@Override protected int getTitleId() { return R.string.fio_dlg_title;	}		
	}
	
	public static class RemarkDlg extends SelectFromListDlg{
		public RemarkDlg(ProcurationEdit owner) { super(owner);}

		@Override protected List<CharSequence> getTitles() { return owner.cmts;	}
		@Override protected void applayVal(String val) { owner.edRemark.setText(val); }
		@Override protected int getTitleId() { return R.string.comment_dlg_title;	}		
	}

	public static class RouteDlg extends SelectFromListDlg{
		public RouteDlg(ProcurationEdit owner) { super(owner);}

		@Override protected void applayVal(String val) { owner.edRoute.setText(val);}
		@Override protected List<CharSequence> getTitles() { return owner.routes;	}
