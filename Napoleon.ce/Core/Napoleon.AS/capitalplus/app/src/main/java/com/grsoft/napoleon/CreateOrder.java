...
//	DateHandler dateHandler;
	...
	protected void init() {
		...
		cbt.setOnCheckedChangeListener((buttonView, isChecked) -> {
			if(isChecked)
				((CheckBox)findViewById(R.id.cbAltProd)).setChecked(false);
		});

		cbt = findViewById(R.id.cbAltProd);
		cbt.setChecked(o.alp > 0);
		cbt.setEnabled(o.items.size() == 0);
		cbt.setOnCheckedChangeListener((buttonView, isChecked) -> {
			if(isChecked)
				((CheckBox)findViewById(R.id.cbTabak)).setChecked(false);
		});
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			o.tabak = ((CheckBox)findViewById(R.id.cbTabak)).isChecked() ? 1 : 0;
			o.alp = ((CheckBox)findViewById(R.id.cbAltProd)).isChecked() ? 1 : 0;
