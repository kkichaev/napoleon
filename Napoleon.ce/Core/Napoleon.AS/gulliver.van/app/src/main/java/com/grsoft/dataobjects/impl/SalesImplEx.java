...
public class SalesImplEx extends SalesImpl {
	...
	public void initFromOrder(OrderImplBase<?> src, GpsCoord location) {
		SalesEx se = (SalesEx)getData();
		OrderEx oe = (OrderEx) src.getData();
		se.dogCode = oe.dogCode;
		se.supplyercode = oe.firmCode;
		...
		if(orgImpl.read()){
			List<OrgDogovor> dogovors = ((OrgEx)orgImpl.getData()).dogovors;
			
			for(OrgDogovor dog : dogovors)
				if(se.dogCode.trim().equals(dog.id.trim())){
					isGen = dog.isGeneral();
					break;
				}
		}
	...
	public boolean updateItem(PriceImpl pi, List<PriceSalesQty> party, int cost) {
		if( !isEditable() )
			return true;
		...
		if(se == null) {
			se = (SalesItemEx) createNewItem(pe, qty, cost, false);
			data.items.add(se);
			priceUpdate = - qty;
		} else {
			priceUpdate = se.qty;
		...
		updateQtyPrice(pi, priceUpdate);
		return true;
	}

	@Override
	public void editItem(long itemRowid, Context context) {
		SalesPriceCount.open(context, itemRowid, (DbObject<Sales>)this);
	}
}
