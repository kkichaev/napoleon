...
public class CreateOrder extends BaseActivity
{
	...
	private List<KeyValue> priceType = new ArrayList<KeyValue>();
...
//	DateHandler dateHandler;
	...
	private void init() {
		cbCash = (CheckBox) findViewById(R.id.cbCreateOrderCash);
		...
		DialogHelper.loadSpinnerWithKey(config, "Организация", firms, spFirma, o.firmCode);
		...
		if(!editMode && o.sumType >= 0 && o.sumType < priceType.size())
		...
		cbCash.setEnabled(org.cashdisable == 0);
...
//			o.date = timeHandler.adjustTime(dateHandler.getDate());
			...
			KeyValue kv = (KeyValue) spFirma.getSelectedItem();
			if(kv != null)
				o.firmCode = kv.key.toString();
			
			Spinner spPrices = (Spinner) findViewById(R.id.spPrices);
			int costType = spPrices.getSelectedItemPosition();
			o.sumType = costType;
			
			kv = (KeyValue) spPrices.getSelectedItem();
			if(kv != null)
				o.prcType = kv.key.toString();
