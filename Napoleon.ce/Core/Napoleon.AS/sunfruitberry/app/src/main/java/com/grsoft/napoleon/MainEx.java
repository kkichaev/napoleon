...
public class MainEx extends Main {
	...
	private static final int ALL_DEBET_INDEX = 3;
...
//			int color = (todayMemos.contains(oe.ido)) ? -16751616 : Color.BLACK;
	...
	String getOrgByColors(int color) {
		...
		for(Entry<String, Integer> kv : debetColorOrgs.entrySet()) {
			if(color == ALL_DEBET_INDEX || kv.getValue() == color) {
				if(ret.length() > 0)
					ret += ",";
				ret += "'" + kv.getKey() + "'";
			}
		...
		if(docType == DebtDoc.instance()) {
			...
			try {
				...
				String stmt = new StringBuilder()
						.append("select distinct o.ido from Org o, AgentMemo am where o.id = am.id and am.created > ")
						.append(cur).append(" and am.topic in (select id from ")
						.append(new MemoType().getTableName()).append(" where type=")
						.append(MemoType.TYPE_UNLOCK).append(")").toString();
