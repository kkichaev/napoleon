...
import com.grsoft.napoleon.memo.Edit;
...
public class AgentMemoImpl extends CreatableDocument<AgentMemo> {
	...
	public static AgentMemoImpl createSendInvoice(Context context, OrderImpl doc) {
		...
		ret.data.topic = MemoType.getType(MemoType.TYPE_INVOICE).id;
	...
	public boolean delete() {
		PicStore p = data.findPicture();
		if(p != null) {
			PicStoreImpl.delete(p.id);
		}
		return super.delete();
	}
}
