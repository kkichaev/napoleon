...
VisualLeakDetector::VisualLeakDetector ()
{
    ...
    pSymSetOptions(SYMOPT_LOAD_LINES | SYMOPT_UNDNAME | SYMOPT_DEFERRED_LOADS);
    ...
    if (m_status & VLD_STATUS_INSTALLED) {
        ...
        if (m_status & VLD_STATUS_NEVER_ENABLED) {
            ...
            if (m_leaksfound == 0) {
                ...
                report(L"Visual Leak Detector detected %u memory leak", m_leaksfound);
...
BOOL CALLBACK AttachToModule(PCWSTR modulepath, DWORD64 modulebase, ULONG modulesize, PVOID curproc)
{
   char mpath[MAX_PATH];
   int len = wcstombs(mpath, modulepath, sizeof(mpath));
   mpath[len] = '\0';
   SymLoadModule64((HANDLE)curproc, NULL, mpath, NULL, modulebase, modulesize);
...
VOID VisualLeakDetector::reportleaks (HANDLE heap)
{
    ...
    for (blockit = blockmap->begin(); blockit != blockmap->end(); ++blockit) {
        ...
        if (m_leaksfound == 0) {
          ...
          wchar_t spath[MAX_PATH], *sp;
          GetModuleFileName(NULL, spath, sizeof(spath)/sizeof(spath[0]));
          sp = wcsrchr(spath, L'\\');
          sp[1] = L'\0';
          SymSetOptions(SYMOPT_LOAD_LINES | SYMOPT_UNDNAME | SYMOPT_DEFERRED_LOADS);
          SymInitialize(currentprocess, spath, FALSE);
          EnumerateLoadedModules64(currentprocess, AttachToModule, (PVOID)currentprocess);
        }
