...
VOID CallStack::dump (BOOL showinternalframes) const
{
    ...
    for (frame = 0; frame < m_size; frame++) {
        ...
        foundline = SymGetLineFromAddr64(currentprocess, programcounter, &displacement, &sourceinfo);
        if (SymFromAddr(currentprocess, programcounter, &displacement64, functioninfo)) {
