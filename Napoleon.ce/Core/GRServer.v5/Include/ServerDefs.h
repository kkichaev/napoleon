...
{
#ifdef UNIX
   return !(*fileName != '\0' && (*fileName == DIR_SEP || *fileName == '~'));
#else
   return !(*fileName != '\0' && fileName[1] != '\0' && ((*fileName == DIR_SEP && fileName[1] == DIR_SEP) || fileName[1] == ':'));
#endif
}

inline bool IsLocalName(const std::string& folder)
{
   if(folder.empty()) return true;

   char sym = *folder.begin();
#ifdef UNIX
   return (sym != DIR_SEP && sym != '~');
#else
   return folder.size() < 2 || !((sym == DIR_SEP && folder.at(1) == DIR_SEP) || folder.at(1) == ':');
#endif
}

inline void MakeFullFileName(std::string* out, const char* fn, const std::string& folder)
{
   if (IsLocalName(fn))
   {
      out->assign(folder);
      if (*folder.rbegin() != DIR_SEP)
         out->append(1, DIR_SEP);
   }
   out->append(fn);
}

inline void MakeFullFileName(std::string* out, const std::string& fn, const std::string& folder)
{
   std::string tv(fn);
   out->clear();
   if (IsLocalName(tv))
