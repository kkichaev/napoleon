...
#ifdef UNIX
static void ChangeDivider(std::string* str)
{
...
#else
		USES_CONVERSION;
		std::wstring fileName(A2W_CP(folder.c_str(), CP_UTF8));
		...
		if (*i->name.begin() == L'$')
		{
			...
			if (!session->Parse(&res, i->value, thisObject))
			{
				if (logger != NULL)
				{
					...
					if (thisObject != NULL)
					{
						error.append("of object ").append(W2A_CP(thisObject->Self()->Name().c_str(), CP_UTF8));
					}
					logger->AddError(false, error.c_str(), W2A_CP(i->name.c_str(), CP_UTF8));
				}
				continue;
			}
			else
				...
				if (!session->Parse(&res, param, thisObject))
				{
					...
					if (logger != NULL)
					{
						...
						if (thisObject != NULL)
						{
							USES_CONVERSION;
							error.append("of object ").append(W2A_CP(thisObject->Self()->Name().c_str(), CP_UTF8))
								.append(" with filter ").append(W2A_CP(filter, CP_UTF8));
						}
