...
bool Thread::Starting(IThreadWorker *worker, IErrorLogger* logger, DWORD maxThreads, bool makeDump, std::string* error)
{
	mutex.Init();
...
//      evStopped = CreateEvent(NULL, TRUE, FALSE, NULL);
   ...
   bool ret = false;
	if (mutex.Acquire(1000))
	{
      ...
      {
         char buf[100];
         sprintf(buf, "create err %d", res);
         error->assign(buf);
      }
   } else 
   {
      error->assign("can't acquire mutex");
   }
