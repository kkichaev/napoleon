...
public:
   ...
   bool WriteTo(JSONArray& out);
   bool ReadFrom(const JSONArray& src);
   ...
   virtual const ParamHelper* GetParamHelper() const { return source.reader != NULL ? source.reader->GetParamHelper() : NULL; }
...
protected:
   ...
   void RestoreWriter(std::vector<IDataSource::IWriter*>::iterator current);
	bool FireEvent(IEvent::Type eventType, Object* o);
...
void ToJSON(JSONArray* rest, const ServObject& object);
// void SetFileFieldBaseFolder(std::string* dest, const IObjectData::Field& src, const IServerConfig& config);
void SetFields(GRServer::Object* dest, const JSONObject& src, FormatList* fmtList);
