...
public:
   ...
   bool HandleCommand(const wchar_t* command, const Member* param, ISession* session);
	...
	size_t SessionMemoryLimit() const { return sessionMemoryLimit; }
	size_t TotalMemoryLimit() const { return totalMemoryLimit; }
   ...
   bool IsBlocked(const std::wstring& category) const { return blocked.find(category) != blocked.end(); }
   void UpdateBlocked(const std::set<std::wstring>& src);

protected:
   std::set<std::wstring> blocked;
