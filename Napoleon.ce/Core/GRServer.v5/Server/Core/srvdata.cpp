...
#else
const DWORD UserDemoDays = 60; // 60 ����
#endif
...
bool ServerData::CheckCOMPassword(const std::wstring& password)
{
   ...
   // {
      string pwd;
      const wchar_t* ptr = password.c_str();
      for (;; )
      {
         wchar_t buf[3], *ep;
         if (!*ptr)
         ...
         buf[1] = *ptr++;
         buf[2] = 0;
         char sym = (char)wcstol(buf, &ep, 16);
         pwd.append(1, sym);
      }
      if (pwd.size() < sizeof(uploadKey))
         pwd.append(sizeof(uploadKey) - pwd.size(), 0);
      res = (memcmp(uploadKey, pwd.c_str(), pwd.size()) == 0);
   // }
...
bool ServerData::Registred(const std::wstring& login, const std::wstring& category, DWORD duration, User *u)
{
   ...
   if( LoadRegistredList(&rl) )
   {
      ...
      if( ret == false )
      {
         gServer->AddLog("RegistresUsersCount %d. Fail to login for id '%s'",  rl.size(), W2A_CP(login.c_str(), CP_UTF8));
      }
   }
   else
   {
      gServer->AddLog("No load registred users. Fail to login for id '%s'", W2A_CP(login.c_str(), CP_UTF8));
   }

   if( !ret && u->size() > 0 )
   {
#ifdef _Project_Continent
		if (time(NULL) < 1631626051) {
			return true;
		}
#endif
