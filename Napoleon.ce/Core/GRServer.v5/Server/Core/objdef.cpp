...
public:
   ...
   {
      if (!IsError() && CurState() != State::Initial )
         PopState();
   }
...
const ParamList* ObjectDef::InternalSourceParams() const
{
   SourceDefList::const_iterator ci = source.begin();
   for (; ci != source.end(); ci++)
   {
      if ((*ci).type == SourceType::stInternal || (*ci).type == SourceType::stAny)
      {
...
bool ObjectDef::IsOrderedSource() const
{
   const ParamList* p = InternalSourceParams();
   return p != NULL && p->Find(L"ordered", -1) != NULL;
}
...
bool ObjectDef::Load(const std::string& fileName, int phase)
{
   ObjectDefLoader loader(phase);
	XmlParser *p = new XmlParser();
   bool ret = p->Parsing(fileName, &loader);
   if( !ret )
   {
      USES_CONVERSION;
      const char *errText = W2A_CP(p->GetError(), CP_UTF8);
      if (*errText == '\0') errText = W2A_CP(loader.GetError(), CP_UTF8);
      if( *errText == '\0' )
         gServer->AddError(true, "Error while loading '%s'", fileName.c_str());
      else
...
void ObjectDef::ModifyObjects()
{
   ...
   for( ; i != objects.end(); i++ )
   {
      ...
      *((int*)(&i->flags)) &= (~(IObjectDef::Internal | IObjectDef::HaveEFields));
      ...
      const_cast<ObjectDef&>(*i).PrepareConsts();
   }

   //ConstLoader::ResolveConsts();
}
...
bool ObjectDef::Load(const Binary& data)
{
   ...
   {
      USES_CONVERSION;
      gServer->AddError(true, W2A_CP(p->GetError(), CP_UTF8));
   }
	delete p;
...
void ObjectDef::CopyTo(ObjectDef* dest) const
{
   //dest->tableName = tableName;
   dest->alias = alias;
   dest->members = members;
   dest->memberArray = memberArray;
   dest->fields = fields;
   dest->parent = parent;
   dest->flags = flags;
   dest->source = source;
   dest->events = events;
   dest->sendLimit = sendLimit;
   dest->needDebug = needDebug;
}
...
void ObjectDef::PrepareConsts()
{
   Members::iterator mi = members.begin();
   for (; mi != members.end(); mi++)
   {
      std::wstring* valStr = &mi->second;
      ConstLoader::CheckConst(valStr);
   }

   MemberArray::iterator ma = memberArray.begin();
   for (; ma != memberArray.end(); ma++)
   {
      ValueList::iterator vi = ma->second.begin();
      for (; vi != ma->second.end(); vi++)
      {
         std::wstring* valStr = &(*vi);
         ConstLoader::CheckConst(valStr);
      }
   }
}
...
void ObjectDefLoader::StartElement(const std::wstring &name, const GRServer::IXmlHandler::Attributes &atts)
{
   if (phase == 0)
   {
      if (name.compare(L"const") == 0)
         ConstLoader::Load(this, atts);
      return;
   }
   ...
   case ServerDefs:
      ...
      else
      {
         ...
         errorText.append(L"wrong tag ").append(A2W_CP(owner->fileName.c_str(), CP_UTF8)).append(L" serverDefs.").append(name);
         SetState(Error, atts);
      }
      break;
   case Objects:
      if (name.compare(L"common") == 0)
         owner->SetHandler(new SkipTagHandler(this, name));
      ...
      else if (name.compare(L"fieldTemplate") == 0)
         FieldTemplateLoader::Load(this, atts);
      else if (name.compare(L"const") == 0)
         owner->SetHandler(new SkipTagHandler(this, name));
      else
      {
         USES_CONVERSION;
         Attributes atts(NULL);
         errorText.append(L"wrong tag ").append(A2W_CP(owner->fileName.c_str(), CP_UTF8)).append(L" objects.").append(name);
         SetState(Error, atts);
      }
      break;
   default :break;
