...
void FileReader::FileBinary::Load() const
{
   ...
   if( f != NULL )
   {
      ...
      if( size > 0 )
      {
         ...
         size_t __u = fread(b, size, sizeof(BYTE), f);
      }
      fclose(f);
   } else
   {
      gServer->AddError(false, "�� ���� ������� ���� '%s' errno=%d (%s)", str, errno, strerror(errno));
   }
}
...
public:
   ...
   virtual bool InSet(const char* value)
   {
      ...
      char *ep;
		std::set<double>::const_iterator fnd = values.find(std::strtod(value, &ep));
...
void DBFReader::Remove()
{
   Close();
	...
	gServer->AddLog("Remove file %s", base.GetName());
...
bool DBFReader::SetValue(Member* member, int index) const
{
   ...
   case 'F':
	case 'L':
	{
      ...
      char *ep;
		member->number = (field->type == 'L') ? ((*value == '1' || *value == 'T') ? 1.0 : 0.0) : std::strtod(value, &ep);
		break;
	}
   case 'D':
...
bool GRServer::GetTableName(std::string* fileName, const ParamList& parameters, const SessionObject& object)
{
   ...
   fileName->assign(config.ExchangeFolder());
	fileName->append(W2A_CP(tableName.value.str->c_str(), CP_UTF8));
#ifdef UNIX
