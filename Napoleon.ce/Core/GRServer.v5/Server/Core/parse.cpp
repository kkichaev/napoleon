...
bool GRServer::SymbolHandler(wchar_t sym, Token *dest, StringStream &stream, const SessionObject* object, IResolver &resolver, const wchar_t *endStmt, bool needDebug)
{
	...
	case L'$':
		...
		{
			USES_CONVERSION;
			gServer->AddLog(IErrorLogger::Full, "Can't resolve (%s)", W2A_CP(var.c_str(), CP_UTF8));
		}
		break;
	case L'(':
		stream.MoveNext();
	...
	case L'+':
		...
		if (error && needDebug)
		{
			...
			std::wstring tv;
			t.ToString(&tv);
			gServer->AddLog(IErrorLogger::Full, "Can't plus %s", W2A_CP(tv.c_str(), CP_UTF8));
		}
	...
	case L'-':
		...
		if (error && needDebug)
		{
			...
			std::wstring tv;
			t.ToString(&tv);
			gServer->AddLog(IErrorLogger::Full, "Can't minus %s", W2A_CP(tv.c_str(), CP_UTF8));
		}
		if (!stream.EOS()) stream.Back();
		break;
	case L'*':
		...
		if (error && needDebug)
		{
			...
			std::wstring tv;
			t.ToString(&tv);
			gServer->AddLog(IErrorLogger::Full, "Can't mul %s", W2A_CP(tv.c_str(), CP_UTF8));
		}
		if (!stream.EOS()) stream.Back();
		break;
	case L'=':
		...
		if (error && needDebug)
		{
			...
			std::wstring tv;
			t.ToString(&tv);
			gServer->AddLog(IErrorLogger::Full, "Can't assign %s", W2A_CP(tv.c_str(), CP_UTF8));
		}
		if (!stream.EOS()) stream.Back();
		break;
...
bool CompareHandler(wchar_t sym, Token *dest, StringStream &stream, const SessionObject* object, IResolver &resolver, const wchar_t *endStmt, bool needDebug)
{
	...
	case '=':
	{
		...
		if (error && needDebug)
		{
			...
			std::wstring tv;
			t.ToString(&tv);
			gServer->AddLog(IErrorLogger::Full, "Wrong compare %s", W2A_CP(tv.c_str(), CP_UTF8));
		}
		if (!stream.EOS()) stream.Back();
		break;
	}
