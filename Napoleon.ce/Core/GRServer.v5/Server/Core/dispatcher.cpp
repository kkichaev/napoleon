...
void Dispatcher::UpdateBlocked(const std::set<std::wstring>& src)
{
   if(blockMutex.Acquire(1000))
   {
      blocked = src;
      blockMutex.Release();
   }
...
bool Dispatcher::OpenSocket()
{
   ...
   {
      //AddError(true, "�������� ����� %d, ��� ������ %d", dataCtrl.Config().port, WSAGetLastError());
      return false;
   }
...
bool Dispatcher::Init(DWORD argc, const char **argv, IProgramLoader *loader)
{
   ...
   if( !config.Load() )
   {
      AddError(true, "Can't load config file '%s'", config.configFile.c_str());
      return false;
   }
   else 
   {
	   AddLog("Starting mem limit %u MB session limit %u MB", (uint32_t)config.memoryLimit, (uint32_t)config.sessionMemoryLimit);
   }

   USES_CONVERSION;
   std::set<std::string>::const_iterator si = config.blocked.begin();
   for( ; si != config.blocked.end(); si++)
   {
      std::wstring str(A2W_CP(si->c_str(), CP_UTF8));
      blocked.insert(str);
   }
   blockMutex.Init();
   ...
   if (config.OpenConsole())
   {
      if (AllocConsole() == FALSE)
         gServer->AddLog(IErrorLogger::Full, "Can't alloc debug console %d", GetLastError());

      //SetConsoleTitle(L"GRServer Python Debug");
      SetConsoleTitle(ServerName());
   ...
   // SYSTEMTIME st;
   // GetLocalTime(&st);
   // AddLog("%02d:%02d:%02d", st.wHour, st.wMinute, st.wSecond);
   ...
   {
      std::string fn;
      MakeFullFileName(&fn, config.defsFile, config.ConfigFolder());

      if (!ObjectDef::Load(fn, phase))
      {
         MakeFullFileName(&fn, config.defsFile, config.ProgFolder());
         if(!ObjectDef::Load(fn, phase))
            return false;
      }
      ...
      {
         MakeFullFileName(&fn, *i, config.ConfigFolder());
         if (!ObjectDef::Load(fn, phase))
            return false;
      }
   }
   ...
   {
#ifdef UNIX
         AddError(true, "Can't bind to port %d, with error %d", dataCtrl.Config().port, errno);
#else
      if( runMode == srmTray )
      {
         if( MessageBox(NULL, L"������ ������� ���������� - ���� �����.\n������� ���� �������� �������?", L"������", MB_YESNO | MB_ICONEXCLAMATION) == IDYES )
   ...
   WSAEventSelect(socket, evAccept, FD_ACCEPT);
	...
	{
		USES_CONVERSION;
		JoinServer::Start(config.jsLogin, config.jsPassword, W2A_CP(PROJECT_NAME, CP_UTF8), config.port, false);
	}
#endif
...
{
   std::string error;
	return Thread::Starting(thread, this, MAX_THREADS, ((ServerConfig&)GetConfig()).makeDumpOnException, &error);
}
...
bool Dispatcher::PrintTableDef(const char* tableName)
{
	...
	if( od != NULL )
	{
		...
		{
			fprintf(stderr, "%s;%s;%s;%d;%d\n", W2A(fi->format.name.c_str()), W2A_CP(fi->data.c_str(), CP_UTF8), TypeToString(*fi),
