         ...
         ret = !name.empty();
			...
			if (ret)
			{
				...
				} else 
				{
					...
					if (so == NULL)
					{
						ISessionObject *iso = const_cast<Session&>(session).LoadObject(object, &thisObject);
						if (iso != NULL)
							so = (const SessionObject*)iso->Self();
					}
					if (so != NULL)
					{
...
FilterValueHead* FilterItemReader::Parse(StringStream& stream, const SessionObject& thisObject)
{
 ...
 //  const IObjectData* myOD = thisObject.GetObjectDef();
   ...
   {
		if (op == FilterValueHead::InSet && !right.name.empty())
		{
			right.isConstValue = false;
		}
