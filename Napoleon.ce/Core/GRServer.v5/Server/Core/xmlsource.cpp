...
{
public:
	XMLUserList(const ISessionObject& object, const ParamList& parameters);
...
class XMLUserTableReader : public XMLReaderBase
{
public:
	XMLUserTableReader(const ISessionObject& object, const ParamList& parameters);
	bool SetFilter(const wchar_t* filter, const ISessionObject& object) { return users.SetFilter(filter, object); }
	virtual bool MoveNext(Object *parentObject);
...
bool XMLReaderBase::Open(const Params& params)
{
   ...
   tfile = W2A_CP(params.tableName.c_str(), CP_UTF8);
   MakeFullFileName(&fileName, tfile.c_str(), config.ExchangeFolder());
	...
	if (params.tableName.empty() == false)
	{
		...
		HANDLE handle = FindFirstFile(A2W_CP(fileName.c_str(), CP_UTF8), &fdata);
   ...
   tfile = W2A_CP(params.archiveFolder.c_str(), CP_UTF8);
   if( !tfile.empty() )
   {
      MakeFullFileName(&archiveFolder, tfile.c_str(), config.ExchangeFolder());
   }
...
XMLUserList::XMLUserList(const ISessionObject& object, const ParamList& parameters) : userIndex(0), isNullUser(false)
{
	const Session& session = (const Session&)object.GetSession();

	USES_CONVERSION;
	std::wstring val;
	const Parameter* fnd = parameters.Find(L"userTable", -1);
	if (fnd && session.Parse(&val, fnd->value, &object))
		userTable = W2A_CP(val.c_str(), CP_UTF8);

	fnd = parameters.Find(L"commonTable", -1);
	if (fnd && session.Parse(&val, fnd->value, &object))
		commonTable = W2A_CP(val.c_str(), CP_UTF8);

	const char* userA = W2A_CP(session.GetUser().ID(), CP_UTF8);
	users.push_back(userA);
}

bool XMLUserList::SetFilter(const wchar_t* filter, const ISessionObject& object)
{
	bool ret = true;
...
const char* XMLUserList::Next() const
{
	const char *uid = "";
...
XMLUserTableReader::XMLUserTableReader(const ISessionObject& object, const ParamList& parameters) :
	XMLReaderBase(object), users(object, parameters), prepared(false)
{
	Params p;
	...
	binder.Prepare(p.docRoot, p.enclosure, object, &data);
}

bool XMLUserTableReader::MoveNext(Object *parentObject)
{
	if (!prepared)
	{
		PrepareBinder(NULL);
		...
		const ServerConfig &config = (const ServerConfig&)object.GetSession().Config();
		string folder = config.ExchangeFolder();

		while (true)
		{
			const char *table = users.Next();
			if (*table == '\0')
				break;
         ...
         USES_CONVERSION;
			...
			HANDLE handle = FindFirstFile(A2W_CP(fileName.c_str(), CP_UTF8), &data);
			if (handle != INVALID_HANDLE_VALUE)
			{
				binder.Load();
				parser->Parsing(fileName, &binder);
				FindClose(handle);
			}
#endif         
		}
...
bool XMLWriter::Open(const Params& params, const ISessionObject& object)
{
   ...
   USES_CONVERSION;
   fileName = W2A_CP(params.tableName.c_str(), CP_UTF8);
...
//----------------------- XMLUserTableCreator --------------------------------
//
IDataSource::IReader* XMLUserTableCreator::CreateReader(const ParamList& parameters, const ISessionObject& object) const
{
	Params p;
	p.Load(parameters, object);
	...
	if (object.Parent() == NULL)
	{
		ret = new XMLUserTableReader(object, parameters);
	}
	//if (!ret->Open(p))
	//{
	//	//USES_CONVERSION;
