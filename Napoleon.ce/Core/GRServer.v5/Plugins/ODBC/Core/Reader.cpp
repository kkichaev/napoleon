...
void Reader::PrepareBinder(const std::wstring& _filter)
{
	...
	{
		USES_CONVERSION;
		gServer->AddLog(IErrorLogger::None, "run stmt %s", W2A_CP(stmt.c_str(), CP_UTF8));
	}
}
 ...
 //     object.GetSession().Parse(&filter, filterP->value, &object);
	...
	if (whF != NULL)
	{
		...
		if (needAssign)
		{
			...
			{
				USES_CONVERSION;
				gServer->AddLog("Error while parse whereFilter of %s", W2A_CP(object.Self()->Name().c_str(), CP_UTF8));
			}
		}
	}
