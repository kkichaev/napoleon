...
bool GRServer::AddErrorsToLog(bool isCritical, SQLSMALLINT ht, SQLHANDLE handle, IErrorLogger::DebugLevel level)
{
   ...
   while( true )
   {
      SQLRETURN rc = SQLGetDiagRec(ht, handle, idx++, (SQLWCHAR*)state, &errCode, (SQLWCHAR*)err, sizeof(err)/sizeof(err[0]),&cn);
      if( rc != SQL_SUCCESS )
         break;
      ...
      ret = true;
      if( isCritical )
         gServer->AddError(isCritical, "ODBC error [%s]: %s", W2A_CP(state, CP_UTF8), W2A_CP(err, CP_UTF8));
      else
         gServer->AddLog(level, "ODBC error [%s]: %s", W2A_CP(state, CP_UTF8), W2A_CP(err, CP_UTF8));
...
bool GRServer::Execute(SQLHDBC hdbc, const std::wstring& sql, bool fetchResult)
{
   ...
   {
      USES_CONVERSION;
      gServer->AddLog(IErrorLogger::Full, "Execute stmt %s", W2A_CP(sql.c_str(), CP_UTF8));
...
bool ODBCDataSource::InitConnection()
{
   ...
   if( connectStr.empty() )
   {
      if( !c.Load(configFile) )
      {
         ...
         err += W2A_CP(configFile.c_str(), CP_UTF8);
         err += "]";
...
bool ODBCDataSource::CheckDBFormat(GRServer::IObjectDef* objDefs)
{
   ...
   for( int pass=0; res && pass < 2; pass++ )
   {
      ...
      for( ; res && i != names->end(); i++ )
      {
         ...
         {
            if( logging )
               gServer->AddLog(IErrorLogger::Full, "Create table %s", W2A_CP(odef->tableName.c_str(), CP_UTF8));
...
bool ODBCDataSource::AlterTable(const IObjectData& objDef, const std::vector<IObjectData::Field>& added)
{
   ...
   {
      USES_CONVERSION;
      gServer->AddLog(IErrorLogger::Full, "Alter table %s", W2A_CP(objDef.tableName.c_str(), CP_UTF8));
   }
