...
bool QueryReader::MoveNext(Object *parentObject)
{
   if( !binder.IsOpened() )
   {
      ...
      objects.insert(objects.begin(), &object);
		...
		{
			USES_CONVERSION;
			gServer->AddLog(IErrorLogger::Full, "Do Query: %s", W2A_CP(stmt.c_str(), CP_UTF8));
		}
...
bool SQLChildQuery::MoveNext(Object *parentObject)
{
	if (requery)
	{
		...
		for (; pi != params.end(); pi++)
		{
			...
			{
				USES_CONVERSION;
				gServer->AddLog("SQLChildQuery parse error %s", W2A_CP(pi->c_str(), CP_UTF8));
			...
			if (!inited)
			{
				...
				{
					USES_CONVERSION;
					gServer->AddLog("SQLChildQuery can't create binder %s", W2A_CP(pi->c_str(), CP_UTF8));
		...
		if (!inited)
		{
			...
			{
				USES_CONVERSION;
				gServer->AddLog(IErrorLogger::None, "Prepare stmt %s", W2A_CP(stmt.c_str(), CP_UTF8));
			}
			inited = true;
		}
