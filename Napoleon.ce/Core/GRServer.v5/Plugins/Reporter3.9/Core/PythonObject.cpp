...
static PyObject* GetAttrO(PythonObject *obj, PyObject *name)
{
   ...
   PyObject* strObj = PyObject_Str(name);
	...
	{
		USES_CONVERSION;
		res = GetAttr(obj, W2A_CP(sname, CP_UTF8));
	}
...
static void ToString(std::wstring* str, const Member& m, const MemberFormat& f)
{
	...
	case MemberFormat::mtNumber:
		...
		//else
#ifdef UNIX
		swprintf(buf, sizeof(buf)/sizeof(buf[0]), L"%.*f", f.format.fraction, m.number);
#else
...
#ifdef UNIX
		swprintf(buf, sizeof(buf)/sizeof(buf[0]), L"%02d/%02d/%d %02d:%02d:%02d", st.wDay, st.wMonth, st.wYear, st.wHour, st.wMinute, st.wSecond);
#else
	...
	{
#ifdef UNIX
		swprintf(buf, sizeof(buf)/sizeof(buf[0]), L"length %d", (m.binary) ? m.binary->Size() : 0);
#else
...
static int SetAttr(PythonObject *obj, const char *name, PyObject *value)
{
   ...
   int idx = obj->src->format.FindMember(A2W(name));
	...
	{
		char buf[500];
		wsprintfA(buf, "No member '%s' in object '%s'", name, W2A_CP(obj->src->format.name.c_str(), CP_UTF8));
...
static int SetAttrO(PythonObject *obj, PyObject *name, PyObject *value)
{
   ...
   PyObject* strObj = PyObject_Str(name);
	...
	{
		USES_CONVERSION;
		res = SetAttr(obj, W2A_CP(sname, CP_UTF8), value);
	}
	PyMem_Free((void*)sname);
...
static PyObject* GetUserAttrO(UserObject *obj, PyObject *name)
{
	...
	{
		USES_CONVERSION;
		res = GetUserAttr(obj, W2A_CP(sname, CP_UTF8));
	}
	PyMem_Free((void*)sname);
