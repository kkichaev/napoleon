...
static PyObject* GetServObject(PyGRServer* self, PyObject* args)
{
	...
	if (iso != NULL)
	{
		...
		USES_CONVERSION;
		const char *aObjName = W2A_CP(objName, CP_UTF8);
		gServer->AddLog(IErrorLogger::Full, "Socket (%d) reading %s:%s", self->session->GetSocket(), aObjName, ((whereStr == NULL) ? "" : W2A_CP(whereStr, CP_UTF8)));
#endif
...
static PyObject* CreateCOMObject(PyGRServer* self, PyObject* args)
{
	...
	if (PyArg_ParseTuple(args, "u", &objName))
	{
		...
		if (SUCCEEDED(res))
		{
			...
			gServer->AddLog("CoCreateInstance %s error %s", objName, W2A_CP(errMsg, CP_UTF8));
		}
		else
		{
			_com_error err(res);
			const wchar_t* errMsg = err.ErrorMessage();
			gServer->AddLog("CLSIDFromProgID %s error %s", objName, W2A_CP(errMsg, CP_UTF8));
		}
	}
...
static PyObject* GetConfig(PyGRServer* self, PyObject* args)
{
	...
	if (PyArg_ParseTuple(args, "s", &key))
	{
		...
		if (fnd != cfg.end())
		{
			const wchar_t* p = A2W_CP(fnd->second.c_str(), CP_UTF8);
			size_t cb = wcslen(p);
...
static PyObject* ServerTZ(PyGRServer* self, PyObject* args)
{
	tzset();
	int res = timezone / 60;
	return PyLong_FromLong(res);
...
static PyMethodDef methods[] = {
   ...
   "Create com object"
	...
	{ "TimeZone", (PyCFunction)ServerTZ, METH_VARARGS,
	"ServerTimeZone in minutes. Below zero east of the UTC"
	},
#ifdef USE_CURL
