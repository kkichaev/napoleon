...
static bool PythonToVariant(VARIANT* ret, PyObject* src)
{
	...
	{
		ret->vt = VT_BSTR;
		wchar_t *tstr = PyUnicode_AsWideCharString(src, NULL);
		ret->bstrVal = SysAllocString(tstr);
		PyMem_Free(tstr);
...
static int SetAttr(PyComObject *obj, const char *name, PyObject *value)
{
	...
	if (SUCCEEDED(hres))
	{
		...
		else
		{
			...
			{
				USES_CONVERSION;
				PyErr_SetString(PyExc_RuntimeError, W2A_CP(ei.bstrDescription, CP_UTF8));
				return NULL;
			}
		}
	}
