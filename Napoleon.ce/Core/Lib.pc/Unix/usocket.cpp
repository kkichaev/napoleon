...
bool Socket::ReadBuf(BYTE *buffer, DWORD bufSize, DWORD timeout, HANDLE evStop)
{
   int res;
   ...
   while( bufSize > 0 )
   {
      ...
      res = (int)recv(socket, (char*)buffer, cb, 0);
...
bool Socket::Write(const Packet& packet)
{
   ...
   unsigned short *p = (unsigned short*)alloca(db);
   ConvHelper((const char*)packet.head.c_str(), (char*)p, cb * sizeof(wchar_t), db, "UTF32", "UTF16");
