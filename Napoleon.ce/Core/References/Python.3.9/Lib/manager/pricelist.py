...
class PriceList(RObj, StObj):
    ...
    def __init__(self, server):
        ...
        self.checkedTypes : dict[str:bool] = dict()
    ...
    def process(self, doc):
        if not doc.GetName in self.checkedTypes:
            canAdd =  hasattr(doc, 'items') and isinstance(doc.items, list) and len(doc.items) > 0 and hasattr(doc.items[0],'id')
            self.checkedTypes[doc.GetName] = canAdd
            
        if self.checkedTypes[doc.GetName]:
            for i in doc.items:
                self.put(i.id)
