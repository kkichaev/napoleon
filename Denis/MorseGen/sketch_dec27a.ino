...
inline void updatePause() {
  ...
  if(pauseCount >= 3 && treeIndex != 0) {
    ...
    else playedSym = morseTree[treeIndex].sym;
...
ISR(TIMER2_OVF_vect) {
  ...
  if(toneMode > 0 ) {
    ...
    if((keyFlag & (DOD | FDASH)) == DOD) {
      ...
      // mark play dot
      if(treeIndex >= 0) treeIndex = morseTree[treeIndex].dot;
    } else if((keyFlag & (DASH|FDASH)) != 0) {
      toneMode = 3 * toneTicks; // play dash
      keyFlag &= ~FDASH;
      
      // mark play dash
      if(treeIndex >= 0) treeIndex = morseTree[treeIndex].dash;
